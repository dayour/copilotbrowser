...
export class InjectedScript {
  ...
  setupHitTargetInterceptor(node: Node, action: 'hover' | 'tap' | 'mouse' | 'drag', hitPoint: { x: number, y: number } | undefined, blockAllEvents: boolean): HitTargetInterceptionResult | 'error:notconnected' | string /* hitTargetDescription */ {
    ...
    const listener = (event: PointerEvent | MouseEvent | TouchEvent) => {
      ...
      // copilotbrowser only issues trusted events, so allow any custom events originating from
  ...
  markTargetElements(markedElements: Set<Element>, callId: string) {
    ...
    const unmarkEvent = new CustomEvent('__copilotbrowser_unmark_target__', {
      bubbles: true,
      cancelable: true,
      detail: callId,
      composed: true,
    ...
    const markEvent = new CustomEvent('__copilotbrowser_mark_target__', {
      bubbles: true,
      cancelable: true,
      detail: callId,
      composed: true,
  ...
  private _setupGlobalListenersRemovalDetection() {
    const customEventName = '__copilotbrowser_global_listeners_check__';
