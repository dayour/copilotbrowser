...
export class ConsoleAPI {
  ...
  install() {
    if (this._injectedScript.window.copilotbrowser)
      return;
    this._injectedScript.window.copilotbrowser = {
    ...
    delete this._injectedScript.window.copilotbrowser.filter;
    delete this._injectedScript.window.copilotbrowser.first;
    delete this._injectedScript.window.copilotbrowser.last;
    delete this._injectedScript.window.copilotbrowser.nth;
    delete this._injectedScript.window.copilotbrowser.and;
    delete this._injectedScript.window.copilotbrowser.or;
  }

  private _querySelector(selector: string, strict: boolean): (Element | undefined) {
    if (typeof selector !== 'string')
      throw new Error(`Usage: copilotbrowser.query('copilotbrowser >> selector').`);
  ...
  private _querySelectorAll(selector: string): Element[] {
    if (typeof selector !== 'string')
      throw new Error(`Usage: copilotbrowser.$$('copilotbrowser >> selector').`);
    const parsed = this._injectedScript.parseSelector(selector);
  ...
  private _inspect(selector: string) {
    if (typeof selector !== 'string')
      throw new Error(`Usage: copilotbrowser.inspect('copilotbrowser >> selector').`);
    this._injectedScript.window.inspect(this._querySelector(selector, false));
  }

  private _selector(element: Element) {
    if (!(element instanceof Element))
      throw new Error(`Usage: copilotbrowser.selector(element).`);
    return this._injectedScript.generateSelectorSimple(element);
  }

  private _generateLocator(element: Element, language?: Language) {
    if (!(element instanceof Element))
      throw new Error(`Usage: copilotbrowser.locator(element).`);
