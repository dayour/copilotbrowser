...
export function inject(globalObj: GlobalThis) {
  if ((globalObj as any).__pwWebSocketDispatch)
    return;
  ...
  function bufferToData(b: Uint8Array): WSData {
    ...
    return { data: globalObj.btoa(s), isBase64: true };
  }
  ...
  function messageToData(message: WebSocketMessage, cb: (data: WSData) => any) {
    if (message instanceof globalObj.Blob)
  ...
  const binding = (globalObj as any).__pwWebSocketBinding as (message: BindingPayload) => void;
  const NativeWebSocket: typeof WebSocket = globalObj.WebSocket;
  const idToWebSocket = new Map<string, WebSocketMock>();
  (globalObj as any).__pwWebSocketDispatch = (request: APIRequest) => {
  ...
  class WebSocketMock extends EventTarget {
    ...
    constructor(url: string | URL, protocols?: string | string[]) {
      ...
      this.url = new URL(url, globalObj.window.document.baseURI).href.replace(/^http/, 'ws');
