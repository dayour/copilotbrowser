...
import logo from '@web/assets/copilotbrowser-logo.svg';
...
const ReportLoader: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    zipReport.load().then(() => {
      ...
      document.getElementById('copilotbrowserReportBase64')?.remove();
      setReport(zipReport);
    });
  }, []);
...
class ZipReport implements LoadedReport {
  ...
  async load() {
    const zipURI = document.getElementById('copilotbrowserReportBase64')!.textContent;
  ...
  async entry(name: string): Promise<Object> {
    ...
    await (reportEntry as zip.FileEntry).getData(writer);
