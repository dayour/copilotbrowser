...
/**
 * copilotbrowser Test supports running multiple test projects at the same time. This is useful for running tests in
 * multiple configurations. For example, consider running tests against multiple browsers. This type describes format
 * of a project in the configuration file, to access resolved configuration parameters at run time use `FullProject`.
 *
 * `TestProject` encapsulates configuration specific to a single project. Projects are configured in
 * **TestConfig.projects** specified in the [configuration file](https://copilotbrowser.dev/docs/test-configuration). Note that all properties
 * of `TestProject` are available in the top-level `TestConfig`, in which case they are shared between all projects.
 *
...
interface TestProject<TestArgs = {}, WorkerArgs = {}> {
  /**
   * Options for all tests in this project, for example **TestOptions.browserName**. Learn more about
   * [configuration](https://copilotbrowser.dev/docs/test-configuration) and see [available options]`TestOptions`.
   *
   * ```js
   * // copilotbrowser.config.ts
  ...
  expect?: {
    ...
    toHaveScreenshot?: {
      ...
      /**
       * See **Page.screenshot.animations** in **Page.screenshot()**. Defaults to `"disabled"`.
       */
      animations?: "allow"|"disabled";

      /**
       * See **Page.screenshot.caret** in **Page.screenshot()**. Defaults to `"hide"`.
       */
      caret?: "hide"|"initial";

      /**
       * See **Page.screenshot.scale** in **Page.screenshot()**. Defaults to `"css"`.
       */
      scale?: "css"|"device";

      /**
       * See **Page.screenshot.style** in **Page.screenshot()**.
       */
    ...
    /**
     * Configuration for the **LocatorAssertions.toMatchAriaSnapshot()** method.
     */
    toMatchAriaSnapshot?: {
      /**
    ...
    /**
     * Configuration for the **SnapshotAssertions.toMatchSnapshot()** method.
     */
    toMatchSnapshot?: {
      /**
       * an acceptable perceived color difference between the same pixel in compared images, ranging from `0` (strict) and
    ...
    /**
     * Configuration for the [expect(value).toPass()](https://copilotbrowser.dev/docs/test-assertions) method.
     */
    toPass?: {
      /**
  ...
  /**
   * copilotbrowser Test runs tests in parallel. In order to achieve that, it runs several worker processes that run at
   * the same time. By default, **test files** are run in parallel. Tests in a single file are run in order, in the same
   * worker process.
   *
   ...
   * run tests with "cart" in the title. Also available globally and in the [command line](https://copilotbrowser.dev/docs/test-cli) with the `-g`
   ...
   *
   * `grep` option is also useful for [tagging tests](https://copilotbrowser.dev/docs/test-annotations#tag-tests).
   */
  grep?: RegExp|Array<RegExp>;

  /**
   * Filter to only run tests with a title **not** matching any of the patterns. This is the opposite of
   * **TestProject.grep**. Also available globally and in the [command line](https://copilotbrowser.dev/docs/test-cli) with the `--grep-invert`
   * option.
   *
   * `grepInvert` option is also useful for [tagging tests](https://copilotbrowser.dev/docs/test-annotations#tag-tests).
   */
   ...
   *
   * **NOTE** copilotbrowser executes the configuration file multiple times. Do not dynamically produce non-stable
   ...
   * **TestProject.outputDir** is created, guaranteeing that test running in parallel do not conflict. This directory
   * can be accessed by **TestInfo.outputDir** and **TestInfo.outputPath()**.
   *
   ...
   * **TestConfig.testDir** nor **TestProject.testDir** are explicitly specified, copilotbrowser will ignore any test
   * files matching `.gitignore` entries. This option allows to override that behavior.
   */
  respectGitIgnore?: boolean;

  /**
   * The maximum number of retry attempts given to failed tests. Learn more about
   * [test retries](https://copilotbrowser.dev/docs/test-retries#retries).
   *
   ...
   * `'snapshots'`, the **TestInfo.snapshotDir** would resolve to `snapshots/a.spec.js-snapshots`.
   */
  snapshotDir?: string;

  /**
   * This option configures a template controlling location of snapshots generated by
   * **PageAssertions.toHaveScreenshot()**, **LocatorAssertions.toMatchAriaSnapshot()** and
   * **SnapshotAssertions.toMatchSnapshot()**.
   *
   ...
   *   - Value: `/home/copilotbrowser/tests` (since `snapshotDir` is not provided in config, it defaults to
   *     `testDir`)
   * - `{testDir}` - Project's **TestProject.testDir**.
   *   - Value: `/home/copilotbrowser/tests` (absolute path since `testDir` is resolved relative to directory with
   *     config)
   ...
   *
   * By default, copilotbrowser looks for files matching the following glob pattern:
   * `**\/*.@(spec|test).?(c|m)`jt`s?(x)`. This means JavaScript or TypeScript files with `".test"` or `".spec"` suffix,
   * for example `login-screen.wrong-credentials.spec.ts`.
   *
   ...
   *
   * This is a base timeout for all tests. Each test can configure its own timeout with **Test.setTimeout()**. Each file
   * or a group of tests can configure the timeout with **Test.describe.configure()**.
   *
   ...
   *
   * Note that the global **TestConfig.workers** limit applies to the total number of worker processes. However,
   * copilotbrowser will limit the number of workers used for this project by the value of **TestProject.workers**.
   *
...
/**
 * Runtime representation of the test project configuration. It is accessible in the tests via **TestInfo.project**
 * and **WorkerInfo.project** and is passed to the test reporters. To see the format of the project in the
 * copilotbrowser configuration file please see `TestProject` instead.
 */
...
/**
 * copilotbrowser Test provides many options to configure how your tests are collected and executed, for example
 * `timeout` or `testDir`. These options are described in the `TestConfig` object in the
 * [configuration file](https://copilotbrowser.dev/docs/test-configuration). This type describes format of the configuration file, to access
 * resolved configuration parameters at run time use `FullConfig`.
 *
 * copilotbrowser Test supports running multiple test projects at the same time. Project-specific options should be
 * put to **TestConfig.projects**, but top-level `TestConfig` can also define base options shared between all
 * projects.
 *
 * ```js
 * // copilotbrowser.config.ts
...
interface TestConfig<TestArgs = {}, WorkerArgs = {}> {
  /**
   * copilotbrowser Test supports running multiple test projects at the same time. See `TestProject` for more
   * information.
   *
   * **Usage**
   *
   ...
   * is not specified, copilotbrowser uses the `'dot'` reporter when the CI environment variable is set, and the
   * `'list'` reporter otherwise.
   *
   * Learn more in the [reporters guide](https://copilotbrowser.dev/docs/test-reporters).
   *
   * **Usage**
   *
   * ```js
  ...
  /**
   * Global options for all tests, for example **TestOptions.browserName**. Learn more about
   * [configuration](https://copilotbrowser.dev/docs/test-configuration) and see [available options]`TestOptions`.
   *
   * **Usage**
   *
   ...
   * **Details**
   *
   * If the port is specified, copilotbrowser Test will wait for it to be available on `127.0.0.1` or `::1`, before
   * running the tests. If the url is specified, copilotbrowser Test will wait for the URL to return a 2xx, 3xx, 400,
   * 401, 402, or 403 status code before running the tests.
   *
   * For continuous integration, you may want to use the `reuseExistingServer: !process.env.CI` option which does not
   * use an existing server on the CI. To see the stdout, you can set the `DEBUG=cb:webserver` environment variable.
   *
   * The `port` (but not the `url`) gets passed over to copilotbrowser as a **TestOptions.baseURL**. For example port
   ...
   *
   * **NOTE** It is also recommended to specify **TestOptions.baseURL** in the config, so that tests could use relative
   * urls.
   *
   * **Usage**
   *
  ...
  /**
   * Configuration for the `expect` assertion library. Learn more about [various timeouts](https://copilotbrowser.dev/docs/test-timeouts).
   *
   * **Usage**
   *
   * ```js
  ...
  expect?: {
    ...
    /**
     * Configuration for the [expect(value).toPass()](https://copilotbrowser.dev/docs/test-assertions#expecttopass) method.
     */
    toPass?: {
      /**
       * Probe intervals for toPass method in milliseconds.
   ...
   *
   * Also available in the [command line](https://copilotbrowser.dev/docs/test-cli) with the `--fail-on-flaky-tests` option.
   *
   * **Usage**
   *
   * ```js
  ...
  /**
   * Whether to exit with an error if any tests or groups are marked as **Test.only()** or **Test.describe.only()**.
   * Useful on CI.
   *
   * **Usage**
   *
   ...
   * function that takes a `FullConfig` argument. Pass an array of paths to specify multiple global setup files.
   *
   * Learn more about [global setup and teardown](https://copilotbrowser.dev/docs/test-global-setup-teardown).
   *
   * **Usage**
   *
   * ```js
   ...
   * function. See also **TestConfig.globalSetup**. Pass an array of paths to specify multiple global teardown files.
   *
   * Learn more about [global setup and teardown](https://copilotbrowser.dev/docs/test-global-setup-teardown).
   *
   * **Usage**
   ...
   * [various timeouts](https://copilotbrowser.dev/docs/test-timeouts).
   *
   * **Usage**
   *
   * ```js
  ...
  /**
   * Filter to only run tests with a title matching one of the patterns. For example, passing `grep: /cart/` should only
   * run tests with "cart" in the title. Also available in the [command line](https://copilotbrowser.dev/docs/test-cli) with the `-g` option. The
   ...
   * **TestConfig.grep**. Also available in the [command line](https://copilotbrowser.dev/docs/test-cli) with the `--grep-invert` option.
   *
   * `grepInvert` option is also useful for [tagging tests](https://copilotbrowser.dev/docs/test-annotations#tag-tests).
   *
   * **Usage**
   ...
   *
   * Also available in the [command line](https://copilotbrowser.dev/docs/test-cli) with the `--max-failures` and `-x` options.
   *
   * **Usage**
   *
   * ```js
   ...
   *
   * This directory is cleaned at the start. When running a test, a unique subdirectory inside the
   * **TestConfig.outputDir** is created, guaranteeing that test running in parallel do not conflict. This directory can
   * be accessed by **TestInfo.outputDir** and **TestInfo.outputPath()**.
   *
   * Here is an example that uses **TestInfo.outputPath()** to create a temporary file.
   *
   * ```js
  ...
  /**
   * **NOTE** Use **TestConfig.snapshotPathTemplate** to configure snapshot paths.
   *
   * The base directory, relative to the config file, for snapshot files created with `toMatchSnapshot`. Defaults to
   * **TestConfig.testDir**.
   *
   ...
   * [CI environments](https://copilotbrowser.dev/docs/test-sharding#merging-reports-from-multiple-environments).
   *
   * Note that each tag must start with `@` symbol. Learn more about [tagging](https://copilotbrowser.dev/docs/test-annotations#tag-tests).
   *
   * **Usage**
   *
   * ```js
   ...
   * **Test.setTimeout()**. Learn more about [various timeouts](https://copilotbrowser.dev/docs/test-timeouts).
   *
   * **Usage**
   *
   * ```js
   ...
   *
   * Learn more about [snapshots](https://copilotbrowser.dev/docs/test-snapshots).
   *
   * **Usage**
   *
   * ```js
   ...
   *
   * copilotbrowser Test uses worker processes to run tests. There is always at least one worker process, but more can
   * be used to speed up test execution.
   *
   * Defaults to half of the number of logical CPU cores. Learn more about
   * [parallelism and sharding](https://copilotbrowser.dev/docs/test-parallel) with copilotbrowser Test.
   *
   * **Usage**
   *
   * ```js
...
/**
 * Resolved configuration which is accessible via **TestInfo.config** and is passed to the test reporters. To see the
 * format of copilotbrowser configuration file, please see `TestConfig` instead.
 */
 ...
 * **Test.beforeEach()**, **Test.afterEach()**, **Test.beforeAll()** and **Test.afterAll()** hooks, and test-scoped
...
export interface TestInfo {
   ...
   * ```
   *
   * **NOTE** **TestInfo.attach()** automatically takes care of copying attached files to a location that is accessible
   * to reporters. You can safely remove the attachment after awaiting the attach call.
   *
  ...
  /**
   * Marks the currently running test as "should fail". copilotbrowser Test runs this test and ensures that it is
   * actually failing. This is useful for documentation purposes to acknowledge that some functionality is broken until
   * it is fixed. This is similar to **Test.fail()**.
   */
  fail(): void;
   ...
   * **Returns:** `string`
   *
   * Returns a path inside the **TestInfo.outputDir** where the test can safely put a temporary file. Guarantees that
   * tests running in parallel will not interfere with each other.
   *
   * ```js
   * import { test, expect } from '@copilotbrowser/copilotbrowser/test';
   * import fs from 'fs';
   ...
   * [various timeouts](https://copilotbrowser.dev/docs/test-timeouts).
   *
   * Timeout is usually specified in the [configuration file](https://copilotbrowser.dev/docs/test-configuration), but it could be useful to
   ...
   * timeout. This is similar to **Test.slow()**.
   ...
   * **Test.describe()** groups the test belongs to and file-level annotations for the test file.
   *
  ...
  /**
   * Processed configuration from the [configuration file](https://copilotbrowser.dev/docs/test-configuration).
   */
  config: FullConfig;

  /**
   * The number of milliseconds the test took to finish. Always zero before the test finishes, either successfully or
   * not. Can be used in **Test.afterEach()** hook.
   */
  duration: number;
  ...
  /**
   * Processed project configuration from the [configuration file](https://copilotbrowser.dev/docs/test-configuration).
   */
  project: FullProject;

  /**
   * Specifies a unique repeat index when running in "repeat each" mode. This mode is enabled by passing `--repeat-each`
   * to the [command line](https://copilotbrowser.dev/docs/test-cli).
   */
  repeatEachIndex: number;

  /**
   * Specifies the retry number when the test is retried after a failure. The first test run has **TestInfo.retry**
   * equal to zero, the first retry has it equal to one, and so on. Learn more about
   * [retries](https://copilotbrowser.dev/docs/test-retries#retries).
   *
   * ```js
   * import { test, expect } from '@copilotbrowser/copilotbrowser/test';
   *
  ...
  /**
   * **NOTE** Use of **TestInfo.snapshotSuffix** is discouraged. Please use **TestConfig.snapshotPathTemplate** to
   ...
   * about [snapshots](https://copilotbrowser.dev/docs/test-snapshots).
   */
  snapshotSuffix: string;

  /**
   * Actual status for the currently running test. Available after the test has finished in **Test.afterEach()** hook
   * and fixtures.
   *
  ...
  /**
   * Tags that apply to the test. Learn more about [tags](https://copilotbrowser.dev/docs/test-annotations#tag-tests).
   *
   ...
   * [various timeouts](https://copilotbrowser.dev/docs/test-timeouts).
   *
   * Timeout is usually specified in the [configuration file](https://copilotbrowser.dev/docs/test-configuration)
   *
   * ```js
   * import { test, expect } from '@copilotbrowser/copilotbrowser/test';
   *
   ...
   *
   * Also available as `process.env.TEST_WORKER_INDEX`. Learn more about [parallelism and sharding](https://copilotbrowser.dev/docs/test-parallel)
   * with copilotbrowser Test.
   * ---
...
export interface TestType<TestArgs extends {}, WorkerArgs extends {}> {
   ...
   * - in the [command line](https://copilotbrowser.dev/docs/test-cli#all-options);
   * - in the config with **TestConfig.grep** and **TestProject.grep**;
   *
   * Learn more about [tagging](https://copilotbrowser.dev/docs/test-annotations#tag-tests).
   *
   * **Annotations**
   *
   ...
   * @param body Test body that takes one or two arguments: an object with fixtures and optional `TestInfo`.
   */
   ...
   *
   * @param title Test title.
   * @param details See **Test.(call)()** for test details description.
   * @param body Test body that takes one or two arguments: an object with fixtures and optional `TestInfo`.
   */
   ...
   * @param callback A callback that is run immediately when calling **Test.describe()**. Any tests declared in this callback will
   * belong to the group.
   */
  describe: {
    /**
     ...
     *
     * @param title Group title.
     * @param details See **Test.describe()** for details description.
     * @param callback A callback that is run immediately when calling **Test.describe.only()**. Any tests added in this callback will
     * belong to the group.
     */
     ...
     * @param title Group title.
     * @param details See **Test.describe()** for details description.
     * @param callback A callback that is run immediately when calling **Test.describe.skip()**. Any tests added in this callback will
     * belong to the group, and will not be run.
     */
    ...
    /**
     * Declares a test group similarly to **Test.describe()**. Tests in this group are marked as "fixme" and will not be
     * executed.
     ...
     * @param title Group title.
     * @param details See **Test.describe()** for details description.
     * @param callback A callback that is run immediately when calling **Test.describe.fixme()**. Any tests added in this callback will
     * belong to the group, and will not be run.
     */
    ...
    /**
     * **NOTE** See **Test.describe.configure()** for the preferred way of configuring the execution mode.
     *
     ...
     * @param title Group title.
     * @param details See **Test.describe()** for details description.
     * @param callback A callback that is run immediately when calling **Test.describe.serial()**. Any tests added in this callback will
     * belong to the group.
     */
    serial: {
      /**
       ...
       * @param title Group title.
       * @param details See **Test.describe()** for details description.
       * @param callback A callback that is run immediately when calling **Test.describe.serial.only()**. Any tests added in this callback
     ...
     * another, but using **Test.describe.parallel()** allows them to run in parallel.
     ...
     * @param title Group title.
     * @param details See **Test.describe()** for details description.
     * @param callback A callback that is run immediately when calling **Test.describe.parallel()**. Any tests added in this callback will
     * belong to the group.
     */
    parallel: {
      /**
       ...
       * **NOTE** See **Test.describe.configure()** for the preferred way of configuring the execution mode.
       *
       * Declares a focused group of tests that could be run in parallel. This is similar to **Test.describe.parallel()**,
       ...
       * @param title Group title.
       * @param details See **Test.describe()** for details description.
       * @param callback A callback that is run immediately when calling **Test.describe.parallel.only()**. Any tests added in this callback
       * will belong to the group.
       */
      only(title: string, callback: () => void): void;
      /**
     ...
     *
     * Learn more about the execution modes [here](https://copilotbrowser.dev/docs/test-parallel).
     *
     * **Usage**
   ...
   * based on some condition. We recommend passing a `description` argument in this case. copilotbrowser will run the
   * test, but abort it immediately after the `test.skip` call.
   *
   * ```js
   * import { test, expect } from '@copilotbrowser/copilotbrowser/test';
   *
   ...
   * ```
   *
   * You can skip all tests in a file or **Test.describe()** group based on some condition with a single
   * `test.skip(callback, description)` call.
   *
   * ```js
   * import { test, expect } from '@copilotbrowser/copilotbrowser/test';
   *
  ...
  /**
   * Mark a test as "fixme", with the intention to fix it. copilotbrowser will not run the test past the `test.fixme()`
   * call.
   *
   ...
   * body based on some condition. We recommend passing a `description` argument in this case. copilotbrowser will run
   * the test, but abort it immediately after the `test.fixme` call.
   *
   * ```js
   * import { test, expect } from '@copilotbrowser/copilotbrowser/test';
   *
   ...
   * ```
   *
   * You can mark all tests in a file or **Test.describe()** group as "fixme" based on some condition with a single
   * `test.fixme(callback, description)` call.
   *
   * ```js
   * import { test, expect } from '@copilotbrowser/copilotbrowser/test';
   *
  ...
  /**
   * Marks a test as "should fail". copilotbrowser runs this test and ensures that it is actually failing. This is
   ...
   * ```
   *
   * You can mark all tests in a file or **Test.describe()** group as "should fail" based on some condition with a
   * single `test.fail(callback, description)` call.
   *
   * ```js
   * import { test, expect } from '@copilotbrowser/copilotbrowser/test';
   *
  ...
  fail: {
     ...
     *
     * @param title Test title.
     * @param details See **Test.describe()** for test details description.
     * @param body Test body that takes one or two arguments: an object with fixtures and optional `TestInfo`.
     */
    only(title: string, body: TestBody<TestArgs & WorkerArgs>): void;
    /**
   ...
   * ```
   *
   * You can mark all tests in a file or **Test.describe()** group as "slow" based on some condition by passing a
   * callback.
   *
   * ```js
   * import { test, expect } from '@copilotbrowser/copilotbrowser/test';
  ...
  /**
   * Changes the timeout for the test. Zero means no timeout. Learn more about [various timeouts](https://copilotbrowser.dev/docs/test-timeouts).
   *
   ...
   * **Test.describe()** group, runs before each test in the group.
   *
   * You can access all the same `Fixtures` as the test body itself, and also the `TestInfo` object that gives a lot of
   ...
   *
   * When called in the scope of a test file, runs after each test in the file. When called inside a **Test.describe()**
   * group, runs after each test in the group.
   *
   * You can access all the same `Fixtures` as the test body itself, and also the `TestInfo` object that gives a lot of
   ...
   * **Test.describe()** group, runs before all tests in the group.
   *
   ...
   * more about [workers and failures](https://copilotbrowser.dev/docs/test-retries).
   *
   * copilotbrowser will continue running all applicable hooks even if some of them have failed.
   *
   * **Usage**
   ...
   *
   * When called in the scope of a test file, runs after all tests in the file. When called inside a **Test.describe()**
   * group, runs after all tests in the group.
   *
   * **Details**
   *
  ...
  /**
   * Specifies options or fixtures to use in a single test file or a **Test.describe()** group. Most useful to set an
  ...
  step: {
     ...
     * planned for a near-term fix. copilotbrowser will not run the step. See also **TestStepInfo.skip()**.
     *
  ...
  /**
   * `expect` function can be used to create test assertions. Read more about [test assertions](https://copilotbrowser.dev/docs/test-assertions).
   *
   * **Usage**
   *
   * ```js
   ...
   * ```
   *
   * Learn more about [fixtures](https://copilotbrowser.dev/docs/test-fixtures) and [parametrizing tests](https://copilotbrowser.dev/docs/test-parameterize).
   * @param fixtures An object containing fixtures and/or options. Learn more about [fixtures format](https://copilotbrowser.dev/docs/test-fixtures).
   */
 ...
 *
 * These options are usually provided in the [configuration file](https://copilotbrowser.dev/docs/test-configuration) through **TestConfig.use**
 * and **TestProject.use**.
 *
 * ```js
 * // copilotbrowser.config.ts
...
export interface copilotbrowserWorkerOptions {
  ...
  /**
   * Options used to launch the browser, as passed to **BrowserType.launch()**. Specific options
   * **TestOptions.headless** and **TestOptions.channel** take priority over this.
   *
   ...
   * ```
   *
   * When connect options are specified, default **Fixtures.browser**, **Fixtures.context** and **Fixtures.page** use
   * the remote browser instead of launching a browser locally, and any launch options like **TestOptions.headless** or
   * **TestOptions.channel** are ignored.
   */
   ...
   * ```
   *
   * Learn more about [automatic screenshots](https://copilotbrowser.dev/docs/test-use-options#recording-options).
   */
   ...
   * ```
   *
   * Learn more about [recording trace](https://copilotbrowser.dev/docs/test-use-options#recording-options).
   */
   ...
   * equal to **TestOptions.viewport** scaled down to fit into 800x800. If `viewport` is not configured explicitly the
   * video size defaults to 800x450. Actual picture of each page will be scaled down if necessary to fit the specified
   * size.
   *
   * **Usage**
   *
   ...
   * ```
   *
   * Learn more about [recording video](https://copilotbrowser.dev/docs/test-use-options#recording-options).
   */
...
export interface copilotbrowserTestOptions {
   ...
   * ```
   *
   * Learn more about [geolocation](https://copilotbrowser.dev/docs/emulation#color-scheme-and-media).
   */
   ...
   * about emulation in our [emulation guide](https://copilotbrowser.dev/docs/emulation#locale--timezone).
   *
   * **Usage**
   *
   * ```js
  ...
  /**
   * Options used to create the context, as passed to **Browser.newContext()**. Specific options like
   * **TestOptions.viewport** take priority over this.
   *
   * **Usage**
   *
   * ```js
   ...
   * ```
   *
   * Learn more about [various timeouts](https://copilotbrowser.dev/docs/test-timeouts).
   */
...
/**
 * copilotbrowser Test is based on the concept of the [test fixtures](https://copilotbrowser.dev/docs/test-fixtures). Test fixtures are used to
 * establish environment for each test, giving the test everything it needs and nothing else.
 *
 * copilotbrowser Test looks at each test declaration, analyses the set of fixtures the test needs and prepares those
 ...
 * ```
 *
 * Given the test above, copilotbrowser Test will set up the `page` fixture before running the test, and tear it down
 * after the test has finished. `page` fixture provides a `Page` object that is available to the test.
 *
 * copilotbrowser Test comes with builtin fixtures listed below, and you can add your own fixtures as well.
 * copilotbrowser Test also [provides options]`TestOptions` to  configure **Fixtures.browser**, **Fixtures.context**
 * and **Fixtures.page**.
 */
export interface copilotbrowserWorkerArgs {
   ...
   *
   * Learn how to [configure browser](https://copilotbrowser.dev/docs/test-configuration) and see [available options]`TestOptions`.
   *
   * **Usage**
   *
   * ```js
...
export interface copilotbrowserTestArgs {
   ...
   *
   * Learn how to [configure context](https://copilotbrowser.dev/docs/test-configuration) and see [available options]`TestOptions`.
   *
  ...
  /**
   * Isolated `Page` instance, created for each test. Pages are isolated between tests due to **Fixtures.context**
   * isolation.
   *
...
interface AsymmetricMatchers {
  /**
   * `expect.any()` matches any object instance created from the **constructor** or a corresponding primitive type. Use
   * it inside **GenericAssertions.toEqual()** to perform pattern matching.
   *
   * **Usage**
   *
   * ```js
  ...
  /**
   * `expect.anything()` matches everything except `null` and `undefined`. Use it inside **GenericAssertions.toEqual()**
  ...
  /**
   * `expect.arrayOf()` matches array of objects created from the **constructor** or a corresponding primitive type. Use
   * it inside **GenericAssertions.toEqual()** to perform pattern matching.
   *
   * **Usage**
   *
  ...
  /**
   * Compares floating point numbers for approximate equality. Use this method inside **GenericAssertions.toEqual()** to
   * perform pattern matching. When just comparing two numbers, prefer **GenericAssertions.toBeCloseTo()**.
   *
   * **Usage**
   *
   * ```js
   ...
   *
   * Use this method inside **GenericAssertions.toEqual()** to perform pattern matching. Object properties can be
   ...
   * **GenericAssertions.toEqual()** to perform pattern matching.
   *
   * **Usage**
   *
   * ```js
   ...
   * inside **GenericAssertions.toEqual()** to perform pattern matching.
   *
   * **Usage**
   *
   * ```js
 ...
 * **copilotbrowserAssertions.expectGeneric()**:
 *
 * ```js
 * import { test, expect } from '@copilotbrowser/copilotbrowser/test';
 *
...
interface GenericAssertions<R> {
  ...
  /**
   * Compares floating point numbers for approximate equality. Use this method instead of **GenericAssertions.toBe()**
   ...
   * performed by **GenericAssertions.toContain()**.
   *
   ...
   *
   * For objects, this method recursively checks equality of all fields, rather than comparing objects by reference as
   * performed by **GenericAssertions.toBe()**.
   *
   ...
   * **Non-strict equality**
   *
   * **GenericAssertions.toEqual()** performs deep equality check that compares contents of the received and expected
   * values. To ensure two objects reference the same instance, use **GenericAssertions.toBe()** instead.
   *
   * **GenericAssertions.toEqual()** ignores `undefined` properties and array items, and does not insist on object types
   * being equal. For stricter matching, use **GenericAssertions.toStrictEqual()**.
   *
   * **Pattern matching**
   *
   * **GenericAssertions.toEqual()** can be also used to perform pattern matching on objects, arrays and primitive
   ...
   * the **expected**. Equality is checked recursively, similarly to **GenericAssertions.toEqual()**.
   *
   * **Usage**
   *
   * ```js
   ...
   * properties to be present in the value, unlike **GenericAssertions.toEqual()**, so you can check just a subset of
   * object properties.
   *
...
interface LocatorAssertions {
  ...
  /**
   * Ensures the `Locator` resolves to an element with the given computed CSS properties. Only the listed properties are
   * checked.
   *
   * **Usage**
   *
  ...
  toBeChecked(options?: {
     ...
     * **LocatorAssertions.toBeChecked.indeterminate** is set to true.
     */
    checked?: boolean;
   ...
   * [non-visible](https://copilotbrowser.dev/docs/actionability#visible) one.
   *
   * **Usage**
   *
   * ```js
  ...
  /**
   * Ensures that `Locator` points to an attached and [visible](https://copilotbrowser.dev/docs/actionability#visible) DOM node.
   *
   ...
   * **Details**
   *
   * When `expected` parameter is a string, copilotbrowser will normalize whitespaces and line breaks both in the actual
  ...
  /**
   * Asserts that the target element matches the given [accessibility snapshot](https://copilotbrowser.dev/docs/aria-snapshots).
   *
   * **Usage**
   *
   * ```js
...
interface PageAssertions {
   ...
   * ```
   *
   * @param url Expected URL string, RegExp, or predicate receiving `URL` to match. When **Browser.newContext.baseURL** is provided
   * via the context options and the `url` argument is a string, the two values are merged via the
...
interface SnapshotAssertions {
  /**
   * **NOTE** To compare screenshots, use **PageAssertions.toHaveScreenshot()** instead.
   *
   ...
   * ```
   *
   * Learn more about [visual comparisons](https://copilotbrowser.dev/docs/test-snapshots).
   *
...
export interface TestStepInfo {
   ...
   * **path** or **body** must be specified, but not both. Calling this method will attribute the attachment to the
   * step, as opposed to **TestInfo.attach()** which stores all attachments at the test level.
   *
   ...
   * ```
   *
   * **NOTE** **TestStepInfo.attach()** automatically takes care of copying attached files to a location that is
   * accessible to reporters. You can safely remove the attachment after awaiting the attach call.
   *
   * @param name Attachment name. The name will also be sanitized and used as the prefix of file name when saving to disk.
   * @param options
   */
