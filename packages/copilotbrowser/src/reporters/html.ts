...
`);
  ...
  _resolveOptions(): { outputFolder: string, open: HtmlReportOpenOption, attachmentsBaseURL: string, host: string | undefined, port: number | undefined } {
    const outputFolder = reportFolderFromEnv() ?? resolveReporterOutputPath('copilotbrowser-report', this._options.configDir, this._options.outputFolder);
    return {
      outputFolder,
      open: getHtmlReportOptionProcessEnv() || this._options.open || 'on-failure',
      attachmentsBaseURL: process.env.copilotbrowser_HTML_ATTACHMENTS_BASE_URL || this._options.attachmentsBaseURL || 'data/',
      host: process.env.copilotbrowser_HTML_HOST || this._options.host,
      port: process.env.copilotbrowser_HTML_PORT ? +process.env.copilotbrowser_HTML_PORT : this._options.port,
    };
  }
  ...
  async onEnd(result: api.FullResult) {
    ...
    if (process.env.copilotbrowser_HTML_NO_SNIPPETS === 'false' || process.env.copilotbrowser_HTML_NO_SNIPPETS === '0')
      noSnippets = false;
    else if (process.env.copilotbrowser_HTML_NO_SNIPPETS)
      noSnippets = true;
    noSnippets = noSnippets || this._options.noSnippets;

    let noCopyPrompt: boolean | undefined;
    if (process.env.copilotbrowser_HTML_NO_COPY_PROMPT === 'false' || process.env.copilotbrowser_HTML_NO_COPY_PROMPT === '0')
      noCopyPrompt = false;
    else if (process.env.copilotbrowser_HTML_NO_COPY_PROMPT)
      noCopyPrompt = true;
    noCopyPrompt = noCopyPrompt || this._options.noCopyPrompt;

    const builder = new HtmlBuilder(this.config, this._outputFolder, this._attachmentsBaseURL, {
      title: process.env.copilotbrowser_HTML_TITLE || this._options.title,
      noSnippets,
      noCopyPrompt,
    });
  ...
  ${packageManagerCommand} copilotbrowser show-report${relativeReportPath}${hostArg}${portArg}
`));
    }
  }
}
...
function reportFolderFromEnv(): string | undefined {
  // Note: copilotbrowser_HTML_REPORT is for backwards compatibility.
  const envValue = process.env.copilotbrowser_HTML_OUTPUT_DIR || process.env.copilotbrowser_HTML_REPORT;
  return envValue ? path.resolve(envValue) : undefined;
}

function getHtmlReportOptionProcessEnv(): HtmlReportOpenOption | undefined {
  // Note: CB_TEST_HTML_REPORT_OPEN is for backwards compatibility.
  const htmlOpenEnv = process.env.copilotbrowser_HTML_OPEN || process.env.CB_TEST_HTML_REPORT_OPEN;
  if (!htmlOpenEnv)
    return undefined;
  if (!isHtmlReportOption(htmlOpenEnv)) {
    writeLine(colors.red(`Configuration Error: HTML reporter Invalid value for copilotbrowser_HTML_OPEN: ${htmlOpenEnv}. Valid values are: ${htmlReportOptions.join(', ')}`));
    return undefined;
  }
  return htmlOpenEnv;
}
...
function standaloneDefaultFolder(): string {
  return reportFolderFromEnv() ?? resolveReporterOutputPath('copilotbrowser-report', process.cwd(), undefined);
}
...
class HtmlBuilder {
  ...
  private async _writeReportData(filePath: string) {
    fs.appendFileSync(filePath, '<script id="copilotbrowserReportBase64" type="application/zip">data:application/zip;base64,');
    await new Promise(f => {
