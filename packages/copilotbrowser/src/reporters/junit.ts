...
class JUnitReporter implements ReporterV2 {
  ...
  constructor(options: JUnitReporterOptions & CommonReporterOptions) {
    this.stripANSIControlSequences = getAsBooleanFromENV('copilotbrowser_JUNIT_STRIP_ANSI', !!options.stripANSIControlSequences);
    this.includeProjectInTestName = getAsBooleanFromENV('copilotbrowser_JUNIT_INCLUDE_PROJECT_IN_TEST_NAME', !!options.includeProjectInTestName);
  ...
  async onEnd(result: FullResult) {
    ...
    const root: XMLEntry = {
      name: 'testsuites',
      attributes: {
        id: process.env[`copilotbrowser_JUNIT_SUITE_ID`] || '',
        name: process.env[`copilotbrowser_JUNIT_SUITE_NAME`] || '',
