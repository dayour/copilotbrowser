...
import { createcopilotbrowser } from '../../copilotbrowser';
...
export async function openTraceViewerApp(url: string, browserName: string, options?: TraceViewerAppOptions): Promise<Page> {
  const traceViewercopilotbrowser = createcopilotbrowser({ sdkLanguage: 'javascript', isInternalcopilotbrowser: true });
  const traceViewerBrowser = isUnderTest() ? 'chromium' : browserName;

  const { context, page } = await launchApp(traceViewercopilotbrowser[traceViewerBrowser as 'chromium'], {
  ...
  await controller.run(async progress => {
    ...
    if (process.env.CBTEST_PRINT_WS_ENDPOINT) {
