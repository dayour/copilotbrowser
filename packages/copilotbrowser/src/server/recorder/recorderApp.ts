...
export class RecorderApp {
  ...
  static async show(context: BrowserContext, params: channels.BrowserContextEnableRecorderParams) {
    if (process.env.CB_CODEGEN_NO_INSPECTOR)
      return;
  ...
  private static async _show(recorder: Recorder, inspectedContext: BrowserContext, params: channels.BrowserContextEnableRecorderParams) {
    ...
    const recordercopilotbrowser = (require('../copilotbrowser').createcopilotbrowser as typeof import('../copilotbrowser').createcopilotbrowser)({ sdkLanguage: 'javascript', isInternalcopilotbrowser: true });
    const { context: appContext, page } = await launchApp(recordercopilotbrowser.chromium, {
      sdkLanguage,
      windowSize: { width: 600, height: 600 },
      windowPosition: { x: 1020, y: 10 },
      persistentContextOptions: {
        noDefaultViewport: true,
        headless: !!process.env.CBTEST_CLI_HEADLESS || (isUnderTest() && !headed),
