...
const ARTIFACTS_FOLDER = path.join(os.tmpdir(), 'copilotbrowser-artifacts-');
...
export class AndroidDevice extends SdkObject {
  ...
  private async _installDriver(progress: Progress): Promise<PipeTransport> {
    debug('cb:android')('Stopping the old driver');
    ...
    if (!this._options.omitDriverInstall) {
      debug('cb:android')('Uninstalling the old driver');
      ...
      debug('cb:android')('Installing the new driver');
      ...
      debug('cb:android')('Skipping the driver installation');
    }

    debug('cb:android')('Starting the new driver');
    this.shell('am instrument -w com.microsoft.copilotbrowser.androiddriver.test/androidx.test.runner.AndroidJUnitRunner').catch(e => debug('cb:android')(e));
  ...
  private async _waitForLocalAbstract(progress: Progress, socketName: string): Promise<SocketBackend> {
    ...
    debug('cb:android')(`Polling the socket localabstract:${socketName}`);
    while (!socket) {
      try {
    ...
    debug('cb:android')(`Connected to localabstract:${socketName}`);
    return socket;
  }
  ...
  async launchBrowser(progress: Progress, pkg: string = 'com.android.chrome', options: channels.AndroidDeviceLaunchBrowserParams): Promise<BrowserContext> {
    debug('cb:android')('Force-stopping', pkg);
    await this._backend.runCommand(`shell:am force-stop ${pkg}`);
    const socketName = isUnderTest() ? 'webview_devtools_remote_copilotbrowser_test' : ('copilotbrowser_' + createGuid() + '_devtools_remote');
    const commandLine = this._defaultArgs(options, socketName).join(' ');
    debug('cb:android')('Starting', pkg, commandLine);
    ...
    try {
      ...
      const cleanupArtifactsDir = async () => {
        ...
        for (let i = 0; i < (errors || []).length; ++i)
          debug('cb:android')(`exception while removing ${artifactsDir}: ${errors[i]}`);
      };
      gracefullyCloseSet.add(cleanupArtifactsDir);
      socket.on('close', async () => {
        gracefullyCloseSet.delete(cleanupArtifactsDir);
        cleanupArtifactsDir().catch(e => debug('cb:android')(`could not cleanup artifacts dir: ${e}`));
      });
      ...
      const browser = await progress.race(CRBrowser.connect(this.attribution.copilotbrowser, androidBrowser, browserOptions));
