...
async function runCommand(command: string, host: string = '127.0.0.1', port: number = 5037, serial?: string): Promise<Buffer> {
  debug('cb:adb:runCommand')(command, serial);
...
class BufferedSocketWrapper extends EventEmitter implements SocketBackend {
  ...
  constructor(command: string, socket: net.Socket) {
    ...
    this._socket.on('data', data => {
      debug('cb:adb:data')(data.toString());
      if (this._isSocket) {
        this.emit('data', data);
        return;
      }
      this._buffer = Buffer.concat([this._buffer, data as Buffer]);
  ...
  async write(data: Buffer) {
    debug('cb:adb:send')(data.toString().substring(0, 100) + '...');
  ...
  close() {
    if (this._isClosed)
      return;
    debug('cb:adb')('Close ' + this._command);
  ...
  async read(length: number): Promise<Buffer> {
    ...
    debug('cb:adb:recv')(result.toString().substring(0, 100) + '...');
    return result;
  }
