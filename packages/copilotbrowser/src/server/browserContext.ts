...
export abstract class BrowserContext<EM extends EventMap = EventMap> extends SdkObject<BrowserContextEventMap | EM> {
  ...
  private _copilotbrowserBindingExposed?: Promise<void>;
  ...
  constructor(browser: Browser, options: types.BrowserContextOptions, browserContextId: string | undefined) {
    ...
    this._devtools = new DevToolsController(this as any);

    this.fetchRequest = new BrowserContextAPIRequestContext(this as any);
    this.tracing = new Tracing(this, browser.options.tracesDir);
    this.clock = new Clock(this as any);
  ...
  async _initialize() {
    ...
    this._debugger = new Debugger(this as any);

    // When PWDEBUG=1, show inspector for each context.
    if (debugMode() === 'inspector')
      await RecorderApp.show(this as any, { pauseOnNextStatement: true });

    // When paused, show inspector.
    if (this._debugger.isPaused())
      RecorderApp.showInspectorNoReply(this as any);
  ...
  private _didCloseInternal() {
    ...
    (this as any).emit(BrowserContext.Events.Close);
  }
  ...
  async exposeBinding(progress: Progress, name: string, needsHandle: boolean, copilotbrowserBinding: frames.FunctionWithSource, forClient?: unknown): Promise<PageBinding> {
  ...
  async close(options: { reason?: string }) {
    if (this._closedStatus === 'open') {
      ...
      (this as any).emit(BrowserContext.Events.BeforeClose);
  ...
  async extendInjectedScript(source: string, arg?: any) {
    ...
    this.on(BrowserContext.Events.Page, installInPage as any);
  ...
  harStart(page: Page | null, options: channels.RecordHarOptions): string {
    ...
    this._harRecorders.set(harId, new HarRecorder(this as any, page, options));
    return harId;
  }
