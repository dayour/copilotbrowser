...
export function getUserAgent(): string {
  ...
  try {
    ...
    cachedUserAgent = 'copilotbrowser/unknown';
  }
  return cachedUserAgent;
}
...
function determineUserAgent(): string {
  ...
  return `copilotbrowser/${getcopilotbrowserVersion()} (${os.arch()}; ${osIdentifier} ${osVersion}) ${embedderName}/${embedderVersion}${serializedTokens}`;
}
...
export function getEmbedderName(): { embedderName: string, embedderVersion: string } {
  ...
  } else if (['node', 'python', 'java', 'csharp'].includes(process.env.CB_LANG_NAME)) {
    embedderName = process.env.CB_LANG_NAME;
    embedderVersion = process.env.CB_LANG_NAME_VERSION ?? 'unknown';
  }
  return { embedderName, embedderVersion };
}

export function getcopilotbrowserVersion(majorMinorOnly = false): string {
  const version = process.env.CB_VERSION_OVERRIDE || require('./../../../package.json').version;
