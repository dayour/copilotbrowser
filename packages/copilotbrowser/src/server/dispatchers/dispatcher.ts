...
export class RootDispatcher extends Dispatcher<SdkObject, any, any> {
  ...
  constructor(connection: DispatcherConnection, private readonly createcopilotbrowser?: (scope: RootDispatcher, options: channels.RootInitializeParams) => Promise<copilotbrowserDispatcher>) {
  ...
  async initialize(params: channels.RootInitializeParams, progress: Progress): Promise<channels.RootInitializeResult> {
    ...
    assert(this.createcopilotbrowser);
