...
if (process.env.CB_TEST_CDN_THAT_SHOULD_WORK) {
  for (let i = 0; i < copilotbrowser_CDN_MIRRORS.length; i++) {
    const cdn = copilotbrowser_CDN_MIRRORS[i];
    if (cdn !== process.env.CB_TEST_CDN_THAT_SHOULD_WORK) {
...
function cftUrl(suffix: string): DownloadPathFunction {
  return ({ browserVersion }) => {
    const customHost = getFromENV('copilotbrowser_CHROMIUM_DOWNLOAD_HOST') || getFromENV('copilotbrowser_DOWNLOAD_HOST');
    if (customHost) {
      return {
...
export const registryDirectory = (() => {
  ...
  const envDefined = getFromENV('copilotbrowser_BROWSERS_PATH');
  if (envDefined === '0') {
    ...
    result = path.join(cacheDirectory, 'ms-copilotbrowser');
  }
...
export class Registry {
  ...
  async install(executablesToInstall: Executable[], options?: { force?: boolean }) {
    ...
    try {
      ...
      if (!getAsBooleanFromENV('copilotbrowser_SKIP_BROWSER_GC'))
  ...
  async validateHostRequirementsForExecutablesIfNeeded(executables: Executable[], sdkLanguage: string) {
    if (getAsBooleanFromENV('copilotbrowser_SKIP_VALIDATE_HOST_REQUIREMENTS')) {
      // eslint-disable-next-line no-restricted-properties
  ...
  private _downloadURLs(descriptor: BrowsersJSONDescriptor): string[] {
    ...
    const customHostOverride = (downloadHostEnv && getFromENV(downloadHostEnv)) || getFromENV('copilotbrowser_DOWNLOAD_HOST');
  ...
  private async _downloadExecutable(descriptor: BrowsersJSONDescriptor, force: boolean, executablePath?: string) {
    ...
    if (!downloadURLs.length)
      throw new Error(`ERROR: copilotbrowser does not support ${descriptor.name} on ${hostPlatform}`);
    if (!isOfficiallySupportedPlatform)
      logPolitely(`BEWARE: your OS is not officially supported by copilotbrowser; downloading fallback build for ${hostPlatform}.`);
    ...
    const downloadFileName = `copilotbrowser-download-${descriptor.name}-${hostPlatform}-${descriptor.revision}.zip`;
    ...
    const downloadSocketTimeoutEnv = getFromENV('copilotbrowser_DOWNLOAD_CONNECTION_TIMEOUT');
...
export function buildcopilotbrowserCLICommand(sdkLanguage: string, parameters: string): string {
  switch (sdkLanguage) {
    case 'python':
...
export async function installBrowsersForNpmInstall(browsers: string[]) {
  ...
  if (getAsBooleanFromENV('copilotbrowser_SKIP_BROWSER_DOWNLOAD')) {
    logPolitely('Skipping browsers download because `copilotbrowser_SKIP_BROWSER_DOWNLOAD` env variable is set');
    return false;
  }
  const executables: Executable[] = [];
  if (process.platform === 'win32')
