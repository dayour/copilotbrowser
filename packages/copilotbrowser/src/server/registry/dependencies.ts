...
import { getcopilotbrowserVersion } from '../utils/userAgent';

import { buildcopilotbrowserCLICommand, registry } from '.';

const BIN_DIRECTORY = path.join(__dirname, '..', '..', '..', 'bin');
const languageBindingVersion = process.env.CB_CLI_DISPLAY_VERSION || require('../../../package.json').version;
...
export async function installDependenciesLinux(targets: Set<DependencyGroup>, dryRun: boolean) {
  ...
  for (const target of targets) {
    ...
    if (!info) {
      console.warn(`Cannot install dependencies for ${platform} with copilotbrowser ${getcopilotbrowserVersion()}!`);  // eslint-disable-line no-console
      return;
    }
...
export async function validateDependenciesLinux(sdkLanguage: string, linuxLddDirectories: string[], dlOpenLibraries: string[]) {
  ...
  if (dockerInfo && !dockerInfo.driverVersion.startsWith(getcopilotbrowserVersion(true /* majorMinorOnly */) + '.')) {
    // We are running in a docker container with unmatching version.
    // In this case, we know how to install dependencies in it.
    const pwVersion = getcopilotbrowserVersion();
    ...
    errorLines.push(...[
      ...
      ``,
      `    ${maybeSudo}${buildcopilotbrowserCLICommand(sdkLanguage, 'install-deps')}`,
      ``,
