...
export class WKBrowser extends Browser {
  ...
  _onDownloadCreated(payload: Protocol.copilotbrowser.downloadCreatedPayload) {
  ...
  _onDownloadFilenameSuggested(payload: Protocol.copilotbrowser.downloadFilenameSuggestedPayload) {
    this._downloadFilenameSuggested(payload.uuid, payload.suggestedFilename);
  }

  _onDownloadFinished(payload: Protocol.copilotbrowser.downloadFinishedPayload) {
    this._downloadFinished(payload.uuid, payload.error);
  }

  _onPageProxyCreated(event: Protocol.copilotbrowser.pageProxyCreatedPayload) {
  ...
  _onPageProxyDestroyed(event: Protocol.copilotbrowser.pageProxyDestroyedPayload) {
    const pageProxyId = event.pageProxyId;
  ...
  _onProvisionalLoadFailed(event: Protocol.copilotbrowser.provisionalLoadFailedPayload) {
    const wkPage = this._wkPages.get(event.pageProxyId);
    if (!wkPage)
      return;
  ...
  _onWindowOpen(event: Protocol.copilotbrowser.windowOpenPayload) {
    const wkPage = this._wkPages.get(event.pageProxyId);
    if (!wkPage)
      return;
...
export class WKBrowserContext extends BrowserContext {
  ...
  async addCookies(cookies: channels.SetNetworkCookie[]) {
    const cc = network.rewriteCookies(cookies).map(c => {
      const { name, value, domain, path, expires, httpOnly, secure, sameSite } = c;
      const copy: Protocol.copilotbrowser.SetCookieParam = {
        name,
        value,
        domain: domain!,
        path: path!,
  ...
  override async doExposecopilotbrowserBinding() {
    for (const page of this.pages())
      await (page.delegate as WKPage).exposecopilotbrowserBinding();
  }
