...
export class DebugController extends SdkObject {
  ...
  constructor(copilotbrowser: copilotbrowser) {
    super({ attribution: { isInternalcopilotbrowser: true }, instrumentation: createInstrumentation() } as any, undefined, 'DebugController');
  ...
  async setRecorderMode(progress: Progress, params: { mode: Mode, testIdAttributeName?: string, generateAutoExpect?: boolean }) {
    ...
    if (!this._copilotbrowser.allBrowsers().length)
      await this._copilotbrowser.chromium.launch(progress, { headless: !!process.env.CB_DEBUG_CONTROLLER_HEADLESS });
    // Create page if none.
  ...
  async highlight(progress: Progress, params: { selector?: string, ariaTemplate?: string }) {
    ...
    const ariaTemplate = params.ariaTemplate ? parseAriaSnapshotUnsafe(yaml as any, params.ariaTemplate) : undefined;
    for (const recorder of await progress.race(this._allRecorders())) {
      if (ariaTemplate)
...
function wireListeners(recorder: Recorder, debugController: DebugController) {
  ...
  const languageGenerator = new JavaScriptLanguageGenerator(/* iscopilotbrowserTest */true);
