...
import { assert, calculateSha1, getcopilotbrowserVersion, isURLAvailable } from '@copilotbrowser/copilotbrowser/lib/utils';
import { colors, debug } from '@copilotbrowser/copilotbrowser/lib/utilsBundle';
import { setExternalDependencies } from '@copilotbrowser/copilotbrowser/lib/transform/compilationCache';
import { resolveHook } from '@copilotbrowser/copilotbrowser/lib/transform/transform';
import { removeDirAndLogToConsole } from '@copilotbrowser/copilotbrowser/lib/util';
import { stoppable } from '@copilotbrowser/copilotbrowser/lib/utilsBundle';
...
// Source rollup's plugin types from vite, which vendors its own copy of them.
// Importing from 'rollup' directly yields a distinct, incompatible type identity.
...
const log = debug('cb:vite');

let stoppableServer: any;
const copilotbrowserVersion = getcopilotbrowserVersion();

export function createPlugin(): TestRunnerPlugin {
  let configDir: string;
  let config: FullConfig;
  return {
    name: 'copilotbrowser-vite-plugin',
    ...
    begin: async (suite: Suite) => {
      ...
      if (isAddressInfo(address)) {
        ...
        process.env.copilotbrowser_TEST_BASE_URL = `${protocol}//${viteConfig.preview.host}:${address.port}`;
      }
    },
...
export async function buildBundle(config: FullConfig, configDir: string): Promise<{ buildInfo: BuildInfo, viteConfig: Record<string, any> } | null> {
  const { registerSourceFile, frameworkPluginFactory } = frameworkConfig(config);
  {
    ...
    if (await isURLAvailable(url, true)) {
      ...
      process.env.copilotbrowser_TEST_BASE_URL = url.toString();
      return null;
    }
  }
  ...
  if (!dirs) {
    // eslint-disable-next-line no-console
    console.log(colors.red(`Component testing template file copilotbrowser/index.html is missing and there is no existing Vite server. Component tests will fail.\n`));
    return null;
  }
...
function vitePlugin(registerSource: string, templateDir: string, buildInfo: BuildInfo, importInfos: Map<string, ImportInfo>, depsCollector: Map<string, string[]>): Plugin {
  ...
  return {
    name: 'copilotbrowser:component-index',

    configResolved(config: ResolvedConfig) {
      moduleResolver = config.createResolver();
    },

    async transform(this: Rollup.TransformPluginContext, content, id) {
