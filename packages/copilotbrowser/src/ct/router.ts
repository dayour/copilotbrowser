...
import type * as copilotbrowser from '@copilotbrowser/copilotbrowser/test';

interface RequestHandler {
  run(args: { request: Request, requestId?: string, resolutionContext?: { baseUrl?: string } }): Promise<{ response?: Response } | null>;
}

type RouteArgs = Parameters<copilotbrowser.BrowserContext['route']>;

let lastRequestId = 0;
let fetchOverrideCounter = 0;
const currentlyInterceptingInContexts = new Map<copilotbrowser.BrowserContext, number>();
...
export class Router {
  private _context: copilotbrowser.BrowserContext;
  private _requestHandlers: RequestHandler[] = [];
  private _requestHandlersRoute: (route: copilotbrowser.Route) => Promise<void>;
  private _requestHandlersActive = false;
  private _routes: RouteArgs[] = [];

  constructor(context: copilotbrowser.BrowserContext, baseURL: string | undefined) {
