...
import { getUserData } from '@copilotbrowser/copilotbrowser/lib/transform/compilationCache';
...
import type { copilotbrowserTestConfig as BasecopilotbrowserTestConfig } from '@copilotbrowser/copilotbrowser/types/test';
import type { FullConfig } from '@copilotbrowser/copilotbrowser/types/testReporter';
import type { InlineConfig, Plugin, TransformResult, UserConfig } from 'vite';


const log = debug('cb:vite');

export type CtConfig = BasecopilotbrowserTestConfig['use'] & {
  ctPort?: number;
...
export async function resolveDirs(configDir: string, config: FullConfig): Promise<ComponentDirs | null> {
  ...
  // This regressed in https://github.com/dayour/copilotbrowser/pull/26526
...
export async function createConfig(dirs: ComponentDirs, config: FullConfig, frameworkPluginFactory: (() => Promise<Plugin>) | undefined, supportJsxInJs: boolean) {
  ...
  // Compose base config from the copilotbrowser config only.
...
export async function populateComponentsFromTests(componentRegistry: ComponentRegistry, componentsByImportingFile?: Map<string, string[]>) {
  const importInfos: Map<string, ImportInfo[]> = await getUserData('copilotbrowser-ct-core');
...
export function frameworkConfig(config: FullConfig): { registerSourceFile: string, frameworkPluginFactory?: () => Promise<Plugin> } {
  return (config as any)['copilotbrowser/ct/core'];
}
