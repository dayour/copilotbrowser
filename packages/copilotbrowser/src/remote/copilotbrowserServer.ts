...
import { copilotbrowserConnection, copilotbrowserInitializeResult } from './copilotbrowserConnection';
import { createcopilotbrowser } from '../server/copilotbrowser';
import { Semaphore } from '../utils/isomorphic/semaphore';
import { DEFAULT_copilotbrowser_LAUNCH_TIMEOUT } from '../utils/isomorphic/time';
import { WSServer } from '../server/utils/wsServer';
import { wrapInASCIIBox } from '../server/utils/ascii';
import { getcopilotbrowserVersion } from '../server/utils/userAgent';
...
import type { copilotbrowser } from '../server/copilotbrowser';
...
export class copilotbrowserServer {
  private _copilotbrowser: copilotbrowser;
  ...
  constructor(options: ServerOptions) {
    ...
    this._copilotbrowser ??= createcopilotbrowser({ sdkLanguage: 'javascript', isServer: true });
    ...
    this._wsServer = new WSServer({
      ...
      onHeaders: headers => {
        if (process.env.CBTEST_SERVER_WS_HEADERS)
          headers.push(process.env.CBTEST_SERVER_WS_HEADERS!);
      },
      ...
      onConnection: (request, url, ws, id) => {
        ...
        let launchOptions: LaunchOptionsWithTimeout = { timeout: DEFAULT_copilotbrowser_LAUNCH_TIMEOUT };
        try {
          launchOptions = JSON.parse(launchOptionsParam || launchOptionsHeaderValue);
          if (!launchOptions.timeout)
            launchOptions.timeout = DEFAULT_copilotbrowser_LAUNCH_TIMEOUT;
        } catch (e) {
        }
        ...
        if (isExtension) {
          ...
          if (connectFilter) {
            ...
            return new copilotbrowserConnection(
                browserSemaphore,
                ws,
                false,
                this._copilotbrowser,
  ...
  private async _initReuseBrowsersMode(browserName: string | null, launchOptions: LaunchOptionsWithTimeout, id: string): Promise<copilotbrowserInitializeResult> {
    ...
    let browser = this._copilotbrowser.allBrowsers().find(b => {
      ...
      if (this._dontReuseBrowsers.has(b))
        return false;
      const existingOptions = launchOptionsHash({ ...b.options.originalLaunchOptions, timeout: DEFAULT_copilotbrowser_LAUNCH_TIMEOUT });
    ...
    if (!browser) {
      const browserType = this._copilotbrowser[(browserName || 'chromium') as 'chromium'];
      const controller = new ProgressController();
      browser = await controller.run(progress => browserType.launch(progress, {
        ...launchOptions,
        headless: !!process.env.CB_DEBUG_CONTROLLER_HEADLESS,
  ...
  private async _initConnectMode(id: string, filter: 'first', browserName: string | null, launchOptions: LaunchOptionsWithTimeout): Promise<copilotbrowserInitializeResult> {
    ...
    if (!browser) {
      const browserType = this._copilotbrowser[browserName as 'chromium'];
      const controller = new ProgressController();
  ...
  private async _initPreLaunchedBrowserMode(id: string): Promise<copilotbrowserInitializeResult> {
  ...
  private async _initPreLaunchedAndroidMode(id: string): Promise<copilotbrowserInitializeResult> {
  ...
  private async _initLaunchBrowserMode(browserName: string | null, proxyValue: string | undefined, launchOptions: LaunchOptionsWithTimeout, id: string): Promise<copilotbrowserInitializeResult> {
...
function userAgentVersionMatchesErrorMessage(userAgent: string) {
  ...
  const expected = getcopilotbrowserVersion(true);
