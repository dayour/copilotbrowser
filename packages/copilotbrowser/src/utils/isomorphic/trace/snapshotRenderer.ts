    ...
    const visit = (n: NodeSnapshot, snapshotIndex: number, parentTag: string | undefined, parentAttrs: [string, string][] | undefined) => {
      ...
      if (typeof n === 'string') {
        ...
        if (parentTag === 'STYLE' || parentTag === 'style')
          ...
          result.push(stringUtils.escapeHTML(n));
        return;
      }
      ...
      if (isSubtreeReferenceSnapshot(n)) {
        ...
        const kCurrentSrcAttribute = '__copilotbrowser_current_src__';
        ...
        for (const [attr, value] of attrs) {
          ...
          if (isFrame && attr.toLowerCase() === 'src') {
            ...
            attrName = '__copilotbrowser_src__';
          }
          ...
          result.push(' ', attrName, '="', stringUtils.escapeHTMLAttribute(attrValue), '"');
        }
        result.push('>');
...
declare global {
  interface Window {
    __copilotbrowser_frame_bounding_rects__: FrameBoundingRectsInfo;
  }
}

function snapshotScript(viewport: ViewportSize, ...targetIds: (string | undefined)[]) {
  function applycopilotbrowserAttributes(viewport: ViewportSize, ...targetIds: (string | undefined)[]) {
    ...
    // Before rendering each iframe, its parent extracts the '__copilotbrowser_canvas_render_info__' attribute
    ...
    const visit = (root: Document | ShadowRoot) => {
      ...
      for (const e of root.querySelectorAll(`[__copilotbrowser_scroll_top_]`))
        scrollTops.push(e);
      for (const e of root.querySelectorAll(`[__copilotbrowser_scroll_left_]`))
        scrollLefts.push(e);

      for (const element of root.querySelectorAll(`[__copilotbrowser_value_]`)) {
        const inputElement = element as HTMLInputElement | HTMLTextAreaElement;
        if (inputElement.type !== 'file')
          inputElement.value = inputElement.getAttribute('__copilotbrowser_value_')!;
        element.removeAttribute('__copilotbrowser_value_');
      }
      for (const element of root.querySelectorAll(`[__copilotbrowser_checked_]`)) {
        (element as HTMLInputElement).checked = element.getAttribute('__copilotbrowser_checked_') === 'true';
        element.removeAttribute('__copilotbrowser_checked_');
      }
      for (const element of root.querySelectorAll(`[__copilotbrowser_selected_]`)) {
        (element as HTMLOptionElement).selected = element.getAttribute('__copilotbrowser_selected_') === 'true';
        element.removeAttribute('__copilotbrowser_selected_');
      }
      for (const element of root.querySelectorAll(`[__copilotbrowser_popover_open_]`)) {
        try {
          (element as HTMLElement).showPopover();
        } catch {
        }
        element.removeAttribute('__copilotbrowser_popover_open_');
      }
      for (const element of root.querySelectorAll(`[__copilotbrowser_dialog_open_]`)) {
        try {
          if (element.getAttribute('__copilotbrowser_dialog_open_') === 'modal')
            (element as HTMLDialogElement).showModal();
          else
            (element as HTMLDialogElement).show();
        } catch {
        }
        element.removeAttribute('__copilotbrowser_dialog_open_');
      }

      for (const targetId of targetIds) {
        for (const target of root.querySelectorAll(`[__copilotbrowser_target__="${targetId}"]`)) {
      ...
      for (const iframe of root.querySelectorAll('iframe, frame')) {
        const boundingRectJson = iframe.getAttribute('__copilotbrowser_bounding_rect__');
        iframe.removeAttribute('__copilotbrowser_bounding_rect__');
        ...
        const src = iframe.getAttribute('__copilotbrowser_src__');
        if (!src) {
      ...
      {
        const body = root.querySelector(`body[__copilotbrowser_custom_elements__]`);
        if (body && win.customElements) {
          const customElements = (body.getAttribute('__copilotbrowser_custom_elements__') || '').split(',');
          for (const elementName of customElements)
            win.customElements.define(elementName, class extends HTMLElement {});
        }
      }

      for (const element of root.querySelectorAll(`template[__copilotbrowser_shadow_root_]`)) {
      ...
      if ('adoptedStyleSheets' in (root as any)) {
        ...
        for (const element of root.querySelectorAll(`template[__copilotbrowser_style_sheet_]`)) {
          const template = element as HTMLTemplateElement;
          const sheet = new CSSStyleSheet();
          (sheet as any).replaceSync(template.getAttribute('__copilotbrowser_style_sheet_'));
    ...
    const onLoad = () => {
      ...
      for (const element of scrollTops) {
        element.scrollTop = +element.getAttribute('__copilotbrowser_scroll_top_')!;
        element.removeAttribute('__copilotbrowser_scroll_top_');
        if (frameBoundingRectsInfo.frames.has(element))
          frameBoundingRectsInfo.frames.get(element)!.scrollTop = element.scrollTop;
      }
      for (const element of scrollLefts) {
        element.scrollLeft = +element.getAttribute('__copilotbrowser_scroll_left_')!;
        element.removeAttribute('__copilotbrowser_scroll_left_');
        if (frameBoundingRectsInfo.frames.has(element))
      ...
      if (canvasElements.length > 0) {
        ...
        img.onload = () => {
          for (const canvas of canvasElements) {
            ...
            const boundingRectAttribute = canvas.getAttribute('__copilotbrowser_bounding_rect__');
            canvas.removeAttribute('__copilotbrowser_bounding_rect__');
            ...
            while (currWindow !== topSnapshotWindow) {
              ...
              const iframeInfo = currWindow['__copilotbrowser_frame_bounding_rects__']?.frames.get(iframe);
            ...
            const { width, height } = topSnapshotWindow['__copilotbrowser_frame_bounding_rects__'].viewport;
            ...
            if (fullyUncaptured) {
              canvas.title = `copilotbrowser couldn't capture canvas contents because it's located outside the viewport.`;
              continue;
            }
            ...
            if (shouldPopulateCanvasFromScreenshot) {
              ...
              if (partiallyUncaptured)
                canvas.title = `copilotbrowser couldn't capture full canvas contents because it's located partially outside the viewport.`;
              else
        ...
        img.onerror = () => {
          for (const canvas of canvasElements) {
            const context = canvas.getContext('2d')!;
            drawCheckerboard(context, canvas);
            canvas.title = `copilotbrowser couldn't show canvas contents because the screenshot failed to load.`;
          }
        };
  ...
  return `\n(${applycopilotbrowserAttributes.toString()})(${JSON.stringify(viewport)}${targetIds.map(id => `, "${id}"`).join('')})`;
}
...
const kLegacyBlobPrefix = 'http://copilotbrowser.bloburl/#';
