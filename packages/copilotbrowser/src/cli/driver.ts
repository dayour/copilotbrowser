...
export function printApiJson() {
  // Note: this file is generated by build-copilotbrowser-driver.sh
...
export function runDriver() {
  ...
  new RootDispatcher(dispatcherConnection, async (rootScope, { sdkLanguage }) => {
    const copilotbrowser = createcopilotbrowser({ sdkLanguage });
    return new copilotbrowserDispatcher(rootScope, copilotbrowser);
  });
  ...
  const isJavaScriptLanguageBinding = !process.env.CB_LANG_NAME || process.env.CB_LANG_NAME === 'javascript';
...
export async function runServer(options: RunServerOptions) {
  ...
  const server = new copilotbrowserServer({ mode: extension ? 'extension' : 'default', path, maxConnections });
...
export async function launchBrowserServer(browserName: string, configFile?: string) {
  ...
  const browserType = (copilotbrowser as any)[browserName] as BrowserType;
