...
function printcopilotbrowserTestError(command: string) {
  ...
  if (packageManager === 'yarn') {
    console.error(`Please install @copilotbrowser/copilotbrowser package before running "yarn copilotbrowser ${command}"`);
    console.error(`  yarn remove ${packages.join(' ')}`);
    console.error('  yarn add -D @copilotbrowser/copilotbrowser');
  } else if (packageManager === 'pnpm') {
    console.error(`Please install @copilotbrowser/copilotbrowser package before running "pnpm exec copilotbrowser ${command}"`);
    console.error(`  pnpm remove ${packages.join(' ')}`);
    console.error('  pnpm add -D @copilotbrowser/copilotbrowser');
  } else {
    console.error(`Please install @copilotbrowser/copilotbrowser package before running "npx copilotbrowser ${command}"`);
    console.error(`  npm uninstall ${packages.join(' ')}`);
    console.error('  npm install -D @copilotbrowser/copilotbrowser');
  }
}
...
function addExternalcopilotbrowserTestCommands() {
  for (const [command, description] of kExternalcopilotbrowserTestCommands) {
    ...
    copilotbrowserTest.description(`${description} Available in @copilotbrowser/copilotbrowser package.`);
