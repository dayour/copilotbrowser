...
program
    .version('Version ' + (process.env.CB_CLI_DISPLAY_VERSION || packageJSON.version))
    .name(buildBasecopilotbrowserCLICommand(process.env.CB_LANG_NAME));
...
commandWithOpenOptions('codegen [url]', 'open page and generate code for user actions',
    [
      ...
      ['--target <language>', `language to generate, one of javascript, copilotbrowser-test, python, python-async, python-pytest, csharp, csharp-mstest, csharp-nunit, java, java-junit`, codegenId()],
...
function printGroupedBycopilotbrowserVersion(browsers: BrowserInfo[]) {
  ...
  const groupedBycopilotbrowserMinorVersion = new Map<string, BrowserInfo[]>();
  for (const browser of browsers) {
    const version = dirToVersion.get(browser.referenceDir)!;
    let entries = groupedBycopilotbrowserMinorVersion.get(version);
    if (!entries) {
      entries = [];
      groupedBycopilotbrowserMinorVersion.set(version, entries);
    }
    entries.push(browser);
  }

  const sortedVersions = [...groupedBycopilotbrowserMinorVersion.keys()].sort((a, b) => {
  ...
  for (const version of sortedVersions) {
    console.log(`\ncopilotbrowser version: ${version}`);
    printInstalledBrowsers(groupedBycopilotbrowserMinorVersion.get(version)!);
  }
}

program
    .command('install [browser...]')
    .description('ensure browsers necessary for this version of copilotbrowser are installed')
    .option('--with-deps', 'install system dependencies for browsers')
    .option('--dry-run', 'do not execute installation, only print information')
    .option('--list', 'prints list of browsers from all copilotbrowser installations')
    ...
    .action(async function(args: string[], options: { withDeps?: boolean, force?: boolean, dryRun?: boolean, list?: boolean, shell?: boolean, noShell?: boolean, onlyShell?: boolean }) {
      if (isLikelyNpxGlobal()) {
        console.error(wrapInASCIIBox([
          `WARNING: It looks like you are running 'npx copilotbrowser install' without first`,
          `installing your project's dependencies.`,
          ``,
          `To avoid unexpected behavior, please install your dependencies first, and`,
          `then run copilotbrowser's install command:`,
          ``,
          `    npm install`,
          `    npx copilotbrowser install`,
          ``,
          `If your project does not yet depend on copilotbrowser, first install the`,
          `applicable npm package (most commonly @copilotbrowser/copilotbrowser), and`,
          `then run copilotbrowser's install command to download the browsers:`,
          ``,
          `    npm install @copilotbrowser/copilotbrowser`,
          `    npx copilotbrowser install`,
          ``,
        ].join('\n'), 1));
      }
      try {
        ...
        if (options.dryRun) {
          ...
          printGroupedBycopilotbrowserVersion(browsers);
        } else {
          await registry.install(executables, { force: options.force });
          await registry.validateHostRequirementsForExecutablesIfNeeded(executables, process.env.CB_LANG_NAME || 'javascript').catch((e: Error) => {
            e.name = 'copilotbrowser Host validation warning';
            console.error(e);
          });
        }
      } catch (e) {
...
program
    .command('uninstall')
    .description('Removes browsers used by this installation of copilotbrowser from the system (chromium, firefox, webkit, ffmpeg). This does not include branded channels.')
    .option('--all', 'Removes all browsers used by any copilotbrowser installation from the system.')
    .action(async (options: { all?: boolean }) => {
      delete process.env.copilotbrowser_SKIP_BROWSER_GC;
      await registry.uninstall(!!options.all).then(({ numberOfBrowsersLeft }) => {
        if (!options.all && numberOfBrowsersLeft > 0) {
          console.log('Successfully uninstalled copilotbrowser browsers for the current copilotbrowser installation.');
          console.log(`There are still ${numberOfBrowsersLeft} browsers left, used by other copilotbrowser installations.\nTo uninstall copilotbrowser browsers for all installations, re-run with --all flag.`);
        }
...
program
    ...
    .action(function(options) {
      runServer({
        ...
        extension: options.mode === 'extension' || !!process.env.CB_EXTENSION_MODE,
      }).catch(logErrorAndExit);
    });
...
async function launchContext(options: Options, extraOptions: LaunchOptions): Promise<{ browser: Browser, browserName: string, launchOptions: LaunchOptions, contextOptions: BrowserContextOptions, context: BrowserContext, closeBrowser: () => Promise<void> }> {
  ...
  const contextOptions: BrowserContextOptions =
    ...
    options.device ? { ...copilotbrowser.devices[options.device] } : {};

  // In headful mode, use host device scale factor for things to look nice.
  // In headless, keep things the way it works in copilotbrowser by default.
...
async function open(options: Options, url: string | undefined) {
  const { context } = await launchContext(options, { headless: !!process.env.CBTEST_CLI_HEADLESS, executablePath: process.env.CBTEST_CLI_EXECUTABLE_PATH });
...
async function codegen(options: Options & { target: string, output?: string, testIdAttribute?: string }, url: string | undefined) {
  ...
  const tracesDir = path.join(os.tmpdir(), `copilotbrowser-recorder-trace-${Date.now()}`);
  const { context, browser, launchOptions, contextOptions, closeBrowser } = await launchContext(options, {
    headless: !!process.env.CBTEST_CLI_HEADLESS,
    executablePath: process.env.CBTEST_CLI_EXECUTABLE_PATH,
    tracesDir,
  });
  const donePromise = new ManualPromise<void>();
  maybeSetupTestHooks(browser, closeBrowser, donePromise);
  dotenv.config({ path: 'copilotbrowser.env' });
...
async function maybeSetupTestHooks(browser: Browser, closeBrowser: () => Promise<void>, donePromise: Promise<void>) {
  if (!process.env.CBTEST_CLI_IS_UNDER_TEST)
    return;

  // Make sure we exit abnormally when browser crashes.
  const logs: string[] = [];
  require('@copilotbrowser/copilotbrowser/lib/utilsBundle').debug.log = (...args: any[]) => {
  ...
  if (process.env.CBTEST_CLI_EXIT_AFTER_TIMEOUT) {
    setTimeout(close, +process.env.CBTEST_CLI_EXIT_AFTER_TIMEOUT);
    return;
  }
...
function lookupBrowserType(options: Options): BrowserType {
  ...
  if (options.device) {
    const device = copilotbrowser.devices[options.device];
    name = device.defaultBrowserType;
  }
  let browserType: any;
  switch (name) {
    case 'chromium': browserType = copilotbrowser.chromium; break;
    case 'webkit': browserType = copilotbrowser.webkit; break;
    case 'firefox': browserType = copilotbrowser.firefox; break;
    case 'cr': browserType = copilotbrowser.chromium; break;
    case 'wk': browserType = copilotbrowser.webkit; break;
    case 'ff': browserType = copilotbrowser.firefox; break;
  }
  if (browserType)
    return browserType;
  program.help();
...
function validateOptions(options: Options) {
  if (options.device && !(options.device in copilotbrowser.devices)) {
    const lines = [`Device descriptor not found: '${options.device}', available devices are:`];
    for (const name in copilotbrowser.devices)
      lines.push(`  "${name}"`);
...
function codegenId(): string {
  return process.env.CB_LANG_NAME || 'copilotbrowser-test';
}
...
function commandWithOpenOptions(command: string, description: string, options: any[][]): Command {
  ...
  return result
      ...
      .option('--timeout <timeout>', 'timeout for copilotbrowser actions in milliseconds, no timeout by default')
      .option('--user-agent <ua string>', 'specify user agent string')
      .option('--user-data-dir <directory>', 'use the specified user data directory instead of a new context')
      .option('--viewport-size <size>', 'specify browser viewport size in pixels, for example "1280, 720"');
}

function buildBasecopilotbrowserCLICommand(cliTargetLang: string | undefined): string {
  switch (cliTargetLang) {
    case 'python':
      return `copilotbrowser`;
    case 'java':
      return `mvn exec:java -e -D exec.mainClass=com.microsoft.copilotbrowser.CLI -D exec.args="...options.."`;
    case 'csharp':
      return `pwsh bin/Debug/netX/copilotbrowser.ps1`;
    default: {
      const packageManagerCommand = getPackageManagerExecCommand();
      return `${packageManagerCommand} copilotbrowser`;
    }
  }
}
