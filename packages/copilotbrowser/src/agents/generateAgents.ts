  ...
  static async init(config: FullConfigInternal, projectName: string, prompts: boolean) {
    ...
    await writeFile('.mcp.json', JSON.stringify({
      mcpServers: {
        'copilotbrowser-test': {
          command: 'npx',
          args: ['copilotbrowser', 'run-test-mcp-server'],
        }
      }
...
export class OpencodeGenerator {
  ...
  static configuration(agents: AgentSpec[]): string {
    ...
    result['mcp']['copilotbrowser-test'] = {
      type: 'local',
      command: ['npx', 'copilotbrowser', 'run-test-mcp-server'],
      enabled: true,
    };
...
export class VSCodeGenerator {
  static async init(config: FullConfigInternal, projectName: string) {
    ...
    const nameMap = new Map<string, string>([
      ['copilotbrowser-test-planner', ' 🎭 planner'],
      ['copilotbrowser-test-generator', '🎭 generator'],
      ['copilotbrowser-test-healer', '🎭 healer'],
    ]);
  ...
  static async appendToMCPJson() {
    ...
    mcpJson.servers['copilotbrowser-test'] = {
      type: 'stdio',
      command: 'npx',
      args: ['copilotbrowser', 'run-test-mcp-server'],
    };
  ...
  static agentSpec(agent: AgentSpec): string {
    ...
    const vscodeMcpName = 'copilotbrowser-test';
