...
export const cacheDir = process.env.CBTEST_CACHE_DIR || (() => {
  if (process.platform === 'win32')
    return path.join(os.tmpdir(), `copilotbrowser-transform-cache`);
  ...
  return path.join(os.tmpdir(), `copilotbrowser-transform-cache-` + process.geteuid?.());
})();
...
// that.
const kcopilotbrowserInternalPrefix = path.resolve(__dirname, '../../../copilotbrowser');
