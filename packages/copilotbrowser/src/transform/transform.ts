...
export function transformHook(originalCode: string, filename: string, moduleUrl?: string): { code: string, serializedCache?: any } {
  ...
  const hasPreprocessor =
    process.env.CB_TEST_SOURCE_TRANSFORM &&
    process.env.CB_TEST_SOURCE_TRANSFORM_SCOPE &&
    process.env.CB_TEST_SOURCE_TRANSFORM_SCOPE.split(pathSeparator).some(f => filename.startsWith(f));
  const pluginsPrologue = _transformConfig.babelPlugins;
  const pluginsEpilogue = hasPreprocessor ? [[process.env.CB_TEST_SOURCE_TRANSFORM!]] as BabelPlugin[] : [];
