...
import { isRegExp } from '@copilotbrowser/copilotbrowser/lib/utils';
...
export async function loadConfig(location: ConfigLocation, overrides?: ConfigCLIOverrides, ignoreProjectDependencies = false, metadata?: Config['metadata']): Promise<FullConfigInternal> {
  ...
  if (!registerESMLoader()) {
    ...
    if (location.resolvedConfigFile && fileIsModule(location.resolvedConfigFile))
      throw errorWithFile(location.resolvedConfigFile, `copilotbrowser requires Node.js 18.19 or higher to load esm modules. Please update your version of Node.js.`);
  }
  ...
  // 2. Load and validate copilotbrowser config.
  ...
  // 3. Load transform options from the copilotbrowser config.
  const babelPlugins = (userConfig as any)['@copilotbrowser/copilotbrowser/test']?.babelPlugins || [];
...
function resolveConfigFile(configFileOrDirectory: string): string | undefined {
  ...
  const resolveConfigFileFromDirectory = (directory: string) => {
    for (const ext of ['.ts', '.js', '.mts', '.mjs', '.cts', '.cjs']) {
      const configFile = resolveConfig(path.resolve(directory, 'copilotbrowser.config' + ext));
      if (configFile)
        return configFile;
    }
  };
