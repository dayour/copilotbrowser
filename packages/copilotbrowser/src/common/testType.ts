  ...
  private _currentSuite(location: Location, title: string): Suite | undefined {
    ...
    if (!suite) {
      throw new Error([
        ...
        `- You have two different versions of @copilotbrowser/copilotbrowser. This usually happens`,
        `  when one of the dependencies in your package.json depends on @copilotbrowser/copilotbrowser.`,
      ].join('\n'));
    }
    return suite;
  }
...
function throwIfRunningInsideJest() {
  if (process.env.JEST_WORKER_ID) {
    ...
    throw new Error(
        `copilotbrowser Test needs to be invoked via '${packageManagerCommand} copilotbrowser test' and excluded from Jest test runs.\n` +
