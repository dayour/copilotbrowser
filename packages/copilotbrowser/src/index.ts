...
if ((process as any)['__pw_initiator__']) {
  ...
  try {
    throw new Error('Requiring @copilotbrowser/copilotbrowser second time, \nFirst:\n' + (process as any)['__pw_initiator__'] + '\n\nSecond: ');
  } finally {
...
const copilotbrowserFixtures: Fixtures<TestFixtures, WorkerFixtures> = ({
  ...
  _reuseContext: [async ({ video, _optionContextReuseMode }, use) => {
    ...
    if (process.env.CB_TEST_REUSE_CONTEXT)
      mode = 'when-possible';
...
function connectOptionsFromEnv() {
  const wsEndpoint = process.env.CB_TEST_CONNECT_WS_ENDPOINT;
  if (!wsEndpoint)
    return undefined;
  const headers = process.env.CB_TEST_CONNECT_HEADERS ? JSON.parse(process.env.CB_TEST_CONNECT_HEADERS) : undefined;
  return {
    wsEndpoint,
    headers,
    exposeNetwork: process.env.CB_TEST_CONNECT_EXPOSE_NETWORK,
  };
}
