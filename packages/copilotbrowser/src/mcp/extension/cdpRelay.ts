...
/**
 * WebSocket server that bridges copilotbrowser MCP and Chrome Extension
 *
 * Endpoints:
 * - /cdp/guid - Full CDP interface for copilotbrowser MCP
...
const debugLogger = debug('cb:mcp:relay');
...
export class CDPRelayServer {
  ...
  private _copilotbrowserConnection: WebSocket | null = null;
  ...
  async ensureExtensionConnectionForMCPContext(clientInfo: ClientInfo, abortSignal: AbortSignal, forceNewTab: boolean) {
    ...
    await Promise.race([
      ...
      new Promise((_, reject) => setTimeout(() => {
        reject(new Error(`Extension connection timeout. Make sure the "copilotbrowser MCP Bridge" extension is installed. See https://github.com/dayour/copilotbrowser-mcp/blob/main/packages/extension/README.md for installation instructions.`));
      }, process.env.CBMCP_TEST_CONNECTION_TIMEOUT ? parseInt(process.env.CBMCP_TEST_CONNECTION_TIMEOUT, 10) : 5_000)),
  ...
  private _connectBrowser(clientInfo: ClientInfo, forceNewTab: boolean) {
    ...
    url.searchParams.set('protocolVersion', process.env.CBMCP_TEST_PROTOCOL_VERSION ?? protocol.VERSION.toString());
    if (forceNewTab)
      url.searchParams.set('newTab', 'true');
    const token = process.env.copilotbrowser_MCP_EXTENSION_TOKEN;
    if (token)
  ...
  closeConnections(reason: string) {
    this._closecopilotbrowserConnection(reason);
  ...
  private _onConnection(ws: WebSocket, request: http.IncomingMessage): void {
    ...
    if (url.pathname === this._cdpPath) {
      this._handlecopilotbrowserConnection(ws);
  ...
  private _handlecopilotbrowserConnection(ws: WebSocket): void {
    if (this._copilotbrowserConnection) {
      debugLogger('Rejecting second copilotbrowser connection');
      ws.close(1000, 'Another CDP client already connected');
      return;
    }
    this._copilotbrowserConnection = ws;
    ws.on('message', async data => {
      try {
        const message = JSON.parse(data.toString());
        await this._handlecopilotbrowserMessage(message);
      } catch (error: any) {
        debugLogger(`Error while handling copilotbrowser message\n${data.toString()}\n`, error);
      }
    });
    ws.on('close', () => {
      if (this._copilotbrowserConnection !== ws)
        return;
      this._copilotbrowserConnection = null;
      this._closeExtensionConnection('copilotbrowser client disconnected');
      debugLogger('copilotbrowser WebSocket closed');
    });
    ws.on('error', error => {
      debugLogger('copilotbrowser WebSocket error:', error);
    });
    debugLogger('copilotbrowser MCP connected');
  }
  ...
  private _closecopilotbrowserConnection(reason: string) {
    if (this._copilotbrowserConnection?.readyState === ws.OPEN)
      this._copilotbrowserConnection.close(1000, reason);
    this._copilotbrowserConnection = null;
  }
  ...
  private _handleExtensionConnection(ws: WebSocket): void {
    ...
    this._extensionConnection.onclose = (c, reason) => {
      ...
      if (this._extensionConnection !== c)
        return;
      this._resetExtensionConnection();
      this._closecopilotbrowserConnection(`Extension disconnected: ${reason}`);
    };
  ...
  private async _handlecopilotbrowserMessage(message: CDPCommand): Promise<void> {
    debugLogger('← copilotbrowser:', `${message.method} (id=${message.id})`);
    const { id, sessionId, method, params } = message;
    try {
      const result = await this._handleCDPCommand(method, params, sessionId);
      this._sendTocopilotbrowser({ id, sessionId, result });
    } catch (e) {
  ...
  private _sendTocopilotbrowser(message: CDPResponse): void {
    debugLogger('→ copilotbrowser:', `${message.method ?? `response(id=${message.id})`}`);
    this._copilotbrowserConnection?.send(JSON.stringify(message));
  }
}
