...
export async function resolveCLIConfig(cliOptions: CLIOptions): Promise<FullConfig> {
  ...
  const browserExplicitlyConfigured = !!(
    cliOptions.browser ||
...
export function configFromCLIOptions(cliOptions: CLIOptions): Config & { configFile?: string } {
  ...
  // Context options
  const contextOptions: copilotbrowser.BrowserContextOptions = cliOptions.device ? devices[cliOptions.device] : {};
...
export function configFromEnv(): Config & { configFile?: string } {
  ...
  if (process.env.copilotbrowser_MCP_CONSOLE_LEVEL)
    options.consoleLevel = enumParser<'error' | 'warning' | 'info' | 'debug'>('--console-level', ['error', 'warning', 'info', 'debug'], process.env.copilotbrowser_MCP_CONSOLE_LEVEL);
  ...
  if (process.env.copilotbrowser_MCP_IMAGE_RESPONSES)
    options.imageResponses = enumParser<'allow' | 'omit'>('--image-responses', ['allow', 'omit'], process.env.copilotbrowser_MCP_IMAGE_RESPONSES);
...
export function outputDir(config: FullConfig, clientInfo: ClientInfo): string {
  ...
  if (rootPath)
    return path.resolve(rootPath, config.skillMode ? '.copilotbrowser-cli' : '.copilotbrowser-mcp');
  const tmpDir = process.env.CB_TMPDIR_FOR_TEST ?? os.tmpdir();
  return path.resolve(tmpDir, 'copilotbrowser-mcp-output', String(clientInfo.timestamp));
}
...
export async function outputFile(config: FullConfig, clientInfo: ClientInfo, fileName: string, options: { origin: 'code' | 'llm' }): Promise<string> {
  ...
  debug('cb:mcp:file')(resolvedFile);
  return resolvedFile;
}
...
function autoDetectBrowserChannel(config: FullConfig): void {
  const candidateChannels = ['chrome', 'msedge', 'chromium'] as const;
  ...
  const currentExecutable = registry.findExecutable(currentChannel);
  ...
  for (const candidate of candidateChannels) {
    if (candidate === currentChannel)
      continue;
    const executable = registry.findExecutable(candidate);
