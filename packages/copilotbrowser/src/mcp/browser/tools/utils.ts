...
export async function waitForCompletion<R>(tab: Tab, callback: () => Promise<R>): Promise<R> {
  const requests: copilotbrowser.Request[] = [];

  const requestListener = (request: copilotbrowser.Request) => requests.push(request);
...
export async function callOnPageNoTrace<T>(page: copilotbrowser.Page, callback: (page: copilotbrowser.Page) => Promise<T>): Promise<T> {
  return await (page as any)._wrapApiCall(() => callback(page), { internal: true });
}

export function eventWaiter<T>(page: copilotbrowser.Page, event: string, timeout: number): { promise: Promise<T | undefined>, abort: () => void } {
