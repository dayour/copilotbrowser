...
const testDebug = debug('cb:mcp:test');
...
export class Context {
  ...
  private _video: {
    allVideos: Set<copilotbrowser.Video>;
    listener: (page: copilotbrowser.Page) => void;
  } | undefined;
  ...
  async newTab(): Promise<Tab> {
    ...
    this._currentTab = this._tabs.find(t => t.page === (page as unknown as typeof t.page))!;
    return this._currentTab;
  }
  ...
  async startVideoRecording(params: VideoParams) {
    ...
    const listener = (page: copilotbrowser.Page) => {
  ...
  private _onPageCreated(page: copilotbrowser.Page) {
  ...
  private async _setupRequestInterception(context: copilotbrowser.BrowserContext) {
  ...
  async ensureBrowserContext(): Promise<copilotbrowser.BrowserContext> {
    const { browserContext } = await this._ensureBrowserContext();
