...
const serverDebug = debug('cb:mcp:server');
const serverDebugResponse = debug('cb:mcp:server:response');
...
export async function connect(factory: ServerBackendFactory, transport: Transport, runPulse: boolean) {
...
export function createServer(name: string, version: string, backend: ServerBackend, runPulse: boolean): Server {
...
const initializeServer = async (server: Server, backend: ServerBackend, runPulse: boolean) => {
...
const startPulse = (server: Server) => {
  const beat = () => {
    Promise.race([
      server.ping(),
