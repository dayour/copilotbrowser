    ...
    setup: async ({ config }) => {
      ...
      await Promise.all(Array.from(outputDirs).map(outputDir => removeFolders([outputDir]).then(async ([error]) => {
        ...
        if ((error as any).code === 'EBUSY') {
          ...
          //   https://github.com/dayour/copilotbrowser/issues/12106
    ...
    setup: async testRun => {
      ...
      for (let i = 0; i < projectToSuite.size; i++) {
        ...
        if (phaseProjects.length) {
          ...
          debug('cb:test:task')(`created phase #${testRun.phases.length} with ${phase.projects.map(p => p.project.project.name).sort()} projects, ${testGroupsInPhase} testGroups`);
