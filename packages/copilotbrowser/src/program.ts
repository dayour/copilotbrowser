...
function addTestCommand(program: Command) {
  ...
  command.description('run tests with copilotbrowser Test');
...
Examples:
  $ npx copilotbrowser test my.spec.ts
  $ npx copilotbrowser test some.spec.ts:42
...
function addClearCacheCommand(program: Command) {
  ...
  command.option('-c, --config <file>', `Configuration file, or a test directory with optional "copilotbrowser.config.{m,c}?{js,ts}"`);
...
Examples:
  $ npx copilotbrowser merge-reports copilotbrowser-report`);
}
...
function addTestMCPServerCommand(program: Command) {
  ...
  command.action(async options => {
    ...
    const factory: mcp.ServerBackendFactory = {
      name: 'copilotbrowser Test Runner',
      nameInConfig: 'copilotbrowser-test-runner',
...
async function runTests(args: string[], opts: { [key: string]: any }) {
  ...
  if (opts.ui || opts.uiHost || opts.uiPort) {
    if (opts.onlyChanged)
      throw new Error(`--only-changed is not supported in UI mode. If you'd like that to change, see https://github.com/dayour/copilotbrowser/issues/15075 for more details.`);
...
function resolveShardWeightsOption(): ConfigCLIOverrides['shardWeights'] {
  const shardWeights = process.env.CBTEST_SHARD_WEIGHTS;
  if (!shardWeights)
    return undefined;
  ...
  return shardWeights.split(':').map(w => {
    ...
    if (isNaN(weight) || weight < 0)
      throw new Error(`CBTEST_SHARD_WEIGHTS="${shardWeights}" weights must be non-negative numbers`);
    return weight;
  });
}
...
const testOptions: [string, { description: string, choices?: string[], preset?: string }][] = [
  ...
  ['-c, --config <file>', { description: `Configuration file, or a test directory with optional "copilotbrowser.config.{m,c}?{js,ts}"` }],
  ['--debug', { description: `Run tests with copilotbrowser Inspector. Shortcut for "PWDEBUG=1" environment variable and "--timeout=0 --max-failures=1 --headed --workers=1" options` }],
  ...
  ['--test-list <file>', { description: `Path to a file containing a list of tests to run. See https://dayour.github.io/copilotbrowser/docs/test-cli for more details.` }],
  ['--test-list-invert <file>', { description: `Path to a file containing a list of tests to skip. See https://dayour.github.io/copilotbrowser/docs/test-cli for more details.` }],
