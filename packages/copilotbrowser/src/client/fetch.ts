...
export type NewContextOptions = Omit<channels.copilotbrowserNewRequestOptions, 'extraHTTPHeaders' | 'clientCertificates' | 'storageState' | 'tracesDir'> & {
...
export class APIRequest implements api.APIRequest {
  ...
  async newContext(options: NewContextOptions & TimeoutOptions = {}): Promise<APIRequestContext> {
    ...
    const context = APIRequestContext.from((await this._copilotbrowser._channel.newRequest({
      ...
      clientCertificates: await toClientCertificatesProtocol(this._copilotbrowser._platform, options.clientCertificates),
    })).request);
