...
// ---------------------------------------------------------------------------
// MCP config file format  (.vscode/mcp.json  /  .github/copilot/mcp.json)
// ---------------------------------------------------------------------------
type McpStdioEntry = {
  type: 'stdio';
  command: string;
...
let runningProcess: ChildProcess | undefined;   // explicit HTTP server (autoStart / manual commands)
let httpServerRunning = false;
let extensionPath: string;  // set during activate(), used to locate bundled CLI

// Sidebar tree view providers (set during activate)
let targetsProvider: TargetsTreeProvider | undefined;
let configProvider: ConfigTreeProvider | undefined;

// MCP provider reference so config commands can trigger a refresh
let mcpProviderRef: copilotbrowserMcpProvider | undefined;
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
      ...
      vscode.commands.registerCommand('copilotbrowser.stopMcpServer', stopMcpServer),
      vscode.commands.registerCommand('copilotbrowser.restartMcpServer', restartMcpServer),
      ...
      vscode.commands.registerCommand('copilotbrowser.showStatus', showStatus),
      vscode.commands.registerCommand('copilotbrowser.showMcpConfig', showMcpConfig),
      vscode.commands.registerCommand('copilotbrowser.openSettings', () => {
      ...
      vscode.commands.registerCommand('copilotbrowser.config.setBrowser', configSetBrowser),
      vscode.commands.registerCommand('copilotbrowser.config.toggleHeadless', configToggleHeadless),
      vscode.commands.registerCommand('copilotbrowser.config.toggleNoSandbox', configToggleNoSandbox),
      vscode.commands.registerCommand('copilotbrowser.config.setCapabilities', configSetCapabilities),
      vscode.commands.registerCommand('copilotbrowser.config.toggleAutoStart', configToggleAutoStart),
      vscode.commands.registerCommand('copilotbrowser.config.toggleProfileMode', configToggleProfileMode),
      vscode.commands.registerCommand('copilotbrowser.config.setConnectedProfile', configSetConnectedProfile),
      vscode.commands.registerCommand('copilotbrowser.installCliSkills', installCliSkills),
  );
  ...
  context.subscriptions.push(
      vscode.window.registerTreeDataProvider('copilotbrowser.targetsView', targetsProvider),
      vscode.window.registerTreeDataProvider('copilotbrowser.configView', configProvider),
      vscode.window.registerTreeDataProvider('copilotbrowser.toolsView', toolsProvider),
      vscode.window.registerTreeDataProvider('copilotbrowser.helpView', helpProvider),
  );
...
async function configureMcpGlobalSettings(context: vscode.ExtensionContext) {
  const config = vscode.workspace.getConfiguration('copilotbrowser');
  ...
  const isFirstRun = !context.globalState.get<boolean>('copilotbrowser.configured');
  if (isFirstRun) {
 ...
 */
async function writeCopilotCliMcpConfig(config: vscode.WorkspaceConfiguration) {
...
// ---------------------------------------------------------------------------
// Explicit HTTP MCP server start / stop / restart
// These commands let users spin up a persistent HTTP-mode MCP server e.g.
// for testing, autoStart scenarios, or debug tooling that prefers HTTP.
// Under normal VS Code + Copilot Chat usage the stdio provider (above) is
// used automatically and these commands are not needed.
// ---------------------------------------------------------------------------
...
function runUtilityCommand(command: string, args: string[]): Promise<CommandResult> {
...
async function forceTerminatePid(pid: number): Promise<void> {
  if (process.platform === 'win32') {
...
async function clearPortConflicts(port: number): Promise<void> {
  const blockedPids = new Set<number>([process.pid]);
...
async function startMcpServer() {
  ...
  try {
    ...
    (proc as any).kill = (...a: Parameters<typeof origKill>): boolean => {
...
async function installCliSkills() {
  ...
  if (!cliPath) {
    return vscode.window.showErrorMessage(
        'Bundled copilotbrowser-cli not found. Try reinstalling the extension.');
  }

  const proc = spawn(resolveNode(), [cliPath, 'install', '--skills'], {
    cwd,
    env: process.env,
    shell: process.platform === 'win32',
  });
...
function buildMcpArgs(config: vscode.WorkspaceConfiguration): string[] {
  ...
  const browser = config.get<string>('mcp.browser', 'msedge');
  if (browser && browser !== 'chromium')
    args.push('--browser', browser);

  if (config.get<boolean>('mcp.headless', false))
    args.push('--headless');

  if (config.get<boolean>('mcp.noSandbox', false))
    args.push('--no-sandbox');

  // Profile mode: isolated (default) vs connected (reuse existing browser profile)
  const profileMode = config.get<string>('mcp.profileMode', 'isolated');
  if (profileMode === 'connected' && (browser === 'msedge' || browser === 'chrome' || browser === 'chromium')) {
    const userDataDir = config.get<string>('mcp.connectedUserDataDir', '').trim()
      || getDefaultUserDataDir(browser);
    if (userDataDir)
      args.push('--user-data-dir', userDataDir);
    const profile = config.get<string>('mcp.connectedProfile', '').trim();
    if (profile)
  ...
  const extra = config.get<string[]>('mcp.extraArgs', []).filter(Boolean);
  args.push(...extra);
...
function getDefaultUserDataDir(browser: string): string {
 ...
 */
async function writeMcpConfigFiles(workspaceRoot: string): Promise<void> {
  const config = vscode.workspace.getConfiguration('copilotbrowser');
  const mcpArgs = buildMcpArgs(config);
  ...
  const serverEntry: McpStdioEntry = cliPath
    ? {
        type: 'stdio',
        command: 'node',
  ...
  for (const targetPath of targets) {
    try {
      ...
      let existing: McpConfigFile = { servers: {} };
      try {
 ...
 */
async function removeCopilotbrowserFromMcpConfigFiles(workspaceRoot: string): Promise<void> {
  const targets = [
    path.join(workspaceRoot, '.vscode', 'mcp.json'),
...
function toRelativePosix(workspaceRoot: string, absPath: string): string {
...
function showStatus() {
  ...
  const config = vscode.workspace.getConfiguration('copilotbrowser');
  const browser = config.get<string>('mcp.browser', 'msedge');
  const headless = config.get<boolean>('mcp.headless', false);
  const noSandbox = config.get<boolean>('mcp.noSandbox', false);
  const caps = config.get<string[]>('mcp.capabilities', []);
  const profileMode = config.get<string>('mcp.profileMode', 'isolated');
  const connectedProfile = config.get<string>('mcp.connectedProfile', '');
  ...
  vscode.window.showInformationMessage(
      statusMessage,
      ...(isRunning ? ['Stop Server', 'Restart Server', 'Open Detailed Config'] : ['Start Server', 'Open Detailed Config']),
  ).then(selection => {
...
async function openInspector() {
  ...
  const config = vscode.workspace.getConfiguration('copilotbrowser');
  const env = { ...process.env, PWDEBUG: '1', ...config.get<Record<string, string>>('env', {}) };
  ...
  proc.on('close', (code: number | null) => {
    ...
    if (proc === runningProcess) runningProcess = undefined;
  });
  runningProcess = proc;
  outputChannel.show(true);
}
...
async function applyConfigChange(key: string, value: unknown) {
  const config = vscode.workspace.getConfiguration('copilotbrowser');
  ...
  if (httpServerRunning) {
    const choice = await vscode.window.showInformationMessage(
        'Configuration changed. Restart the MCP server to apply?',
        'Restart Now', 'Later',
    );
...
async function configSetBrowser() {
  const config = vscode.workspace.getConfiguration('copilotbrowser');
  const current = config.get<string>('mcp.browser', 'msedge');
...
async function configToggleHeadless() {
  const config = vscode.workspace.getConfiguration('copilotbrowser');
  const current = config.get<boolean>('mcp.headless', false);
  const next = !current;
...
async function configToggleNoSandbox() {
  const config = vscode.workspace.getConfiguration('copilotbrowser');
  const current = config.get<boolean>('mcp.noSandbox', false);
  const choice = await vscode.window.showQuickPick(
      [
...
async function configSetCapabilities() {
  const config = vscode.workspace.getConfiguration('copilotbrowser');
  const current = config.get<string[]>('mcp.capabilities', []);
  const options: vscode.QuickPickItem[] = [
    {
      label: 'vision',
...
async function configToggleAutoStart() {
  const config = vscode.workspace.getConfiguration('copilotbrowser');
  const current = config.get<boolean>('autoStart', false);
  const choice = await vscode.window.showQuickPick(
      [
...
async function configToggleProfileMode() {
  const config = vscode.workspace.getConfiguration('copilotbrowser');
  const current = config.get<string>('mcp.profileMode', 'isolated');
  const browser = config.get<string>('mcp.browser', 'msedge');
  ...
  if (newValue === 'connected' && !supportsConnected) {
    vscode.window.showWarningMessage(
        `Connected profile mode requires Microsoft Edge or Chrome. Switch the browser setting first.`,
        'Switch to Edge',
    ).then(choice => {
  ...
  if (newValue === 'connected') {
    const profileChoice = await vscode.window.showInformationMessage(
        'Connected mode enabled. Would you like to select which browser profile to use?',
        'Select Profile', 'Use Default',
    );
...
async function configSetConnectedProfile() {
  const config = vscode.workspace.getConfiguration('copilotbrowser');
  const browser = config.get<string>('mcp.browser', 'msedge');
  const currentProfile = config.get<string>('mcp.connectedProfile', '');
  const currentUdd = config.get<string>('mcp.connectedUserDataDir', '');
  ...
  let profileItems: vscode.QuickPickItem[] = [
    {
  ...
  if (userDataDir) {
    try {
      ...
      const localState = JSON.parse(raw) as {
        profile?: { info_cache?: Record<string, { gaia_name?: string; user_name?: string; name?: string }> };
      };
  ...
  const uddChoice = await vscode.window.showInformationMessage(
      `Profile set to: ${profileFolder || 'Default'}. User data directory: ${userDataDir || '(auto-detect)'}`,
      'Change Directory', 'OK',
  );
...
class TargetsTreeProvider implements vscode.TreeDataProvider<CopilotBrowserTreeItem> {
  private _onDidChangeTreeData = new vscode.EventEmitter<void>();
  readonly onDidChangeTreeData = this._onDidChangeTreeData.event;

  refresh() { this._onDidChangeTreeData.fire(); }

  getTreeItem(element: CopilotBrowserTreeItem): vscode.TreeItem { return element; }

  getChildren(element?: CopilotBrowserTreeItem): CopilotBrowserTreeItem[] {
    if (element) return [];
...
class ConfigTreeProvider implements vscode.TreeDataProvider<CopilotBrowserTreeItem> {
  private _onDidChangeTreeData = new vscode.EventEmitter<void>();
  readonly onDidChangeTreeData = this._onDidChangeTreeData.event;
  ...
  getChildren(element?: CopilotBrowserTreeItem): CopilotBrowserTreeItem[] {
    ...
    const items: CopilotBrowserTreeItem[] = [
    ...
    items.push(
      new CopilotBrowserTreeItem('Capabilities', vscode.TreeItemCollapsibleState.None, {
...
class ToolsTreeProvider implements vscode.TreeDataProvider<CopilotBrowserTreeItem> {
  getTreeItem(element: CopilotBrowserTreeItem): vscode.TreeItem { return element; }
...
class HelpTreeProvider implements vscode.TreeDataProvider<CopilotBrowserTreeItem> {
  getTreeItem(element: CopilotBrowserTreeItem): vscode.TreeItem { return element; }
