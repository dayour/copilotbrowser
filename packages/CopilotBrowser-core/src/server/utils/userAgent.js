...
exports.getcopilotbrowserVersion = getcopilotbrowserVersion;
const child_process_1 = require("child_process");
const os_1 = __importDefault(require("os"));
const linuxUtils_1 = require("../utils/linuxUtils");
let cachedUserAgent;
...
function determineUserAgent() {
    ...
    if (process.platform === 'win32') {
        const version = os_1.default.release().split('.');
        osIdentifier = 'windows';
        osVersion = `${version[0]}.${version[1]}`;
    }
    else if (process.platform === 'darwin') {
        const version = (0, child_process_1.execSync)('sw_vers -productVersion', { stdio: ['ignore', 'pipe', 'ignore'] }).toString().trim().split('.');
        osIdentifier = 'macOS';
        osVersion = `${version[0]}.${version[1]}`;
    }
    ...
    return `copilotbrowser/${getcopilotbrowserVersion()} (${os_1.default.arch()}; ${osIdentifier} ${osVersion}) ${embedderName}/${embedderVersion}${serializedTokens}`;
}
...
function getcopilotbrowserVersion(majorMinorOnly = false) {
