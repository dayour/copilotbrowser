...
exports.gracefullyCloseSet = new Set();
const killSet = new Set();
async function gracefullyCloseAll() {
    await Promise.all(Array.from(exports.gracefullyCloseSet).map(gracefullyClose => gracefullyClose().catch(e => { })));
}
...
async function launchProcess(options) {
    ...
    const cleanup = async () => {
        ...
        const errors = await (0, fileUtils_1.removeFolders)(options.tempDirectories);
    ...
    spawnedProcess.once('close', (exitCode, signal) => {
        ...
        processClosed = true;
        exports.gracefullyCloseSet.delete(gracefullyClose);
    ...
    exports.gracefullyCloseSet.add(gracefullyClose);
    ...
    function killProcessAndCleanup() {
        ...
        for (const dir of options.tempDirectories) {
            try {
                fs_1.default.rmSync(dir, { force: true, recursive: true, maxRetries: 5 });
            }
            catch (e) {
