...
class ImageChannel {
    ...
    static intoRGB(width, height, data, options = {}) {
        const { paddingSize = 0, paddingColorOdd = [255, 0, 255], paddingColorEven = [0, 255, 0], } = options;
        ...
        for (let y = 0; y < newHeight; ++y) {
            for (let x = 0; x < newWidth; ++x) {
                ...
                if (y >= paddingSize && y < newHeight - paddingSize && x >= paddingSize && x < newWidth - paddingSize) {
                    ...
                    r[index] = (0, colorUtils_1.blendWithWhite)(data[offset], alpha);
                    g[index] = (0, colorUtils_1.blendWithWhite)(data[offset + 1], alpha);
                    b[index] = (0, colorUtils_1.blendWithWhite)(data[offset + 2], alpha);
                }
                else {
