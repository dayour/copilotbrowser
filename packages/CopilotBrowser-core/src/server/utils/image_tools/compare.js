...
const imageChannel_1 = require("./imageChannel");
...
function compare(actual, expected, diff, width, height, options = {}) {
    ...
    const [r1, g1, b1] = imageChannel_1.ImageChannel.intoRGB(width, height, expected, {
        paddingSize,
        paddingColorEven,
        paddingColorOdd,
    });
    const [r2, g2, b2] = imageChannel_1.ImageChannel.intoRGB(width, height, actual, {
        paddingSize,
        paddingColorEven,
        paddingColorOdd,
    });
    ...
    const drawGrayPixel = diff ? (x, y) => {
        const gray = (0, colorUtils_1.rgb2gray)(r1.get(x, y), g1.get(x, y), b1.get(x, y));
        const value = (0, colorUtils_1.blendWithWhite)(gray, 0.1);
    ...
    for (let y = paddingSize; y < r1.height - paddingSize; ++y) {
        for (let x = paddingSize; x < r1.width - paddingSize; ++x) {
            ...
            const delta = (0, colorUtils_1.colorDeltaE94)([r1.get(x, y), g1.get(x, y), b1.get(x, y)], [r2.get(x, y), g2.get(x, y), b2.get(x, y)]);
            if (delta <= maxColorDeltaE94) {
                drawGrayPixel(x, y);
                continue;
            }
            if (!fastR || !fastG || !fastB) {
                fastR = new stats_1.FastStats(r1, r2);
                fastG = new stats_1.FastStats(g1, g2);
                fastB = new stats_1.FastStats(b1, b2);
            }
            ...
            const ssimRGB = ((0, stats_1.ssim)(fastR, ssimX1, ssimY1, ssimX2, ssimY2) + (0, stats_1.ssim)(fastG, ssimX1, ssimY1, ssimX2, ssimY2) + (0, stats_1.ssim)(fastB, ssimX1, ssimY1, ssimX2, ssimY2)) / 3.0;
