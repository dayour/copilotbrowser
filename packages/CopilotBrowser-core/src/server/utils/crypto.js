...
class DER {
    ...
    static encodeInteger(data) {
        (0, assert_1.assert)(data >= -128 && data <= 127);
    ...
    static encodeSet(data) {
        (0, assert_1.assert)(data.length === 1, 'Only one item in the set is supported. We\'d need to sort the data to support more.');
...
function generateSelfSignedCertificate() {
    const { privateKey, publicKey } = crypto_1.default.generateKeyPairSync('rsa', { modulusLength: 2048 });
        ...
        DER.encodeSequence([
            ...
            DER.encodeSet([
                DER.encodeSequence([
                    ...
                    DER.encodePrintableString('copilotbrowser Client Certificate Support')
                ])
            ])
        ]), // issuer
        DER.encodeSequence([
    ...
    const signature = crypto_1.default.sign('sha256', tbsCertificate, privateKey);
