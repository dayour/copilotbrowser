...
class AndroidDeviceDispatcher extends dispatcher_1.Dispatcher {
    ...
    constructor(scope, device) {
        ...
        this.addObjectListener(android_1.AndroidDevice.Events.WebViewAdded, webView => this._dispatchEvent('webViewAdded', { webView }));
        this.addObjectListener(android_1.AndroidDevice.Events.WebViewRemoved, socketName => this._dispatchEvent('webViewRemoved', { socketName }));
        this.addObjectListener(android_1.AndroidDevice.Events.Close, () => this._dispatchEvent('close'));
    }
    ...
    async launchBrowser(params, progress) {
        ...
        return { context: browserContextDispatcher_1.BrowserContextDispatcher.from(this, context) };
    }
    async close(params, progress) {
        await this._object.close();
    }
    async connectToWebView(params, progress) {
        return { context: browserContextDispatcher_1.BrowserContextDispatcher.from(this, await this._object.connectToWebView(progress, params.socketName)) };
    }
}
exports.AndroidDeviceDispatcher = AndroidDeviceDispatcher;
class SocketSdkObject extends instrumentation_1.SdkObject {
    _socket;
    _eventListeners;
    constructor(parent, socket) {
        ...
        this._eventListeners = [
            eventsHelper_1.eventsHelper.addEventListener(socket, 'data', data => this.emit('data', data)),
            eventsHelper_1.eventsHelper.addEventListener(socket, 'close', () => {
