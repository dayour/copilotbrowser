...
class JSHandleDispatcher extends dispatcher_1.Dispatcher {
    ...
    async evaluateExpressionHandle(params, progress) {
        ...
        return { handle: elementHandlerDispatcher_1.ElementHandleDispatcher.fromJSOrElementHandle(this.parentScope(), jsHandle) };
    }
    ...
    async getPropertyList(params, progress) {
        ...
        for (const [name, value] of map) {
            // If "jsHandle" is an ElementHandle, it belongs to the same frame as "this".
            properties.push({ name, value: elementHandlerDispatcher_1.ElementHandleDispatcher.fromJSOrElementHandle(this.parentScope(), value) });
        }
        return { properties };
    }
...
function parseArgument(arg) {
    return (0, serializers_1.parseSerializedValue)(arg.value, arg.handles.map(a => a._object));
}
function parseValue(v) {
    return (0, serializers_1.parseSerializedValue)(v, []);
}
function serializeResult(arg) {
    return (0, serializers_1.serializeValue)(arg, value => ({ fallThrough: value }));
}
