...
export class LocalUtilsDispatcher extends Dispatcher<SdkObject, channels.LocalUtilsChannel, RootDispatcher> implements channels.LocalUtilsChannel {
  ...
  constructor(scope: RootDispatcher, copilotbrowser: copilotbrowser) {
    const localUtils = new SdkObject(copilotbrowser, 'localUtils', 'localUtils');
  ...
  async connect(params: channels.LocalUtilsConnectParams, progress: Progress): Promise<channels.LocalUtilsConnectResult> {
    ...
    const transport = await WebSocketTransport.connect(progress, wsEndpoint, { headers: wsHeaders, followRedirects: true, debugLogHeader: 'x-copilotbrowser-debug-log' });
