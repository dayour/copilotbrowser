...
class Helper {
    ...
    static waitForEvent(progress, emitter, event, predicate) {
        const listeners = [];
        const dispose = () => eventsHelper_1.eventsHelper.removeEventListeners(listeners);
        const promise = progress.race(new Promise((resolve, reject) => {
            listeners.push(eventsHelper_1.eventsHelper.addEventListener(emitter, event, eventArg => {
                try {
    ...
    static debugProtocolLogger(protocolLogger) {
        return (direction, message) => {
            ...
            if (debugLogger_1.debugLogger.isEnabled('protocol')) {
                ...
                debugLogger_1.debugLogger.log('protocol', (direction === 'send' ? 'SEND ► ' : '◀ RECV ') + text);
            }
        };
    }
