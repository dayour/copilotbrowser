...
async function saveProtocolStream(client, handle, path) {
    let eof = false;
    await (0, fileUtils_1.mkdirIfNeeded)(path);
    const fd = await fs_1.default.promises.open(path, 'w');
    while (!eof) {
...
function exceptionToError(exceptionDetails) {
    ...
    const { name, message } = (0, stackTrace_1.splitErrorMessage)(messageWithName);
