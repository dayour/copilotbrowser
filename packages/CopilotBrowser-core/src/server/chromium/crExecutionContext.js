...
class CRExecutionContext {
    ...
    async rawEvaluateJSON(expression) {
        ...
        if (exceptionDetails)
            throw new js.JavaScriptErrorInEvaluate((0, crProtocolHelper_1.getExceptionMessage)(exceptionDetails));
    ...
    async evaluateWithArguments(expression, returnByValue, utilityScript, values, handles) {
        ...
        if (exceptionDetails)
            throw new js.JavaScriptErrorInEvaluate((0, crProtocolHelper_1.getExceptionMessage)(exceptionDetails));
        return returnByValue ? (0, utilityScriptSerializers_1.parseEvaluationResultValue)(remoteObject.value) : createHandle(utilityScript._context, remoteObject);
    }
    ...
    async releaseHandle(handle) {
        if (!handle._objectId)
            return;
        await (0, crProtocolHelper_1.releaseObject)(this._client, handle._objectId);
    }
}
...
function rewriteError(error) {
    ...
    if (error instanceof TypeError && error.message.startsWith('Converting circular structure to JSON'))
        (0, stackTrace_1.rewriteErrorMessage)(error, error.message + ' Are you passing a nested JSHandle?');
