...
const crServiceWorker_1 = require("./crServiceWorker");
class CRBrowser extends browser_1.Browser {
    _connection;
    _session;
    _clientRootSessionPromise = null;
    _contexts = new Map();
    _crPages = new Map();
    _serviceWorkers = new Map();
    _devtools;
    _version = '';
    _majorVersion = 0;
    _revision = '';
    ...
    static async connect(parent, transport, options, devtools) {
        ...
        options = { ...options };
        const connection = new crConnection_1.CRConnection(parent, transport, options.protocolLogger, options.browserLogsCollector);
    ...
    constructor(parent, connection, options) {
        ...
        this._connection.on(crConnection_1.ConnectionEvents.Disconnected, () => this._didDisconnect());
    ...
    async doCreateNewContext(options) {
        ...
        if (proxy) {
            if (process.env.copilotbrowser_DISABLE_FORCED_CHROMIUM_PROXIED_LOOPBACK)
    ...
    _onAttachedToTarget({ targetInfo, sessionId, waitingForDebugger }) {
        ...
        (0, assert_1.assert)(targetInfo.browserContextId, 'targetInfo: ' + JSON.stringify(targetInfo, null, 2));
        ...
        (0, assert_1.assert)(!this._crPages.has(targetInfo.targetId), 'Duplicate target ' + targetInfo.targetId);
        (0, assert_1.assert)(!this._serviceWorkers.has(targetInfo.targetId), 'Duplicate target ' + targetInfo.targetId);
        if (targetInfo.type === 'page' || treatOtherAsPage) {
            const opener = targetInfo.openerId ? this._crPages.get(targetInfo.openerId) || null : null;
            const crPage = new crPage_1.CRPage(session, targetInfo.targetId, context, opener, { hasUIWindow: targetInfo.type === 'page' });
            this._crPages.set(targetInfo.targetId, crPage);
            return;
        }
        if (targetInfo.type === 'service_worker') {
            const serviceWorker = new crServiceWorker_1.CRServiceWorker(context, session, targetInfo.url);
    ...
    _onDownloadWillBegin(payload) {
        ...
        if (!page) {
            ...
            // See https://github.com/dayour/copilotbrowser/issues/22551.
            return;
        }
    ...
    async startTracing(page, options = {}) {
        (0, assert_1.assert)(!this._tracingRecording, 'Cannot start recording trace while already recording trace.');
    ...
    async stopTracing() {
        (0, assert_1.assert)(this._tracingClient, 'Tracing was not started.');
        const [event] = await Promise.all([
            new Promise(f => this._tracingClient.once('Tracing.tracingComplete', f)),
            this._tracingClient.send('Tracing.end')
        ]);
        const tracingPath = path_1.default.join(this.options.artifactsDir, (0, crypto_1.createGuid)() + '.crtrace');
        await (0, crProtocolHelper_1.saveProtocolStream)(this._tracingClient, event.stream, tracingPath);
        this._tracingRecording = false;
        const artifact = new artifact_1.Artifact(this, tracingPath);
...
class CRBrowserContext extends browserContext_1.BrowserContext {
    ...
    async _initialize() {
        (0, assert_1.assert)(!Array.from(this._browser._crPages.values()).some(page => page._browserContext === this));
    ...
    async doExposecopilotbrowserBinding() {
        for (const page of this._crPages())
            await page.exposecopilotbrowserBinding();
    }
    ...
    async newCDPSession(page) {
        ...
        else if (page instanceof frames_1.Frame) {
