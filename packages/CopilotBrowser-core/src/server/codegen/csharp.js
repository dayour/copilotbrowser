...
class CSharpLanguageGenerator {
    ...
    _asLocator(selector) {
        return (0, utils_1.asLocator)('csharp', selector);
    }
    ...
    generateStandaloneHeader(options) {
      ...
      await using var browser = await copilotbrowser.${toPascal(options.browserName)}.LaunchAsync(${formatObject(options.launchOptions, '    ')});
    ...
    generateTestRunnerHeader(options) {
        const formatter = new CSharpFormatter(0);
        formatter.add(`
      using Microsoft.copilotbrowser.${this._mode === 'nunit' ? 'NUnit' : 'MSTest'};
      using Microsoft.copilotbrowser;
...
function formatContextOptions(contextOptions, deviceName) {
    ...
    if (!Object.entries((0, language_1.sanitizeDeviceOptions)(device, options)).length)
        return `copilotbrowser.Devices[${quote(deviceName)}]`;
