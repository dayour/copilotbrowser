...
class PythonLanguageGenerator {
    ...
    _asLocator(selector) {
        return (0, utils_1.asLocator)('python', selector);
    }
...
import asyncio
import re
from copilotbrowser.async_api import copilotbrowser, async_copilotbrowser, expect


async def run(copilotbrowser: copilotbrowser) -> None {
    browser = await copilotbrowser.${options.browserName}.launch(${formatOptions(options.launchOptions, false)})
    context = await browser.new_context(${formatContextOptions(options.contextOptions, options.deviceName)})`);
            ...
            formatter.add(`
import re
from copilotbrowser.sync_api import copilotbrowser, sync_copilotbrowser, expect


def run(copilotbrowser: copilotbrowser) -> None {
    browser = copilotbrowser.${options.browserName}.launch(${formatOptions(options.launchOptions, false)})
...
function formatOptions(value, hasArguments, asDict) {
    ...
    return (hasArguments ? ', ' : '') + keys.map(key => {
        if (asDict)
            return `"${(0, utils_1.toSnakeCase)(key)}": ${formatValue(value[key])}`;
        return `${(0, utils_1.toSnakeCase)(key)}=${formatValue(value[key])}`;
    }).join(', ');
}
function formatContextOptions(options, deviceName, asDict) {
    ...
    return `**copilotbrowser.devices[${quote(deviceName)}]` + formatOptions((0, language_1.sanitizeDeviceOptions)(device, options), true, asDict);
}
