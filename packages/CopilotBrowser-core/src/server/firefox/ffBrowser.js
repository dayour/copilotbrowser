...
class FFBrowser extends browser_1.Browser {
    ...
    _onAttachedToTarget(payload) {
        ...
        (0, utils_1.assert)(type === 'page');
        const context = browserContextId ? this._contexts.get(browserContextId) : this._defaultContext;
        (0, utils_1.assert)(context, `Unknown context id:${browserContextId}, _defaultContext: ${this._defaultContext}`);
...
class FFBrowserContext extends browserContext_1.BrowserContext {
    ...
    async _initialize() {
        (0, utils_1.assert)(!this._ffPages().length);
    ...
    async doExposecopilotbrowserBinding() {
        this._browser.session.send('Browser.addBinding', { browserContextId: this._browserContextId, name: page_1.PageBinding.kBindingName, script: '' });
    }
    onClosePersistent() { }
    async clearCache() {
...
// Should all be moved to `copilotbrowser.cfg`.
