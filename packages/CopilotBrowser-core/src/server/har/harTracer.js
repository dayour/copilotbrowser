...
class HarTracer {
    ...
    constructor(context, page, delegate, options) {
        ...
        this._baseURL = context instanceof fetch_1.APIRequestContext ? context._defaultOptions().baseURL : context._options.baseURL;
    }
    start(options) {
        if (this._started)
            return;
        ...
        this._started = true;
        const apiRequest = this._context instanceof fetch_1.APIRequestContext ? this._context : this._context.fetchRequest;
        this._eventListeners = [
            eventsHelper_1.eventsHelper.addEventListener(apiRequest, fetch_1.APIRequestContext.Events.Request, (event) => this._onAPIRequest(event)),
            eventsHelper_1.eventsHelper.addEventListener(apiRequest, fetch_1.APIRequestContext.Events.RequestFinished, (event) => this._onAPIRequestFinished(event)),
        ];
        if (this._context instanceof browserContext_1.BrowserContext) {
            this._eventListeners.push(eventsHelper_1.eventsHelper.addEventListener(this._context, browserContext_1.BrowserContext.Events.Page, (page) => this._createPageEntryIfNeeded(page)), eventsHelper_1.eventsHelper.addEventListener(this._context, browserContext_1.BrowserContext.Events.Request, (request) => this._onRequest(request)), eventsHelper_1.eventsHelper.addEventListener(this._context, browserContext_1.BrowserContext.Events.RequestFinished, ({ request, response }) => this._onRequestFinished(request, response).catch(() => { })), eventsHelper_1.eventsHelper.addEventListener(this._context, browserContext_1.BrowserContext.Events.RequestFailed, request => this._onRequestFailed(request)), eventsHelper_1.eventsHelper.addEventListener(this._context, browserContext_1.BrowserContext.Events.Response, (response) => this._onResponse(response)), eventsHelper_1.eventsHelper.addEventListener(this._context, browserContext_1.BrowserContext.Events.RequestAborted, request => this._onRequestAborted(request)), eventsHelper_1.eventsHelper.addEventListener(this._context, browserContext_1.BrowserContext.Events.RequestFulfilled, request => this._onRequestFulfilled(request)), eventsHelper_1.eventsHelper.addEventListener(this._context, browserContext_1.BrowserContext.Events.RequestContinued, request => this._onRequestContinued(request)));
            for (const page of this._context.pages())
                this._createPageEntryIfNeeded(page);
        }
    }
    _shouldIncludeEntryWithUrl(urlString) {
        return !this._options.urlFilter || (0, utils_2.urlMatches)(this._baseURL, urlString, this._options.urlFilter);
    }
    ...
    _createPageEntryIfNeeded(page) {
        ...
        if (!pageEntry) {
            ...
            page.mainFrame().on(frames_1.Frame.Events.AddLifecycle, (event) => {
                if (event === 'load')
    ...
    _onRequest(request) {
        ...
        (0, utils_1.assert)(this._started);
        this._delegate.onEntryStarted(harEntry);
    }
    ...
    async _onRequestFinished(request, response) {
        ...
        // it here to not hang in case of long chunked responses, see https://github.com/dayour/copilotbrowser/issues/21182.
        if (!this._options.omitServerIP) {
        ...
        const compressionCalculationBarrier = this._options.omitSizes ? undefined : {
            _encodedBodySize: -1,
            _decodedBodySize: -1,
            barrier: new manualPromise_1.ManualPromise(),
            _check: function () {
        ...
        harEntry.timings.receive = response.request()._responseEndTiming !== -1 ? helper_1.helper.millisToRoundishMillis(response.request()._responseEndTiming - timing.responseStart) : -1;
        this._computeHarEntryTotalTime(harEntry);
        if (!this._options.omitSizes) {
    ...
    _storeResponseContent(buffer, content, resourceType) {
        ...
        if (this._options.content === 'embed') {
            ...
            if ((0, utils_2.isTextualMimeType)(content.mimeType) && resourceType !== 'font') {
            ...
            const sha1 = (0, utils_1.calculateSha1)(buffer) + '.' + (utilsBundle_1.mime.getExtension(content.mimeType) || 'dat');
    ...
    stop() {
        ...
        const context = this._context instanceof browserContext_1.BrowserContext ? this._context : undefined;
        const log = {
            version: '1.2',
            creator: {
                name: 'copilotbrowser',
                version: (0, utils_2.getcopilotbrowserVersion)(),
            },
            browser: {
    ...
    _postDataForBuffer(postData, contentType, content) {
        ...
        if (content === 'attach') {
            const sha1 = (0, utils_1.calculateSha1)(postData) + '.' + (utilsBundle_1.mime.getExtension(contentType) || 'dat');
            if (this._options.includeTraceInfo)
                result._sha1 = sha1;
            else
                result._file = sha1;
...
function createHarEntry(pageRef, method, url, frameref, options) {
    const harEntry = {
        ...
        _monotonicTime: options.includeTraceInfo ? (0, utils_1.monotonicTime)() : undefined,
    };
    return harEntry;
}
