...
export class WebKit extends BrowserType {
  ...
  override connectToTransport(transport: ConnectionTransport, options: BrowserOptions): Promise<WKBrowser> {
    return WKBrowser.connect(this.attribution.copilotbrowser, transport, options);
  }
  ...
  override attemptToGracefullyCloseBrowser(transport: ConnectionTransport): void {
    ...
    transport.send({ method: 'copilotbrowser.close', params: {}, id: kBrowserCloseMessageId });
  }
...
export async function translatePathToWSL(path: string): Promise<string> {
  const { stdout } = await spawnAsync('wsl.exe', ['-d', 'copilotbrowser', '--cd', '/home/pwuser', 'wslpath', path.replace(/\\/g, '\\\\')]);
