...
exports.registry = exports.Registry = exports.registryDirectory = exports.writeDockerVersion = void 0;
exports.browserDirectoryToMarkerFilePath = browserDirectoryToMarkerFilePath;
exports.buildcopilotbrowserCLICommand = buildcopilotbrowserCLICommand;
exports.installBrowsersForNpmInstall = installBrowsersForNpmInstall;
exports.findChromiumChannelBestEffort = findChromiumChannelBestEffort;
...
Object.defineProperty(exports, "writeDockerVersion", { enumerable: true, get: function () { return dependencies_3.writeDockerVersion; } });
...
if (process.env.PW_TEST_CDN_THAT_SHOULD_WORK) {
    for (let i = 0; i < copilotbrowser_CDN_MIRRORS.length; i++) {
        const cdn = copilotbrowser_CDN_MIRRORS[i];
        if (cdn !== process.env.PW_TEST_CDN_THAT_SHOULD_WORK) {
            const parsedCDN = new URL(cdn);
            parsedCDN.hostname = parsedCDN.hostname + '.does-not-resolve.copilotbrowser.dev';
            copilotbrowser_CDN_MIRRORS[i] = parsedCDN.toString();
        }
    }
}
const EXECUTABLE_PATHS = {
    ...
    'webkit': {
        '<unknown>': undefined,
        'linux-x64': ['cb_run.sh'],
        'linux-arm64': ['cb_run.sh'],
        'mac-x64': ['cb_run.sh'],
        'mac-arm64': ['cb_run.sh'],
        'win-x64': ['copilotbrowser.exe'],
    },
    'ffmpeg': {
        '<unknown>': undefined,
...
function cftUrl(suffix) {
    return ({ browserVersion }) => {
        const customHost = (0, utils_1.getFromENV)('copilotbrowser_CHROMIUM_DOWNLOAD_HOST') || (0, utils_1.getFromENV)('copilotbrowser_DOWNLOAD_HOST');
        if (customHost) {
            return {
        ...
        return {
            ...
            mirrors: [
                'https://github.com/dayour/copilotbrowser/releases/download',
...
exports.registryDirectory = (() => {
    let result;
    const envDefined = (0, utils_1.getFromENV)('copilotbrowser_BROWSERS_PATH');
    if (envDefined === '0') {
        ...
        result = path_1.default.join(cacheDirectory, 'ms-copilotbrowser');
    }
    if (!path_1.default.isAbsolute(result)) {
        ...
        result = path_1.default.resolve((0, utils_1.getFromENV)('INIT_CWD') || process.cwd(), result);
    }
    return result;
})();
...
function readDescriptors(browsersJSON) {
    return (browsersJSON['browsers']).map(obj => {
        const name = obj.name;
        const revisionOverride = (obj.revisionOverrides || {})[hostPlatform_1.hostPlatform];
        const revision = revisionOverride || obj.revision;
        const browserDirectoryPrefix = revisionOverride ? `${name}_${hostPlatform_1.hostPlatform}_special` : `${name}`;
        const descriptor = {
            name,
            revision,
            ...
            dir: path_1.default.join(exports.registryDirectory, browserDirectoryPrefix.replace(/-/g, '_') + '-' + revision),
        };
        return descriptor;
    });
}
...
class Registry {
    ...
    constructor(browsersJSON) {
        ...
        const findExecutablePath = (dir, name) => {
            const tokens = EXECUTABLE_PATHS[name][hostPlatform_1.shortPlatform];
            return tokens ? path_1.default.join(dir, ...tokens) : undefined;
        };
        const executablePathOrDie = (name, e, installByDefault, sdkLanguage) => {
            if (!e)
                throw new Error(`${name} is not supported on ${hostPlatform_1.hostPlatform}`);
            const installCommand = buildcopilotbrowserCLICommand(sdkLanguage, `install${installByDefault ? '' : ' ' + name}`);
            if (!(0, fileUtils_1.canAccessFile)(e)) {
                const currentDockerVersion = (0, dependencies_1.readDockerVersionSync)();
                const preferredDockerVersion = currentDockerVersion ? (0, dependencies_1.dockerVersion)(currentDockerVersion.dockerImageNameTemplate) : null;
                ...
                if (isOutdatedDockerImage) {
                    prettyMessage = [
                        `Looks like copilotbrowser was just updated to ${preferredDockerVersion.driverVersion}.`,
                    ...
                    prettyMessage = [
                        ...
                        ``,
                        `    ${buildcopilotbrowserCLICommand(sdkLanguage, 'install ffmpeg')}`,
                        ``,
                        `<3 copilotbrowser Team`,
                    ].join('\n');
                }
                    ...
                    prettyMessage = [
                        `Looks like copilotbrowser was just installed or updated.`,
                ...
                throw new Error(`Executable doesn't exist at ${e}\n${(0, ascii_1.wrapInASCIIBox)(prettyMessage, 1)}`);
            }
            return e;
        };
        this._executables = [];
        ...
        const webkitLinuxLddDirectories = [
            path_1.default.join('minibrowser-gtk'),
            path_1.default.join('minibrowser-gtk', 'bin'),
            path_1.default.join('minibrowser-gtk', 'lib'),
            path_1.default.join('minibrowser-gtk', 'sys', 'lib'),
            path_1.default.join('minibrowser-wpe'),
            path_1.default.join('minibrowser-wpe', 'bin'),
            path_1.default.join('minibrowser-wpe', 'lib'),
            path_1.default.join('minibrowser-wpe', 'sys', 'lib'),
        ];
        this._executables.push({
            name: 'webkit',
            browserName: 'webkit',
        ...
        this._executables.push({
            ...
            _install: async () => {
                ...
                const script = path_1.default.join(BIN_PATH, 'install_webkit_wsl.ps1');
                const { code } = await (0, spawnAsync_1.spawnAsync)('powershell.exe', [
    ...
    _createChromiumChannel(name, lookAt, install) {
        const executablePath = (sdkLanguage, shouldThrow) => {
            ...
            for (const prefix of prefixes) {
                const executablePath = path_1.default.join(prefix, suffix);
                if ((0, fileUtils_1.canAccessFile)(executablePath))
                    return executablePath;
            }
            if (!shouldThrow)
                return undefined;
            const location = prefixes.length ? ` at ${path_1.default.join(prefixes[0], suffix)}` : ``;
            const installation = install ? `\nRun "${buildcopilotbrowserCLICommand(sdkLanguage, 'install ' + name)}"` : '';
    ...
    async _validateHostRequirements(sdkLanguage, browserDirectory, linuxLddDirectories, dlOpenLibraries, windowsExeAndDllDirectories) {
        if (os_1.default.platform() === 'linux')
            return await (0, dependencies_2.validateDependenciesLinux)(sdkLanguage, linuxLddDirectories.map(d => path_1.default.join(browserDirectory, d)), dlOpenLibraries);
        if (os_1.default.platform() === 'win32' && os_1.default.arch() === 'x64')
            return await (0, dependencies_2.validateDependenciesWindows)(sdkLanguage, windowsExeAndDllDirectories.map(d => path_1.default.join(browserDirectory, d)));
    }
    async installDeps(executablesToInstallDeps, dryRun) {
        ...
        if (os_1.default.platform() === 'win32')
            return await (0, dependencies_2.installDependenciesWindows)(targets, dryRun);
        if (os_1.default.platform() === 'linux')
            return await (0, dependencies_2.installDependenciesLinux)(targets, dryRun);
    }
    async install(executablesToInstall, options) {
        const executables = this._dedupe(executablesToInstall);
        await fs_1.default.promises.mkdir(exports.registryDirectory, { recursive: true });
        const lockfilePath = path_1.default.join(exports.registryDirectory, '__dirlock');
        const linksDir = path_1.default.join(exports.registryDirectory, '.links');
        let releaseLock;
        try {
            releaseLock = await utilsBundle_1.lockfile.lock(exports.registryDirectory, {
                retries: {
            ...
            await fs_1.default.promises.writeFile(path_1.default.join(linksDir, (0, utils_1.calculateSha1)(PACKAGE_PATH)), PACKAGE_PATH);
            // Remove stale browsers.
            if (!(0, utils_1.getAsBooleanFromENV)('copilotbrowser_SKIP_BROWSER_GC'))
                await this._validateInstallationCache(linksDir);
            // Install browsers for this package.
            for (const executable of executables) {
                if (!executable._install)
                    throw new Error(`ERROR: copilotbrowser does not support installing ${executable.name}`);
                if (!(0, utils_1.getAsBooleanFromENV)('CI') && !executable._isHermeticInstallation && !options?.force && executable.executablePath()) {
                    const { embedderName } = (0, userAgent_1.getEmbedderName)();
                    const command = buildcopilotbrowserCLICommand(embedderName, 'install --force ' + executable.name);
            ...
            if (e.code === 'ELOCKED') {
                ...
                throw new Error('\n' + (0, ascii_1.wrapInASCIIBox)([
                    ...
                    ``,
                    `Either:`,
                    `- wait a few minutes if other copilotbrowser is installing browsers in parallel`,
    ...
    async uninstall(all) {
        const linksDir = path_1.default.join(exports.registryDirectory, '.links');
        if (all) {
            const links = await fs_1.default.promises.readdir(linksDir).catch(() => []);
            for (const link of links)
                await fs_1.default.promises.unlink(path_1.default.join(linksDir, link));
        }
        else {
            await fs_1.default.promises.unlink(path_1.default.join(linksDir, (0, utils_1.calculateSha1)(PACKAGE_PATH))).catch(() => { });
        }
        // Remove stale browsers.
        await this._validateInstallationCache(linksDir);
        return {
            numberOfBrowsersLeft: (await fs_1.default.promises.readdir(exports.registryDirectory).catch(() => [])).filter(browserDirectory => isBrowserDirectory(browserDirectory)).length
        };
    }
    async validateHostRequirementsForExecutablesIfNeeded(executables, sdkLanguage) {
        if ((0, utils_1.getAsBooleanFromENV)('copilotbrowser_SKIP_VALIDATE_HOST_REQUIREMENTS')) {
            // eslint-disable-next-line no-restricted-properties
            process.stderr.write('Skipping host requirements validation logic because `copilotbrowser_SKIP_VALIDATE_HOST_REQUIREMENTS` env variable is set.\n');
            return;
        }
        for (const executable of executables)
    ...
    async _validateHostRequirementsForExecutableIfNeeded(executable, sdkLanguage) {
        ...
        if (!executable.directory)
            return;
        const markerFile = path_1.default.join(executable.directory, 'DEPENDENCIES_VALIDATED');
        // Executable is already validated.
        if (await fs_1.default.promises.stat(markerFile).then(stat => (Date.now() - stat.mtime.getTime()) < kMaximumReValidationPeriod).catch(() => false))
            return;
        debugLogger_1.debugLogger.log('install', `validating host requirements for "${executable.name}"`);
        try {
            await executable._validateHostRequirements(sdkLanguage);
            debugLogger_1.debugLogger.log('install', `validation passed for ${executable.name}`);
        }
        catch (error) {
            debugLogger_1.debugLogger.log('install', `validation failed for ${executable.name}`);
            throw error;
        }
        await fs_1.default.promises.writeFile(markerFile, '').catch(() => { });
    }
    _downloadURLs(descriptor) {
        const paths = DOWNLOAD_PATHS[descriptor.name];
        const downloadPathTemplate = paths[hostPlatform_1.hostPlatform] || paths['<unknown>'];
        ...
        if (typeof downloadPathTemplate === 'function') {
            ...
            mirrors = copilotbrowser_CDN_MIRRORS;
        }
        let downloadHostEnv;
        if (descriptor.name.startsWith('chromium'))
            downloadHostEnv = 'copilotbrowser_CHROMIUM_DOWNLOAD_HOST';
        else if (descriptor.name.startsWith('firefox'))
            downloadHostEnv = 'copilotbrowser_FIREFOX_DOWNLOAD_HOST';
        else if (descriptor.name.startsWith('webkit'))
            downloadHostEnv = 'copilotbrowser_WEBKIT_DOWNLOAD_HOST';
        const customHostOverride = (downloadHostEnv && (0, utils_1.getFromENV)(downloadHostEnv)) || (0, utils_1.getFromENV)('copilotbrowser_DOWNLOAD_HOST');
    ...
    async _downloadExecutable(descriptor, force, executablePath) {
        ...
        if (!downloadURLs.length)
            throw new Error(`ERROR: copilotbrowser does not support ${descriptor.name} on ${hostPlatform_1.hostPlatform}`);
        if (!hostPlatform_1.isOfficiallySupportedPlatform)
            (0, browserFetcher_1.logPolitely)(`BEWARE: your OS is not officially supported by copilotbrowser; downloading fallback build for ${hostPlatform_1.hostPlatform}.`);
        if (descriptor.hasRevisionOverride) {
            const message = `You are using a frozen ${descriptor.name} browser which does not receive updates anymore on ${hostPlatform_1.hostPlatform}. Please update to the latest version of your operating system to test up-to-date browsers.`;
            if (process.env.GITHUB_ACTIONS)
                console.log(`::warning title=copilotbrowser::${message}`); // eslint-disable-line no-console
            else
                (0, browserFetcher_1.logPolitely)(message);
        }
        const title = this.calculateDownloadTitle(descriptor);
        const downloadFileName = `copilotbrowser-download-${descriptor.name}-${hostPlatform_1.hostPlatform}-${descriptor.revision}.zip`;
        // copilotbrowser_DOWNLOAD_CONNECTION_TIMEOUT is a misnomer, it actually controls the socket's
        // max idle timeout. Unfortunately, we cannot rename it without breaking existing user workflows.
        const downloadSocketTimeoutEnv = (0, utils_1.getFromENV)('copilotbrowser_DOWNLOAD_CONNECTION_TIMEOUT');
        const downloadSocketTimeout = +(downloadSocketTimeoutEnv || '0') || network_1.NET_DEFAULT_TIMEOUT;
        await (0, browserFetcher_1.downloadBrowserWithProgressBar)(title, descriptor.dir, executablePath, downloadURLs, downloadFileName, downloadSocketTimeout, force).catch(e => {
    ...
    calculateDownloadTitle(descriptor) {
        ...
        return `${title}${version} (copilotbrowser ${descriptor.name} v${descriptor.revision})`;
    }
    async _installMSEdgeChannel(channel, scripts) {
        const scriptArgs = [];
        if (process.platform !== 'linux') {
            const products = lowercaseAllKeys(JSON.parse(await (0, network_1.fetchData)(undefined, { url: 'https://edgeupdates.microsoft.com/api/products' })));
            const productName = {
                'msedge': 'Stable',
                'msedge-beta': 'Beta',
                'msedge-dev': 'Dev',
    ...
    async _installChromiumChannel(channel, scripts, scriptArgs = []) {
        ...
        if (isPowerShell) {
            const args = [
                ...
                path_1.default.join(BIN_PATH, scriptName),
                ...scriptArgs
            ];
            const { code } = await (0, spawnAsync_1.spawnAsync)('powershell.exe', args, { cwd, stdio: 'inherit' });
            if (code !== 0)
                throw new Error(`Failed to install ${channel}`);
        }
        else {
            const { command, args, elevatedPermissions } = await (0, dependencies_1.transformCommandsForRoot)([`bash "${path_1.default.join(BIN_PATH, scriptName)}" ${scriptArgs.join('')}`]);
            if (elevatedPermissions)
                console.log('Switching to root user to install dependencies...'); // eslint-disable-line no-console
            const { code } = await (0, spawnAsync_1.spawnAsync)(command, args, { cwd, stdio: 'inherit' });
            if (code !== 0)
                throw new Error(`Failed to install ${channel}`);
        }
    }
    async listInstalledBrowsers() {
        ...
        return browsers.filter(browser => fs_1.default.existsSync(browser.browserPath));
    }
    ...
    async _traverseBrowserInstallations(linksDir) {
        ...
        for (const fileName of await fs_1.default.promises.readdir(linksDir)) {
            ...
            try {
                ...
                const browsersJSON = require(path_1.default.join(linkTarget, 'browsers.json'));
                const descriptors = readDescriptors(browsersJSON);
                for (const browserName of allDownloadableDirectoriesThatEverExisted) {
    ...
    async _deleteStaleBrowsers(browserList) {
        ...
        for (const browser of browserList) {
            ...
            if (!shouldHaveMarkerFile || (await (0, fileUtils_1.existsAsync)(browserDirectoryToMarkerFilePath(browserPath))))
                usedBrowserPaths.add(browserPath);
        }
        let downloadedBrowsers = (await fs_1.default.promises.readdir(exports.registryDirectory)).map(file => path_1.default.join(exports.registryDirectory, file));
        downloadedBrowsers = downloadedBrowsers.filter(file => isBrowserDirectory(file));
        const directories = new Set(downloadedBrowsers);
        for (const browserDirectory of usedBrowserPaths)
            directories.delete(browserDirectory);
        for (const directory of directories)
            (0, browserFetcher_1.logPolitely)('Removing unused browser at ' + directory);
        await (0, fileUtils_1.removeFolders)([...directories]);
    }
...
function buildcopilotbrowserCLICommand(sdkLanguage, parameters) {
    switch (sdkLanguage) {
        case 'python':
            return `copilotbrowser ${parameters}`;
        case 'java':
            return `mvn exec:java -e -D exec.mainClass=com.microsoft.copilotbrowser.CLI -D exec.args="${parameters}"`;
        case 'csharp':
            return `pwsh bin/Debug/netX/copilotbrowser.ps1 ${parameters}`;
        default: {
            const packageManagerCommand = (0, utils_1.getPackageManagerExecCommand)();
            return `${packageManagerCommand} copilotbrowser ${parameters}`;
        }
    }
}
async function installBrowsersForNpmInstall(browsers) {
    // copilotbrowser_SKIP_BROWSER_DOWNLOAD should have a value of 0 or 1
    if ((0, utils_1.getAsBooleanFromENV)('copilotbrowser_SKIP_BROWSER_DOWNLOAD')) {
        (0, browserFetcher_1.logPolitely)('Skipping browsers download because `copilotbrowser_SKIP_BROWSER_DOWNLOAD` env variable is set');
        return false;
    }
    const executables = [];
    if (process.platform === 'win32')
        executables.push(exports.registry.findExecutable('winldd'));
    for (const browserName of browsers) {
        const executable = exports.registry.findExecutable(browserName);
        if (!executable || executable.installType === 'none')
            throw new Error(`Cannot install ${browserName}`);
        executables.push(executable);
    }
    await exports.registry.install(executables);
}
...
function findChromiumChannelBestEffort(sdkLanguage) {
    ...
    for (const name of ['chromium', 'chrome', 'msedge']) {
        try {
            exports.registry.findExecutable(name).executablePathOrDie(sdkLanguage);
    ...
    if (channel === null) {
        const installCommand = buildcopilotbrowserCLICommand(sdkLanguage, `install chromium`);
        const prettyMessage = [
...
exports.registry = new Registry(require('../../../browsers.json'));
