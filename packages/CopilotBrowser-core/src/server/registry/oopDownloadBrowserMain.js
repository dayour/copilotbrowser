...
function downloadFile(options) {
    ...
    (0, network_1.httpRequest)({
        ...
        const file = fs_1.default.createWriteStream(options.zipPath);
        file.on('finish', () => {
...
async function main(options) {
    ...
    await (0, fileUtils_1.removeFolders)([options.browserDirectory]);
    log(`extracting archive`);
    await (0, zipBundle_1.extract)(options.zipPath, { dir: options.browserDirectory });
    if (options.executablePath) {
        log(`fixing permissions at ${options.executablePath}`);
        await fs_1.default.promises.chmod(options.executablePath, 0o755);
    }
    await fs_1.default.promises.writeFile(browserDirectoryToMarkerFilePath(options.browserDirectory), '');
}
