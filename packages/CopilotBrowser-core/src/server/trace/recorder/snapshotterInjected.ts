...
export function frameSnapshotStreamer(snapshotStreamer: string, removeNoScript: boolean) {
  ...
  class Streamer {
    ...
    private _refreshListeners() {
      (document as any).addEventListener('__copilotbrowser_mark_target__', (event: CustomEvent) => {
        if (!event.detail)
          return;
        const callId = event.detail as string;
        (event.composedPath()[0] as any).__copilotbrowser_target__ = callId;
      });
      (document as any).addEventListener('__copilotbrowser_unmark_target__', (event: CustomEvent) => {
        if (!event.detail)
          return;
        const callId = event.detail as string;
        if ((event.composedPath()[0] as any).__copilotbrowser_target__ === callId)
          delete (event.composedPath()[0] as any).__copilotbrowser_target__;
      });
    }
      ...
      const visitNode = (node: Node | ShadowRoot): { equals: boolean, n: NodeSnapshot } | undefined => {
        ...
        if (nodeType === Node.ELEMENT_NODE) {
          ...
          if ('__copilotbrowser_target__' in element) {
            ...
            attrs[kTargetAttribute] = element['__copilotbrowser_target__'] as string;
          }
        }
