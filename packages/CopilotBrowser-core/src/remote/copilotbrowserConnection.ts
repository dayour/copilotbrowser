...
import { DispatcherConnection, copilotbrowserDispatcher, RootDispatcher } from '../server';
...
import type { DispatcherScope, copilotbrowser } from '../server';
...
export class copilotbrowserConnection {
  ...
  constructor(semaphore: Semaphore, ws: WebSocket, controller: boolean, copilotbrowser: copilotbrowser, initialize: () => Promise<copilotbrowserInitializeResult>, id: string) {
    ...
    if (controller) {
      ...
      this._root = new DebugControllerDispatcher(this._dispatcherConnection, copilotbrowser.debugController);
      return;
    }
    ...
    this._root = new RootDispatcher(this._dispatcherConnection, async (scope, params) => {
      ...
      const dispatcher = new copilotbrowserDispatcher(scope, copilotbrowser, options);
