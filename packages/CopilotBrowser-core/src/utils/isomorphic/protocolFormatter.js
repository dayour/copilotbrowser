...
const protocolMetainfo_1 = require("./protocolMetainfo");
...
function renderTitleForCall(metadata) {
    const titleFormat = metadata.title ?? protocolMetainfo_1.methodMetainfo.get(metadata.type + '.' + metadata.method)?.title ?? metadata.method;
...
function getActionGroup(metadata) {
    return protocolMetainfo_1.methodMetainfo.get(metadata.type + '.' + metadata.method)?.group;
}
