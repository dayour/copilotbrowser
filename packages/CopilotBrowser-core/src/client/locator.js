...
class Locator {
    ...
    constructor(frame, selector, options) {
        ...
        if (options?.hasText)
            this._selector += ` >> internal:has-text=${(0, stringUtils_1.escapeForTextSelector)(options.hasText, false)}`;
        if (options?.hasNotText)
            this._selector += ` >> internal:has-not-text=${(0, stringUtils_1.escapeForTextSelector)(options.hasNotText, false)}`;
        if (options?.has) {
    ...
    async _withElement(task, options) {
        ...
        const deadline = timeout ? (0, time_1.monotonicTime)() + timeout : 0;
        return await this._frame._wrapApiCall(async () => {
            ...
            const handle = elementHandle_1.ElementHandle.fromNullable(result.element);
            if (!handle)
                throw new Error(`Could not resolve ${this._selector} to DOM Element`);
            try {
                return await task(handle, deadline ? deadline - (0, time_1.monotonicTime)() : 0);
            }
            finally {
    ...
    locator(selectorOrLocator, options) {
        if ((0, rtti_1.isString)(selectorOrLocator))
    ...
    getByTestId(testId) {
        return this.locator((0, locatorUtils_1.getByTestIdSelector)(testIdAttributeName(), testId));
    }
    ...
    description() {
        return (0, locatorGenerators_1.locatorCustomDescription)(this._selector) || null;
    }
    first() {
    ...
    toString() {
        return (0, locatorGenerators_1.asLocatorDescription)('javascript', this._selector);
    }
}
