...
function captureLibraryStackTrace(platform) {
    const stack = (0, stackTrace_1.captureRawStack)();
    let parsedFrames = stack.map(line => {
        const frame = (0, stackTrace_1.parseStackFrame)(line, platform.pathSeparator, platform.showInternalStackFrames());
        if (!frame || !frame.file)
            return null;
        const iscopilotbrowserLibrary = !!platform.coreDir && frame.file.startsWith(platform.coreDir);
        const parsed = {
            frame,
            frameText: line,
            iscopilotbrowserLibrary
        };
        return parsed;
    }).filter(Boolean);
    let apiName = '';
    ...
    for (let i = 0; i < parsedFrames.length - 1; i++) {
        ...
        if (parsedFrame.iscopilotbrowserLibrary && !parsedFrames[i + 1].iscopilotbrowserLibrary) {
