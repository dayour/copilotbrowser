...
export class Request extends ChannelOwner<channels.RequestChannel> {
...
export class Route extends ChannelOwner<channels.RouteChannel> {
  ...
  async fetch(options: FallbackOverrides & { maxRedirects?: number, maxRetries?: number, timeout?: number } = {}): Promise<APIResponse> {
    return await this._wrapApiCall(async () => {
      return await this._context.request._innerFetch({ request: this.request() as any, data: options.postData, ...options });
    });
  }
...
export class Response extends ChannelOwner<channels.ResponseChannel> {
  private _provisionalHeaders: RawHeaders;
  private _actualHeadersPromise: Promise<RawHeaders> | undefined;
...
export class WebSocket extends ChannelOwner<channels.WebSocketChannel> {
  private _page: Page;
