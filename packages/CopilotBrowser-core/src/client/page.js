...
class Page extends channelOwner_1.ChannelOwner {
    ...
    _workers = new Set();
    _closed = false;
    _closedOrCrashedScope = new manualPromise_1.LongStandingScope();
    _viewportSize;
    _routes = [];
    _webSocketRoutes = [];
    coverage;
    ...
    constructor(parent, type, guid, initializer) {
        ...
        this._timeoutSettings = new timeoutSettings_1.TimeoutSettings(this._platform, this._browserContext._timeoutSettings);
        this.keyboard = new input_1.Keyboard(this);
        this.mouse = new input_1.Mouse(this);
        this.request = this._browserContext.request;
        this.touchscreen = new input_1.Touchscreen(this);
        ...
        this._video = new video_1.Video(this, this._connection, initializer.video ? artifact_1.Artifact.from(initializer.video) : undefined);
        ...
        this._channel.on('route', ({ route }) => this._onRoute(network_1.Route.from(route)));
        this._channel.on('webSocketRoute', ({ webSocketRoute }) => this._onWebSocketRoute(network_1.WebSocketRoute.from(webSocketRoute)));
        ...
        this.once(events_1.Events.Page.Close, () => this._closedOrCrashedScope.close(this._closeErrorWithReason()));
        this.once(events_1.Events.Page.Crash, () => this._closedOrCrashedScope.close(new errors_1.TargetClosedError()));
        this._setEventToSubscriptionMapping(new Map([
            [events_1.Events.Page.Console, 'console'],
            [events_1.Events.Page.Dialog, 'dialog'],
            [events_1.Events.Page.Request, 'request'],
            [events_1.Events.Page.Response, 'response'],
            [events_1.Events.Page.RequestFinished, 'requestFinished'],
            [events_1.Events.Page.RequestFailed, 'requestFailed'],
            [events_1.Events.Page.FileChooser, 'fileChooser'],
        ]));
    }
    ...
    frame(frameSelector) {
        const name = (0, rtti_1.isString)(frameSelector) ? frameSelector : frameSelector.name;
        const url = (0, rtti_1.isObject)(frameSelector) ? frameSelector.url : undefined;
        (0, assert_1.assert)(name || url, 'Either name or url matcher should be specified');
        return this.frames().find(f => {
            if (name)
                return f.name() === name;
            return (0, urlMatch_1.urlMatches)(this._browserContext._options.baseURL, f.url(), url);
        }) || null;
    }
    frames() {
        return [...this._frames];
    ...
    async setExtraHTTPHeaders(headers) {
        (0, network_1.validateHeaders)(headers);
    ...
    async reload(options = {}) {
        const waitUntil = (0, frame_1.verifyLoadState)('waitUntil', options.waitUntil === undefined ? 'load' : options.waitUntil);
        return network_1.Response.fromNullable((await this._channel.reload({ ...options, waitUntil, timeout: this._timeoutSettings.navigationTimeout(options) })).response);
    }
    ...
    async waitForRequest(urlOrPredicate, options = {}) {
        const predicate = async (request) => {
            if ((0, rtti_1.isString)(urlOrPredicate) || (0, rtti_1.isRegExp)(urlOrPredicate))
                return (0, urlMatch_1.urlMatches)(this._browserContext._options.baseURL, request.url(), urlOrPredicate);
            return await urlOrPredicate(request);
        };
        const trimmedUrl = trimUrl(urlOrPredicate);
        const logLine = trimmedUrl ? `waiting for request ${trimmedUrl}` : undefined;
        return await this._waitForEvent(events_1.Events.Page.Request, { predicate, timeout: options.timeout }, logLine);
    }
    async waitForResponse(urlOrPredicate, options = {}) {
        const predicate = async (response) => {
            if ((0, rtti_1.isString)(urlOrPredicate) || (0, rtti_1.isRegExp)(urlOrPredicate))
                return (0, urlMatch_1.urlMatches)(this._browserContext._options.baseURL, response.url(), urlOrPredicate);
            return await urlOrPredicate(response);
        };
        const trimmedUrl = trimUrl(urlOrPredicate);
        const logLine = trimmedUrl ? `waiting for response ${trimmedUrl}` : undefined;
        return await this._waitForEvent(events_1.Events.Page.Response, { predicate, timeout: options.timeout }, logLine);
    }
    async waitForEvent(event, optionsOrPredicate = {}) {
        return await this._waitForEvent(event, optionsOrPredicate, `waiting for event "${event}"`);
    }
    _closeErrorWithReason() {
        return new errors_1.TargetClosedError(this._closeReason || this._browserContext._effectiveCloseReason());
    }
    async _waitForEvent(event, optionsOrPredicate, logLine) {
        return await this._wrapApiCall(async () => {
            ...
            if (event !== events_1.Events.Page.Close)
                waiter.rejectOnEvent(this, events_1.Events.Page.Close, () => this._closeErrorWithReason());
    ...
    async goBack(options = {}) {
        const waitUntil = (0, frame_1.verifyLoadState)('waitUntil', options.waitUntil === undefined ? 'load' : options.waitUntil);
        return network_1.Response.fromNullable((await this._channel.goBack({ ...options, waitUntil, timeout: this._timeoutSettings.navigationTimeout(options) })).response);
    }
    async goForward(options = {}) {
        const waitUntil = (0, frame_1.verifyLoadState)('waitUntil', options.waitUntil === undefined ? 'load' : options.waitUntil);
        return network_1.Response.fromNullable((await this._channel.goForward({ ...options, waitUntil, timeout: this._timeoutSettings.navigationTimeout(options) })).response);
    }
    async requestGC() {
    ...
    async route(url, handler, options = {}) {
        this._routes.unshift(new network_1.RouteHandler(this._platform, this._browserContext._options.baseURL, url, handler, options.times));
    ...
    async routeWebSocket(url, handler) {
        this._webSocketRoutes.unshift(new network_1.WebSocketRouteHandler(this._browserContext._options.baseURL, url, handler));
    ...
    async _updateInterceptionPatterns(options) {
        const patterns = network_1.RouteHandler.prepareInterceptionPatterns(this._routes);
    ...
    async _updateWebSocketInterceptionPatterns(options) {
        const patterns = network_1.WebSocketRouteHandler.prepareInterceptionPatterns(this._webSocketRoutes);
    ...
    async screenshot(options = {}) {
        ...
        if (!copy.type)
            copy.type = (0, elementHandle_1.determineScreenshotType)(options);
        if (mask) {
    ...
    async close(options = {}) {
        ...
        try {
            ...
            if ((0, errors_1.isTargetClosedError)(e) && !options.runBeforeUnload)
                return;
            throw e;
        }
    }
    ...
    async consoleMessages() {
        ...
        return messages.map(message => new consoleMessage_1.ConsoleMessage(this._platform, message, this, null));
    }
    ...
    async pageErrors() {
        ...
        return errors.map(error => (0, errors_1.parseError)(error));
    }
    ...
    async requests() {
        ...
        return requests.map(request => network_1.Request.from(request));
    }
    workers() {
    ...
    async agent(options = {}) {
        ...
        const pageAgent = pageAgent_1.PageAgent.from(agent);
...
class BindingCall extends channelOwner_1.ChannelOwner {
    ...
    async call(func) {
        try {
            ...
            this._channel.resolve({ result: (0, jsHandle_1.serializeArgument)(result) }).catch(() => { });
        }
        catch (e) {
            this._channel.reject({ error: (0, errors_1.serializeError)(e) }).catch(() => { });
        }
    }
}
exports.BindingCall = BindingCall;
function trimUrl(param) {
    if ((0, rtti_1.isRegExp)(param))
        return `/${(0, stringUtils_1.trimStringWithEllipsis)(param.source, 50)}/${param.flags}`;
    if ((0, rtti_1.isString)(param))
        return `"${(0, stringUtils_1.trimStringWithEllipsis)(param, 50)}"`;
}
