...
$copilotbrowserCoreRoot = Resolve-Path (Join-Path $pwshDirname "..")
...
sudo -u $Username copilotbrowser_SKIP_BROWSER_GC=1 /home/$Username/node/bin/node cli.js install webkit
"@ -replace "\r\n", "`n"

wsl -d $Distribution --cd $copilotbrowserCoreRoot -u root -- bash -c "$initScript"
Write-Host "Done!"