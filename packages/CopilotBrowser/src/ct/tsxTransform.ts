...
import { declare, traverse, types } from '@copilotbrowser/copilotbrowser/lib/transform/babelBundle';
import { setTransformData } from '@copilotbrowser/copilotbrowser/lib/transform/transform';

import type { BabelAPI, PluginObj, T } from '@copilotbrowser/copilotbrowser/lib/transform/babelBundle';
...
export default declare((api: BabelAPI) => {
  ...
  const result: PluginObj = {
    name: 'copilotbrowser-debug-transform',
    visitor: {
      Program: {
        enter(path) {
        ...
        exit(path) {
          ...
          setTransformData('copilotbrowser-ct-core', [...importInfos.values()]);
        }
      },
