...
const locatorGenerators_1 = require("./locatorGenerators");
    ...
    return parts.map(t => {
        ...
        t = t
            ...
            if (t.startsWith('internal:attr') || t.startsWith('internal:testid') || t.startsWith('internal:role'))
                return (0, stringUtils_1.escapeForAttributeSelector)(new RegExp(param.text), false) + (suffix || '');
            return (0, stringUtils_1.escapeForTextSelector)(new RegExp(param.text, suffix), false);
        })
            ...
            if (t.startsWith('internal:testid'))
                return (0, stringUtils_1.escapeForAttributeSelector)(param.text, true);
            if (t.startsWith('internal:attr') || t.startsWith('internal:role'))
                return (0, stringUtils_1.escapeForAttributeSelector)(param.text, suffix === 's');
            return (0, stringUtils_1.escapeForTextSelector)(param.text, suffix === 's');
        });
        return t;
    }).join(' >> ');
}
...
function unsafeLocatorOrSelectorAsSelector(language, locator, testIdAttributeName) {
    try {
        (0, selectorParser_1.parseSelector)(locator);
        return locator;
    }
    catch (e) {
    }
    const { selector, preferredQuote } = parseLocator(locator, testIdAttributeName);
    const locators = (0, locatorGenerators_1.asLocators)(language, selector, undefined, undefined, preferredQuote);
