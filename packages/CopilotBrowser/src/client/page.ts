...
export class Page extends ChannelOwner<channels.PageChannel> {
  ...
  async _expectScreenshot(options: ExpectScreenshotOptions): Promise<{ actual?: Buffer, previous?: Buffer, diff?: Buffer, errorMessage?: string, log?: string[], timedOut?: boolean}> {
    const mask = options?.mask ? options?.mask.map(locator => ({
      frame: (locator as unknown as Locator)._frame._channel,
      selector: (locator as unknown as Locator)._selector,
    })) : undefined;
    const locator = options.locator ? {
      frame: (options.locator as unknown as Locator)._frame._channel,
      selector: (options.locator as unknown as Locator)._selector,
    } : undefined;
  ...
  async call(func: (source: structs.BindingSource, ...args: any[]) => any) {
    try {
      ...
      if (this._initializer.handle)
        result = await func(source as any, JSHandle.from(this._initializer.handle));
      else
        result = await func(source as any, ...this._initializer.args!.map(parseResult));
