...
const clientStackTrace_1 = require("./clientStackTrace");
const stackTrace_1 = require("../utils/isomorphic/stackTrace");
class ChannelOwner extends eventEmitter_1.EventEmitter {
    _connection;
    _parent;
    _objects = new Map();
    _type;
    ...
    constructor(parent, type, guid, initializer) {
        ...
        this._channel = this._createChannel(new eventEmitter_1.EventEmitter(connection._platform));
        ...
        const channel = new Proxy(base, {
            get: (obj, prop) => {
                if (typeof prop === 'string') {
                    const validator = (0, validator_1.maybeFindValidator)(this._type, prop, 'Params');
                    const { internal } = protocolMetainfo_1.methodMetainfo.get(this._type + '.' + prop) || {};
                    if (validator) {
                        return async (params) => {
    ...
    async _wrapApiCall(func, options) {
        ...
        const stackTrace = (0, clientStackTrace_1.captureLibraryStackTrace)(this._platform);
        ...
        try {
            ...
            const stackFrames = '\n' + (0, stackTrace_1.stringifyStackFrames)(stackTrace.frames).join('\n') + innerError;
            if (stackFrames.trim())
