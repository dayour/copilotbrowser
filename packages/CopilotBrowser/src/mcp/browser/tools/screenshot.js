...
const screenshotSchema = mcpBundle_1.z.object({
    ...
    filename: mcpBundle_1.z.string().optional().describe('File name to save the screenshot to. Defaults to `page-{timestamp}.{png|jpeg}` if not specified. Prefer relative file names to stay within the output directory.'),
    element: mcpBundle_1.z.string().optional().describe('Human-readable element description used to obtain permission to screenshot the element. If not provided, the screenshot will be taken of viewport. If element is provided, ref must be provided too.'),
    ref: mcpBundle_1.z.string().optional().describe('Exact target element reference from the page snapshot. If not provided, the screenshot will be taken of viewport. If ref is provided, element must be provided too.'),
    fullPage: mcpBundle_1.z.boolean().optional().describe('When true, takes a screenshot of the full scrollable page, instead of the currently visible viewport. Cannot be used with element screenshots.'),
});
const screenshot = (0, tool_1.defineTabTool)({
    ...
    handle: async (tab, params, response) => {
        ...
        if (ref)
            response.addCode(`await page.${ref.resolved}.screenshot(${(0, utils_2.formatObject)({ ...options, path: resolvedFile.relativeName })});`);
        else
            response.addCode(`await page.screenshot(${(0, utils_2.formatObject)({ ...options, path: resolvedFile.relativeName })});`);
...
function scaleImageToFitMessage(buffer, imageType) {
    ...
    const image = imageType === 'png' ? utilsBundle_1.PNG.sync.read(buffer) : utilsBundle_1.jpegjs.decode(buffer, { maxMemoryUsageInMB: 512 });
    ...
    const scaledImage = (0, utils_1.scaleImageToSize)(image, { width, height });
    return imageType === 'png' ? utilsBundle_1.PNG.sync.write(scaledImage) : utilsBundle_1.jpegjs.encode(scaledImage, 80).data;
}
exports.default = [
    screenshot,
];
