...
function isSuccessfulResponse(request: copilotbrowser.Request): boolean {
  if (request.failure())
    return false;
...
export function isFetch(request: copilotbrowser.Request): boolean {
  return ['fetch', 'xhr'].includes(request.resourceType());
}

export async function renderRequest(request: copilotbrowser.Request): Promise<string> {
  const response = request.existingResponse();
