...
export class Tab extends EventEmitter<TabEventsInterface> {
  ...
  private _requests: copilotbrowser.Request[] = [];
  ...
  static async collectConsoleMessages(page: copilotbrowser.Page): Promise<ConsoleMessage[]> {
  ...
  private async _initialize() {
    for (const message of await Tab.collectConsoleMessages(this.page as any))
      this._handleConsoleMessage(message);
    const requests = await this.page.requests().catch(() => []);
    for (const request of requests.filter(r => r.existingResponse() || r.failure()))
      this._requests.push(request as any);
  ...
  private _dialogShown(dialog: copilotbrowser.Dialog) {
    this.setModalState({
      type: 'dialog',
  ...
  private async _downloadStarted(download: copilotbrowser.Download) {
  ...
  private _handleRequest(request: copilotbrowser.Request) {
  ...
  private _handleResponse(response: copilotbrowser.Response) {
  ...
  private _handleRequestFailed(request: copilotbrowser.Request) {
    this._requests.push(request);
  ...
  async headerSnapshot(): Promise<TabHeader & { changed: boolean }> {
    ...
    await this._raceAgainstModalStates(async () => {
      title = await callOnPageNoTrace(this.page as any, page => page.title());
    });
  ...
  async waitForLoadState(state: 'load', options?: { timeout?: number }): Promise<void> {
    ...
    await callOnPageNoTrace(this.page as any, page => page.waitForLoadState(state, options).catch(logUnhandledError));
  }
  ...
  async navigate(url: string) {
    ...
    const { promise: downloadEvent, abort: abortDownloadEvent } = eventWaiter<copilotbrowser.Download>(this.page as any, 'download', 3000);
    try {
  ...
  async consoleMessages(level: ConsoleMessageLevel): Promise<ConsoleMessage[]> {
    ...
    const messages = await this.page.consoleMessages();
    for (const message of messages) {
      const cm = messageToConsoleMessage(message as any);
  ...
  async requests(): Promise<copilotbrowser.Request[]> {
    await this._initializedPromise;
  ...
  async waitForTimeout(time: number) {
    ...
    await callOnPageNoTrace(this.page as any, page => {
...
function messageToConsoleMessage(message: copilotbrowser.ConsoleMessage): ConsoleMessage {
  return {
    type: message.type(),
