...
export function createCustomMessageHandler(testInfo: TestInfo, context: copilotbrowser.BrowserContext) {
...
async function generatePausedMessage(testInfo: TestInfo, context: copilotbrowser.BrowserContext) {
  const lines: string[] = [];
  ...
  for (let i = 0; i < context.pages().length; i++) {
    ...
    lines.push(
        `- Page Snapshot:`,
        '```yaml',
        (await (page as unknown as Page)._snapshotForAI()).full,
        '```',
    );
  }
