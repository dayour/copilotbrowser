...
export class BrowserServerLauncherImpl implements BrowserServerLauncher {
  ...
  async launchServer(options: LaunchServerOptions & { _sharedBrowser?: boolean, _userDataDir?: string } = {}): Promise<BrowserServer> {
    ...
    let launchOptions = {
      ...
      timeout: options.timeout ?? DEFAULT_copilotbrowser_LAUNCH_TIMEOUT,
    };
    ...
    try {
      ...
      browser = await controller.run(async progress => {
        if (options._userDataDir !== undefined) {
          ...
          const context = await copilotbrowser[this._browserName].launchPersistentContext(progress, options._userDataDir, launchOptions);
          ...
          return await copilotbrowser[this._browserName].launch(progress, launchOptions, toProtocolLogger(options.logger));
        }
      });
    } catch (e) {
    ...
    // 2. Start the server
    const server = new copilotbrowserServer({ mode: options._sharedBrowser ? 'launchServerShared' : 'launchServer', path, maxConnections: Infinity, preLaunchedBrowser: browser });
