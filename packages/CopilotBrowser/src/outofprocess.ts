...
import type { copilotbrowser } from './client/copilotbrowser';


export async function start(env: any = {}): Promise<{ copilotbrowser: copilotbrowser, stop: () => Promise<void> }> {
  const client = new copilotbrowserClient(env);
  const copilotbrowser = await client._copilotbrowser;
  (copilotbrowser as any).driverProcess = client._driverProcess;
  return { copilotbrowser, stop: () => client.stop() };
}

class copilotbrowserClient {
  _copilotbrowser: Promise<copilotbrowser>;
  ...
  constructor(env: any) {
    ...
    this._copilotbrowser = connection.initializecopilotbrowser();
  }
