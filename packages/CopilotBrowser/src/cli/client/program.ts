...
function resolveSessionName(sessionName?: string): string {
  if (sessionName)
    return sessionName;
  if (process.env.copilotbrowser_CLI_SESSION)
    return process.env.copilotbrowser_CLI_SESSION;
  return 'default';
}
...
async function program() {
  ...
  if (args.help || args.h) {
    if (command) {
      ...
      console.log('copilotbrowser-cli - run copilotbrowser mcp commands from terminal\n');
  ...
  switch (commandName) {
    ...
    default: {
      ...
      if (!defaultEntry) {
        ...
        console.log('');
        console.log(`  copilotbrowser-cli${sessionName !== 'default' ? ` -s=${sessionName}` : ''} open [params]`);
        process.exit(1);
      }
...
async function install(args: MinimistArgs) {
  ...
  // Create .copilotbrowser folder to mark workspace root
  const copilotbrowserDir = path.join(cwd, '.copilotbrowser');
  await fs.promises.mkdir(copilotbrowserDir, { recursive: true });
  console.log(`✅ Workspace initialized at \`${cwd}\`.`);

  if (args.skills) {
    const skillSourceDir = path.join(__dirname, '../../skill');
    const skillDestDir = path.join(cwd, '.claude', 'skills', 'copilotbrowser-cli');
...
function daemonSocketPath(clientInfo: ClientInfo, sessionName: string): string {
  ...
  const socketsDir = process.env.copilotbrowser_DAEMON_SOCKETS_DIR || path.join(os.tmpdir(), 'copilotbrowser-cli');
  return path.join(socketsDir, clientInfo.workspaceDirHash, socketName);
}

function defaultConfigFile(): string {
  return path.resolve('.copilotbrowser', 'cli.config.json');
}
