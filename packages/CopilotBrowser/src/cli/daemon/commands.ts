...
const open = declareCommand({
  ...
  options: z.object({
    ...
    config: z.string().optional().describe('Path to the configuration file, defaults to .copilotbrowser/cli.config.json'),
...
const runCode = declareCommand({
  ...
  category: 'devtools',
  args: z.object({
    code: z.string().describe('A JavaScript function containing copilotbrowser code to execute. It will be invoked with a single argument, page, which you can use for any page interaction.'),
  }),
