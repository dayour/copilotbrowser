...
function generateCommandHelp(command: AnyCommandSchema) {
  ...
  const lines: string[] = [
    `copilotbrowser-cli ${command.name} ${commandArgsText(args)}`,
    '',
    command.description,
    '',
  ];
...
export function generateHelp() {
  const lines: string[] = [];
  lines.push('Usage: copilotbrowser-cli <command> [args] [options]');
  lines.push('Usage: copilotbrowser-cli -s=<session> <command> [args] [options]');
...
function generateReadmeEntry(command: AnyCommandSchema): string {
  const args = commandArgs(command);
  const prefix = `copilotbrowser-cli ${command.name} ${commandArgsText(args)}`;
  const suffix = '# ' + command.description.toLowerCase();
