        ...
        for (const entry of Object.entries(fixtures)) {
            ...
            if (previous && options) {
                if (previous.scope !== options.scope) {
                    this._addLoadError(`Fixture "${name}" has already been registered as a { scope: '${previous.scope}' } fixture defined in ${(0, util_1.formatLocation)(previous.location)}.`, location);
                    continue;
                }
                if (previous.auto !== options.auto) {
                    this._addLoadError(`Fixture "${name}" has already been registered as a { auto: '${previous.scope}' } fixture defined in ${(0, util_1.formatLocation)(previous.location)}.`, location);
                    continue;
                }
            }
            else if (previous) {
...
function formatPotentiallyInternalLocation(location) {
    const isUserFixture = location && (0, util_1.filterStackFile)(location.file);
    return isUserFixture ? (0, util_1.formatLocation)(location) : '<builtin>';
}
