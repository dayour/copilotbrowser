...
import { resolveLLMProvider } from './agents/llmProvider';
...
import type { ClientInstrumentationListener } from './client/clientInstrumentation';
import type { copilotbrowser as copilotbrowserImpl } from './client/copilotbrowser';
import type { Browser as BrowserImpl } from './client/browser';
import type { BrowserContext as BrowserContextImpl } from './client/browserContext';
import type { APIRequestContext as APIRequestContextImpl, NewContextOptions as APIRequestContextOptions } from './client/fetch';
import type { ChannelOwner } from './client/channelOwner';
import type { Page as PageImpl } from './client/page';
...
if ((process as any)['__pw_initiator__']) {
  ...
  try {
    throw new Error('Requiring @copilotbrowser/test second time, \nFirst:\n' + (process as any)['__pw_initiator__'] + '\n\nSecond: ');
  } finally {
...
type WorkerFixtures = copilotbrowserWorkerArgs & copilotbrowserWorkerOptions & {
  copilotbrowser: copilotbrowserImpl;
  _browserOptions: LaunchOptions;
  _optionContextReuseMode: ContextReuseMode,
  _optionConnectOptions: copilotbrowserWorkerOptions['connectOptions'],
  _reuseContext: boolean,
};

const copilotbrowserFixtures: Fixtures<TestFixtures, WorkerFixtures> = ({
  ...
  _browserOptions: [async ({ copilotbrowser, headless, channel, launchOptions }, use) => {
    ...
    copilotbrowser._defaultLaunchOptions = options;
    await use(options);
    copilotbrowser._defaultLaunchOptions = undefined;
  }, { scope: 'worker', auto: true, box: true }],

  browser: [async ({ copilotbrowser, browserName, _browserOptions, connectOptions }, use) => {
    if (!['chromium', 'firefox', 'webkit'].includes(browserName))
      throw new Error(`Unexpected browserName "${browserName}", must be one of "chromium", "firefox" or "webkit"`);

    if (connectOptions) {
      const browser = await copilotbrowser[browserName].connect({
        ...connectOptions,
        exposeNetwork: connectOptions.exposeNetwork ?? (connectOptions as any)._exposeNetwork,
        headers: {
          // HTTP headers are ASCII only (not UTF-8).
          'x-copilotbrowser-launch-options': jsonStringifyForceASCII(_browserOptions),
    ...
    const browser = await copilotbrowser[browserName].launch();
    await use(browser);
    await browser.close({ reason: 'Test ended.' });
  ...
  baseURL: [async ({ }, use) => {
    await use(process.env.copilotbrowser_TEST_BASE_URL);
  ...
  _setupContextOptions: [async ({ copilotbrowser, actionTimeout, navigationTimeout, testIdAttribute }, use, testInfo) => {
    if (testIdAttribute)
      copilotbrowserLibrary.selectors.setTestIdAttribute(testIdAttribute);
    testInfo.snapshotSuffix = process.platform;
    if (debugMode() === 'inspector')
      (testInfo as TestInfoImpl)._setDebugMode();

    copilotbrowser._defaultContextTimeout = actionTimeout || 0;
    copilotbrowser._defaultContextNavigationTimeout = navigationTimeout || 0;
    await use();
    copilotbrowser._defaultContextTimeout = undefined;
    copilotbrowser._defaultContextNavigationTimeout = undefined;
  }, { auto: 'all-hooks-included',  title: 'context configuration', box: true } as any],

  _setupArtifacts: [async ({ copilotbrowser, screenshot, _combinedContextOptions }, use, testInfo) => {
    ...
    const artifactsRecorder = new ArtifactsRecorder(copilotbrowser, tracing().artifactsDir(), screenshot);
    ...
    const clientInstrumentation = copilotbrowser._instrumentation;
  ...
  _contextFactory: [async ({ browser, video, _reuseContext, _combinedContextOptions /** mitigate dep-via-auto lack of traceability */ }, use, testInfo) => {
    ...
    await (use as any)(async (options?: any) => {
      ...
      if (hook === 'beforeAll' || hook === 'afterAll') {
        throw new Error([
          ...
          `If you would like to reuse a single page between tests, create context manually with browser.newContext(). See https://aka.ms/copilotbrowser/reuse-page for details.`,
      ...
      const context = await browser.newContext({ ...videoOptions, ...options }) as unknown as BrowserContextImpl;
      ...
      const close = async () => {
        ...
        if (preserveVideo) {
          const { pagesWithVideo: pagesForVideo } = contexts.get(context as any)!;
      ...
      contexts.set(context as any, contextData);
  ...
  context: async ({ browser, _reuseContext, _contextFactory }, use, testInfo) => {
    const browserImpl = browser as unknown as BrowserImpl;
    ...
    (testInfo as TestInfoImpl)._onCustomMessageCallback = createCustomMessageHandler(testInfo, context as any);
  ...
  agent: async ({ page, agentOptions }, use, testInfo) => {
    ...
    const provider = resolveLLMProvider({ agentOptions, runAgentsMode: testInfo.config.runAgents });
    if (provider)
  ...
  request: async ({ copilotbrowser }, use) => {
    ...
    if (hook === 'beforeAll') {
      await request.dispose({ reason: [
        ...
        `See https://dayour.github.io/copilotbrowser/docs/api-testing#sending-api-requests-from-ui-tests for more details.`,
      ].join('\n') });
    } else {
...
type ScreenshotOption = copilotbrowserWorkerOptions['screenshot'] | undefined;
...
function attachConnectedHeaderIfNeeded(testInfo: TestInfo, browser: BrowserImpl | null) {
  ...
  for (const header of connectHeaders) {
    if (header.name !== 'x-copilotbrowser-attachment')
      continue;
...
type ClientCertificates = NonNullable<copilotbrowserTestOptions['clientCertificates']>;
...
class SnapshotRecorder {
  ...
  async maybeCapture() {
    ...
    await Promise.all(this._artifactsRecorder._copilotbrowser._allPages().map(page => this._snapshotPage(page, false)));
  }
...
class ArtifactsRecorder {
  ...
  _copilotbrowser: copilotbrowserImpl;
  ...
  constructor(copilotbrowser: copilotbrowserImpl, artifactsDir: string, screenshot: ScreenshotOption) {
  ...
  async willStartTest(testInfo: TestInfoImpl) {
    ...
    await Promise.all(this._copilotbrowser._allContexts().map(context => this.didCreateBrowserContext(context)));
    const existingApiRequests = Array.from(this._copilotbrowser.request._contexts);
  ...
  private async _takePageSnapshot(context: BrowserContextImpl) {
    if (process.env.copilotbrowser_NO_COPY_PROMPT)
      return;
  ...
  async didFinishTest() {
    ...
    const leftoverContexts = this._copilotbrowser._allContexts();
    const leftoverApiRequests = Array.from(this._copilotbrowser.request._contexts);
...
export const test = _baseTest.extend<TestFixtures, WorkerFixtures>(copilotbrowserFixtures);
