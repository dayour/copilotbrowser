...
const debugWebServer = (0, utilsBundle_1.debug)('pw:webserver');
class WebServerPlugin {
    _isAvailableCallback;
    _killProcess;
    _processExitedPromise;
    _options;
    _checkPortOnly;
    _reporter;
    _waitForStdioPromise;
    name = 'copilotbrowser:webserver';
    ...
    async setup(config, configDir, reporter) {
        ...
        this._options.cwd = this._options.cwd ? path_1.default.resolve(configDir, this._options.cwd) : configDir;
        try {
    ...
    async _startProcess() {
        ...
        const { launchedProcess, gracefullyClose } = await (0, utils_1.launchProcess)({
        ...
        if (this._options.wait?.stdout || this._options.wait?.stderr)
            this._waitForStdioPromise = new utils_1.ManualPromise();
    ...
    async _waitForProcess() {
        ...
        const deadline = (0, utils_1.monotonicTime)() + launchTimeout;
        const racingPromises = [this._processExitedPromise];
        if (this._isAvailableCallback)
            racingPromises.push((0, utils_1.raceAgainstDeadline)(() => waitFor(this._isAvailableCallback, cancellationToken), deadline));
        if (this._waitForStdioPromise)
            racingPromises.push((0, utils_1.raceAgainstDeadline)(() => this._waitForStdioPromise, deadline));
...
function getIsAvailableFunction(url, checkPortOnly, ignoreHTTPSErrors, onStdErr) {
    ...
    if (!checkPortOnly)
        return () => (0, utils_1.isURLAvailable)(urlObject, ignoreHTTPSErrors, debugWebServer, onStdErr);
    ...
    for (const webServerConfig of config.webServers) {
        ...
        if (webServerConfig.port || webServerConfig.url) {
            ...
            if (shouldSetBaseUrl && !webServerConfig.url)
                process.env.copilotbrowser_TEST_BASE_URL = url;
        }
...
function prefixOutputLines(output, prefixName = 'WebServer') {
    ...
    if (lastIsNewLine)
        lines.pop();
    lines = lines.map(line => utils_2.colors.dim(`[${prefixName}] `) + line);
    if (lastIsNewLine)
        lines.push('');
    return lines.join('\n');
}
