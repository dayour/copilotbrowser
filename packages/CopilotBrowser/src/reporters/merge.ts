...
import { ZipFile } from '@copilotbrowser/copilotbrowser/lib/utils';
...
function findMetadata(events: JsonEvent[], file: string): BlobReportMetadata {
  ...
  if (metadata.version > currentBlobReportVersion)
    throw new Error(`Blob report ${file} was created with a newer version of copilotbrowser.`);
  return metadata;
}
  ...
  if (rootDirOverride) {
    ...
    if (rootDirs.size > 1) {
      throw new Error([
        ...
        `if the tests ran with different copilotbrowser configs.`,
        ``,
