...
export const terminalScreen: TerminalScreen = (() => {
  ...
  if (process.env.copilotbrowser_FORCE_TTY === 'false' || process.env.copilotbrowser_FORCE_TTY === '0') {
    isTTY = false;
    ttyWidth = 0;
    ttyHeight = 0;
  } else if (process.env.copilotbrowser_FORCE_TTY === 'true' || process.env.copilotbrowser_FORCE_TTY === '1') {
    isTTY = true;
    ttyWidth = originalProcessStdout.columns || DEFAULT_TTY_WIDTH;
    ttyHeight = originalProcessStdout.rows || DEFAULT_TTY_HEIGHT;
  } else if (process.env.copilotbrowser_FORCE_TTY) {
    isTTY = true;
    const sizeMatch = process.env.copilotbrowser_FORCE_TTY.match(/^(\d+)x(\d+)$/);
    if (sizeMatch) {
      ttyWidth = +sizeMatch[1];
      ttyHeight = +sizeMatch[2];
    } else {
      ttyWidth = +process.env.copilotbrowser_FORCE_TTY;
...
export class TerminalReporter implements ReporterV2 {
  ...
  private _printSlowTests() {
    ...
    if (slowTests.length)
      this.writeLine(this.screen.colors.yellow('  Consider running tests from slow files in parallel. See: https://dayour.github.io/copilotbrowser/docs/test-parallel'));
  }
    ...
    for (let i = 0; i < attachmentGroups.length; ++i) {
      ...
      if (attachment.actual?.path) {
        ...
        if (attachment.name === 'trace') {
          ...
          resultLines.push('');
          resultLines.push(screen.colors.dim(`        ${packageManagerCommand} copilotbrowser show-trace ${quotePathIfNeeded(relativePath)}`));
          resultLines.push('');
        }
      } else {
...
export function resolveOutputFile(reporterName: string, options: {
  ...
  let outputFile = resolveFromEnv(`copilotbrowser_${name}_OUTPUT_FILE`);
  if (!outputFile && options.outputFile)
    outputFile = path.resolve(options.configDir, options.outputFile);
  if (outputFile)
    return { outputFile };

  let outputDir = resolveFromEnv(`copilotbrowser_${name}_OUTPUT_DIR`);
  ...
  const reportName = process.env[`copilotbrowser_${name}_OUTPUT_NAME`] ?? options.fileName ?? options.default?.fileName;
  if (!reportName)
    return undefined;
