...
exports.WebSocket = exports.Response = exports.Route = exports.Request = exports.kMaxCookieExpiresDateInSeconds = void 0;
exports.filterCookies = filterCookies;
exports.isLocalHostname = isLocalHostname;
exports.applyHeadersOverrides = applyHeadersOverrides;
...
exports.kMaxCookieExpiresDateInSeconds = 253402300799;
function rewriteCookies(cookies) {
    return cookies.map(c => {
        (0, utils_1.assert)(c.url || (c.domain && c.path), 'Cookie should have a url or a domain/path pair');
        (0, utils_1.assert)(!(c.url && c.domain), 'Cookie should have either url or domain');
        (0, utils_1.assert)(!(c.url && c.path), 'Cookie should have either url or path');
        (0, utils_1.assert)(!(c.expires && c.expires < 0 && c.expires !== -1), 'Cookie should have a valid expires, only -1 or a positive number for the unix timestamp in seconds is allowed');
        (0, utils_1.assert)(!(c.expires && c.expires > 0 && c.expires > exports.kMaxCookieExpiresDateInSeconds), 'Cookie should have a valid expires, only -1 or a positive number for the unix timestamp in seconds is allowed');
        const copy = { ...c };
        if (copy.url) {
            (0, utils_1.assert)(copy.url !== 'about:blank', `Blank page can not have cookie "${c.name}"`);
            (0, utils_1.assert)(!copy.url.startsWith('data:'), `Data URL page can not have cookie "${c.name}"`);
...
class Request extends instrumentation_1.SdkObject {
    ...
    _serviceWorker = null;
    _context;
    _rawRequestHeadersPromise = new manualPromise_1.ManualPromise();
    _waitForResponsePromise = new manualPromise_1.ManualPromise();
    ...
    constructor(context, frame, serviceWorker, redirectedFrom, documentId, url, resourceType, method, postData, headers) {
        ...
        (0, utils_1.assert)(!url.startsWith('data:'), 'Data urls should not fire requests');
...
class Route extends instrumentation_1.SdkObject {
    ...
    async abort(errorCode = 'failed') {
        this._startHandling();
        this._request._context.emit(browserContext_1.BrowserContext.Events.RequestAborted, this._request);
        await this._delegate.abort(errorCode);
        this._endHandling();
    }
    redirectNavigationRequest(url) {
        this._startHandling();
        (0, utils_1.assert)(this._request.isNavigationRequest());
    ...
    async fulfill(overrides) {
        ...
        if (body === undefined) {
            if (overrides.fetchResponseUid) {
                const buffer = this._request._context.fetchRequest.fetchResponses.get(overrides.fetchResponseUid) || fetch_1.APIRequestContext.findResponseBody(overrides.fetchResponseUid);
                (0, utils_1.assert)(buffer, 'Fetch response has been disposed');
        ...
        this._request._context.emit(browserContext_1.BrowserContext.Events.RequestFulfilled, this._request);
    ...
    // See https://github.com/dayour/copilotbrowser/issues/12929
    ...
    async continue(overrides) {
        ...
        if (!overrides.isFallback)
            this._request._context.emit(browserContext_1.BrowserContext.Events.RequestContinued, this._request);
        this._startHandling();
        await this._delegate.continue(overrides);
        this._endHandling();
    }
    _startHandling() {
        (0, utils_1.assert)(!this._handled, 'Route is already handled!');
        this._handled = true;
...
class Response extends instrumentation_1.SdkObject {
    ...
    _timing;
    _serverAddrPromise = new manualPromise_1.ManualPromise();
    _securityDetailsPromise = new manualPromise_1.ManualPromise();
    _rawResponseHeadersPromise = new manualPromise_1.ManualPromise();
    _httpVersion;
    _fromServiceWorker;
    _encodedBodySizePromise = new manualPromise_1.ManualPromise();
    _transferSizePromise = new manualPromise_1.ManualPromise();
    _responseHeadersSizePromise = new manualPromise_1.ManualPromise();
