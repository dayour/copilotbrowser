...
function frameSnapshotStreamer(snapshotStreamer, removeNoScript) {
    // Communication with copilotbrowser.
    if (window[snapshotStreamer])
        return;
    // Attributes present in the snapshot.
    const kShadowAttribute = '__copilotbrowser_shadow_root_';
    const kValueAttribute = '__copilotbrowser_value_';
    const kCheckedAttribute = '__copilotbrowser_checked_';
    const kSelectedAttribute = '__copilotbrowser_selected_';
    const kScrollTopAttribute = '__copilotbrowser_scroll_top_';
    const kScrollLeftAttribute = '__copilotbrowser_scroll_left_';
    const kStyleSheetAttribute = '__copilotbrowser_style_sheet_';
    const kTargetAttribute = '__copilotbrowser_target__';
    const kCustomElementsAttribute = '__copilotbrowser_custom_elements__';
    const kCurrentSrcAttribute = '__copilotbrowser_current_src__';
    const kBoundingRectAttribute = '__copilotbrowser_bounding_rect__';
    const kPopoverOpenAttribute = '__copilotbrowser_popover_open_';
    const kDialogOpenAttribute = '__copilotbrowser_dialog_open_';
    // Symbols for our own info on Nodes/StyleSheets.
    const kSnapshotFrameId = Symbol('__copilotbrowser_snapshot_frameid_');
    const kCachedData = Symbol('__copilotbrowser_snapshot_cache_');
    const kEndOfList = Symbol('__copilotbrowser_end_of_list_');
    ...
    class Streamer {
        _lastSnapshotNumber = 0;
        _staleStyleSheets = new Set();
        _modifiedStyleSheets = new Set();
        _readingStyleSheet = false; // To avoid invalidating due to our own reads.
        _fakeBase;
        _observer;
        constructor() {
        ...
        _refreshListenersWhenNeeded() {
            ...
            const customEventName = '__copilotbrowser_snapshotter_global_listeners_check__';
            let seenEvent = false;
        ...
        _refreshListeners() {
            document.addEventListener('__copilotbrowser_mark_target__', (event) => {
                if (!event.detail)
                    return;
                const callId = event.detail;
                event.composedPath()[0].__copilotbrowser_target__ = callId;
            });
            document.addEventListener('__copilotbrowser_unmark_target__', (event) => {
                if (!event.detail)
                    return;
                const callId = event.detail;
                if (event.composedPath()[0].__copilotbrowser_target__ === callId)
                    delete event.composedPath()[0].__copilotbrowser_target__;
            });
        }
        ...
        captureSnapshot(needsReset) {
            ...
            const visitNode = (node) => {
                ...
                // See https://github.com/dayour/copilotbrowser/issues/12005.
                ...
                if (nodeType === Node.ELEMENT_NODE) {
                    ...
                    if ('__copilotbrowser_target__' in element) {
                        expectValue(kTargetAttribute);
                        expectValue(element['__copilotbrowser_target__']);
                        attrs[kTargetAttribute] = element['__copilotbrowser_target__'];
                    }
                }
                if (nodeName === 'HEAD') {
                    ++headNesting;
