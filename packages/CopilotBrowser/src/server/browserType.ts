...
import { DEFAULT_copilotbrowser_TIMEOUT } from '../utils/isomorphic/time';
...
export abstract class BrowserType extends SdkObject {
  ...
  private async _prepareToLaunch(options: types.LaunchOptions, isPersistent: boolean, userDataDir: string | undefined) {
    ...
    const artifactsDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'copilotbrowser-artifacts-'));
    ...
    if (userDataDir) {
      ...
      userDataDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), `copilotbrowser_${this._name}dev_profile-`));
    ...
    if (executablePath) {
      ...
      await registry.validateHostRequirementsForExecutablesIfNeeded([registryExecutable], this.attribution.copilotbrowser.options.sdkLanguage);
    }
  ...
  private async _launchProcess(progress: Progress, options: types.LaunchOptions, isPersistent: boolean, browserLogsCollector: RecentLogsCollector, userDataDir?: string): Promise<{ browserProcess: BrowserProcess, artifactsDir: string, userDataDir: string, transport: ConnectionTransport }> {
    ...
    browserProcess = {
      ...
      close: () => closeOrKill((options as any).__testHookBrowserCloseTimeout || DEFAULT_copilotbrowser_TIMEOUT),
      kill
    };
    try {
      ...
      await closeOrKill(DEFAULT_copilotbrowser_TIMEOUT).catch(() => {});
      throw error;
    }
  }
