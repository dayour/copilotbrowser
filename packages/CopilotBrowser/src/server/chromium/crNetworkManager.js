...
class CRNetworkManager {
    ...
    _responseExtraInfoTracker = new ResponseExtraInfoTracker();
    _sessions = new Map();
    ...
    async addSession(session, workerFrame, isMain) {
        ...
        sessionInfo.eventListeners = [
            eventsHelper_1.eventsHelper.addEventListener(session, 'Fetch.requestPaused', this._onRequestPaused.bind(this, sessionInfo)),
            eventsHelper_1.eventsHelper.addEventListener(session, 'Fetch.authRequired', this._onAuthRequired.bind(this, sessionInfo)),
            eventsHelper_1.eventsHelper.addEventListener(session, 'Network.requestWillBeSent', this._onRequestWillBeSent.bind(this, sessionInfo)),
            eventsHelper_1.eventsHelper.addEventListener(session, 'Network.requestWillBeSentExtraInfo', this._onRequestWillBeSentExtraInfo.bind(this)),
            eventsHelper_1.eventsHelper.addEventListener(session, 'Network.requestServedFromCache', this._onRequestServedFromCache.bind(this)),
            eventsHelper_1.eventsHelper.addEventListener(session, 'Network.responseReceived', this._onResponseReceived.bind(this, sessionInfo)),
            eventsHelper_1.eventsHelper.addEventListener(session, 'Network.responseReceivedExtraInfo', this._onResponseReceivedExtraInfo.bind(this)),
            eventsHelper_1.eventsHelper.addEventListener(session, 'Network.loadingFinished', this._onLoadingFinished.bind(this, sessionInfo)),
            eventsHelper_1.eventsHelper.addEventListener(session, 'Network.loadingFailed', this._onLoadingFailed.bind(this, sessionInfo)),
        ];
        if (this._page) {
            sessionInfo.eventListeners.push(...[
                eventsHelper_1.eventsHelper.addEventListener(session, 'Network.webSocketCreated', e => this._page.frameManager.onWebSocketCreated(e.requestId, e.url)),
                eventsHelper_1.eventsHelper.addEventListener(session, 'Network.webSocketWillSendHandshakeRequest', e => this._page.frameManager.onWebSocketRequest(e.requestId)),
                eventsHelper_1.eventsHelper.addEventListener(session, 'Network.webSocketHandshakeResponseReceived', e => this._page.frameManager.onWebSocketResponse(e.requestId, e.response.status, e.response.statusText)),
                eventsHelper_1.eventsHelper.addEventListener(session, 'Network.webSocketFrameSent', e => e.response.payloadData && this._page.frameManager.onWebSocketFrameSent(e.requestId, e.response.opcode, e.response.payloadData)),
                eventsHelper_1.eventsHelper.addEventListener(session, 'Network.webSocketFrameReceived', e => e.response.payloadData && this._page.frameManager.webSocketFrameReceived(e.requestId, e.response.opcode, e.response.payloadData)),
                eventsHelper_1.eventsHelper.addEventListener(session, 'Network.webSocketClosed', e => this._page.frameManager.webSocketClosed(e.requestId)),
                eventsHelper_1.eventsHelper.addEventListener(session, 'Network.webSocketFrameError', e => this._page.frameManager.webSocketError(e.requestId, e.errorMessage)),
            ]);
        }
    ...
    removeSession(session) {
        ...
        if (info)
            eventsHelper_1.eventsHelper.removeEventListeners(info.eventListeners);
    ...
    async _setExtraHTTPHeadersForSession(info, initial) {
        if (initial && !this._extraHTTPHeaders.length)
            return;
        await info.session.send('Network.setExtraHTTPHeaders', { headers: (0, utils_1.headersArrayToObject)(this._extraHTTPHeaders, false /* lowerCase */) });
    }
    async clearCache() {
    ...
    _onRequest(requestWillBeSentSessionInfo, requestWillBeSentEvent, requestPausedSessionInfo, requestPausedEvent) {
        ...
        if (route) {
            ...
            request.request.setRawRequestHeaders((0, utils_1.headersObjectToArray)(requestPausedEvent.request.headers, '\n'));
        }
    ...
    _createResponse(request, responsePayload, hasExtraInfo) {
        ...
        const response = new network.Response(request.request, responsePayload.status, responsePayload.statusText, (0, utils_1.headersObjectToArray)(responsePayload.headers), timing, getResponseBody, !!responsePayload.fromServiceWorker, responsePayload.protocol);
...
class InterceptableRequest {
    ...
    constructor(options) {
        ...
        this.request = new network.Request(context, frame, serviceWorker, redirectedFrom?.request || null, documentId, url, toResourceType(requestWillBeSentEvent.type || 'Other'), method, postDataBuffer, headersOverride || (0, utils_1.headersObjectToArray)(headers));
    }
}
class RouteImpl {
    _session;
    ...
    async abort(errorCode = 'failed') {
        ...
        (0, utils_1.assert)(errorReason, 'Unknown error code: ' + errorCode);
        await catchDisallowedErrors(async () => {
...
async function catchDisallowedErrors(callback) {
    try {
        ...
        if ((0, protocolError_1.isProtocolError)(e) && e.message.includes('Invalid http status code or phrase'))
            throw e;
        if ((0, protocolError_1.isProtocolError)(e) && e.message.includes('Unsafe header'))
            throw e;
    }
}
...
class ResponseExtraInfoTracker {
    ...
    _patchHeaders(info, index) {
        ...
        if (response && requestExtraInfo) {
            response.request().setRawRequestHeaders((0, utils_1.headersObjectToArray)(requestExtraInfo.headers, '\n'));
        ...
        if (response && responseExtraInfo) {
            ...
            response.setRawResponseHeaders((0, utils_1.headersObjectToArray)(responseExtraInfo.headers, '\n'));
