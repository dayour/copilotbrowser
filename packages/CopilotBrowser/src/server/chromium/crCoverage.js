...
class CRCoverage {
    ...
    async startJSCoverage(progress, options) {
        await (0, progress_1.raceUncancellableOperationWithCleanup)(progress, () => this._jsCoverage.start(options), () => this._jsCoverage.stop());
    }
    async stopJSCoverage() {
        return await this._jsCoverage.stop();
    }
    async startCSSCoverage(progress, options) {
        await (0, progress_1.raceUncancellableOperationWithCleanup)(progress, () => this._cssCoverage.start(options), () => this._cssCoverage.stop());
    }
...
class JSCoverage {
    ...
    async start(options) {
        (0, utils_1.assert)(!this._enabled, 'JSCoverage is already enabled');
        ...
        this._eventListeners = [
            eventsHelper_1.eventsHelper.addEventListener(this._client, 'Debugger.scriptParsed', this._onScriptParsed.bind(this)),
            eventsHelper_1.eventsHelper.addEventListener(this._client, 'Runtime.executionContextsCleared', this._onExecutionContextsCleared.bind(this)),
            eventsHelper_1.eventsHelper.addEventListener(this._client, 'Debugger.paused', this._onDebuggerPaused.bind(this)),
        ];
        await Promise.all([
...
class CSSCoverage {
    ...
    async start(options) {
        (0, utils_1.assert)(!this._enabled, 'CSSCoverage is already enabled');
        ...
        this._eventListeners = [
            eventsHelper_1.eventsHelper.addEventListener(this._client, 'CSS.styleSheetAdded', this._onStyleSheet.bind(this)),
            eventsHelper_1.eventsHelper.addEventListener(this._client, 'Runtime.executionContextsCleared', this._onExecutionContextsCleared.bind(this)),
        ];
        await Promise.all([
