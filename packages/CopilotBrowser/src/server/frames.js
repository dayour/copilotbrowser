...
exports.Frame = exports.FrameManager = exports.NavigationAbortedError = void 0;
const browserContext_1 = require("./browserContext");
const dom = __importStar(require("./dom"));
const errors_1 = require("./errors");
const fileUploadUtils_1 = require("./fileUploadUtils");
const frameSelectors_1 = require("./frameSelectors");
...
class FrameManager {
    ...
    _frames = new Map();
    _mainFrame;
    _consoleMessageTags = new Map();
    _signalBarriers = new Set();
    _webSockets = new Map();
    _nextFrameSeq = 0;
    constructor(page) {
        this._page = page;
    ...
    frameAttached(frameId, parentFrameId) {
        ...
        if (!parentFrame) {
            if (this._mainFrame) {
                ...
                (0, utils_1.assert)(!this._frames.has(frameId));
    ...
    async waitForSignalsCreatedBy(progress, waitAfter, action) {
        ...
        try {
            ...
            await new Promise((0, utils_1.makeWaitForNextTask)());
            return result;
        }
        finally {
    ...
    requestStarted(request, route) {
        ...
        this._page.emitOnContext(browserContext_1.BrowserContext.Events.Request, request);
        if (route)
    ...
    requestReceivedResponse(response) {
        if (response.request()._isFavicon)
            return;
        this._page.emitOnContext(browserContext_1.BrowserContext.Events.Response, response);
    }
    reportRequestFinished(request, response) {
        this._inflightRequestFinished(request);
        if (request._isFavicon)
            return;
        this._page.emitOnContext(browserContext_1.BrowserContext.Events.RequestFinished, { request, response });
    }
    requestFailed(request, canceled) {
        ...
        if (request._isFavicon)
            return;
        this._page.emitOnContext(browserContext_1.BrowserContext.Events.RequestFailed, request);
    }
...
class Frame extends instrumentation_1.SdkObject {
    ...
    _parentFrame;
    _url = '';
    _contextData = new Map();
    _childFrames = new Set();
    _name = '';
    _inflightRequests = new Set();
    _networkIdleTimer;
    _setContentCounter = 0;
    _detachedScope = new utils_1.LongStandingScope();
    _raceAgainstEvaluationStallingEventsPromises = new Set();
    _redirectedNavigations = new Map(); // documentId -> data
    selectors;
    constructor(page, id, parentFrame) {
        ...
        this.selectors = new frameSelectors_1.FrameSelectors(this);
        this._contextData.set('main', { contextPromise: new manualPromise_1.ManualPromise(), context: null });
        this._contextData.set('utility', { contextPromise: new manualPromise_1.ManualPromise(), context: null });
    ...
    _recalculateNetworkIdle(frameThatAllowsRemovingNetworkIdle) {
        ...
        if (isNetworkIdle && !this._firedLifecycleEvents.has('networkidle')) {
            ...
            if (this === this._page.mainFrame() && this._url !== 'about:blank')
                debugLogger_1.debugLogger.log('api', `  "networkidle" event fired`);
        }
    ...
    async raceNavigationAction(progress, action) {
        return utils_1.LongStandingScope.raceMultiple([
            this._detachedScope,
    ...
    async goto(progress, url, options = {}) {
        const constructedNavigationURL = (0, utils_1.constructURLBasedOnBaseURL)(this._page.browserContext._options.baseURL, url);
    ...
    async gotoImpl(progress, url, options) {
        ...
        const navigateResult = await progress.race(this._page.delegate.navigateFrame(this, url, referer)).finally(() => this.off(Frame.Events.InternalNavigation, collectNavigations));
        let event;
        if (navigateResult.newDocumentId) {
            ...
            if (events.length)
                ...
                event = await helper_1.helper.waitForEvent(progress, this, Frame.Events.InternalNavigation, predicate).promise;
    ...
    async _waitForNavigation(progress, requiresNewDocument, options) {
        ...
        const navigationEvent = await helper_1.helper.waitForEvent(progress, this, Frame.Events.InternalNavigation, (event) => {
    ...
    async querySelector(selector, options) {
        debugLogger_1.debugLogger.log('api', `    finding element using the selector "${selector}"`);
    ...
    async setContent(progress, html, options) {
        const tag = `--copilotbrowser--set--content--${this._id}--${++this._setContentCounter}--`;
        await this.raceNavigationAction(progress, async () => {
            ...
            const tagPromise = new manualPromise_1.ManualPromise();
    ...
    async _raceWithCSPError(func) {
        ...
        const errorPromise = new Promise(resolve => {
            listeners.push(eventsHelper_1.eventsHelper.addEventListener(this._page.browserContext, browserContext_1.BrowserContext.Events.Console, (message) => {
    ...
    async retryWithProgressAndTimeouts(progress, timeouts, action) {
        ...
        while (true) {
            ...
            if (timeout) {
                ...
                await progress.race(utils_1.LongStandingScope.raceMultiple([
                    this._page.openScope,
                    this._detachedScope,
                ], actionPromise));
            }
    ...
    isNonRetriableError(e) {
        ...
        if (js.isJavaScriptErrorInEvaluate(e) || (0, protocolError_1.isSessionClosedError)(e))
            return true;
        // Certain errors opt-out of the retries, throw.
        if (dom.isNonRecoverableDOMError(e) || (0, selectorParser_1.isInvalidSelectorError)(e))
            return true;
    ...
    async setInputFiles(progress, selector, params) {
        const inputFileItems = await (0, fileUploadUtils_1.prepareFilesForUpload)(this, params);
    ...
    async expect(progress, selector, options) {
        progress.log(`${(0, utils_1.renderTitleForCall)(progress.metadata)}${options.timeoutForLogs ? ` with timeout ${options.timeoutForLogs}ms` : ''}`);
        ...
        try {
            ...
            const result = { matches: options.isNot, log: (0, callLog_1.compressCallLog)(progress.metadata.log) };
            if ((0, selectorParser_1.isInvalidSelectorError)(e)) {
    ...
    async waitForFunctionExpression(progress, expression, isFunction, arg, options, world = 'main') {
        if (typeof options.pollingInterval === 'number')
            (0, utils_1.assert)(options.pollingInterval > 0, 'Cannot poll with non-positive interval: ' + options.pollingInterval);
    ...
    _setContext(world, context) {
        ...
        if (context)
            ...
            data.contextPromise = new manualPromise_1.ManualPromise();
    }
    ...
    _startNetworkIdleTimer() {
        (0, utils_1.assert)(!this._networkIdleTimer);
    ...
    _asLocator(selector) {
        return (0, utils_1.asLocator)(this._page.browserContext._browser.sdkLanguage(), selector);
    }
}
exports.Frame = Frame;
class SignalBarrier {
    _progress;
    _protectCount = 0;
    _promise = new manualPromise_1.ManualPromise();
    ...
    addFrameNavigation(frame) {
        ...
        if (frame.parentFrame())
            return;
        this.retain();
        const waiter = helper_1.helper.waitForEvent(this._progress, frame, Frame.Events.InternalNavigation, (e) => {
            if (!e.isPublic)
                return false;
        ...
        utils_1.LongStandingScope.raceMultiple([
