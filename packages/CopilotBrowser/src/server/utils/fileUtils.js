...
exports.SerializedFS = exports.existsAsync = void 0;
...
async function removeFolders(dirs) {
    return await Promise.all(dirs.map((dir) => fs_1.default.promises.rm(dir, { recursive: true, force: true, maxRetries: 10 }).catch(e => e)));
}
...
async function copyFileAndMakeWritable(from, to) {
    ...
    await fs_1.default.promises.chmod(to, 0o664);
}
...
class SerializedFS {
    _buffers = new Map(); // Should never be accessed from within appendOperation.
    _error;
    _operations = [];
    _operationsDone;
    constructor() {
        this._operationsDone = new manualPromise_1.ManualPromise();
    ...
    async _performOperation(op) {
        switch (op.op) {
            case 'mkdir': {
                await fs_1.default.promises.mkdir(op.dir, { recursive: true });
                return;
            }
            case 'writeFile': {
                ...
                if (op.skipIfExists)
                    await fs_1.default.promises.writeFile(op.file, op.content, { flag: 'wx' }).catch(() => { });
                else
                    await fs_1.default.promises.writeFile(op.file, op.content);
                return;
            }
            case 'copyFile': {
                await fs_1.default.promises.copyFile(op.from, op.to);
                return;
            }
            case 'appendFile': {
                await fs_1.default.promises.appendFile(op.file, op.content);
                return;
            }
            case 'zip': {
                ...
                zipFile.outputStream
                    .pipe(fs_1.default.createWriteStream(op.zipFileName))
