...
class WKInterceptableRequest {
    ...
    constructor(session, frame, event, redirectedFrom, documentId) {
        ...
        this.request = new network.Request(frame._page.browserContext, frame, null, redirectedFrom?.request || null, documentId, event.request.url, resourceType, event.request.method, postDataBuffer, (0, utils_1.headersObjectToArray)(event.request.headers));
    }
    ...
    createResponse(responsePayload) {
        ...
        const setCookieSeparator = process.platform === 'darwin' ? ',' : 'copilotbrowser-set-cookie-separator';
        const response = new network.Response(this.request, responsePayload.status, responsePayload.statusText, (0, utils_1.headersObjectToArray)(responsePayload.headers, ',', setCookieSeparator), timing, getResponseBody, responsePayload.source === 'service-worker');
        ...
        if (responsePayload.requestHeaders && Object.keys(responsePayload.requestHeaders).length) {
            ...
            this.request.setRawRequestHeaders((0, utils_1.headersObjectToArray)(headers));
        }
        else {
...
class WKRouteImpl {
    ...
    async abort(errorCode) {
        ...
        (0, utils_1.assert)(errorType, 'Unknown error code: ' + errorCode);
    ...
    async fulfill(response) {
        ...
        const headers = (0, utils_1.headersArrayToObject)(response.headers, true /* lowerCase */);
    ...
    async continue(overrides) {
        ...
        await this._session.sendMayFail('Network.interceptWithRequest', {
            ...
            headers: overrides.headers ? (0, utils_1.headersArrayToObject)(overrides.headers, false /* lowerCase */) : undefined,
