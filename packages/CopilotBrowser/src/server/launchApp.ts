...
import { buildcopilotbrowserCLICommand, findChromiumChannelBestEffort } from './registry';
  ...
  try {
    ...
    if (channel) {
      error = rewriteErrorMessage(error, [
        ...
        wrapInASCIIBox(`${buildcopilotbrowserCLICommand(options.sdkLanguage, 'install')}`, 2),
      ].join('\n'));
    }
    throw error;
  }
