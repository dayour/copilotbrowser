...
exports.TestTracing = exports.testTraceEntryName = void 0;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const utils_1 = require("@copilotbrowser/copilotbrowser/lib/utils");
const zipBundle_1 = require("@copilotbrowser/copilotbrowser/lib/zipBundle");
const util_1 = require("../util");
exports.testTraceEntryName = 'test.trace';
const version = 8;
let traceOrdinal = 0;
class TestTracing {
    _testInfo;
    ...
    _liveTraceFile;
    _traceEvents = [];
    _temporaryTraceFiles = [];
    _artifactsDir;
    _tracesDir;
    _contextCreatedEvent;
    _didFinishTestFunctionAndAfterEachHooks = false;
    constructor(testInfo, artifactsDir) {
        this._testInfo = testInfo;
        this._artifactsDir = artifactsDir;
        this._tracesDir = path_1.default.join(this._artifactsDir, 'traces');
        this._contextCreatedEvent = {
            version,
            type: 'context-options',
            origin: 'testRunner',
            browserName: '',
            copilotbrowserVersion: (0, utils_1.getcopilotbrowserVersion)(),
            options: {},
            platform: process.platform,
            wallTime: Date.now(),
            monotonicTime: (0, utils_1.monotonicTime)(),
    ...
    async startIfNeeded(value) {
        ...
        if (!this._liveTraceFile && this._options._live) {
            ...
            this._liveTraceFile = { file: path_1.default.join(this._tracesDir, `${this._testInfo.testId}-test.trace`), fs: new utils_1.SerializedFS() };
            this._liveTraceFile.fs.mkdir(path_1.default.dirname(this._liveTraceFile.file));
    ...
    traceTitle() {
        return [path_1.default.relative(this._testInfo.project.testDir, this._testInfo.file) + ':' + this._testInfo.line, ...this._testInfo.titlePath.slice(1)].join(' › ');
    }
    ...
    _generateNextTraceRecordingPath() {
        const file = path_1.default.join(this._artifactsDir, (0, utils_1.createGuid)() + '.zip');
    ...
    async stopIfNeeded() {
        ...
        if (this._options?.sources) {
            ...
            for (const sourceFile of sourceFiles) {
                await fs_1.default.promises.readFile(sourceFile, 'utf8').then(source => {
                    zipFile.addBuffer(Buffer.from(source), 'resources/src@' + (0, utils_1.calculateSha1)(sourceFile) + '.txt');
                }).catch(() => { });
            }
        }
        const sha1s = new Set();
        for (const event of this._traceEvents.filter(e => e.type === 'after')) {
            for (const attachment of (event.attachments || [])) {
                let contentPromise;
                if (attachment.path)
                    contentPromise = fs_1.default.promises.readFile(attachment.path).catch(() => undefined);
                else if (attachment.base64)
                    contentPromise = Promise.resolve(Buffer.from(attachment.base64, 'base64'));
                const content = await contentPromise;
                if (content === undefined)
                    continue;
                const sha1 = (0, utils_1.calculateSha1)(content);
        ...
        zipFile.addBuffer(traceContent, exports.testTraceEntryName);
        await new Promise(f => {
            zipFile.end(undefined, () => {
                zipFile.outputStream.pipe(fs_1.default.createWriteStream(this._generateNextTraceRecordingPath())).on('close', f);
            });
        });
    ...
    appendForError(error) {
        ...
        const stack = rawStack ? (0, util_1.filteredStackTrace)(rawStack) : [];
    ...
    appendStdioToTrace(type, chunk) {
        this._appendTraceEvent({
            type,
            timestamp: (0, utils_1.monotonicTime)(),
    ...
    appendAfterActionForStep(callId, error, attachments = [], annotations) {
        this._appendTraceEvent({
            type: 'after',
            callId,
            endTime: (0, utils_1.monotonicTime)(),
...
async function mergeTraceFiles(fileName, temporaryTraceFiles) {
    temporaryTraceFiles = temporaryTraceFiles.filter(file => fs_1.default.existsSync(file));
    if (temporaryTraceFiles.length === 1) {
        await fs_1.default.promises.rename(temporaryTraceFiles[0], fileName);
        return;
    }
    const mergePromise = new utils_1.ManualPromise();
    const zipFile = new zipBundle_1.yazl.ZipFile();
    ...
    for (let i = temporaryTraceFiles.length - 1; i >= 0; --i) {
        ...
        zipBundle_1.yauzl.open(tempFile, (err, inZipFile) => {
            ...
            inZipFile.on('entry', entry => {
                ...
                if (entry.fileName === exports.testTraceEntryName) {
    ...
    zipFile.end(undefined, () => {
        zipFile.outputStream.pipe(fs_1.default.createWriteStream(fileName)).on('close', () => {
            void Promise.all(temporaryTraceFiles.map(tempFile => fs_1.default.promises.unlink(tempFile))).then(() => {
