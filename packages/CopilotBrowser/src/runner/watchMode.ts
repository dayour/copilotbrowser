...
let showBrowserServer: copilotbrowserServer | undefined;
...
function printConfiguration(options: WatchModeOptions, title?: string) {
  ...
  const tokens: string[] = [];
  tokens.push(`${packageManagerCommand} copilotbrowser test`);
  if (options.projects)
...
async function toggleShowBrowser() {
  if (!showBrowserServer) {
    showBrowserServer = new copilotbrowserServer({ mode: 'extension', path: '/' + createGuid(), maxConnections: 1 });
