...
export interface Page {
   ...
   *
   * If the function passed to the **Page.evaluate()** returns a `Promise`, then **Page.evaluate()** would wait for the
   * promise to resolve and return its value.
   *
   * If the function passed to the **Page.evaluate()** returns a non-`Serializable` value, then
   *
   * **Page.evaluate()** resolves to `undefined`. copilotbrowser also supports transferring some additional values that
   ...
   * @param arg Optional argument to pass to **expression**.
   */
   ...
   *
   * The only difference between **Page.evaluate()** and **Page.evaluateHandle()** is that **Page.evaluateHandle()**
   * returns `JSHandle`.
   *
   * If the function passed to the **Page.evaluateHandle()** returns a `Promise`, then **Page.evaluateHandle()** would
  ...
  /**
   * **NOTE** Use web assertions that assert visibility or a locator-based **Locator.waitFor()** instead. Read more about
   * [locators](https://copilotbrowser.dev/docs/locators).
   *
   ...
   * `detached`.
   *
   * **NOTE** copilotbrowser automatically waits for element to be ready before performing an action. Using `Locator`
   ...
   * If at the moment of calling the method **selector** already satisfies the condition, the method will return
   * immediately. If the selector doesn't satisfy the condition for the **timeout** milliseconds, the function will
   * throw.
   *
   * **Usage**
   *
   ...
   * @param callback Callback function that will be called in the copilotbrowser's context.
   * @param options
   */
   ...
   * ```
   *
   * **NOTE** When no **Page.event('dialog')** or **BrowserContext.event('dialog')** listeners are present, all dialogs
  ...
  /**
   * Emitted when a file chooser is supposed to appear, such as after clicking the  `<input type=file>`. copilotbrowser
   ...
   * to this network request, use **BrowserContext.route()** and **BrowserContext.event('request')** respectively
   ...
   * ```
   *
   * **NOTE** Use **Page.waitForLoadState()** to wait until the page gets to a particular state (you should not need it
   * in most cases).
   *
   */
   ...
   * will complete with **Page.event('requestFinished')** event and not with **Page.event('requestFailed')**. A request
   ...
   * - copilotbrowser checks for the overlay every time before executing or retrying an action that requires an
   *   [actionability check](https://copilotbrowser.dev/docs/actionability), or before performing an auto-waiting assertion check. When overlay
   *   is visible, copilotbrowser calls the handler first, and then proceeds with the action/assertion. Note that the
   *   handler is only called when you perform an action/assertion - if the overlay becomes visible but you don't
   *   perform any actions, the handler will not be triggered.
   * - After executing the handler, copilotbrowser will ensure that overlay that triggered the handler is not visible
   ...
   *
   * For example, consider a test that calls **Locator.focus()** followed by **Keyboard.press()**. If your handler
   * clicks a button between these two actions, the focused element most likely will be wrong, and key press will happen
   * on the unexpected element. Use **Locator.press()** instead to avoid this problem.
   *
   * Another example is a series of mouse actions, where **Mouse.move()** is followed by **Mouse.down()**. Again, when
   * the handler runs between these two actions, the mouse position will be wrong during the mouse down. Prefer
   ...
   * @param handler Function that should be run once **locator** appears. This function should get rid of the element that blocks
   * actions like click.
   * @param options
   */
  addLocatorHandler(locator: Locator, handler: ((locator: Locator) => Promise<any>), options?: {
    /**
     * By default, after calling the handler copilotbrowser will wait until the overlay becomes hidden, and only then
     * copilotbrowser will continue with the action/assertion that triggered the handler. This option allows to opt-out of
  ...
  /**
   * **NOTE** Use locator-based **Locator.check()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   * This method checks an element matching **selector** by performing the following steps:
   * 1. Find an element matching **selector**. If there is none, wait until a matching element is attached to the
   *    DOM.
   ...
   * 1. Wait for [actionability](https://copilotbrowser.dev/docs/actionability) checks on the matched element, unless **force** option is set.
  ...
  check(selector: string, options?: {
    /**
     * Whether to bypass the [actionability](https://copilotbrowser.dev/docs/actionability) checks. Defaults to `false`.
     */
    force?: boolean;
     ...
     * option in the config, or by using the **BrowserContext.setDefaultTimeout()** or
     *
    ...
    /**
     * When set, this method only performs the [actionability](https://copilotbrowser.dev/docs/actionability) checks and skips the action. Defaults
  ...
  /**
   * Clears all stored page errors from this page. Subsequent calls to **Page.pageErrors()** will only return errors
   * thrown after the clear.
   */
  clearPageErrors(): Promise<void>;

  /**
   * **NOTE** Use locator-based **Locator.click()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
  ...
  /**
   * **NOTE** Use locator-based **Locator.dblclick()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
  ...
  /**
   * **NOTE** Use locator-based **Locator.dispatchEvent()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   ...
   *
   * @param name Name of the function on the window object
   * @param callback Callback function which will be called in copilotbrowser's context.
   */
  exposeFunction(name: string, callback: Function): Promise<void>;

  /**
   * **NOTE** Use locator-based **Locator.fill()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   * This method waits for an element matching **selector**, waits for [actionability](https://copilotbrowser.dev/docs/actionability) checks,
  ...
  /**
   * **NOTE** Use locator-based **Locator.focus()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
  ...
  /**
   * **NOTE** Use locator-based **Locator.getAttribute()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   * **Returns:** `null|string`
   *
   ...
   *
   * See also **Locator.filter()** that allows to match by another criteria, like an accessible role, and then filter by
   * the text content.
   *
   * **Usage**
   *
  ...
  goBack(options?: {
     ...
     * `navigationTimeout` option in the config, or by using the **BrowserContext.setDefaultNavigationTimeout()**,
     *
   ...
   *
   * @param url URL to navigate page to. The url should include scheme, e.g. `https://`. When a **Browser.newContext.baseURL** via
   * the context options was provided and the passed URL is a path, it gets merged via the
  ...
  /**
   * **NOTE** Use locator-based **Locator.hover()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
  ...
  /**
   * **NOTE** Use locator-based **Locator.innerHTML()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   * **Returns:** `string`
   *
  ...
  /**
   * **NOTE** Use locator-based **Locator.innerText()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   * **Returns:** `string`
   *
  ...
  /**
   * **NOTE** Use locator-based **Locator.inputValue()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   * **Returns:** `string`
   *
  ...
  /**
   * **NOTE** Use locator-based **Locator.isChecked()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   * **Returns:** `boolean`
   *
  ...
  /**
   * **NOTE** Use locator-based **Locator.isDisabled()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   * **Returns:** `boolean`
   *
   * Returns whether the element is disabled, the opposite of [enabled](https://copilotbrowser.dev/docs/actionability#enabled).
   * @param selector A selector to search for an element. If there are multiple elements satisfying the selector, the first will be
   * used.
   * @param options
   */
  ...
  /**
   * **NOTE** Use locator-based **Locator.isEditable()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   * **Returns:** `boolean`
   *
   * Returns whether the element is [editable](https://copilotbrowser.dev/docs/actionability#editable).
   * @param selector A selector to search for an element. If there are multiple elements satisfying the selector, the first will be
   * used.
   * @param options
   */
  ...
  /**
   * **NOTE** Use locator-based **Locator.isEnabled()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   * **Returns:** `boolean`
   *
   * Returns whether the element is [enabled](https://copilotbrowser.dev/docs/actionability#enabled).
   * @param selector A selector to search for an element. If there are multiple elements satisfying the selector, the first will be
   * used.
   * @param options
   */
  ...
  /**
   * **NOTE** Use locator-based **Locator.isHidden()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   * **Returns:** `boolean`
   *
   * Returns whether the element is hidden, the opposite of [visible](https://copilotbrowser.dev/docs/actionability#visible).  **selector** that
  ...
  /**
   * **NOTE** Use locator-based **Locator.isVisible()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   * **Returns:** `boolean`
   *
   * Returns whether the element is [visible](https://copilotbrowser.dev/docs/actionability#visible). **selector** that does not match any
   ...
   *
   * [Learn more about locators](https://copilotbrowser.dev/docs/locators).
   * @param selector A selector to use when resolving DOM element.
   * @param options
   */
  locator(selector: string, options?: {
     ...
     * `article` that has `text=copilotbrowser` matches `<article><div>copilotbrowser</div></article>`.
     *
     * Inner locator **must be relative** to the outer locator and is queried starting with the outer locator match, not
     * the document root. For example, you can find `content` that has `div` in
     * `<article><content><div>copilotbrowser</div></content></article>`. However, looking for `content` that has `article
     ...
     * outer one. For example, `article` that does not have `div` matches
     * `<article><span>copilotbrowser</span></article>`.
     *
     * Note that outer and inner locators must belong to the same frame. Inner locator must not contain `FrameLocator`s.
     */
    hasNot?: Locator;
     ...
     * passed a `string`, matching is case-insensitive and searches for a substring. For example, `"copilotbrowser"`
     * matches `<article><div>copilotbrowser</div></article>`.
     */
  ...
  /**
   * Pauses script execution. copilotbrowser will stop executing the script and wait for the user to either press the
   * 'Resume' button in the page overlay or to call `copilotbrowser.resume()` in the DevTools console.
   *
   ...
   *
   * **NOTE** This method requires copilotbrowser to be started in a headed mode, with a falsy
   * **BrowserType.launch.headless** option.
   *
   */
  pause(): Promise<void>;
  ...
  /**
   * **NOTE** Use locator-based **Locator.press()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   ...
   * @param locator Locator passed to **Page.addLocatorHandler()**.
   */
   ...
   *
   * Note that requests reported through the **Page.event('request')** request are not collected, so there is a trade
   * off between efficient memory usage with **Page.requests()** and the amount of available information reported
   * through **Page.event('request')**.
   */
   ...
   * [this](https://github.com/dayour/copilotbrowser/issues/1090) issue. We recommend disabling Service Workers when
   * using request interception by setting **Browser.newContext.serviceWorkers** to `'block'`.
   *
   * **NOTE** **Page.route()** will not intercept the first request of a popup page. Use **BrowserContext.route()**
   * instead.
   *
   * **Usage**
   *
   ...
   * **Browser.newContext.baseURL** is set in the context options and the provided URL is a string that does not start
   * with `*`, it is resolved using the [`new URL()`](https://developer.mozilla.org/en-US/docs/Web/API/URL/URL)
   * constructor.
   ...
   * [Replaying from HAR](https://copilotbrowser.dev/docs/mock#replaying-from-har).
   *
   * copilotbrowser will not serve requests intercepted by Service Worker from the HAR file. See
   * [this](https://github.com/dayour/copilotbrowser/issues/1090) issue. We recommend disabling Service Workers when
   * using request interception by setting **Browser.newContext.serviceWorkers** to `'block'`.
   ...
   * **Browser.newContext.baseURL** context option.
  ...
  /**
   * **NOTE** Use locator-based **Locator.selectOption()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
  ...
  /**
   * **NOTE** Use locator-based **Locator.setChecked()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
  ...
  /**
   * **NOTE** Use locator-based **Locator.setInputFiles()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
  ...
  /**
   * **NOTE** Use locator-based **Locator.tap()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   ...
   * Passing zero timeout disables this.
   *
   * **NOTE** **Page.tap()** the method will throw if **Browser.newContext.hasTouch** option of the browser context is
   * false.
   *
   * @param selector A selector to search for an element. If there are multiple elements satisfying the selector, the first will be
   * used.
  ...
  /**
   * **NOTE** Use locator-based **Locator.textContent()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   * **Returns:** `null|string`
   *
  ...
  /**
   * **NOTE** Use locator-based **Locator.uncheck()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   ...
   * **Returns:** `Video`
   *
   * Video object associated with this page. Can be used to control video recording with **Video.start()** and
   * **Video.stop()**, or to access the video file when using the `recordVideo` context option.
   */
  video(): Video;
   ...
   * immediately.
   *
   * **NOTE** Most of the time, this method is not needed because copilotbrowser
   * [auto-waits before every action](https://copilotbrowser.dev/docs/actionability).
   *
   * **Usage**
   *
   * ```js
   ...
   * **Returns:** `Request`
   *
   * Waits for the matching request and returns it. See [waiting for event](https://copilotbrowser.dev/docs/events#waiting-for-event) for more
   ...
   * **Returns:** `Response`
   *
   * Returns the matched response. See [waiting for event](https://copilotbrowser.dev/docs/events#waiting-for-event) for more details about
   * events.
   *
   * **Usage**
   *
   ...
   * ```
   *
   * @param urlOrPredicate Request URL string, regex or predicate receiving `Response` object. When a **Browser.newContext.baseURL** via the
   ...
   *
   * Browser-specific Coverage implementation. See [Coverage](https://copilotbrowser.dev/docs/api/class-coverage) for
   * more details.
   */
  coverage: Coverage;
...
export interface PageAgent {
  ...
  expect(expectation: string, options?: {
    /**
     * All the agentic actions are converted to the copilotbrowser calls and are cached. By default, they are cached
     * globally with the `task` as a key. This option allows controlling the cache key explicitly.
     */
    cacheKey?: string;
     ...
     * the config, or by specifying the `expect` property of the **Page.agent.expect** option. Pass `0` to disable
     * timeout.
     */
    timeout?: number;
  }): Promise<void>;
  ...
  perform(task: string, options?: {
     ...
     * the config, or by using the **BrowserContext.setDefaultTimeout()** or
     *
 ...
 * - **Page.event('frameAttached')** - fired when the frame gets attached to the page. A Frame can be attached to
 *   the page only once.
 * - **Page.event('frameNavigated')** - fired when the frame commits navigation to a different URL.
 * - **Page.event('frameDetached')** - fired when the frame gets detached from the page.  A Frame can be detached
 *   from the page only once.
 *
...
export interface Frame {
   ...
   *
   * If the function passed to the **Frame.evaluate()** returns a `Promise`, then **Frame.evaluate()** would wait for
   * the promise to resolve and return its value.
   *
   * If the function passed to the **Frame.evaluate()** returns a non-`Serializable` value, then
   *
   * **Frame.evaluate()** returns `undefined`. copilotbrowser also supports transferring some additional values that are
   ...
   *    If the element is detached during the checks, the whole action is retried.
   * 1. Scroll the element into view if needed.
   * 1. Use **Page.mouse** to double click in the center of the element, or the specified **position**. if the first
 ...
 * context.
 *
 * copilotbrowser allows creating isolated non-persistent browser contexts with **Browser.newContext()** method.
...
export interface BrowserContext {
   ...
   * called, the function executes **callback** and returns a `Promise` which resolves to the return value of
   *
   ...
   * will also fire for popup pages. See also **Page.event('popup')** to receive events about popups relevant to a
   * specific page.
   *
   * The earliest moment that page is available is when it has navigated to the initial url. For example, when opening a
   * popup with `window.open('http://example.com')`, this event will fire when the network request to
   ...
   * only listen for requests from a particular page, use **Page.event('request')**.
   *
   ...
   * use **Page.event('requestFailed')**.
   *
   * **NOTE** HTTP Error responses, such as 404 or 503, are still successful responses from HTTP standpoint, so request
   * will complete with **BrowserContext.event('requestFinished')** event and not with
   ...
   * particular page, use **Page.event('requestFinished')**.
   */
  on(event: 'requestfinished', listener: (request: Request) => any): this;
   ...
   * page, use **Page.event('pageError')** instead.
   */
   ...
   * ```
   *
   * @param storageState Learn more about [storage state and auth](https://copilotbrowser.dev/docs/auth).
   *
   ...
   * @param handler Optional handler function used to register a routing with **BrowserContext.route()**.
   */
  unroute(url: string|RegExp|URLPattern|((url: URL) => boolean), handler?: ((route: Route, request: Request) => Promise<any>|any)): Promise<void>;
...
export interface Browser {
   ...
   * calling **Browser.close()**. This will ensure the `context` is closed gracefully and any artifacts—like HARs and
  ...
  newPage(options?: {
    ...
    /**
     * When using **Page.goto()**, **Page.route()**, **Page.waitForURL()**, **Page.waitForRequest()**, or
     * **Page.waitForResponse()** it takes the base URL in consideration by using the
     ...
     * work by replacing `localhost` with `local.copilotbrowser`.
     *
     */
     ...
     *
     * **Page.emulateMedia()** for more details. Passing `null` resets emulation to system defaults. Defaults to
     * `'light'`.
     */
    colorScheme?: null|"light"|"dark"|"no-preference";
     ...
     * [emulating devices with device scale factor](https://copilotbrowser.dev/docs/emulation#devices).
     */
    ...
    /**
     * Emulates `'forced-colors'` media feature, supported values are `'active'`, `'none'`. See **Page.emulateMedia()**
     ...
     * [mobile emulation](https://copilotbrowser.dev/docs/emulation#devices).
     */
    hasTouch?: boolean;
     ...
     * about [mobile emulation](https://copilotbrowser.dev/docs/emulation#ismobile).
     */
    isMobile?: boolean;

    /**
     * Whether or not to enable JavaScript in the context. Defaults to `true`. Learn more about
     * [disabling JavaScript](https://copilotbrowser.dev/docs/emulation#javascript-enabled).
     */
     ...
     * locale. Learn more about emulation in our [emulation guide](https://copilotbrowser.dev/docs/emulation#locale--timezone).
     */
    locale?: string;
     ...
     * [network emulation](https://copilotbrowser.dev/docs/emulation#offline).
     */
    offline?: boolean;
     ...
     * If not specified, the HAR is not recorded. Make sure to await **BrowserContext.close()** for the HAR to be saved.
     */
    recordHar?: {
      /**
      ...
      /**
       * A glob or regex pattern to filter requests that are stored in the HAR. When a **Browser.newContext.baseURL** via
       * the context options was provided and the passed URL is a path, it gets merged via the
    ...
    /**
     * Learn more about [storage state and auth](https://copilotbrowser.dev/docs/auth).
     *
     * Populates context with given storage state. This option can be used to initialize context with logged-in
     * information obtained via **BrowserContext.storageState()**.
     */
     ...
     * viewport emulation. Learn more about [viewport emulation](https://copilotbrowser.dev/docs/emulation#viewport).
     *
   ...
   * chromium-specific debugging tool. API to control [copilotbrowser Tracing](https://copilotbrowser.dev/docs/trace-viewer) could be found
   * [here](https://copilotbrowser.dev/docs/api/class-tracing).
   *
   * You can use **Browser.startTracing()** and **Browser.stopTracing()** to create a trace file that can be opened in
...
export interface Worker {
   ...
   * Returns the return value of **expression**.
   *
   * If the function passed to the **Worker.evaluate()** returns a `Promise`, then **Worker.evaluate()** would wait for
   * the promise to resolve and return its value.
   *
   * If the function passed to the **Worker.evaluate()** returns a non-`Serializable` value, then **Worker.evaluate()**
   * returns `undefined`. copilotbrowser also supports transferring some additional values that are not serializable by
   ...
   *
   * **Worker.evaluateHandle()** is that **Worker.evaluateHandle()** returns `JSHandle`.
   *
   * If the function passed to the **Worker.evaluateHandle()** returns a `Promise`, then **Worker.evaluateHandle()**
...
export interface ElementHandle<T=Node> extends JSHandle<T> {
  ...
  /**
   * **NOTE** Use web assertions that assert visibility or a locator-based **Locator.waitFor()** instead.
   *
   * **Returns:** `null|ElementHandle`
   *
   ...
   * `detached`.
   *
   * Wait for the **selector** relative to the element handle to satisfy **state** option (either appear/disappear from
   * dom, or become visible/hidden). If at the moment of calling the method **selector** already satisfies the
   * condition, the method will return immediately. If the selector doesn't satisfy the condition for the
   *
   ...
   * 1. Wait for [actionability](https://copilotbrowser.dev/docs/actionability) checks on the element, unless **force** option is set.
   * 1. Scroll the element into view if needed.
   * 1. Use **Page.mouse** to click in the center of the element.
   * 1. Ensure that the element is now checked. If not, this method throws.
   *
  ...
  click(options?: {
    ...
    /**
     * Defaults to 1. Sends `n` interpolated `mousemove` events to represent travel between copilotbrowser's current
     * cursor position and the provided destination. When set to 1, emits a single `mousemove` event at the destination
     * location.
     */
    steps?: number;
   ...
   * **NOTE** Use locator-based **Locator.fill()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   * This method waits for [actionability](https://copilotbrowser.dev/docs/actionability) checks, focuses the element, fills it and triggers an
   ...
   * **Returns:** `boolean`
   *
   * Returns whether the element is hidden, the opposite of [visible](https://copilotbrowser.dev/docs/actionability#visible).
   */
   ...
   * **Returns:** `boolean`
   *
   * Returns whether the element is [visible](https://copilotbrowser.dev/docs/actionability#visible).
   */
  ...
  /**
   * **NOTE** Use locator-based **Locator.screenshot()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   * **Returns:** `Buffer`
   *
   ...
   *
   * This method waits for the [actionability](https://copilotbrowser.dev/docs/actionability) checks, then scrolls element into view before taking
  ...
  screenshot(options?: {
     ...
     * invisible elements, see [Matching only visible elements](https://copilotbrowser.dev/docs/locators#matching-only-visible-elements) to disable
     * that.
     */
    mask?: Array<Locator>;
  ...
  /**
   * **NOTE** Use locator-based **Locator.scrollIntoViewIfNeeded()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   * This method waits for [actionability](https://copilotbrowser.dev/docs/actionability) checks, then tries to scroll element into view, unless
   ...
   *
   * See [scrolling](https://copilotbrowser.dev/docs/input#scrolling) for alternative ways to scroll.
   * @param options
   */
   ...
   * **Returns:** `Array<string>`
   *
   * This method waits for [actionability](https://copilotbrowser.dev/docs/actionability) checks, waits until all specified options are present in
  ...
  /**
   * **NOTE** Use locator-based **Locator.selectText()** instead. Read more about [locators](https://copilotbrowser.dev/docs/locators).
   *
   * This method waits for [actionability](https://copilotbrowser.dev/docs/actionability) checks, then focuses the element and selects all its
   * text content.
   *
   ...
   *
   * Depending on the **state** parameter, this method waits for one of the [actionability](https://copilotbrowser.dev/docs/actionability) checks
   * to pass. This method throws when the element is detached while waiting, unless waiting for the `"hidden"` state.
   * - `"visible"` Wait until the element is [visible](https://copilotbrowser.dev/docs/actionability#visible).
   * - `"hidden"` Wait until the element is [not visible](https://copilotbrowser.dev/docs/actionability#visible) or not attached. Note that
   *   waiting for hidden does not throw when the element detaches.
   * - `"stable"` Wait until the element is both [visible](https://copilotbrowser.dev/docs/actionability#visible) and
   *   [stable](https://copilotbrowser.dev/docs/actionability#stable).
   * - `"enabled"` Wait until the element is [enabled](https://copilotbrowser.dev/docs/actionability#enabled).
   * - `"disabled"` Wait until the element is [not enabled](https://copilotbrowser.dev/docs/actionability#enabled).
   * - `"editable"` Wait until the element is [editable](https://copilotbrowser.dev/docs/actionability#editable).
   *
...
/**
 * Locators are the central piece of copilotbrowser's auto-waiting and retry-ability. In a nutshell, locators
 * represent a way to find element(s) on the page at any moment. A locator can be created with the **Page.locator()**
 * method.
 *
 * [Learn more about locators](https://copilotbrowser.dev/docs/locators).
 */
export interface Locator {
   ...
   * **Details**
   *
   * Returns the return value of **expression** as a[JSHandle](https://copilotbrowser.dev/docs/api/class-jshandle),
   * called with the matching element as a first argument, and **arg** as a second argument.
   *
   ...
   * **Returns:** `string`
   *
   * Returns a human-readable representation of the locator, using the **Locator.description()** if one exists;
   ...
   *
   * **NOTE** If you need to assert text on the page, prefer **LocatorAssertions.toHaveText()** with
   * **LocatorAssertions.toHaveText.useInnerText** option to avoid flakiness. See
   * [assertions guide](https://copilotbrowser.dev/docs/test-assertions) for more details.
   *
   * **Usage**
   *
   * ```js
   ...
   * **Returns:** `string`
   *
   * Captures the aria snapshot of the given element. Read more about [aria snapshots](https://copilotbrowser.dev/docs/aria-snapshots) and
   ...
   * **Details**
   *
   * This method waits for [actionability](https://copilotbrowser.dev/docs/actionability) checks, focuses the element, clears it and triggers an
   ...
   *
   * **NOTE** If you need to assert the number of elements on the page, prefer **LocatorAssertions.toHaveCount()** to
   * avoid flakiness. See [assertions guide](https://copilotbrowser.dev/docs/test-assertions) for more details.
   *
   * **Usage**
   *
   * ```js
   ...
   * flakiness. See [assertions guide](https://copilotbrowser.dev/docs/test-assertions) for more details.
   *
   * @param name Attribute name to get the value for.
   * @param options
   */
   ...
   * Returns whether the element is checked. Throws if the element is not a checkbox or radio input.
   *
   * **NOTE** If you need to assert that checkbox is checked, prefer **LocatorAssertions.toBeChecked()** to avoid
   * flakiness. See [assertions guide](https://copilotbrowser.dev/docs/test-assertions) for more details.
   *
   * **Usage**
   *
   ...
   * **Returns:** `boolean`
   *
   * Returns whether the element is [editable](https://copilotbrowser.dev/docs/actionability#editable). If the target element is not an `<input>`,
   ...
   * Returns whether the element is hidden, the opposite of [visible](https://copilotbrowser.dev/docs/actionability#visible).
   *
   * **NOTE** If you need to assert that element is hidden, prefer **LocatorAssertions.toBeHidden()** to avoid
   * flakiness. See [assertions guide](https://copilotbrowser.dev/docs/test-assertions) for more details.
   *
   * **Usage**
   *
   ...
   * Returns whether the element is [visible](https://copilotbrowser.dev/docs/actionability#visible).
   *
   * **NOTE** If you need to assert that element is visible, prefer **LocatorAssertions.toBeVisible()** to avoid
   * flakiness. See [assertions guide](https://copilotbrowser.dev/docs/test-assertions) for more details.
   *
   * **Usage**
   *
   ...
   * a [locator strictness](https://copilotbrowser.dev/docs/locators#strictness) violation.
   *
   * **Usage**
   *
   ...
   * possibly throwing the ["strict mode violation" error](https://copilotbrowser.dev/docs/locators#strictness). In this case, you can use
  ...
  /**
   * **NOTE** In most cases, you should use **Locator.fill()** instead. You only need to press keys one by one if there
   ...
   * dispatching touch events, see the [emulating legacy touch events](https://copilotbrowser.dev/docs/touch-events) page.
   *
   * **Details**
   *
   * This method taps the element by performing the following steps:
...
export interface BrowserType<Unused = {}> {
   ...
   *
   * **NOTE** This connection is significantly lower fidelity than the copilotbrowser protocol connection via
   ...
   * **Returns:** `Browser`
   *
   * This method attaches copilotbrowser to an existing browser instance created via `BrowserType.launchServer` in
   * Node.js.
   *
   * **NOTE** The major and minor version of the copilotbrowser instance that connects needs to match the version of
   * copilotbrowser that launches the browser (1.2.3 → is compatible with 1.2.x).
   *
   * @param wsEndpoint A copilotbrowser browser websocket endpoint to connect to. You obtain this endpoint via `BrowserServer.wsEndpoint`.
   * @param options
   */
  ...
  launchPersistentContext(userDataDir: string, options?: {
    ...
    /**
     * **NOTE** Use custom browser args at your own risk, as some of them may break copilotbrowser functionality.
     *
     ...
     *
     * Use "chromium" to [opt in to new headless mode](https://copilotbrowser.dev/docs/browsers#chromium-new-headless-mode).
     *
     * Use "chrome", "chrome-beta", "chrome-dev", "chrome-canary", "msedge", "msedge-beta", "msedge-dev", or
     * "msedge-canary" to use branded [Google Chrome and Microsoft Edge](https://copilotbrowser.dev/docs/browsers#google-chrome--microsoft-edge).
     */
    channel?: string;
    ...
    /**
     * Path to a browser executable to run instead of the bundled one. If **executablePath** is a relative path, then it
     * is resolved relative to the current working directory. Note that copilotbrowser only works with the bundled
     ...
     * `copilotbrowser_FIREFOX_POLICIES_JSON` environment variable.
     */
    ...
    /**
     * Slows down copilotbrowser operations by the specified amount of milliseconds. Useful so that you can see what is
     * going on.
     */
    slowMo?: number;
   ...
   *
   * Returns the browser app instance. You can connect to it via **BrowserType.connect()**, which requires the
  ...
  launchServer(options?: {
     ...
     *
     * **NOTE** Any process or web page (including those running in copilotbrowser) with knowledge of the `wsPath` can
     * take control of the OS user. For this reason, you should use an unguessable token when using this option.
     *
     */
    wsPath?: string;
 ...
 * ```
 *
 * Since we do not call **WebSocketRoute.connectToServer()** inside the WebSocket route handler, copilotbrowser
 * assumes that WebSocket will be mocked, and opens the WebSocket inside the page automatically.
 *
 ...
 * them. Calling **WebSocketRoute.connectToServer()** returns a server-side `WebSocketRoute` instance that you can
 * send messages to, or handle incoming messages.
 *
 ...
 *
 * However, if you call **WebSocketRoute.onMessage()** on the original route, messages from the page to the server
 * **will not be forwarded** anymore, but should instead be handled by the **WebSocketRoute.onMessage.handler**.
 *
 * Similarly, calling **WebSocketRoute.onMessage()** on the server-side WebSocket will **stop forwarding messages**
 * from the server to the page, and **WebSocketRoute.onMessage.handler** should take care of them.
 *
 * The following example blocks some messages in both directions. Since it calls **WebSocketRoute.onMessage()** in
 * both directions, there is no automatic forwarding at all.
 *
 * ```js
 * await page.routeWebSocket('/ws', ws => {
 *   const server = ws.connectToServer();
...
export interface WebSocketRoute {
   ...
   * messages by responding to them with **WebSocketRoute.send()**, forwarding them to the server-side connection
   * returned by **WebSocketRoute.connectToServer()** or do something else.
   *
   ...
   *   in the page will be **automatically forwarded** to the server, unless **WebSocketRoute.onMessage()** is called
 ...
 * - extends: `Error`
 *
 * TimeoutError is emitted whenever certain operations are terminated due to timeout, e.g. **Locator.waitFor()** or
 * **BrowserType.launch()**.
 *
 * ```js
...
/**
 * Electron application representation. You can use **Electron.launch()** to obtain the application instance. This
...
export interface ElectronApplication {
   ...
   *
   * **ElectronApplication.evaluate()** returns `undefined`. copilotbrowser also supports transferring some additional
   ...
   * Returns the return value of **expression** as a `JSHandle`.
   *
   * The only difference between **ElectronApplication.evaluate()** and **ElectronApplication.evaluateHandle()** is that
   * **ElectronApplication.evaluateHandle()** returns `JSHandle`.
   *
   ...
   * used for copilotbrowser automation.
   */
...
/**
 * copilotbrowser has **experimental** support for Android automation. This includes Chrome for Android and Android
 * WebView.
 *
 * *Requirements*
...
export interface Android {
   ...
   *
   * This methods attaches copilotbrowser to an existing Android device. Use **Android.launchServer()** to launch a new
  ...
  devices(options?: {
    ...
    /**
     * Prevents automatic copilotbrowser driver installation on attach. Assumes that the drivers have been installed
     * already.
     */
...
export interface AndroidDevice {
  ...
  drag(selector: AndroidSelector, dest: {
     ...
     * by using the **AndroidDevice.setDefaultTimeout()** method.
     */
    timeout?: number;
  }): Promise<void>;
 ...
 * **AndroidDevice.open()** to open a socket.
 */
 ...
 * **copilotbrowser.request**. For more information see `APIRequestContext`.
 */
export interface APIRequest {
  ...
  newContext(options?: {
    ...
    /**
     * Populates context with given storage state. This option can be used to initialize context with logged-in
     * information obtained via **BrowserContext.storageState()** or **APIRequestContext.storageState()**. Either a path
     * to the file with saved storage, or the value returned by one of **BrowserContext.storageState()** or
     *
 ...
 *
 * Each copilotbrowser browser context has associated with it `APIRequestContext` instance which shares cookie storage
 * with the browser context and can be accessed via **BrowserContext.request** or **Page.request**. It is also
...
export interface BrowserServer {
   ...
   *
   * Browser websocket endpoint which can be used as an argument to **BrowserType.connect()** to establish connection to
   * the browser.
   *
 ...
 * more about [clock emulation](https://copilotbrowser.dev/docs/clock).
 *
...
export interface Clock {
   ...
   * Once the page has fully loaded, you can safely use **Clock.pauseAt()** to pause the clock.
   *
   * ```js
   ...
   * scenarios, use **Clock.install()** instead. Read docs on [clock emulation](https://copilotbrowser.dev/docs/clock) to learn more.
   *
   * **Usage**
   *
   * ```js
...
/**
 * `ConsoleMessage` objects are dispatched by page via the **Page.event('console')** event. For each console message
 * logged in the page there will be corresponding event in the copilotbrowser context.
 *
 * ```js
...
export interface ConsoleMessage {
  ...
  /**
   * **Returns:**
   * `ConsoleMessageType<"log"|"debug"|"info"|"error"|"warning"|"dir"|"dirxml"|"table"|"trace"|"clear"|"startGroup"|"startGroupCollapsed"|"endGroup"|"assert"|"profile"|"profileEnd"|"count"|"time"|"timeEnd">`
   */
...
/**
 * Selectors can be used to install custom selector engines. See [extensibility](https://copilotbrowser.dev/docs/extensibility) for more
 * information.
 */
...
export interface Coverage {
   ...
   * created on the page using `eval` or `new Function`. If **reportAnonymousScripts** is set to `true`, anonymous
   * scripts will have `__copilotbrowser_evaluation_script__` as their URL.
   *
   * @param options
   */
 ...
 * ```
 *
 * **NOTE** Dialogs are dismissed automatically, unless there is a **Page.event('dialog')** listener. When listener is
 * present, it **must** either **Dialog.accept()** or **Dialog.dismiss()** the dialog - otherwise the page will
 * [freeze](https://developer.mozilla.org/en-US/docs/Web/JavaScript/EventLoop#never_blocking) waiting for the dialog,
 * and actions like click will never finish.
 *
 */
 ...
 *
 * For finer control, you can use **Keyboard.down()**, **Keyboard.up()**, and **Keyboard.insertText()** to manually
...
export interface Keyboard {
  ...
  /**
   * **NOTE** In most cases, you should use **Locator.fill()** instead. You only need to press keys one by one if there
   * is special keyboard handling on the page - in this case use **Locator.pressSequentially()**.
   *
 ...
 *
 * **NOTE** If you want to debug where the mouse moved, you can use the [Trace viewer](https://copilotbrowser.dev/docs/trace-viewer-intro) or
 * [copilotbrowser Inspector](https://copilotbrowser.dev/docs/running-tests). A red dot showing the location of the mouse will be shown for
 * every mouse action.
 *
...
export interface Mouse {
   ...
   * [scrolling](https://copilotbrowser.dev/docs/input#scrolling) for alternative ways to scroll.
   *
 ...
 * event), the  **Page.event('requestFailed')** event is emitted.
 *
 * **NOTE** HTTP Error responses, such as 404 or 503, are still successful responses from HTTP standpoint, so request
...
export interface Request {
   ...
   *
   * Unlike **Request.response()**, this method does not wait for the response to arrive. It returns immediately with
   * the response object if the response has been received, or `null` if the response has not been received yet.
   */
   ...
   *
   * When the server responds with a redirect, copilotbrowser creates a new `Request` object. The two requests are
...
/**
 * Whenever a network route is set up with **Page.route()** or **BrowserContext.route()**, the `Route` object allows
 * to handle the route.
 *
 * Learn more about [networking](https://copilotbrowser.dev/docs/network).
 */
export interface Route {
   ...
   * **Details**
   *
   * The **headers** option applies to both the routed request and any redirects it initiates. However, **url**,
   * **method**, and **postData** only apply to the original request and are not carried over to redirected requests.
   *
   * **Route.continue()** will immediately send the request to the network, other matching handlers won't be invoked.
   * Use **Route.fallback()** If you want next matching handler in the chain to be invoked.
   *
  ...
  /**
   * Continues route's request with optional overrides. The method is similar to **Route.continue()** with the
   ...
   * ```
   *
   * Use **Route.continue()** to immediately send the request to the network, other matching handlers won't be invoked
   * in that case.
   * @param options
   */
  fallback(options?: {
   ...
   * **Details**
   *
   * Note that **headers** option will apply to the fetched request as well as any redirects initiated by it. If you
   * want to only apply **headers** to the original request, but not to redirects, look into **Route.continue()**
   * instead.
   * @param options
   */
  fetch(options?: {
 ...
 * touch events, see the [emulating legacy touch events](https://copilotbrowser.dev/docs/touch-events) page.
 */
...
/**
 * API for collecting and saving copilotbrowser traces. copilotbrowser traces can be opened in
 * [Trace Viewer](https://copilotbrowser.dev/docs/trace-viewer) after copilotbrowser script runs.
 *
 * **NOTE** You probably want to
 * [enable tracing in your config file](https://dayour.github.io/copilotbrowser/docs/api/class-testoptions#test-options-trace)
 ...
 * [enabling tracing through copilotbrowser Test configuration](https://dayour.github.io/copilotbrowser/docs/api/class-testoptions#test-options-trace),
...
export interface Tracing {
   ...
   * **Tracing.groupEnd()** is called. Groups can be nested and will be visible in the trace viewer.
   *
   * **Usage**
   *
   * ```js
  ...
  start(options?: {
     ...
     * **BrowserType.launch.tracesDir** directory specified in **BrowserType.launch()**. To specify the final trace zip
     * file name, you need to pass `path` option to
     *
 ...
 * ```
 *
 * Alternatively, you can use **Video.start()** and **Video.stop()** to record video manually. This approach is
 ...
 * **BrowserContext.event('webError')** event.
 *
 * ```js
 * // Log all uncaught errors to the terminal
...
export interface ConnectOverCDPOptions {
  ...
  /**
   * Tells copilotbrowser that it runs on the same host as the CDP server. It will enable certain optimizations that
   * rely upon the file system being the same between copilotbrowser and the Browser.
   */
  isLocal?: boolean;
...
interface PageWaitForFunctionOptions {
  /**
   * If **polling** is `'raf'`, then **expression** is constantly executed in `requestAnimationFrame` callback. If
   ...
   * `actionTimeout` option in the config, or by using the **BrowserContext.setDefaultTimeout()** or
   * **Page.setDefaultTimeout()** methods.
   */
  timeout?: number;
}
