...
async function innerLoadTrace(traceUri: string, progress: Progress): Promise<LoadedTrace> {
  ...
  try {
    ...
    if (error?.message?.includes('Cannot find .trace file') && await traceLoader.hasEntry('index.html'))
      throw new Error('Could not load trace. Did you upload a copilotbrowser HTML report instead? Make sure to extract the archive first and then double-click the index.html file or put it on a web server.');
    if (error instanceof TraceVersionError)
      throw new Error(`Could not load trace from ${traceUri}. ${error.message}`);

    let message = `Could not load trace from ${traceUri}. Make sure a valid copilotbrowser Trace is accessible over this url.`;
