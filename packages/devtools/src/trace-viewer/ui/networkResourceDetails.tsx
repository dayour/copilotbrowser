...
const CopyDropdown: React.FC<{
  ...
  const copyAscopilotbrowser = async () => getAPIRequestCodeGen(sdkLanguage).generatecopilotbrowserRequestCall(resource.request, requestBody?.text);
  return (
    <div className='copy-request-dropdown'>
      ...
      <div className='copy-request-dropdown-menu'>
        ...
        <CopyToClipboardTextButton description='Copy as copilotbrowser' copiedDescription={copiedDescription} value={copyAscopilotbrowser}/>
      </div>
    </div>
  );
};
