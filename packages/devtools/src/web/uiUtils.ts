...
// Returns Date.now() in a React Compiler-safe way.
// When active is true, returns the current timestamp (updated each render cycle).
// When active is false, returns 0 to keep dependencies stable.
const _nowSubscribe = () => () => {};
const _nowSnapshot = () => Date.now();
const _zeroSnapshot = () => 0;
export function useNow(active: boolean): number {
  return React.useSyncExternalStore(_nowSubscribe, active ? _nowSnapshot : _zeroSnapshot);
}
...
export function useSetting<S>(name: string | undefined, defaultValue: S): [S, React.Dispatch<React.SetStateAction<S>>] {
  const resolvedDefault = name ? settings.getObject(name, defaultValue) : defaultValue;
  const [value, setValue] = React.useState<S>(resolvedDefault);
  ...
  React.useEffect(() => {
    if (name) {
      const onStoreChange = () => setValue(settings.getObject(name, resolvedDefault));
