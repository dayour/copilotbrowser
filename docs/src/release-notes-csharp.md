---
id: release-notes-csharp
title: "Release notes"
toc_max_heading_level: 2
sidebar_label: "Release notes (C#)"
---
...
- Option `Devtools` from **BrowserType.launch()** has been removed. Use `Args = new[] { "--auto-open-devtools-for-tabs" }` instead.
...
- **Worker.event('console')** event is emitted when JavaScript within the worker calls one of console API methods, e.g. console.log or console.dir.
...
- Added support for Xunit v3 as part of [`Microsoft.copilotbrowser.Xunit.v3`](https://www.nuget.org/packages/Microsoft.copilotbrowser.Xunit.v3)
- Added support for MSTest v4 as part of [`Microsoft.copilotbrowser.MSTest.v4`](https://www.nuget.org/packages/Microsoft.copilotbrowser.MSTest.v4)
...
- New option `--user-data-dir` in multiple commands. You can specify the same user data dir to reuse browsing state, like authentication, between sessions.
  ```bash
  pwsh bin/Debug/netX/copilotbrowser.ps1 codegen --user-data-dir=./user-data
  ```

- `pwsh bin/Debug/netX/copilotbrowser.ps1 open` does not open the test recorder anymore. Use `pwsh bin/Debug/netX/copilotbrowser.ps1 codegen` instead.
...
- New method **Locator.describe()** to describe a locator. Used for trace viewer.
  ...
  ```
- `pwsh bin/Debug/netX/copilotbrowser.ps1 install --list` will now list all installed browsers, versions and locations.
...
* New option **BrowserContext.storageState.indexedDB** for **BrowserContext.storageState()** allows to save and restore IndexedDB contents. Useful when your application uses [IndexedDB API](https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB_API) to store authentication tokens, like Firebase Authentication.
  ...
  {
      Path = "../../../copilotbrowser/.auth/state.json",
      IndexedDB = true
  });
  ...
  {
      StorageStatePath = "../../../copilotbrowser/.auth/state.json"
  });
  ```
...
* Support for xUnit 2.8+ via [Microsoft.copilotbrowser.Xunit](https://www.nuget.org/packages/Microsoft.copilotbrowser.Xunit). Follow our [Getting Started](./intro.md) guide to learn more.
...
await Page.Context.Tracing.GroupAsync("Open copilotbrowser.dev > API");
await Page.GotoAsync("https://dayour.github.io/copilotbrowser/");
...
- The `mcr.microsoft.com/copilotbrowser/dotnet:v1.47.0` now serves a copilotbrowser image based on Ubuntu 24.04 Noble.
  To use the 22.04 jammy-based image, please use `mcr.microsoft.com/copilotbrowser/dotnet:v1.47.0-jammy` instead.
...
- New method **FormData.append()** allows to specify repeating fields with the same name in [`Multipart`](./api/class-apirequestcontext#api-request-context-fetch-option-multipart) option in `APIRequestContext.FetchAsync()`:
  ```csharp
...
- **PageAssertions.toHaveURL()** now supports `IgnoreCase` [option](./api/class-pageassertions#page-assertions-to-have-url-option-ignore-case).
...
- copilotbrowser [docker image](./docker.md) now comes with .NET 8 (new LTS).
...
* New `uninstall` CLI command to uninstall browser binaries:
  ```bash
  $ pwsh bin/Debug/netX/copilotbrowser.ps1 uninstall # remove browsers installed by this installation
  $ pwsh bin/Debug/netX/copilotbrowser.ps1 uninstall --all # remove all ever-install copilotbrowser browsers
  ```
...
* The `mcr.microsoft.com/copilotbrowser/dotnet:v1.33.0` now serves a copilotbrowser image based on Ubuntu Jammy.
  To use the focal-based image, please use `mcr.microsoft.com/copilotbrowser/dotnet:v1.33.0-focal` instead.
...
- **APIResponseAssertions.toBeOK()** is now available.
...
A bunch of copilotbrowser APIs already support the `WaitUntil: WaitUntilState.DOMContentLoaded` option.
For example:

```csharp
await Page.GotoAsync("https://copilotbrowser.dev", new() { WaitUntil = WaitUntilState.DOMContentLoaded });
```
...
`Microsoft.copilotbrowser.NUnit` and `Microsoft.copilotbrowser.MSTest` will now consider the `.runsettings` file and passed settings via the CLI when running end-to-end tests. See in the [documentation](./test-runners) for a full list of supported settings.
...
We rewrote our Getting Started docs to be more end-to-end testing focused. Check them out on [copilotbrowser Github.io](./intro).

## Version 1.23

### API Testing

copilotbrowser for .NET 1.23 introduces new [API Testing](./api/class-apirequestcontext) that lets you send requests to the server directly from .NET!
Now you can:
...
```bash
pwsh bin/Debug/netX/copilotbrowser.ps1 open --save-har=example.har --save-har-glob="**/api/**" https://example.com
```
...
* copilotbrowser for .NET now supports **linux-arm64** and provides a **arm64 Ubuntu 20.04 Docker image** for it.
...
- We now ship a designated .NET docker image `mcr.microsoft.com/copilotbrowser/dotnet`. Read more in [our documentation](./docker).
...
copilotbrowser for .NET 1.20 introduces [Web-First Assertions](./test-assertions).
...
- run trace viewer with `pwsh bin/Debug/netX/copilotbrowser.ps1 show-trace` and drop trace files to the trace viewer PWA
...
- copilotbrowser for .NET v1.12 is now stable!
