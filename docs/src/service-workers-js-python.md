...
:::note
If you're looking to do general network mocking, routing, and interception, please see the [Network Guide](./network.md) first. copilotbrowser provides built-in APIs for this use case that don't require the information below. However, if you're interested in requests made by Service Workers themselves, please read below.
:::
...
copilotbrowser allows to disable Service Workers during testing. This makes tests more predictable and performant. However, if your actual page uses a Service Worker, the behavior might be different.
...
You can use **BrowserContext.serviceWorkers()** to list the Service `Worker`s, or specifically watch for the Service `Worker` if you anticipate a page will trigger its [registration](https://developer.mozilla.org/en-US/docs/Web/API/ServiceWorkerContainer/register):
...
**BrowserContext.event('serviceWorker')** event is fired ***before*** the Service Worker has taken control over the page, so ***before*** evaluating in the worker with **Worker.evaluate()** you should wait on its activation.
...
* **BrowserContext.event('request')**, **BrowserContext.event('requestFinished')**, **BrowserContext.event('response')** and **BrowserContext.event('requestFailed')** are fired
...
| -                                 | -                | -                              | -      | -                                      |
| **BrowserContext.event('request')** | `Frame`          | index.html                     | Yes    |                                        |
| **Page.event('request')**           | `Frame`          | index.html                     | Yes    |                                        |
| **BrowserContext.event('request')** | Service `Worker` | transparent-service-worker.js  | Yes    |                                        |
| **BrowserContext.event('request')** | Service `Worker` | data.json                      | Yes    |                                        |
| **BrowserContext.event('request')** | `Frame`          | data.json                      |        | Yes                                    |
| **Page.event('request')**           | `Frame`          | data.json                      |        | Yes                                    |



Since the example Service Worker just acts a basic transparent "proxy":

* There's 2 **BrowserContext.event('request')** events for `data.json`; one `Frame`-owned, the other Service `Worker`-owned.
...
Requests for updated Service Worker main script code currently cannot be routed (https://github.com/dayour/copilotbrowser/issues/14711).
