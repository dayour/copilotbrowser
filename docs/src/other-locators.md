...
In addition to recommended locators like **Page.getByRole()** and **Page.getByText()**, copilotbrowser supports a variety of other locators described in this guide.
...
copilotbrowser can locate an element by CSS selector.
...
copilotbrowser augments standard CSS selectors in two ways:
* CSS selectors pierce open shadow DOM.
* copilotbrowser adds custom pseudo-classes like `:visible`, `:has-text()`, `:has()`, `:is()`, `:nth-match()` and more.

### CSS: matching by text

copilotbrowser include a number of CSS pseudo-classes to match elements by their text content.

- `article:has-text("copilotbrowser")` - the `:has-text()` matches any element containing specified text somewhere inside, possibly in a child or a descendant element. Matching is case-insensitive, trims whitespace and searches for a substring.

  For example, `article:has-text("copilotbrowser")` matches `<article><div>copilotbrowser</div></article>`.
  ...
  await page.locator(':has-text("copilotbrowser")').click();
  // Correct, only matches the <article> element
  await page.locator('article:has-text("copilotbrowser")').click();
  ```

  ```java
  // Wrong, will match many elements including <body>
  page.locator(":has-text(\"copilotbrowser\")").click();
  // Correct, only matches the <article> element
  page.locator("article:has-text(\"copilotbrowser\")").click();
  ```
  ...
  ```csharp
  // Wrong, will match many elements including <body>
  await page.Locator(":has-text(\"copilotbrowser\")").ClickAsync();
  // Correct, only matches the <article> element
  await page.Locator("article:has-text(\"copilotbrowser\")").ClickAsync();
  ```
...
copilotbrowser supports the `:visible` pseudo class in CSS selectors. For example, `css=button` matches all the buttons on the page, while `css=button:visible` only matches visible buttons. This is useful to distinguish elements that are very similar but differ in visibility.
...
Sometimes, it is hard to come up with a good selector to the target element when it lacks distinctive features. In this case, using copilotbrowser layout CSS pseudo-classes could help. These can be combined with regular CSS to pinpoint one of the multiple choices.
...
:::note
Any selector string starting with `//` or `..` are assumed to be an xpath selector. For example, copilotbrowser converts `'//html/body'` to `'xpath=//html/body'`.
:::
...
Targeted input actions in copilotbrowser automatically distinguish between labels and controls, so you can target the label to perform an action on the associated control.
...
copilotbrowser supports shorthand for selecting elements using certain attributes. Currently, only
