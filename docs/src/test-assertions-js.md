---
id: test-assertions-js
title: "Assertions"
sidebar_label: "Assertions (JavaScript)"
---

## Introduction

copilotbrowser includes test assertions in the form of `expect` function. To make an assertion, call `expect(value)` and choose a matcher that reflects the expectation. There are many [generic matchers](./api/class-genericassertions.md) like `toEqual`, `toContain`, `toBeTruthy` that can be used to assert any conditions.

```js
expect(success).toBeTruthy();
```

copilotbrowser also includes web-specific [async matchers](./api/class-locatorassertions.md) that will wait until
...
| **GenericAssertions.toContain()** | String contains a substring |
| **GenericAssertions.toContain()** | Array or set contains an element |
...
By default, failed assertion will terminate test execution. copilotbrowser also
...
Note that soft assertions only work with copilotbrowser test runner.
...
:::note
Do not confuse copilotbrowser's `expect` with the [`expect` library](https://jestjs.io/docs/expect). The latter is not fully integrated with copilotbrowser test runner, so make sure to use copilotbrowser's own `expect`.
:::
