...
copilotbrowser tests can be run on any CI provider. This guide covers one way of running tests on GitHub using GitHub Actions. If you would like to learn more, or how to configure other CI providers, check out our detailed [doc on Continuous Integration](./ci.md).
...
copilotbrowser tests can be run on any CI provider. In this section we cover running tests on GitHub using GitHub Actions. If you would like to see how to configure other CI providers, check out our detailed [doc on Continuous Integration](./ci.md).
...
When [installing copilotbrowser](./intro.md) using the [VS Code extension](./getting-started-vscode.md) or with `npm init copilotbrowser@latest`, you are given the option to add a [GitHub Actions](https://docs.github.com/en/actions) workflow. This creates a `copilotbrowser.yml` file inside a `.github/workflows` folder containing everything you need so that your tests run on each push and pull request into the main/master branch. Here's how that file looks:

```yml js title=".github/workflows/copilotbrowser.yml"
...
1. Install copilotbrowser Browsers
1. Run copilotbrowser tests
...
To add a [GitHub Actions](https://docs.github.com/en/actions) file, first create `.github/workflows` folder and inside it add a `copilotbrowser.yml` file containing the example code below so that your tests run on each push and pull request for the main/master branch.

```yml python title=".github/workflows/copilotbrowser.yml"
name: copilotbrowser Tests
on:
  push:
    branches: [ main, master ]
...
```yml java title=".github/workflows/copilotbrowser.yml"
name: copilotbrowser Tests
on:
  push:
    branches: [ main, master ]
...
```yml csharp title=".github/workflows/copilotbrowser.yml"
name: copilotbrowser Tests
on:
  push:
    branches: [ main, master ]
...
![dotnet repo on github](https://github.com/dayour/copilotbrowser/assets/13063165/4f1b4cc3-b850-4d60-a99e-24057eaf91ad)
...
![opening the workflow](https://github.com/dayour/copilotbrowser/assets/13063165/71793c09-0815-4faa-866b-85684a1f87e5)
...
Clicking on the workflow run shows you all the actions that GitHub performed and clicking on **Run copilotbrowser tests** shows the error messages, what was expected and what was received as well as the call log.
...
![viewing the test logs](https://github.com/dayour/copilotbrowser/assets/13063165/ba2d8d7b-ffce-42de-95e0-bcb35c421975)
...
In the Artifacts section, click on the **copilotbrowser-report** to download your report in the format of a zip file.
...
Locally opening the report does not work as expected as you need a web server for everything to work correctly. First, extract the zip, preferably in a folder that already has copilotbrowser installed. Using the command line, change into the directory where the report is and use `npx copilotbrowser show-report` followed by the name of the extracted folder. This serves up the report and enables you to view it in your browser.


```bash
npx copilotbrowser show-report name-of-my-extracted-copilotbrowser-report
```

![viewing the HTML report](https://github.com/dayour/copilotbrowser/assets/13063165/c5f60e56-fb75-4a2d-a4b6-054b8c5d69c1)

To learn more about reports, check out our detailed guide on [HTML Reporter](/test-reporters.md#html-reporter)

## Viewing the Trace

Once you have served the report using `npx copilotbrowser show-report`, click on the trace icon next to the test's file name as seen in the image above. You can then view the trace of your tests and inspect each action to try to find out why the tests are failing.

![copilotbrowser trace viewer](https://github.com/dayour/copilotbrowser/assets/13063165/10fe3585-8401-4051-b1c2-b2e92ac4c274)

## Viewing the Trace

[trace.copilotbrowser.dev](https://trace.copilotbrowser.dev) is a statically hosted variant of the Trace Viewer. You can upload trace files using drag and drop.

![copilotbrowser trace viewer](./images/getting-started/trace-viewer-failed-test.png)

## Viewing the Trace

You can upload Traces which get created on your CI like GitHub Actions as artifacts. This requires [starting and stopping the trace](./trace-viewer-intro#recording-a-trace). We recommend only recording traces for failing tests. Once your traces have been uploaded to CI, they can then be downloaded and opened using [trace.copilotbrowser.dev](https://trace.copilotbrowser.dev), which is a statically hosted variant of the Trace Viewer. You can upload trace files using drag and drop.
...
1. Add a step that uploads the HTML report to Azure Storage.
    ...
    ```yaml title=".github/workflows/copilotbrowser.yml"
    ...
        - name: Upload HTML report to Azure
          shell: bash
          run: |
            REPORT_DIR='run-${{ github.run_id }}-${{ github.run_attempt }}'
            azcopy cp --recursive "./copilotbrowser-report/*" "https://<STORAGE_ACCOUNT_NAME>.blob.core.windows.net/\$web/$REPORT_DIR"
