...
By default, copilotbrowser runs test files in [parallel](./test-parallel.md) and strives for optimal utilization of CPU cores on your machine. In order to achieve even greater parallelisation, you can further scale copilotbrowser test execution by running tests on multiple machines simultaneously. We call this mode of operation "sharding". Sharding in copilotbrowser means splitting your tests into smaller parts called "shards". Each shard is like a separate job that can run independently. The whole purpose is to divide your tests to speed up test runtime.
...
```bash
npx copilotbrowser test --shard=1/4
npx copilotbrowser test --shard=2/4
npx copilotbrowser test --shard=3/4
npx copilotbrowser test --shard=4/4
```

Now, if you run these shards in parallel on different jobs, your test suite completes four times faster.

Note that copilotbrowser can only shard tests that can be run in parallel. By default, this means copilotbrowser will shard test files. Learn about other options in the [parallelism guide](./test-parallel.md).
...
When `fullyParallel: true` is enabled, copilotbrowser Test runs individual tests in parallel across multiple shards, ensuring each shard receives an even distribution of tests. This allows for test-level granularity, meaning each shard will attempt to balance the number of individual tests it runs. This is the preferred mode for ensuring even load distribution when sharding, as copilotbrowser can optimize shard execution based on the total number of tests.

**Sharding without fullyParallel**

Without the fullyParallel setting, copilotbrowser Test defaults to file-level granularity, meaning entire test files are assigned to shards (note that the same file may be assigned to different shards across different projects). In this case, the number of tests per file can greatly influence shard distribution. If your test files are not evenly sized (i.e., some files contain many more tests than others), certain shards may end up running significantly more tests, while others may run fewer or even none.
...
Blob report contains information about all the tests that were run and their results as well as all test attachments such as traces and screenshot diffs. Blob reports can be merged and converted to any other copilotbrowser report. By default, blob report will be generated into `blob-report` directory. You can learn about [blob report options here](./test-reporters.md#blob-reporter).
...
Afterwards, run `npx copilotbrowser merge-reports` command:

```bash
npx copilotbrowser merge-reports --reporter html ./all-blob-reports
```

This will produce a standard HTML report into `copilotbrowser-report` directory.
...
The following example shows you how to configure a job to run your tests on four machines in parallel and then merge the reports into a single report. Don't forget to add `reporter: process.env.CI ? 'blob' : 'html',` to your `copilotbrowser.config.ts` file as in the example above.
...
1. Then we run our copilotbrowser tests with the `--shard=${{ matrix.shardIndex }}/${{ matrix.shardTotal }}` option. This will run our test command for each shard.
...
jobs:
  copilotbrowser-tests:
    ...
    - name: Run copilotbrowser tests
      run: npx copilotbrowser test --shard=${{ matrix.shardIndex }}/${{ matrix.shardTotal }}
...
...
  merge-reports:
    ...
    - name: Merge into HTML Report
      run: npx copilotbrowser merge-reports --reporter html ./all-blob-reports
...
<img width="875" alt="image" src="https://github.com/dayour/copilotbrowser/assets/9798949/b69dac59-fc19-4b98-8f49-814b1c29ca02" />
...
`npx copilotbrowser merge-reports path/to/blob-reports-dir` reads all blob reports from the passed directory and merges them into a single report.
...
- `--reporter reporter-to-use`
  ...
  ```bash
  npx copilotbrowser merge-reports --reporter=html,github ./blob-reports
  ```

- `--config path/to/config/file`

  Specifies the copilotbrowser configuration file with output reporters. Use this option to pass
  ...
  ```bash
  npx copilotbrowser merge-reports --config=merge.config.ts ./blob-reports
  ```
