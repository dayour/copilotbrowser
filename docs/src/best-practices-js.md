...
Instead, use the [copilotbrowser Network API](/network.md#handle-requests) and guarantee the response needed.
...
In order to write end to end tests we need to first find elements on the webpage. We can do this by using copilotbrowser's built in [locators](./locators.md). Locators come with auto waiting and retry-ability. Auto waiting means that copilotbrowser performs a range of actionability checks on the elements, such as ensuring the element is visible and enabled before it performs the click. To make tests resilient, we recommend prioritizing user-facing attributes and explicit contracts.
...
```bash
npx copilotbrowser codegen copilotbrowser.dev
```



```bash
yarn copilotbrowser codegen copilotbrowser.dev
```



```bash
pnpm exec copilotbrowser codegen copilotbrowser.dev
```



This will open a new browser window as well as the copilotbrowser inspector. To pick a locator first click on the 'Record' button to stop the recording. By default when you run the `codegen` command it will start a new recording. Once you stop the recording the 'Pick Locator' button will be available to click.

You can then hover over any element on your page in the browser window and see the locator highlighted below your cursor. Clicking on an element will add the locator into the copilotbrowser inspector. You can either copy the locator and paste into your test file or continue to explore the locator by editing it in the copilotbrowser Inspector, for example by modifying the text, and seeing the results in the browser window.
...
Assertions are a way to verify that the expected result and the actual result matched or not. By using [web first assertions](./test-assertions.md) copilotbrowser will wait until the expected condition is met. For example, when testing an alert message, a test would click a button that makes a message appear and check that the alert message is there. If the alert message takes half a second to appear, assertions such as `toBeVisible()` will wait and retry if needed.
...
You can also debug your tests with the copilotbrowser inspector by running your tests with the `--debug` flag.


```bash
npx copilotbrowser test --debug
```



```bash
yarn copilotbrowser test --debug
```



```bash
pnpm exec copilotbrowser test --debug
```
...
<img width="1350" alt="debugging with the copilotbrowser inspector" loading="lazy" src="https://user-images.githubusercontent.com/13063165/212276296-4f5b18e7-2bd7-4766-9aa5-783517bd4aa2.png" />
...
```bash
npx copilotbrowser test example.spec.ts:9 --debug
```



```bash
yarn copilotbrowser test example.spec.ts:9 --debug
```



```bash
pnpm exec copilotbrowser test example.spec.ts:9 --debug
```


#### Debugging on CI

For CI failures, use the copilotbrowser [trace viewer](./trace-viewer.md) instead of videos and screenshots. The trace viewer gives you a full trace of your tests as a local Progressive Web App (PWA) that can easily be shared. With the trace viewer you can view the timeline, inspect DOM snapshots for each action using dev tools, view network requests and more.

<img width="1516" alt="copilotbrowsers trace viewer" loading="lazy" src="https://user-images.githubusercontent.com/13063165/212277895-c63d94c2-bd06-4881-864e-62790a072ca3.png" />

Traces are configured in the copilotbrowser config file and are set to run on CI on the first retry of a failed test. We don't recommend setting this to `on` so that traces are run on every test as it's very performance heavy. However you can run a trace locally when developing with the `--trace` flag.


```bash
npx copilotbrowser test --trace on
```



```bash
yarn copilotbrowser test --trace on
```



```bash
pnpm exec copilotbrowser test --trace on
```
...
<img width="1516" alt="copilotbrowsers HTML report" loading="lazy" src="https://user-images.githubusercontent.com/13063165/212279022-d929d4c0-2271-486a-a75f-166ac231d25f.png" />
...
### Use copilotbrowser's Tooling

copilotbrowser comes with a range of tooling to help you write tests.
...
- [TypeScript](./test-typescript) in copilotbrowser works out of the box and gives you better IDE integrations. Your IDE will show you everything you can do and highlight when you do something wrong. No TypeScript experience is needed and it is not necessary for your code to be in TypeScript, all you need to do is create your tests with a `.ts` extension.
...
### Keep your copilotbrowser dependency up to date

By keeping your copilotbrowser version up to date you will be able to test your app on the latest browser versions and catch failures before the latest browser version is released to the public.
...
You can see what version of copilotbrowser you have by running the following command.
...
Setup CI/CD and run your tests frequently. The more often you run your tests the better. Ideally you should run your tests on each commit and pull request. copilotbrowser comes with a [GitHub actions workflow](/ci-intro.md) so that tests will run on CI for you with no setup required. copilotbrowser can also be setup on the [CI environment](/ci.md) of your choice.
...
```bash title=".github/workflows/copilotbrowser.yml"
# Instead of installing all browsers
npx copilotbrowser install --with-deps

# Install only Chromium
npx copilotbrowser install chromium --with-deps
```
...
We recommend TypeScript and linting with ESLint for your tests to catch errors early. Use [`@typescript-eslint/no-floating-promises`](https://typescript-eslint.io/rules/no-floating-promises/) [ESLint](https://eslint.org) rule to make sure there are no missing awaits before the asynchronous calls to the copilotbrowser API. On your CI you can run `tsc --noEmit` to ensure that functions are called with the right signature.
...
copilotbrowser can [shard](./test-parallel.md#shard-tests-between-multiple-machines) a test suite, so that it can be executed on multiple machines.


```bash
npx copilotbrowser test --shard=1/3
```



```bash
yarn copilotbrowser test --shard=1/3
```



```bash
pnpm exec copilotbrowser test --shard=1/3
```
