...
For every attachment downloaded by the page, **Page.event('download')** event is emitted. All these attachments are downloaded into a temporary folder. You can obtain the download url, file name and payload stream using the `Download` object from the event.
