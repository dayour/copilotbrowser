---
id: api-testing-csharp
title: "API testing"
sidebar_label: "API testing (C#)"
---

## Introduction

copilotbrowser can be used to get access to the [REST](https://en.wikipedia.org/wiki/Representational_state_transfer) API of
your application.
...
The following examples rely on the [`Microsoft.copilotbrowser.MSTest`](./test-runners.md) package which creates a copilotbrowser and Page instance for each test.

## Writing API Test

`APIRequestContext` can send all kinds of HTTP(S) requests over network.

The following example demonstrates how to use copilotbrowser to test issues creation via [GitHub API](https://docs.github.com/en/rest). The test suite will do the following:
...
[TestClass]
public class TestGitHubAPI : copilotbrowserTest
{
    ...
    private async Task CreateAPIRequestContext()
    {
        ...
        Request = await this.copilotbrowser.APIRequest.NewContextAsync(new() {
...
public class TestGitHubAPI : copilotbrowserTest
{
    ...
    private async Task CreateAPIRequestContext()
    {
        ...
        Request = await copilotbrowser.APIRequest.NewContextAsync(new()
        {
            // All requests we send go to this API endpoint.
            BaseURL = "https://api.github.com",
            ExtraHTTPHeaders = headers,
...
class TestGitHubAPI : PageTest
{
    ...
    public async Task LastCreatedIssueShouldBeFirstInTheList()
    {
        ...
        // When inheriting from 'copilotbrowserTest' it only gives you a copilotbrowser instance. To get a Page instance, either start
...
copilotbrowser provides **APIRequestContext.storageState()** method that can be used to
...
```csharp
var requestContext = await copilotbrowser.APIRequest.NewContextAsync(new()
{
    HttpCredentials = new()
    {
        Username = "user",
