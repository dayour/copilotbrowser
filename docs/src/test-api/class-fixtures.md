...
copilotbrowser Test is based on the concept of the [test fixtures](../test-fixtures.md). Test fixtures are used to establish environment for each test, giving the test everything it needs and nothing else.
...
Given the test above, copilotbrowser Test will set up the `page` fixture before running the test, and tear it down after the test has finished. `page` fixture provides a `Page` object that is available to the test.

copilotbrowser Test comes with builtin fixtures listed below, and you can add your own fixtures as well. copilotbrowser Test also [provides options]`TestOptions` to  configure **Fixtures.browser**, **Fixtures.context** and **Fixtures.page**.
