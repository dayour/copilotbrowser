...
copilotbrowser provides a powerful command line interface for running tests, generating code, debugging, and more. The most up to date list of commands and arguments available on the CLI can always be retrieved via `npx copilotbrowser --help`.

## Essential Commands

### Run Tests

Run your copilotbrowser tests. [Read more about running tests](./running-tests.md).

#### Syntax

```bash
npx copilotbrowser test [options] [test-filter...]
```
...
npx copilotbrowser test --project=chromium

# Get help
npx copilotbrowser test --help
```

**Disable [parallelization](./test-parallel.md)**

```bash
npx copilotbrowser test --workers=1
```

**Run in debug mode with [copilotbrowser Inspector](./debug.md)**
...
| Option | Description |
| :--- | :--- |
| `--debug` | Run tests with copilotbrowser Inspector. Shortcut for `PWDEBUG=1` environment variable and `--timeout=0 --max-failures=1 --headed --workers=1` options. |
...
| `-c <file>` or `--config <file>` | Configuration file, or a test directory with optional "copilotbrowser.config.&#123;m,c&#125;?&#123;js,ts&#125;". Defaults to `copilotbrowser.config.ts` or `copilotbrowser.config.js` in the current directory. |
| `--debug` | Run tests with copilotbrowser Inspector. Shortcut for `PWDEBUG=1` environment variable and `--timeout=0 --max-failures=1 --headed --workers=1` options. |
...
```bash
npx copilotbrowser show-report [report] [options]
```
...
npx copilotbrowser show-report copilotbrowser-report/

# Show report on custom port
npx copilotbrowser show-report --port 8080
```
...
Install browsers required by copilotbrowser. [Read more about copilotbrowser's browser support](./browsers.md).

#### Syntax

```bash
npx copilotbrowser install [options] [browser...]
npx copilotbrowser install-deps [options] [browser...]
...
npx copilotbrowser install chromium webkit
...
```bash
npx copilotbrowser codegen [options] [url]
```
...
npx copilotbrowser codegen

# Record on specific site
npx copilotbrowser codegen https://copilotbrowser.dev

# Generate Python code
npx copilotbrowser codegen --target=python
```
...
| `--target <language>` | Language to use: javascript, copilotbrowser-test, python, etc. |
...
```bash
npx copilotbrowser show-trace [options] [trace]
```
...
npx copilotbrowser show-trace

# View a trace file
npx copilotbrowser show-trace trace.zip

# View trace from directory
npx copilotbrowser show-trace trace/
```
...
```bash
npx copilotbrowser merge-reports [options] <blob dir>
```
...
npx copilotbrowser merge-reports ./reports
```
...
Clear all copilotbrowser caches.

#### Syntax

```bash
npx copilotbrowser clear-cache
```
