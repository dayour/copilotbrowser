...
- Protractor’s `ElementFinder` ⇄ [copilotbrowser Test Locator](./api/class-locator)
- Protractor’s [`waitForAngular`] ⇄ copilotbrowser Test [auto-waiting](./actionability.md)
- Don’t forget to await in copilotbrowser Test

## Cheat Sheet

| Protractor                                        | copilotbrowser Test                         |
|---------------------------------------------------|-----------------------------------------|
...
1. Almost all copilotbrowser calls are prefixed with `await`
1. Locator creation with **Page.locator()** is one of the few methods that is sync.

## Polyfilling `waitForAngular`

copilotbrowser Test has built-in [auto-waiting](./actionability.md) that makes protractor's [`waitForAngular`] unneeded in general case.

However, it might come handy in some edge cases.
Here's how to polyfill `waitForAngular` function in copilotbrowser Test:
