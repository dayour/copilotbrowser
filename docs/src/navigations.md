...
copilotbrowser can navigate to URLs and handle navigations caused by the page interactions.
...
In copilotbrowser you can interact with the page at any moment. It will automatically
...
For the scenario above, copilotbrowser will wait for the text to become visible,
will wait for the rest of the actionability checks to pass for that element,
and will click it.

copilotbrowser operates as a very fast user - the moment it sees the button, it
clicks it. In the general case, you don't need to worry about whether all the
resources loaded, etc. 

## Hydration

At some point in time, you'll stumble upon a use case where copilotbrowser performs
...
copilotbrowser will start interacting with the page the moment it sees it. And if
the button on a page is enabled, but the listeners have not yet been added,
copilotbrowser will do its job, but the click won't have any effect.
...
copilotbrowser splits the process of showing a new document in a page into **navigation** and **loading**.
...
- **Page.event('DOMContentLoaded')** event is fired
- page executes some scripts and loads resources like stylesheets and images
- **Page.event('load')** event is fired
