...
sidebar_label: "Release notes (Java)"
---
...
- **Worker.event('console')** event is emitted when JavaScript within the worker calls one of console API methods, e.g. console.log or console.dir. **Worker.waitForConsoleMessage()** can be used to wait for it.
...
- New option `--user-data-dir` in multiple commands. You can specify the same user data dir to reuse browsing state, like authentication, between sessions.
  ```bash
  mvn exec:java -e -D exec.mainClass=com.microsoft.copilotbrowser.CLI -D exec.args="codegen --user-data-dir=./user-data"
  ```

- `mvn exec:java -e -D exec.mainClass=com.microsoft.copilotbrowser.CLI -D exec.args=open` command does not open the test recorder anymore. Use `mvn exec:java -e -D exec.mainClass=com.microsoft.copilotbrowser.CLI -D exec.args=codegen` instead.
...
- New method **Locator.describe()** to describe a locator. Used for trace viewer.
  ...
  button.click();
  ```
- `mvn exec:java -e -D exec.mainClass=com.microsoft.copilotbrowser.CLI -D exec.args="install --list"` will now list all installed browsers, versions and locations.
...
- The `mcr.microsoft.com/copilotbrowser/java:v1.47.0` now serves a copilotbrowser image based on Ubuntu 24.04 Noble.
  To use the 22.02 jammy-based image, please use `mcr.microsoft.com/copilotbrowser/java:v1.47.0-jammy` instead.
...
- TLS client certificates can now be passed from memory by passing **Browser.newContext.clientCertificates.cert** and **Browser.newContext.clientCertificates.key** as byte arrays instead of file paths.
...
- After executing the handler added with **Page.addLocatorHandler()**, copilotbrowser will now wait until the overlay that triggered the handler is not visible anymore. You can opt-out of this behavior with the new `setNoWaitAfter` option.
...
- New method **FormData.append()** allows to specify repeating fields with the same name in [`setMultipart`](./api/class-requestoptions#request-options-set-multipart) option in `RequestOptions`:
  ```java
...
Add new [`@Usecopilotbrowser`](./junit.md) annotation to your test classes to start using copilotbrowser
fixtures for `Page`, `BrowserContext`, `Browser`, `APIRequestContext` and `copilotbrowser` in the
test methods.
...
public class TestExample {
  void shouldNavigateToInstallationGuide(Page page) {
    page.navigate("https://dayour.github.io/copilotbrowser/java/");
...
- New methods **BrowserContext.newCDPSession()** and **Browser.newBrowserCDPSession()** create a [Chrome DevTools Protocol](https://chromedevtools.github.io/devtools-protocol/) session for the page and browser respectively.
...
* New `uninstall` CLI command to uninstall browser binaries:
  ```bash
  $ mvn exec:java -e -D exec.mainClass=com.microsoft.copilotbrowser.CLI -D exec.args="uninstall" # remove browsers installed by this installation
  $ mvn exec:java -e -D exec.mainClass=com.microsoft.copilotbrowser.CLI -D exec.args="uninstall --all" # remove all ever-install copilotbrowser browsers
  ```
...
- Native support for Apple Silicon - copilotbrowser now runs without Rosetta
- Added Ubuntu 22.04 (Jammy) Docker image

### ⚠️ Breaking change

* The `mcr.microsoft.com/copilotbrowser/java:v1.33.0` now serves a copilotbrowser image based on Ubuntu Jammy.
  To use the focal-based image, please use `mcr.microsoft.com/copilotbrowser/java:v1.33.0-focal` instead.
...
A bunch of copilotbrowser APIs already support the `setWaitUntil(WaitUntilState.DOMCONTENTLOADED)` option.
For example:

```java
page.navigate("https://copilotbrowser.dev", new Page.NavigateOptions().setWaitUntil(WaitUntilState.DOMCONTENTLOADED));
```
...
- Default assertions timeout now can be changed with [`setDefaultAssertionTimeout`](./api/class-copilotbrowserassertions#copilotbrowser-assertions-set-default-assertion-timeout).
...
```bash
mvn exec:java -e -D exec.mainClass=com.microsoft.copilotbrowser.CLI -D exec.args="open --save-har=example.har --save-har-glob='**/api/**' https://example.com"
```
...
- copilotbrowser for Java now supports **Ubuntu 20.04 ARM64** and **Apple M1**.
  You can now run copilotbrowser for Java tests on Apple M1, inside Docker on Apple M1, and on Raspberry Pi.
...
copilotbrowser for Java 1.18 introduces new [API Testing](./api/class-apirequestcontext) that lets you send requests to the server directly from Java!
Now you can:
...
copilotbrowser for Java 1.18 introduces [Web-First Assertions](./test-assertions).
...
- run trace viewer with `mvn exec:java -e -D exec.mainClass=com.microsoft.copilotbrowser.CLI -D exec.args="show-trace"` and drop trace files to the trace viewer PWA
...
```sh
mvn exec:java -e -D exec.mainClass=com.microsoft.copilotbrowser.CLI -D exec.args="show-trace trace.zip"
```
...
- copilotbrowser now includes command line interface, former copilotbrowser-cli.
  ```bash java
  mvn exec:java -e -D exec.mainClass=com.microsoft.copilotbrowser.CLI -D exec.args="--help"
  ```
