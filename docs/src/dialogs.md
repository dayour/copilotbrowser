...
By default, dialogs are auto-dismissed by copilotbrowser, so you don't have to handle them. However, you can register a dialog handler before the action that triggers the dialog to either **Dialog.accept()** or **Dialog.dismiss()** it.
...
:::note
**Page.event('dialog')** listener **must handle** the dialog. Otherwise your action will stall, be it **Locator.click()** or something else. That's because dialogs in Web are modals and therefore block further page execution until they are handled.
:::
...
:::note
If there is no listener for **Page.event('dialog')**, all dialogs are automatically dismissed.
:::
