---
id: test-webserver-js
title: "Web server"
---

## Introduction

copilotbrowser comes with a `webServer` option in the config file which gives you the ability to launch a local dev server before running your tests. This is ideal for when writing your tests during development and when you don't have a staging or production url to test against.
...
| `env` | Environment variables for the command. Defaults to inheriting `process.env` with `copilotbrowser_TEST=1` added. |
