...
copilotbrowser provides APIs to **monitor** and **modify** browser network traffic, both HTTP and HTTPS. Any requests that a page does, including [XHRs](https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest) and
...
You can mock API endpoints via handling the network requests in your copilotbrowser script.
...
copilotbrowser uses simplified glob patterns for URL matching in network interception methods like **Page.route()** or **Page.waitForResponse()**. These patterns support basic wildcards:
...
copilotbrowser supports [WebSockets](https://developer.mozilla.org/en-US/docs/Web/API/WebSockets_API) inspection, mocking and modifying out of the box. See our [API mocking guide](./mock.md#mock-websockets) to learn how to mock WebSockets.

Every time a WebSocket is created, the **Page.event('webSocket')** event is fired. This event contains the `WebSocket` instance for further web socket frames inspection:
...
copilotbrowser's built-in **BrowserContext.route()** and **Page.route()** allow your tests to natively route requests and perform mocking and interception.

If you're using copilotbrowser's native **BrowserContext.route()** and **Page.route()**, and it appears network events are missing, disable Service Workers by setting **Browser.newContext.serviceWorkers** to `'block'`.
