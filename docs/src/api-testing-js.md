...
id: api-testing-js
title: "API testing"
sidebar_label: "API testing (JavaScript)"
---
...
copilotbrowser Test comes with the built-in `request` fixture that respects configuration options like `baseURL` or `extraHTTPHeaders` we specified and is ready to send some requests.
...
test.beforeAll(async ({ copilotbrowser }) => {
  apiContext = await copilotbrowser.request.newContext({
    // All requests we send go to this API endpoint.
    baseURL: 'https://api.github.com',
    extraHTTPHeaders: {
      // We set this header per GitHub guidelines.
