...
With copilotbrowser's Snapshot testing you can assert the accessibility tree of a page against a predefined snapshot template.

```js
await page.goto('https://dayour.github.io/copilotbrowser/');
await expect(page.getByRole('banner')).toMatchAriaSnapshot(`
  - banner:
    - heading /copilotbrowser enables reliable end-to-end/ [level=1]
    - link "Get started":
      - /url: /docs/intro
...
await Expect(page.Locator("banner")).ToMatchAriaSnapshotAsync(@"
  - banner:
    - heading ""copilotbrowser enables reliable end-to-end testing for modern web apps."" [level=1]
    - link ""Get started"":
      - /url: /docs/intro
    - link ""Star microsoft/copilotbrowser on GitHub"":
      - /url: https://github.com/dayour/copilotbrowser
...
### Assertion testing
Assertion testing is a targeted approach where you assert specific values or conditions about elements or components. For instance, with copilotbrowser, **LocatorAssertions.toHaveText()**
...
In copilotbrowser, aria snapshots provide a YAML representation of the accessibility tree of a page.
...
The **LocatorAssertions.toMatchAriaSnapshot()** assertion method in copilotbrowser compares the accessible
...
Creating aria snapshots in copilotbrowser helps ensure and maintain your application's structure.
You can generate snapshots in various ways depending on your testing setup and workflow.

### Generating snapshots with the copilotbrowser code generator

If you're using copilotbrowser's [Code Generator](./codegen.md), generating aria snapshots is streamlined with its
...
Note that copilotbrowser will wait for the maximum expect timeout specified in the test runner configuration to ensure the
...
When updating snapshots, copilotbrowser creates patch files that capture differences. These patch files can be reviewed,
...
```bash
npx copilotbrowser test --update-snapshots --update-source-method=3way
```
...
By default, snapshots from a test file `example.spec.ts` are placed in the `example.spec.ts-snapshots` directory. As snapshots should be the same across browsers, only one snapshot is saved even if testing with multiple browsers. Should you wish, you can customize the [snapshot path template](./api/class-testconfig.md) using the following configuration:
...
The **Locator.ariaSnapshot()** method allows you to programmatically create a YAML representation of accessible
