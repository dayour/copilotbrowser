...
copilotbrowser supports custom selector engines, registered with **Selectors.register()**.
...
export const test = base.extend<{}, { selectorRegistration: void }>({
  ...
  selectorRegistration: [async ({ copilotbrowser }, use) => {
...
await copilotbrowser.Selectors.Register("tag", new() {
  Script = @"
  // Must evaluate to a selector engine instance.
  {
    // Returns the first element matching given selector in the root's subtree.
