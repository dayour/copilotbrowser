...
Web APIs are usually implemented as HTTP endpoints. copilotbrowser provides APIs to **mock** and **modify** network traffic, both HTTP and HTTPS. Any requests that a page does, including [XHRs](https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest) and
[fetch](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) requests, can be tracked, modified and mocked. With copilotbrowser you can also mock using HAR files that contain multiple network requests made by the page.
...
test("mocks a fruit and doesn't call api", async ({ page }) => {
  ...
  // Go to the page
  await page.goto('https://demo.copilotbrowser.dev/api-mocking');
...
def test_mock_the_fruit_api(page: Page):
    ...
    # Go to the page
    page.goto("https://demo.copilotbrowser.dev/api-mocking")
...
// Go to the page
await page.GotoAsync("https://demo.copilotbrowser.dev/api-mocking");
...
// Go to the page
page.navigate("https://demo.copilotbrowser.dev/api-mocking");
...
![api mocking trace](https://github.com/dayour/copilotbrowser/assets/13063165/3dc14cbf-c100-4efc-ac21-d7b52d698b53)
...
async def test_gets_the_json_from_api_and_adds_a_new_fruit(page: Page):
    ...
    await page.route("https://demo.copilotbrowser.dev/api-mocking/api/v1/fruits", handle)
...
def test_gets_the_json_from_api_and_adds_a_new_fruit(page: Page):
    ...
    page.route("https://demo.copilotbrowser.dev/api-mocking/api/v1/fruits", handle)
...
![trace of test showing api being called and fulfilled](https://github.com/dayour/copilotbrowser/assets/13063165/8b8dd82d-1b3e-428e-871b-840581fed439)

By inspecting the response we can see that our new fruit was added to the list.
![trace of test showing the mock response](https://github.com/dayour/copilotbrowser/assets/13063165/03e6c87c-4ecc-47e8-9ca0-30fface25e9d)
...
test('gets the json from HAR and checks the new fruit has been added', async ({ page }) => {
  ...
  // Assert that the copilotbrowser fruit is visible
  await expect(page.getByText('copilotbrowser', { exact: true })).toBeVisible();
});
```
...
async def test_gets_the_json_from_har_and_checks_the_new_fruit_has_been_added(page: Page):
    ...
    # Assert that the copilotbrowser fruit is visible
    await expect(page.get_by_text("copilotbrowser", exact=True)).to_be_visible()
```
...
def test_gets_the_json_from_har_and_checks_the_new_fruit_has_been_added(page: Page):
    ...
    # Assert that the copilotbrowser fruit is visible
    expect(page.get_by_text("copilotbrowser", exact=True)).to_be_visible()
```
...
// Assert that the copilotbrowser fruit is visible
await page.ExpectByTextAsync("copilotbrowser", new() { Exact = true }).ToBeVisibleAsync();
```
...
// Assert that the copilotbrowser fruit is visible
assertThat(page.getByText("copilotbrowser", new Page.GetByTextOptions()
...
![trace showing the HAR file being used](https://github.com/dayour/copilotbrowser/assets/13063165/1bd7ab66-ea4f-43c2-a4e5-ca17d4837ff1)
...
![trace showing response from HAR file](https://github.com/dayour/copilotbrowser/assets/13063165/db3117fc-7b02-4973-9a51-29e213261a6a)
...
```bash python
# Save API requests from example.com as "example.har" archive.
copilotbrowser open --save-har=example.har --save-har-glob="**/api/**" https://example.com
```
