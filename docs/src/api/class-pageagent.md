...
Expect timeout in milliseconds. Defaults to `5000`. The default value can be changed via `expect.timeout` option in the config, or by specifying the `expect` property of the **Page.agent.expect** option. Pass `0` to disable timeout.
...
Extract timeout in milliseconds. Defaults to `5000`. The default value can be changed via `actionTimeout` option in the config, or by using the **BrowserContext.setDefaultTimeout()** or
**Page.setDefaultTimeout()** methods. Pass `0` to disable timeout.
...
Perform timeout in milliseconds. Defaults to `5000`. The default value can be changed via `actionTimeout` option in the config, or by using the **BrowserContext.setDefaultTimeout()** or
**Page.setDefaultTimeout()** methods. Pass `0` to disable timeout.
