...
are fired unless **Clock.runFor()**, **Clock.fastForward()**, **Clock.pauseAt()** or **Clock.resume()** is called.
...
Use this method for simple scenarios where you only need to test with a predefined time. For more advanced scenarios, use **Clock.install()** instead. Read docs on [clock emulation](../clock.md) to learn more.
