...
API for collecting and saving copilotbrowser traces. copilotbrowser traces can be opened in [Trace Viewer](../trace-viewer.md) after copilotbrowser script runs.

:::note
You probably want to [enable tracing in your config file](https://dayour.github.io/copilotbrowser/docs/api/class-testoptions#test-options-trace) instead of using `context.tracing`.

The `context.tracing` API captures browser operations and network activity, but it doesn't record test assertions (like `expect` calls). We recommend [enabling tracing through copilotbrowser Test configuration](https://dayour.github.io/copilotbrowser/docs/api/class-testoptions#test-options-trace), which includes those assertions and provides a more complete trace for debugging test failures.
:::
...
expect(page.url()).toBe('https://copilotbrowser.dev');
...
:::note
You probably want to [enable tracing in your config file](https://dayour.github.io/copilotbrowser/docs/api/class-testoptions#test-options-trace) instead of using `Tracing.start`.
...
given name prefix inside the **BrowserType.launch.tracesDir** directory specified in **BrowserType.launch()**.
...
must be provided via `copilotbrowser_JAVA_SRC` environment variable (the paths should be separated by ';' on Windows
...
# All actions between group and group_end
# will be shown in the trace viewer as a group.
await page.context.tracing.group("Open copilotbrowser.dev > API")
