...
public class Example {
  public static void main(String[] args) {
    try (copilotbrowser copilotbrowser = copilotbrowser.create()) {
      BrowserType webkit = copilotbrowser.webkit();
...
copilotbrowser has ability to mock clock and passage of time.
...
try {
  ...
  // Or while waiting for an event.
  page.waitForPopup(() -> {});
} catch (copilotbrowserException e) {
  // When the page crashes, exception message contains "crash".
}
```
...
:::note
When no **Page.event('dialog')** or **BrowserContext.event('dialog')** listeners are present, all dialogs are automatically dismissed.
:::
...
Emitted when a file chooser is supposed to appear, such as after clicking the  `<input type=file>`. copilotbrowser can
...
**BrowserContext.event('page')**, but only for popups relevant to this page.

The earliest moment that page is available is when it has navigated to the initial url. For example, when opening a
popup with `window.open('http://example.com')`, this event will fire when the network request to "http://example.com" is
done and its response has started loading in the popup. If you would like to route/listen to this network request, use **BrowserContext.route()** and **BrowserContext.event('request')** respectively instead of similar methods on the `Page`.
...
:::note
Use **Page.waitForLoadState()** to wait until the page gets to a particular state (you should not need it in most
cases).
:::
...
with **Page.event('requestFinished')** event and not with **Page.event('requestFailed')**. A request will only be considered
...
```js
// In your copilotbrowser script, assuming the preload.js file is in same directory
...
manually via **Page.event('dialog')** event.
:::
...
**Page.evaluate()** resolves to `undefined`. copilotbrowser also supports transferring some
...
class PageExamples
{
    ...
    {
        using var copilotbrowser = await copilotbrowser.CreateAsync();
        await using var browser = await copilotbrowser.Webkit.LaunchAsync(new()
        {
            Headless = false,
        });
        var page = await browser.NewPageAsync();
...
Callback function that will be called in the copilotbrowser's context.

### option: Page.exposeBinding.handle
> *Added in: v1.8*
>
> ⚠️ **Deprecated.** This option will be removed in the future.
- `handle` <`boolean`>
...
(async () => {
  ...
  await page.setContent(`
    <script>
      async function onClick() {
        document.querySelector('div').textContent = await window.sha256('copilotbrowser');
      }
    </script>
    <button onclick="onClick()">Click me</button>
    <div></div>
...
Callback function which will be called in copilotbrowser's context.
...
When a **Browser.newContext.baseURL** via the context options was provided and the passed URL is a path,
...
> *Added in: v1.8*
>
> ⚠️ **Deprecated.** This option is ignored. **Page.isHidden()** does not wait for the
...
> *Added in: v1.8*
>
> ⚠️ **Deprecated.** This option is ignored. **Page.isVisible()** does not wait
...
Returns up to (currently) 200 last console messages from this page. See **Page.event('console')** for more details.
...
Returns up to (currently) 200 last page errors from this page. See **Page.event('pageError')** for more details.
...
Receives the `Dialog` object, it **must** either **Dialog.accept()** or **Dialog.dismiss()** the dialog - otherwise
...
Pauses script execution. copilotbrowser will stop executing the script and wait for the user to either press the 'Resume'
button in the page overlay or to call `copilotbrowser.resume()` in the DevTools console.

User can inspect selectors or perform manual steps while paused. Resume will continue running the original script from
the place it was paused.

:::note
This method requires copilotbrowser to be started in a headed mode, with a falsy **BrowserType.launch.headless** option.
:::
...
The file path to save the PDF to. If **path** is a relative path, then it is resolved relative to the current
...
**height** or **format** options. Defaults to `false`, which will scale the content to fit the paper
size.
...
Returns up to (currently) 100 last network request from this page. See **Page.event('request')** for more details.
...
Note that requests reported through the **Page.event('request')** request are not collected, so there is a trade off between efficient memory usage with **Page.requests()** and the amount of available information reported through **Page.event('request')**.
...
* copilotbrowser checks for the overlay every time before executing or retrying an action that requires an [actionability check](../actionability.md), or before performing an auto-waiting assertion check. When overlay is visible, copilotbrowser calls the handler first, and then proceeds with the action/assertion. Note that the handler is only called when you perform an action/assertion - if the overlay becomes visible but you don't perform any actions, the handler will not be triggered.
* After executing the handler, copilotbrowser will ensure that overlay that triggered the handler is not visible anymore. You can opt-out of this behavior with **noWaitAfter**.
...
By default, after calling the handler copilotbrowser will wait until the overlay becomes hidden, and only then copilotbrowser will continue with the action/assertion that triggered the handler. This option allows to opt-out of this behavior, so that overlay can stay visible after the handler has run.
...
await page.goto('https://copilotbrowser.dev', { waitUntil: 'domcontentloaded' });
...
**BrowserContext.request** on the page's context. See **BrowserContext.request** for more details.
...
:::note
**Page.route()** will not intercept requests intercepted by Service Worker. See [this](https://github.com/dayour/copilotbrowser/issues/1090) issue. We recommend disabling Service Workers when using request interception by setting **Browser.newContext.serviceWorkers** to `'block'`.
:::
...
A glob pattern, regex pattern, URL pattern, or predicate that receives a `URL` to match during routing. If **Browser.newContext.baseURL** is set in the context options and the provided URL is a string that does not start with `*`, it is resolved using the [`new URL()`](https://developer.mozilla.org/en-US/docs/Web/API/URL/URL) constructor.
...
A glob pattern, regex pattern, or predicate that receives a `URL` to match during routing. If **Browser.newContext.baseURL** is set in the context options and the provided URL is a string that does not start with `*`, it is resolved using the [`new URL()`](https://developer.mozilla.org/en-US/docs/Web/API/URL/URL) constructor.
...
copilotbrowser will not serve requests intercepted by Service Worker from the HAR file. See [this](https://github.com/dayour/copilotbrowser/issues/1090) issue. We recommend disabling Service Workers when using request interception by setting **Browser.newContext.serviceWorkers** to `'block'`.
...
Only WebSockets with the url matching this pattern will be routed. A string pattern can be relative to the **Browser.newContext.baseURL** context option.
...
viewport size before navigating to the page. **Page.setViewportSize()** will also reset `screen` size, use **Browser.newContext()** with `screen` and `viewport` parameters if you need better control of these properties.
...
Removes a route created with **Page.route()**. When **handler** is not specified, removes all routes for
the **url**.
...
Video object associated with this page. Can be used to control video recording with **Video.start()**
and **Video.stop()**, or to access the video file when using the `recordVideo` context option.
...
Will throw an error if the page is closed before the **Page.event('console')** event is fired.
...
> *Added in: v1.8*
>
> ⚠️ **Deprecated.** This method is inherently racy, please use **Page.waitForURL()** instead.
>
> **Languages:** *(all)*
**Returns:** `null|Response`
...
Will throw an error if the page is closed before the **Page.event('requestFinished')** event is fired.
...
The method will block until the condition returns true. All copilotbrowser events will
...
:::note
In most cases, you should use **Page.waitForEvent()**.
:::
