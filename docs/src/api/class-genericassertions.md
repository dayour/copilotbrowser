...
Compares floating point numbers for approximate equality. Use this method instead of **GenericAssertions.toBe()** when comparing floating point numbers.
...
For objects, this method recursively checks equality of all fields, rather than comparing objects by reference as performed by **GenericAssertions.toContain()**.

For primitive values, this method is equivalent to **GenericAssertions.toContain()**.
...
For objects, this method recursively checks equality of all fields, rather than comparing objects by reference as performed by **GenericAssertions.toBe()**.

For primitive values, this method is equivalent to **GenericAssertions.toBe()**.
...
**GenericAssertions.toEqual()** performs deep equality check that compares contents of the received and expected values. To ensure two objects reference the same instance, use **GenericAssertions.toBe()** instead.

**GenericAssertions.toEqual()** ignores `undefined` properties and array items, and does not insist on object types being equal. For stricter matching, use **GenericAssertions.toStrictEqual()**.

**Pattern matching**

**GenericAssertions.toEqual()** can be also used to perform pattern matching on objects, arrays and primitive types, with the help of the following matchers:
...
Ensures that property at provided `keyPath` exists on the object and optionally checks that property is equal to the **expected**. Equality is checked recursively, similarly to **GenericAssertions.toEqual()**.
...
Compares contents of the value with contents of **expected**, performing "deep equality" check. Allows extra properties to be present in the value, unlike **GenericAssertions.toEqual()**, so you can check just a subset of object properties.
...
Differences from **GenericAssertions.toEqual()**:
...
An alias for **GenericAssertions.toThrow()**.
...
`expect.any()` matches any object instance created from the **constructor** or a corresponding primitive type. Use it inside **GenericAssertions.toEqual()** to perform pattern matching.
...
`expect.anything()` matches everything except `null` and `undefined`. Use it inside **GenericAssertions.toEqual()** to perform pattern matching.
...
Use this method inside **GenericAssertions.toEqual()** to perform pattern matching.
...
`expect.arrayOf()` matches array of objects created from the **constructor** or a corresponding primitive type. Use it inside **GenericAssertions.toEqual()** to perform pattern matching.
...
Compares floating point numbers for approximate equality. Use this method inside **GenericAssertions.toEqual()** to perform pattern matching. When just comparing two numbers, prefer **GenericAssertions.toBeCloseTo()**.
...
Use this method inside **GenericAssertions.toEqual()** to perform pattern matching. Object properties can be matchers to further relax the expectation. See examples.
...
`expect.stringContaining()` matches a string that contains the expected substring. Use this method inside **GenericAssertions.toEqual()** to perform pattern matching.
...
`expect.stringMatching()` matches a received string that in turn matches the expected pattern. Use this method inside **GenericAssertions.toEqual()** to perform pattern matching.
