...
copilotbrowser allows creating isolated non-persistent browser contexts with **Browser.newContext()** method. Non-persistent browser
...
var browser = await copilotbrowser.Firefox.LaunchAsync(new() { Headless = false });
// Create a new incognito browser context
...
> *Added in: v1.11*
>
> ⚠️ **Deprecated.** Background pages have been removed from Chromium together with Manifest V2 extensions.
- argument: <`Page`>
...
also fire for popup pages. See also **Page.event('popup')** to receive events about popups relevant to a specific page.
...
context. To listen for errors from a particular page, use **Page.event('pageError')** instead.
...
failed requests from a particular page, use **Page.event('requestFailed')**.

:::note
HTTP Error responses, such as 404 or 503, are still successful responses from HTTP standpoint, so request will complete
with **BrowserContext.event('requestFinished')** event and not with **BrowserContext.event('requestFailed')**.
:::
...
successful requests from a particular page, use **Page.event('requestFinished')**.
...
from a particular page, use **Page.event('response')**.
...
using var copilotbrowser = await copilotbrowser.CreateAsync();
var browser = await copilotbrowser.Webkit.LaunchAsync(new() { Headless = false });
var context = await browser.NewContextAsync();
...
:::note
**BrowserContext.route()** will not intercept requests intercepted by Service Worker. See [this](https://github.com/dayour/copilotbrowser/issues/1090) issue. We recommend disabling Service Workers when using request interception by setting **Browser.newContext.serviceWorkers** to `'block'`.
:::
...
:::note
**Page.setDefaultNavigationTimeout()** and **Page.setDefaultTimeout()** take priority over
...
:::note
**Page.setDefaultNavigationTimeout()**, **Page.setDefaultTimeout()** and
...
:::note
Consider using **BrowserContext.grantPermissions()** to grant permissions for the browser context pages to read
its geolocation.
:::
...
> **Languages:** JavaScript
>
> ⚠️ **Deprecated.** Browsers may cache credentials after successful authentication. Create a new browser context instead.
...
Removes a route created with **BrowserContext.route()**. When **handler** is not specified, removes all
routes for the **url**.
...
Will throw an error if the page is closed before the **BrowserContext.event('console')** event is fired.
...
- `optionsOrPredicate` ?<`function`|`Object`>
  ...
  - `timeout` ?<`float`> Maximum time to wait for in milliseconds. Defaults to `0` - no timeout. The default value can be changed via `actionTimeout` option in the config, or by using the **BrowserContext.setDefaultTimeout()** method.
...
:::note
In most cases, you should use **BrowserContext.waitForEvent()**.
:::
