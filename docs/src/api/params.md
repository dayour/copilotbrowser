...
## input-no-wait-after
> ⚠️ **Deprecated.** This option will default to `true` in the future.
- `noWaitAfter` <`boolean`>
...
Populates context with given storage state. This option can be used to initialize context with logged-in information
obtained via **BrowserContext.storageState()**. Path to the file with saved storage state.
...
- `recordHar` <`Object`>
  ...
  - `urlFilter` ?<`string`|`RegExp`> A glob or regex pattern to filter requests that are stored in the HAR. When a **Browser.newContext.baseURL** via the context options was provided and the passed URL is a path, it gets merged via the [`new URL()`](https://developer.mozilla.org/en-US/docs/Web/API/URL/URL) constructor. Defaults to none.
...
call **BrowserContext.close()** for the HAR to be saved.
...
not recorded. Make sure to call **BrowserContext.close()** for videos to be saved.
...
* `'block'`: copilotbrowser will block all registration of Service Workers.
...
Path to a browser executable to run instead of the bundled one. If **executablePath** is a relative path, then
it is resolved relative to the current working directory. Note that copilotbrowser only works with the bundled Chromium,
...
You can also provide a path to a custom [`policies.json` file](https://mozilla.github.io/policy-templates/) via `copilotbrowser_FIREFOX_POLICIES_JSON` environment variable.
...
* `{snapshotDir}` - Project's **TestProject.snapshotDir**.
  * Value: `/home/copilotbrowser/tests` (since `snapshotDir` is not provided in config, it defaults to `testDir`)
* `{testDir}` - Project's **TestProject.testDir**.
  * Value: `/home/copilotbrowser/tests` (absolute path since `testDir` is resolved relative to directory with config)
