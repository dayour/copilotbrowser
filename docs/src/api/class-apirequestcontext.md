...
Each copilotbrowser browser context has associated with it `APIRequestContext` instance which shares cookie storage with
the browser context and can be accessed via **BrowserContext.request** or **Page.request**.
...
async def run(copilotbrowser: copilotbrowser):
    ...
    # api_request_context = await copilotbrowser.request.new_context(base_url="https://api.github.com")
