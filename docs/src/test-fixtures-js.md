...
The `{ page }` argument tells copilotbrowser Test to set up the `page` fixture and provide it to your test function.
...
`TodoPage` is a class that helps us interact with a "todo list" page of the web app, following the [Page Object Model](./pom.md) pattern. It uses copilotbrowser's `page` internally.
...
export class TodoPage {
  ...
  async goto() {
    await this.page.goto('https://demo.copilotbrowser.dev/todomvc/');
  }
...
- Fixtures are **reusable** between test files - you can define them once and use them in all your tests. That's how copilotbrowser's built-in `page` fixture works. So if you have a helper function that is used in multiple tests, consider turning it into a fixture.
- Fixtures are **on-demand** - you can define as many fixtures as you'd like, and copilotbrowser Test will setup only the ones needed by your test and nothing else.
...
test.use({ baseURL: 'https://copilotbrowser.dev' });
```
...
export const test = base.extend<{}, { account: Account }>({
  account: [async ({ browser }, use, workerInfo) => {
    ...
    // Create the account with copilotbrowser.
...
copilotbrowser Test supports running multiple test projects that can be configured separately. You can use "option" fixtures to make your configuration options declarative and type safe. Learn more about [parameterizing tests](./test-parameterize.md).
