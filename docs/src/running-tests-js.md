...
You can run your tests with the `copilotbrowser test` command. This runs your tests on all browsers as configured in the `copilotbrowser.config` file, and results appear in the terminal. Tests run in headless mode by default, meaning no browser window opens while running the tests.
...
To run your tests in headed mode, use the `--headed` flag. This gives you the ability to visually see how copilotbrowser interacts with the website.

```bash
npx copilotbrowser test --headed
```
...
```bash
npx copilotbrowser test landing-page.spec.ts
```

To run a set of test files from different directories, pass in the directory names that you want to run the tests in.

```bash
npx copilotbrowser test tests/todo-page/ tests/landing-page/
```

To run files that have `landing` or `login` in the file name, simply pass in these keywords to the CLI.

```bash
npx copilotbrowser test landing login
```

To run a test with a specific title, use the `-g` flag followed by the title of the test.

```bash
npx copilotbrowser test -g "add a todo item"
```
...
Since copilotbrowser runs in Node.js, you can debug it with your debugger of choice, e.g. using `console.log`, inside your IDE, or directly in VS Code with the [VS Code Extension](./getting-started-vscode.md). copilotbrowser comes with [UI Mode](./test-ui-mode.md), where you can easily walk through each step of the test, see logs, errors, network requests, inspect the DOM snapshot, and more. You can also use the [copilotbrowser Inspector](./debug.md#copilotbrowser-inspector), which allows you to step through copilotbrowser API calls, see their debug logs, and explore [locators](./locators.md).
...
While debugging you can use the Pick Locator button to select an element on the page and see the locator that copilotbrowser would use to find that element. You can also edit the locator in the locator playground and see it highlighting live in the browser window. Use the Copy Locator button to copy the locator to your clipboard and then paste it into your test.

![pick locator in ui mode](./images/getting-started/ui-mode-pick-locator.png)

Check out our [detailed guide on UI Mode](./test-ui-mode.md) to learn more about its features.

### Debug tests with the copilotbrowser Inspector

To debug all tests, run the copilotbrowser test command followed by the `--debug` flag.

```bash
npx copilotbrowser test --debug
```

![Debugging Tests with the copilotbrowser inspector](./images/getting-started/run-tests-debug.png)

This command opens a browser window as well as the copilotbrowser Inspector. You can use the step over button at the top of the inspector to step through your test. Or, press the play button to run your test from start to finish. Once the test finishes, the browser window closes.

To debug one test file, run the copilotbrowser test command with the test file name that you want to debug followed by the `--debug` flag.

```bash
npx copilotbrowser test example.spec.ts --debug
```
...
![Locator picker in the copilotbrowser Inspector](./images/getting-started/run-tests-pick-locator.png)


Check out our [debugging guide](./debug.md) to learn more about debugging with the [VS Code debugger](./debug.md#vs-code-debugger), UI Mode, and the [copilotbrowser Inspector](./debug.md#copilotbrowser-inspector) as well as debugging with [Browser Developer tools](./debug.md#browser-developer-tools).
