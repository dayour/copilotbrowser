...
You can debug your test live in VS Code. After running a test with the `Show Browser` option checked, click on any of the locators in VS Code and it will be highlighted in the Browser window. copilotbrowser will also show you if there are multiple matches.
...
You can also edit the locators in VS Code and copilotbrowser will show you the changes live in the browser window.
...
copilotbrowser will look at your page and figure out the best locator, prioritizing [role, text and test id locators](./locators.md). If copilotbrowser finds multiple elements matching the locator, it will improve the locator to make it resilient and uniquely identify the target element, so you don't have to worry about failing tests due to locators.
...
The copilotbrowser Inspector is a GUI tool to help you debug your copilotbrowser tests. It allows you to step through your tests, live edit locators, pick locators and see actionability logs.

<img width="864" alt="copilotbrowser Inspector" src="https://user-images.githubusercontent.com/13063165/212924587-4b84e5f6-b147-40e9-8c75-d7b9ab6b7ca1.png" />

### Run in debug mode

Run your tests with the `--debug` flag to open the inspector. This configures copilotbrowser for debugging and opens the inspector. Additional useful defaults are configured when `--debug` is used:
...
To debug one test on a specific line, run the test command followed by the name of the test file and the line number of the test you want to debug, followed by the `--debug` flag. This will run a single test in each browser configured in your [`copilotbrowser.config`](./test-projects.md#configure-projects-for-multiple-browsers) and open the inspector.

```bash
npx copilotbrowser test example.spec.ts:10 --debug
```
#### Debug on a specific browser

In copilotbrowser you can configure projects in your [`copilotbrowser.config`](./test-projects.md#configure-projects-for-multiple-browsers). Once configured you can then debug your tests on a specific browser or mobile viewport using the `--project` flag followed by the name of the project configured in your `copilotbrowser.config`.

```bash
npx copilotbrowser test --project=chromium --debug
npx copilotbrowser test --project="Mobile Safari" --debug
npx copilotbrowser test --project="Microsoft Edge" --debug
```
...
```bash
npx copilotbrowser test example.spec.ts:10 --project=webkit --debug
```
### Run in debug mode

Set the `PWDEBUG` environment variable to run your copilotbrowser tests in debug mode. This
configures copilotbrowser for debugging and opens the inspector. Additional useful defaults are configured when `PWDEBUG=1` is set:
...
To tell copilotbrowser where to look for the source code that you are debugging, pass
a list of the source directories via `copilotbrowser_JAVA_SRC` environment variable. Paths in
...
PWDEBUG=1 copilotbrowser_JAVA_SRC=<java source dirs> mvn test
```

```batch tab=bash-batch lang=java
# Source directories in the list are separated by : on macos and linux and by ; on win.
set copilotbrowser_JAVA_SRC=<java source dirs>
set PWDEBUG=1
mvn test
```
...
# Source directories in the list are separated by : on macos and linux and by ; on win.
$env:copilotbrowser_JAVA_SRC="<java source dirs>"
$env:PWDEBUG=1
mvn test
```
...
<img width="1340" alt="copilotbrowser Inspector and browser" src="https://user-images.githubusercontent.com/13063165/212936618-84b87acc-bc2e-46ed-994b-32b2ef742e60.png" />
...
By the time copilotbrowser has paused on a click action, it has already performed [actionability checks](./actionability.md) that can be found in the log. This can help you understand what happened during your test and what copilotbrowser did or tried to do. The log tells you if the element was visible, enabled and stable, if the locator resolved to an element, scrolled into view, and so much more. If actionability can't be reached, it will show the action as pending.
...
When running in Debug Mode with `PWDEBUG=console`, a `copilotbrowser` object is available in the Developer tools console. Developer tools can help you to:
...
<img width="1399" alt="Browser Developer Tools with copilotbrowser object" src="https://user-images.githubusercontent.com/13063165/219128002-898f604d-9697-4b7f-95b5-a6a8260b7282.png" />
...
PWDEBUG=console npx copilotbrowser test
```
...
```bash tab=bash-bash lang=java
# Source directories in the list are separated by : on macos and linux and by ; on win.
PWDEBUG=console copilotbrowser_JAVA_SRC=<java source dirs> mvn test
```
...
Once copilotbrowser launches the browser window, you can open the developer tools.
The `copilotbrowser` object will be available in the console panel.

#### copilotbrowser.$(selector)

Query the copilotbrowser selector, using the actual copilotbrowser query engine, for example:

```bash
copilotbrowser.$('.auth-form >> text=Log in');
...
Same as `copilotbrowser.$`, but returns all matching elements.

```bash
copilotbrowser.$$('li >> text=John')

[<li>, <li>, <li>, <li>]
```

#### copilotbrowser.inspect(selector)

Reveal element in the Elements panel.

```bash
copilotbrowser.inspect('text=Log in')
```

#### copilotbrowser.locator(selector)

Create a locator and query matching elements, for example:

```bash
copilotbrowser.locator('.auth-form', { hasText: 'Log in' });
...
prevent the copilotbrowser script from executing any further and
will reset pre-configured user agent and device emulation.
:::

## Headed mode

copilotbrowser runs browsers in headless mode by default. To change this behavior,
...
```csharp
// Chromium, Firefox, or WebKit
await using var browser = await copilotbrowser.Chromium.LaunchAsync(new()
{
    Headless = false,
    SlowMo = 100
});
