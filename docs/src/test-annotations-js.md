---
id: test-annotations-js
title: "Annotations"
---

## Introduction

copilotbrowser supports tags and annotations that are displayed in the test report.

You can add your own tags and annotations at any moment, but copilotbrowser comes with a few built-in ones:
- **Test.skip()** marks the test as irrelevant. copilotbrowser does not run such a test. Use this annotation when the test is not applicable in some configuration.
- **Test.fail()** marks the test as failing. copilotbrowser will run this test and ensure it does indeed fail. If the test does not fail, copilotbrowser will complain.
- **Test.fixme()** marks the test as failing. copilotbrowser will not run this test, as opposed to the `fail` annotation. Use `fixme` when running the test is slow or crashes.
...
```bash tab=bash-bash
npx copilotbrowser test --grep-invert @fast
```
...
```bash tab=bash-bash
npx copilotbrowser test --grep "@fast|@slow"
```
...
If you would like to annotate your tests with something more substantial than a tag, you can do that when declaring a test. Annotations have a `type` and a `description` for more context and available in reporter API. copilotbrowser's built-in HTML reporter shows all annotations, except those where `type` starts with `_` symbol.
