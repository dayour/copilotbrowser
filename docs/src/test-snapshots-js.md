...
The snapshot name and path can be configured with **TestConfig.snapshotPathTemplate** in the copilotbrowser config.
...
Apart from screenshots, you can use `expect(value).toMatchSnapshot(snapshotName)` to compare text or arbitrary binary data. copilotbrowser Test auto-detects the content type and uses the appropriate comparison algorithm.
