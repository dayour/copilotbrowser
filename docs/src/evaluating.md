...
copilotbrowser scripts run in your copilotbrowser environment. Your page scripts run in the browser page environment. Those environments don't intersect, they are running in different virtual machines in different processes and even potentially on different computers.

The **Page.evaluate()** API can run a JavaScript function in the context
of the web page and bring results back to the copilotbrowser environment. Browser globals like
...
copilotbrowser evaluation methods like **Page.evaluate()** take a single optional argument. This argument can be a mix of `Serializable` values and `JSHandle` instances. Handles are automatically converted to the value they represent.
