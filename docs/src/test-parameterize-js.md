...
copilotbrowser Test supports running multiple test projects at the same time. In the following example, we'll run two projects with different options.
...
```bash tab=bash-bash
USER_NAME=me PASSWORD=secret npx copilotbrowser test
```
...
```bash tab=bash-bash
STAGING=1 npx copilotbrowser test
```
...
The copilotbrowser test-runner runs in Node.js, this means you can directly read files from the file system and parse them with your preferred CSV library.
