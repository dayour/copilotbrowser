...
The following MCP commands are used during the agent creation and extension process:

- `mcp_copilotbrowse_browser_navigate(url)`: Navigate to specific URLs or pages within Copilot Studio.
- `mcp_copilotbrowse_browser_click(element)`: Click UI elements like tabs, buttons, and menu options.
- `mcp_copilotbrowse_browser_type(element, text)`: Input text into fields such as topic names, descriptions, and messages.
- `mcp_copilotbrowse_browser_wait_for(time)`: Wait for UI operations to complete (e.g., saving, publishing).
- `mcp_copilotbrowse_browser_snapshot()`: Capture the current page accessibility tree for inspection.
- `mcp_copilotbrowse_browser_take_screenshot(type)`: Capture screenshots for documentation and validation.

> **Important:** Issue ONE browser command per turn and wait for the result before proceeding.
