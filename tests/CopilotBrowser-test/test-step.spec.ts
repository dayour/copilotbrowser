...
import { FullConfig, Location, Reporter, Suite, TestStep } from '@copilotbrowser/test/reporter';
...
test('step timeout is errors.TimeoutError', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'a.test.ts': `
      import { test, expect, errors } from '@copilotbrowser/test';
...
test('should step w/o box', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    ...
    'a.test.ts':
    ` /*1*/ import { test, expect } from '@copilotbrowser/test';
      /*2*/ test('fail', async () => {
...
test('should allow passing location to test.step', async ({ runInlineTest, runTSC }) => {
  const result = await runInlineTest({
    'reporter.ts': stepIndentReporter,
    'helper.ts': `
      import { Location, TestType } from '@copilotbrowser/test';
...
test('should box fixtures', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    ...
    'copilotbrowser.config.ts': `module.exports = { reporter: './reporter' };`,
    'a.test.ts': `
      import { test as baseTest, expect, type Page } from '@copilotbrowser/test';
