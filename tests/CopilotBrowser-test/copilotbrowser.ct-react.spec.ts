...
test('should not add React import to minified .js file that already imports React after closing brace', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/38906' }
}, async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'copilotbrowser.config.ts': copilotbrowserCtConfigText,
    'copilotbrowser/index.html': `<script type="module" src="./index.js"></script>`,
...
test('should throw test error when template index.html is not provided', async ({ runInlineTest }) => {
  ...
  expect(result.output).toContain('Component testing template file copilotbrowser/index.html is missing and there is no existing Vite server. Component tests will fail.');
  expect(result.results[0].error.message).toBe('Error: Component testing could not determine the base URL of your component under test. Ensure you have supplied a template copilotbrowser/index.html or have set the copilotbrowser_TEST_BASE_URL environment variable.');
});
