...
test('should display annotations', async ({ runUITest }) => {
    ...
    'a.test.ts': `
      ...
      test.describe('suite', {
        ...
        test('annotation test', {
          annotation: { type: 'bug report', description: 'Report https://github.com/dayour/copilotbrowser/issues/30095 is here' }
        }, async () => {
          test.info().annotations.push({ type: 'test repo', description: 'https://github.com/dayour/copilotbrowser' });
        });
      });
    `,
  });
  ...
  await expect(annotations.locator('.annotation-item').filter({ hasText: 'bug report' }).locator('a'))
      .toHaveAttribute('href', 'https://github.com/dayour/copilotbrowser/issues/30095');
  ...
  await expect(annotations.locator('.annotation-item').filter({ hasText: 'test repo' }).locator('a'))
      .toHaveAttribute('href', 'https://github.com/dayour/copilotbrowser');
});
