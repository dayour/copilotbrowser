...
import { test, expect } from './copilotbrowser-test-fixtures';
import fs from 'fs';
...
for (const useIntermediateMergeReport of [false, true] as const) {
  test.describe(`${useIntermediateMergeReport ? 'merged' : 'created'}`, () => {
    ...
    test('should render attachment paths relative to report file when report file name is specified', async ({ runInlineTest }, testInfo) => {
      test.skip(useIntermediateMergeReport, 'Blob report hashes attachment paths');
      const result = await runInlineTest({
        'project/copilotbrowser.config.ts': `
        ...
        `,
      }, { reporter: '', config: './project/copilotbrowser.config.ts' });
    ...
    test('should render annotations to custom testcase properties', async ({ runInlineTest }) => {
      const result = await runInlineTest({
        'copilotbrowser.config.ts': `module.exports = { reporter: 'junit' };`,
        'a.test.js': `
          import { test, expect } from '@copilotbrowser/test';
          test('one', async ({}, testInfo) => {
    ...
    test.describe('report location', () => {
      ...
      test('with env var should create relative to cwd', async ({ runInlineTest }, testInfo) => {
        const result = await runInlineTest({
          ...
          `
        }, { 'reporter': 'junit,line' }, { 'copilotbrowser_JUNIT_OUTPUT_NAME': '../my-report.xml' }, {
      ...
      test('support copilotbrowser_JUNIT_OUTPUT_FILE', async ({ runInlineTest }, testInfo) => {
        const result = await runInlineTest({
          'foo/package.json': `{ "name": "foo" }`,
          'foo/bar/baz/tests/a.spec.js': `
            import { test, expect } from '@copilotbrowser/test';
          ...
          `
        }, { 'reporter': 'junit,line' }, { 'copilotbrowser_JUNIT_OUTPUT_FILE': '../my-report.xml' }, {
          cwd: 'foo/bar/baz/tests',
        });
        expect(result.exitCode).toBe(0);
        expect(result.passed).toBe(1);
      ...
      test('copilotbrowser_JUNIT_OUTPUT_FILE should take precedence over config', async ({ runInlineTest }, testInfo) => {
        const result = await runInlineTest({
          'package.json': `{ "name": "foo" }`,
          'bar/copilotbrowser.config.js': `
            module.exports = {
          ...
          `
        }, { 'config': 'bar/copilotbrowser.config.js' }, { 'copilotbrowser_JUNIT_OUTPUT_FILE': 'bar/my-report.xml' });
        expect(result.exitCode).toBe(0);
        expect(result.passed).toBe(1);
        expect(fs.existsSync(testInfo.outputPath('bar', 'my-report.xml'))).toBe(true);
      });

      test('support copilotbrowser_JUNIT_OUTPUT_DIR and copilotbrowser_JUNIT_OUTPUT_NAME', async ({ runInlineTest }, testInfo) => {
        const result = await runInlineTest({
          ...
          `
        }, { 'reporter': 'junit,line' }, { 'copilotbrowser_JUNIT_OUTPUT_DIR': 'foo/bar', 'copilotbrowser_JUNIT_OUTPUT_NAME': 'baz/my-report.xml' });
        expect(result.exitCode).toBe(0);
        expect(result.passed).toBe(1);
        expect(fs.existsSync(testInfo.outputPath('foo', 'bar', 'baz', 'my-report.xml'))).toBe(true);
      });

      test('copilotbrowser_JUNIT_OUTPUT_NAME should take precedence over config', async ({ runInlineTest }, testInfo) => {
        const result = await runInlineTest({
          'package.json': `{ "name": "foo" }`,
          'bar/copilotbrowser.config.js': `
            module.exports = {
          ...
          `
        }, { 'config': 'bar/copilotbrowser.config.js' }, { 'copilotbrowser_JUNIT_OUTPUT_NAME': 'foo/my-report.xml' });
        expect(result.exitCode).toBe(0);
        expect(result.passed).toBe(1);
    ...
    test('testsuites time is test run wall time', async ({ runInlineTest }) => {
      test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/30518' });
      const result = await runInlineTest({
        'a.test.js': `
          import { test, expect } from '@copilotbrowser/test';
          test('one', async ({}) => {
