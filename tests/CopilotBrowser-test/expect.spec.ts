...
test('should work with custom copilotbrowserTest namespace', async ({ runTSC }) => {
  const result = await runTSC({
    ...
    `,
    'a.spec.ts': `
      import { test, expect, type Page, type APIResponse } from '@copilotbrowser/test';
      test.expect.extend({
        toBeWithinRange() {
          return {
            pass: true,
...
test('should chain expect matchers and expose matcher utils (TSC)', async ({ runTSC }) => {
  const result = await runTSC({
    'a.spec.ts': `
    import { test, expect as baseExpect } from '@copilotbrowser/test';
    import type { Page, Locator, ExpectMatcherState, Expect } from '@copilotbrowser/test';
...
test('should support mergeExpects (TSC)', async ({ runTSC }) => {
  const result = await runTSC({
    'a.spec.ts': `
      import { test, mergeExpects, expect as baseExpect } from '@copilotbrowser/test';
...
test('should respect timeout from configured expect when used outside of the test runner', async ({ runInlineTest, writeFiles, runTSC }) => {
  ...
  const files = {
    'script.mjs': `
      import { test, expect as baseExpect, chromium } from '@copilotbrowser/test';
...
test('should expose timeout to custom matchers', async ({ runInlineTest, runTSC }) => {
  const files = {
    ...
    `,
    'a.test.ts': `
      import type { ExpectMatcherState, MatcherReturnType } from '@copilotbrowser/test';
      import { test, expect as base } from '@copilotbrowser/test';
...
test('should throw error when using .equals()', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'helper.ts': `
      import { test as base, expect as baseExpect } from '@copilotbrowser/test';
...
test('custom asymmetric matchers should work with expect.extend', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'expect-test.spec.ts': `
      import { test, expect as baseExpect, mergeExpects } from '@copilotbrowser/test';
      const expect1 = baseExpect.extend({
