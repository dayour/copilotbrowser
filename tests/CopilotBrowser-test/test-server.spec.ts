...
import { TestServerConnection } from '../../packages/copilotbrowser/lib/isomorphic/testServerConnection';
import { copilotbrowserCtConfigText } from './copilotbrowser-test-fixtures';
import ws from 'ws';
...
test('copilotbrowser_TEST environment variable', async ({ startTestServer, writeFiles }) => {
  const testServerConnection = await startTestServer();
  await testServerConnection.initialize({});
  await writeFiles({
    'a.test.ts': `
      import { test, expect } from '@copilotbrowser/test';
      test('foo', () => {
        expect(process.env.copilotbrowser_TEST).toBe('1');
      });
      `,
  });
