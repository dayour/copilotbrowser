...
test('should respect path resolver', async ({ runInlineTest }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/11656' });
...
test('should not use baseurl for relative imports', async ({ runInlineTest }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/15891' });
  const result = await runInlineTest({
    ...
    'frontend/copilotbrowser/utils.ts': `
      export const foo = 42;
    `,
    'frontend/copilotbrowser/tests/forms_cms_standard.spec.ts': `
...
test('should not use baseurl for relative imports when dir with same name exists', async ({ runInlineTest }) => {
  ...
  const result = await runInlineTest({
    ...
    'frontend/src/.bar.js': `
      export const bar = 42;
    `,
    'frontend/copilotbrowser/tests/utils.ts': `
      export const foo = 42;
    `,
    'frontend/copilotbrowser/tests/index.js': `
...
test('should respect tsconfig project references', async ({ runInlineTest }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/29256' });
...
test('should respect config.tsconfig option', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    ...
    'tests/a.test.ts': `
      ...
      import { configFoo } from '../copilotbrowser.config';
      import { test, expect } from '@copilotbrowser/test';
      test('test', ({}) => {
...
test.describe('directory imports', () => {
  ...
  test('should resolve index.js after path mapping in CJS', async ({ runInlineTest, runTSC }) => {
    test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/31811' });
