...
test('render trace attachment', async ({ runInlineTest }) => {
  ...
  expect(text).toContain('npx copilotbrowser show-trace "test-results/a-one/my dir with space/trace.zip"');
  expect(text).toContain('    ────────────────────────────────────────────────────────────────────────────────────────────────');
  expect(result.exitCode).toBe(1);
});
...
test('attaching inside boxed fixture should not log error', { annotation: [{ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/37147' }, { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/37747' }] }, async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'a.test.ts': `
