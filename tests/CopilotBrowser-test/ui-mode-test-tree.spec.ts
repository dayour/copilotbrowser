...
test('should list all tests from projects with clashing names', async ({ runUITest }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/30396' });
...
test('should expand all', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/32825' }
}, async ({ runUITest }) => {
  const { page } = await runUITest(basicTestTree);
