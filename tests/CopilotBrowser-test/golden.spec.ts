...
import { test, expect, createWhiteImage, paintBlackPixels } from './copilotbrowser-test-fixtures';
...
test('should generate separate actual results for repeating names', async ({ runInlineTest }, testInfo) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/29719' });
  const result = await runInlineTest({
    ...files,
...
for (const updateSnapshots of ['all', 'changed', 'missing', 'none']) {
  test(`should update snapshot with the update-snapshots=${updateSnapshots} (config)`, async ({ runInlineTest }, testInfo) => {
    const result = await runInlineTest({
      'copilotbrowser.config.ts': `export default { updateSnapshots: '${updateSnapshots}' };`,
      ...files,
      'a.spec.js-snapshots/snapshot.txt': 'Hello world',
      'a.spec.js': `
        const { test, expect } = require('./helper');
  ...
  expect(attachments).toEqual([
    {
      ...
      path: 'golden-should-attach-expected-actual-diff-with-snapshot-path-copilotbrowser-test/a.spec.js-snapshots/test/path/snapshot.png'
    },
    {
  ...
  expect(attachments).toEqual([
    {
      ...
      contentType: 'image/png',
      path: 'golden-should-attach-expected-actual-diff-copilotbrowser-test/a.spec.js-snapshots/snapshot.png'
    },
    {
