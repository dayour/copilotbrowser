...
test('merge into list report by default', async ({ runInlineTest, mergeReports }) => {
  ...
  const { exitCode, output } = await mergeReports(reportDir, { PW_TEST_DEBUG_REPORTERS: '1', copilotbrowser_LIST_PRINT_STEPS: '1', copilotbrowser_FORCE_TTY: '80' }, { additionalArgs: ['--reporter', 'list'] });
  expect(exitCode).toBe(0);
...
test('support copilotbrowser_BLOB_OUTPUT_NAME env variable', async ({ runInlineTest, mergeReports }) => {
  ...
  await runInlineTest(files, undefined, { copilotbrowser_BLOB_OUTPUT_NAME: 'report-two.zip', PWTEST_BLOB_DO_NOT_REMOVE: '1' });
