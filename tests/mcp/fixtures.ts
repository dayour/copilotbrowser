...
import { parseResponse } from '../../packages/copilotbrowser/lib/mcp/browser/response';
...
async function createTransport(mcpServerType: TestOptions['mcpServerType'], options: { args: string[], env: NodeJS.ProcessEnv, cwd: string }): Promise<{
  ...
  const profilesDir = test.info().outputPath('ms-copilotbrowser');
...
export const testMcpServerPath = [path.join(__dirname, '../../packages/copilotbrowser-test/cli.js'), 'run-test-mcp-server'];
