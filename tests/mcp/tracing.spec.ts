...
test('check that trace is saved with browser_start_tracing (no output dir)', async ({ startClient, server }, testInfo) => {
  ...
  const folders = await fs.promises.readdir(path.join(tmpDir, 'copilotbrowser-mcp-output'));
  expect(folders.length).toBe(1);
  expect(folders[0]).toMatch(/\d+/);
  const files = await fs.promises.readdir(path.join(tmpDir, 'copilotbrowser-mcp-output', folders[0], 'traces'));
  expect(files).toEqual([
    'resources',
    expect.stringMatching(/trace-\d+\.network/),
    expect.stringMatching(/trace-\d+\.stacks/),
