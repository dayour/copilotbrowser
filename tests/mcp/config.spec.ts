...
import { resolveCLIConfig } from '../../packages/copilotbrowser/lib/mcp/browser/config';
...
test.describe(() => {
  test.use({ mcpBrowser: '' });
  test('browserName', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser-mcp/issues/458' } }, async ({ startClient }, testInfo) => {
    const config: Config = {
      browser: {
        browserName: 'firefox',
      },
...
test('browser_get_config returns merged config from file, env and cli', async ({ startClient }) => {
  const { client } = await startClient({
    ...
    args: ['--isolated'],
    env: {
      copilotbrowser_MCP_TIMEOUT_NAVIGATION: '45000',
    },
  });
