...
export const test = baseTest.extend<{
  ...
  cli: async ({ mcpBrowser, mcpHeadless, childProcess }, use) => {
    ...
    await fs.promises.mkdir(test.info().outputPath('.copilotbrowser'), { recursive: true });
...
async function runCli(childProcess: CommonFixtures['childProcess'], args: string[], cliOptions: { cwd?: string, env?: Record<string, string> }, options: { mcpBrowser: string, mcpHeadless: boolean }, sessions: { name: string, pid: number }[]) {
  ...
  return await test.step(stepTitle, async () => {
    ...
    const cli = childProcess({
      command: [process.execPath, require.resolve('../../packages/copilotbrowser/lib/cli/client/program.js'), ...args],
      cwd: cliOptions.cwd ?? testInfo.outputPath(),
      env: {
        ...process.env,
        ...cliOptions.env,
        copilotbrowser_DAEMON_SESSION_DIR: testInfo.outputPath('daemon'),
        copilotbrowser_DAEMON_SOCKETS_DIR: path.join(testInfo.project.outputDir, 'daemon-sockets'),
        copilotbrowser_MCP_BROWSER: options.mcpBrowser,
        copilotbrowser_MCP_HEADLESS: String(options.mcpHeadless),
        ...cliOptions.env,
      },
    });
...
function loadAttachments(output: string) {
  // attachments look like md links  - [Page as pdf](.copilotbrowser-cli/page-2026-01-22T23-13-56-347Z.pdf)
