...
test('workspace isolation - sessions in different workspaces are isolated', async ({ cli, server }, testInfo) => {
  ...
  expect(fs.existsSync(path.join(workspace1, '.copilotbrowser'))).toBe(true);
  expect(fs.existsSync(path.join(workspace2, '.copilotbrowser'))).toBe(true);
...
test('newer client with older daemon is compatible', async ({ cli, server }) => {
  ...
  const { exitCode } = await cli('eval', '1+1', { env: { copilotbrowser_CLI_VERSION_FOR_TEST: '9.9.9' } });
...
test('older client with newer daemon fails with version mismatch error', async ({ cli, server }) => {
  ...
  const { output, error, exitCode } = await cli('eval', '1+1', { env: { copilotbrowser_CLI_VERSION_FOR_TEST: '0.0.1' } });
  expect(exitCode).not.toBe(0);
  const fullOutput = output + error;
  expect(fullOutput).toContain('Client is v0.0.1');
  expect(fullOutput).toContain('copilotbrowser-cli open');
...
test('older client with newer daemon - named session includes session name in error', async ({ cli, server }) => {
  ...
  const { output, error, exitCode } = await cli('-s', 'mysession', 'eval', '1+1', { env: { copilotbrowser_CLI_VERSION_FOR_TEST: '0.0.1' } });
  expect(exitCode).not.toBe(0);
  const fullOutput = output + error;
  expect(fullOutput).toContain('Client is v0.0.1');
  expect(fullOutput).toContain(`session 'mysession'`);
  expect(fullOutput).toContain('copilotbrowser-cli -s=mysession open');
});

test('older client with newer daemon - list shows incompatible warning', async ({ cli, server }) => {
  await cli('open', server.HELLO_WORLD);

  const { output } = await cli('list', { env: { copilotbrowser_CLI_VERSION_FOR_TEST: '0.0.1' } });
