...
test('config-print prints merged config from file, env and cli', async ({ cli, server }, testInfo) => {
  ...
  await fs.promises.writeFile(testInfo.outputPath('.copilotbrowser', 'cli.config.json'), JSON.stringify(fileConfig, null, 2));

  // Env var overrides navigation timeout (30000 from file → 45000 from env).
  const env = { copilotbrowser_MCP_TIMEOUT_NAVIGATION: '45000' };
  ...
  // From copilotbrowser cli defaults.
