...
test('daemon shuts down on browser launch failure', async ({ cli, server }) => {
  const first = await cli('open', server.PREFIX, { env: { copilotbrowser_MCP_EXECUTABLE_PATH: '/nonexistent/browser/path' } });
...
test('install workspace', async ({ cli }, testInfo) => {
  ...
  const copilotbrowserDir = testInfo.outputPath('.copilotbrowser');
  expect(fs.existsSync(copilotbrowserDir)).toBe(true);
});

test('install workspace w/skills', async ({ cli }, testInfo) => {
  const { output } = await cli('install', '--skills');
  expect(output).toContain(`Skills installed to \`.claude${path.sep}skills${path.sep}copilotbrowser-cli\`.`);

  const skillFile = testInfo.outputPath('.claude', 'skills', 'copilotbrowser-cli', 'SKILL.md');
  expect(fs.existsSync(skillFile)).toBe(true);

  const referencesDir = testInfo.outputPath('.claude', 'skills', 'copilotbrowser-cli', 'references');
