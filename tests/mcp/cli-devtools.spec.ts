...
test('tracing-start-stop', async ({ cli, server }, testInfo) => {
  ...
  expect(tracingStopOutput).toContain(`- [Trace](.copilotbrowser-cli${path.sep}traces${path.sep}trace-${timestamp}.trace)`);
  expect(tracingStopOutput).toContain(`- [Network log](.copilotbrowser-cli${path.sep}traces${path.sep}trace-${timestamp}.network)`);
  expect(tracingStopOutput).toContain(`- [Resources](.copilotbrowser-cli${path.sep}traces${path.sep}resources)`);

  expect(fs.existsSync(testInfo.outputPath('.copilotbrowser-cli', 'traces', 'resources'))).toBeTruthy();
  expect(fs.existsSync(testInfo.outputPath('.copilotbrowser-cli', 'traces', `trace-${timestamp}.trace`))).toBeTruthy();
  expect(fs.existsSync(testInfo.outputPath('.copilotbrowser-cli', 'traces', `trace-${timestamp}.network`))).toBeTruthy();
});
