...
test('android.launchServer should connect to a device', async ({ copilotbrowser }) => {
  const browserServer = await copilotbrowser._android.launchServer();
  const device = await copilotbrowser._android.connect(browserServer.wsEndpoint());
...
test('android.launchServer should work with host', async ({ copilotbrowser }) => {
  const host = '0.0.0.0';
  const browserServer = await copilotbrowser._android.launchServer({ host });
...
test('android.launchServer should handle close event correctly', async ({ copilotbrowser }) => {
...
test('android.launchServer should be able to reconnect to a device', async ({ copilotbrowser }) => {
  const browserServer = await copilotbrowser._android.launchServer();
  try {
    {
      const device = await copilotbrowser._android.connect(browserServer.wsEndpoint());
...
test('android.launchServer should throw if there is no device with a specified serial number', async ({ copilotbrowser }) => {
  await expect(copilotbrowser._android.launchServer({
    deviceSerialNumber: 'does-not-exist',
  })).rejects.toThrow(`No device with serial number 'does-not-exist'`);
});

test('android.launchServer should not allow multiple connections', async ({ copilotbrowser }) => {
  const browserServer = await copilotbrowser._android.launchServer();
  try {
    await copilotbrowser._android.connect(browserServer.wsEndpoint());
    await expect(copilotbrowser._android.connect(browserServer.wsEndpoint(), { timeout: 2_000 })).rejects.toThrow('android.connect: Timeout 2000ms exceeded');
  } finally {
    await browserServer.close();
  }
});

test('android.launchServer BrowserServer.close() will disconnect the device', async ({ copilotbrowser }) => {
  const browserServer = await copilotbrowser._android.launchServer();
  try {
    const device = await copilotbrowser._android.connect(browserServer.wsEndpoint());
    await browserServer.close();
...
test('android.launchServer BrowserServer.kill() will disconnect the device',  async ({ copilotbrowser }) => {
  const browserServer = await copilotbrowser._android.launchServer();
  try {
    const device = await copilotbrowser._android.connect(browserServer.wsEndpoint());
...
test('android.launchServer should terminate WS connection when device gets disconnected', async  ({ copilotbrowser }) => {
  const browserServer = await copilotbrowser._android.launchServer();
  ...
  try {
    const device = await copilotbrowser._android.connect(`ws://localhost:${(forwardingServer.address() as ws.AddressInfo).port}/connect`);
...
test('android.launchServer should be able to launch browser', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/36911' } }, async ({ copilotbrowser }) => {
  const browserServer = await copilotbrowser._android.launchServer();
  const device = await copilotbrowser._android.connect(browserServer.wsEndpoint());
