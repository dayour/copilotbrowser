...
test('select webview from socketName', async function({ androidDevice }) {
  ...
  const webview = await androidDevice.webView({ socketName: 'webview_devtools_remote_copilotbrowser_test' });
...
test.fail('should be able to receive webView cookies', {
  annotation: {
    type: 'issue',
    description: 'https://github.com/dayour/copilotbrowser/issues/35392',
  }
