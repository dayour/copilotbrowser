...
test('androidDevice.close', async function({ copilotbrowser }) {
  const devices = await copilotbrowser._android.devices();
...
test('should be able to use a custom port', async function({ copilotbrowser }) {
  ...
  const devices = await copilotbrowser._android.devices({ port: proxyPort });
