...
test('render a component as slot', async ({ mount }) => {
  const component = await mount(DefaultSlot, {
    slots: {
      default: '<Button title="Submit" />', // component is registered globally in /copilotbrowser/index.ts
    },
    hooksConfig: {
      components: { Button }
    }
...
test('updating default slot should work', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/32809' } }, async ({ mount }) => {
