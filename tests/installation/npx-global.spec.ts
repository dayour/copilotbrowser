...
test('npx copilotbrowser --help should not download browsers', async ({ exec, checkInstalledSoftwareOnDisk }) => {
  const result = await exec('npx copilotbrowser --help');
...
test('npx copilotbrowser codegen', async ({ exec, checkInstalledSoftwareOnDisk }) => {
  const stdio = await exec('npx copilotbrowser codegen', { expectToExitWithError: true });
...
test('npx copilotbrowser install global', async ({ exec, checkInstalledSoftwareOnDisk }) => {
