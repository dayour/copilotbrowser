...
test('typescript types should work', async ({ exec, tsc, writeFiles }) => {
  ...
  await tsc('copilotbrowser-test-types.ts');
  await tsc('--module nodenext copilotbrowser-test-types.ts');

  await writeFiles({
    'test.ts':
      `import { AndroidDevice, _android, AndroidWebView, Page } from 'copilotbrowser';`,
  });
  await tsc('test.ts');
});
