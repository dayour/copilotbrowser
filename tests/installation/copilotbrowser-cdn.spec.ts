...
test(`copilotbrowser cdn should race with a timeout`, async ({ exec }) => {
...
test(`copilotbrowser cdn should not timeout on redirect`, {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/34686' }
}, async ({ exec }) => {
...
test(`npx copilotbrowser install should not hang when CDN closes the connection`, async ({ exec }) => {
  let retryCount = 0;
...
test(`npx copilotbrowser install should not hang when CDN TCP connection stalls`, async ({ exec }) => {
  let retryCount = 0;
