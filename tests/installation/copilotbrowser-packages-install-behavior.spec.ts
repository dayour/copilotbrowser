...
for (const browser of ['chromium', 'firefox', 'webkit']) {
  test(`@copilotbrowser-${browser} should work`, async ({ exec, checkInstalledSoftwareOnDisk }) => {
    ...
    const result2 = await exec('npm i --foreground-scripts copilotbrowser');
    expect(result2).toHaveLoggedSoftwareDownload([]);
    await checkInstalledSoftwareOnDisk(expectedSoftware);

    await exec('node sanity.js copilotbrowser', browser);
...
test(`copilotbrowser should work`, async ({ exec, checkInstalledSoftwareOnDisk }) => {
  ...
  const result2 = await exec('npx copilotbrowser install');
  ...
  await exec('node esm-copilotbrowser.mjs');
});

test(`copilotbrowser should work with chromium --no-shell`, async ({ exec, checkInstalledSoftwareOnDisk }) => {
  const result1 = await exec('npm i --foreground-scripts copilotbrowser');
  expect(result1).toHaveLoggedSoftwareDownload([]);
  await checkInstalledSoftwareOnDisk([]);
  const result2 = await exec('npx copilotbrowser install chromium --no-shell');
...
test(`copilotbrowser should work with chromium --only-shell`, async ({ exec, checkInstalledSoftwareOnDisk }) => {
  const result1 = await exec('npm i --foreground-scripts copilotbrowser');
  expect(result1).toHaveLoggedSoftwareDownload([]);
  await checkInstalledSoftwareOnDisk([]);
  const result2 = await exec('npx copilotbrowser install --only-shell');
...
test('@copilotbrowser/test should work', async ({ exec, checkInstalledSoftwareOnDisk }) => {
  ...
  await exec('npx copilotbrowser test -c . sample.spec.js', { expectToExitWithError: true, message: 'should not be able to run tests without installing browsers' });
