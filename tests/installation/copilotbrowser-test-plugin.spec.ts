...
test('npm: @copilotbrowser/test plugin should work', async ({ exec, tmpWorkspace }) => {
  ...
  const output = await exec('npx copilotbrowser test -c . --browser=chromium --reporter=line plugin.spec.ts');
...
test('pnpm: @copilotbrowser/test plugin should work', async ({ exec, tmpWorkspace }) => {
  ...
  const output = await exec('pnpm exec copilotbrowser test -c . --browser=chromium --reporter=line plugin.spec.ts');
  expect(output).toContain('plugin value: hello from plugin');
  expect(output).toContain('1 passed');
...
test('yarn: @copilotbrowser/test plugin should work', async ({ exec, tmpWorkspace }) => {
  ...
  const output = await exec('yarn copilotbrowser test -c . --browser=chromium --reporter=line plugin.spec.ts');
  expect(output).toContain('plugin value: hello from plugin');
  expect(output).toContain('1 passed');

  await exec('yarn add typescript@5.2.2 @types/node@18');
  await exec('yarn tsc copilotbrowser-test-plugin-types.ts');
});
