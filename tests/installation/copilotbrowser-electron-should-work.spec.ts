...
import fs from 'fs';
import { expect } from 'packages/copilotbrowser-test';
import path from 'path';

test('electron should work', async ({ exec, tsc, writeFiles }) => {
  await exec('npm i copilotbrowser electron@19.0.11');
  await exec('node sanity-electron.js');
  await writeFiles({
    'test.ts':
      `import { Page, _electron, ElectronApplication, Electron } from 'copilotbrowser';`
  });
  await tsc('test.ts');
});

test('electron should work with special characters in path', async ({ exec, tmpWorkspace }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/30755' });
...
test('should work when wrapped inside @copilotbrowser/copilotbrowser and trace is enabled', async ({ exec, tmpWorkspace, writeFiles }) => {
  await exec('npm i -D @copilotbrowser/copilotbrowser electron@31');
  await writeFiles({
    'electron-with-tracing.spec.ts': `
      import { test, expect, _electron } from '@copilotbrowser/copilotbrowser/test';
      ...
      test('should work', async ({ trace }) => {
        ...
        await window.goto('data:text/html,<title>copilotbrowser</title><h1>copilotbrowser</h1>');
        await expect(window).toHaveTitle(/copilotbrowser/);
        await expect(window.getByRole('heading')).toHaveText('copilotbrowser');
  ...
  await exec('npx copilotbrowser test --trace=on --reporter=json electron-with-tracing.spec.ts', {
    env: { copilotbrowser_JSON_OUTPUT_NAME: jsonOutputName }
  });
  const traces = [
    // our actual trace.
