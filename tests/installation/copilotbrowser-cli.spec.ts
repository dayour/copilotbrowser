...
test('cli should work', async ({ exec, tmpWorkspace }) => {
  ...
  await test.step('codegen with user data dir', async () => {
    const userDataDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'copilotbrowser-test-custom-user-data-dir'));
    const outputFile = test.info().outputPath('codegen.output');
  ...
  await test.step('codegen --target=javascript', async () => {
    ...
    expect(contents).toContain(`copilotbrowser`);
  });
  ...
  await test.step('screenshot', async () => {
    await exec(path.join('node_modules', '.bin', 'copilotbrowser'), 'screenshot about:blank one.png');
    await fs.promises.stat(path.join(tmpWorkspace, 'one.png'));

    await exec('npx copilotbrowser screenshot about:blank two.png');
    await fs.promises.stat(path.join(tmpWorkspace, 'two.png'));
  });

  await test.step('show-trace', async () => {
    const result = await exec('npx copilotbrowser show-trace i-do-not-exist.zip', { expectToExitWithError: true });
    expect(result).toContain(`Trace file i-do-not-exist.zip does not exist`);
  });

  await test.step('show-report', async () => {
    const result = await exec('npx copilotbrowser show-report', { expectToExitWithError: true });
    expect(result).toContain(`No report found at "${path.join(fs.realpathSync(tmpWorkspace), 'copilotbrowser-report')}"`);
  });
});
