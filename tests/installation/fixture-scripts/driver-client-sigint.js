...
(async () => {
  ...
  const { copilotbrowser, stop } = await start();
  console.log('launched driver')
  try {
    const browser = await copilotbrowser.chromium.launch({ handleSIGINT: false });
    const context = await browser.newContext();
    const page = await context.newPage();
    // let things settle down
    await page.waitForTimeout(100);
    // send SIGINT to driver
    process.kill(copilotbrowser.driverProcess.pid, 'SIGINT');
