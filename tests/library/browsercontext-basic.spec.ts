...
it('should be able to click across browser contexts', async function({ browser }) {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/29096' });
  expect(browser.contexts().length).toBe(0);
...
it('fetch with keepalive should throw when offline', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35701' },
...
it('should emulate offline event', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/37295' } }, async ({ browser }) => {
  const context = await browser.newContext();
  const page = await context.newPage();
...
it('should create two pages in parallel in various contexts', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/34586' }
}, async ({ browser }) => {
  const context1 = await browser.newContext();
  const context2 = await browser.newContext();
  await Promise.all([
