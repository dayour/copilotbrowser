...
it.describe('block', () => {
  ...
  it('blocks service worker registration', async ({ page, server }) => {
    await Promise.all([
      page.waitForEvent('console', evt => evt.text() === 'Service Worker registration blocked by copilotbrowser'),
      page.goto(server.PREFIX + '/serviceworkers/empty/sw.html'),
    ]);
  });

  it('should not throw error on about:blank', async ({ page }) => {
    it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/32292' });
    const errors = [];
