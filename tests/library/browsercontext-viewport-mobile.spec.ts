...
it.describe('mobile viewport', () => {
  ...
  it('should support mobile emulation', async ({ copilotbrowser, browser, server }) => {
    const iPhone = copilotbrowser.devices['iPhone 6'];
  ...
  it('should support touch emulation', async ({ copilotbrowser, browser, server }) => {
    const iPhone = copilotbrowser.devices['iPhone 6'];
    const context = await browser.newContext({ ...iPhone });
    const page = await context.newPage();
    await page.goto(server.PREFIX + '/mobile.html');
  ...
  it('should be detectable', async ({ copilotbrowser, browser }) => {
    const iPhone = copilotbrowser.devices['iPhone 6'];
    const context = await browser.newContext({ ...iPhone });
    const page = await context.newPage();
  ...
  it('should support landscape emulation', async ({ copilotbrowser, browser, server }) => {
    const iPhone = copilotbrowser.devices['iPhone 6'];
    const iPhoneLandscape = copilotbrowser.devices['iPhone 6 landscape'];
  ...
  it('should emulate the hover media feature', async ({ copilotbrowser, browser }) => {
    const iPhone = copilotbrowser.devices['iPhone 6'];
  ...
  it('should scroll mobile page with background-attachment: fixed', {
    annotation: [
      { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/31551' },
      { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/23573' },
    ]
  }, async ({ copilotbrowser, browser, server }) => {
    const iPhone = copilotbrowser.devices['iPhone 12'];
    const context = await browser.newContext({ ...iPhone });
    const page = await context.newPage();
  ...
  it('view scale should reset after navigation', async ({ browser }) => {
    it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/26876' });
    const context = await browser.newContext({
