...
it.describe('device', () => {
  ...
  it('should work @smoke', async ({ copilotbrowser, browser, server }) => {
  ...
  it('should support clicking', async ({ copilotbrowser, browser, server }) => {
    const iPhone = copilotbrowser.devices['iPhone 6'];
    const context = await browser.newContext({ ...iPhone });
    const page = await context.newPage();
  ...
  it('should scroll twice when emulated', async ({ contextFactory, copilotbrowser }) => {
  ...
  it('should reset scroll top after a navigation', async ({ server, contextFactory, copilotbrowser, browserName }) => {
  ...
  it('should scroll to a precise position with mobile scale', async ({ server, contextFactory, copilotbrowser, browserName }) => {
    it.skip(browserName === 'webkit');
  ...
  it('should emulate viewport and screen size', async ({ contextFactory, copilotbrowser }) => {
    const device = copilotbrowser.devices['iPhone 12'];
    const context = await contextFactory(device);
    const page = await context.newPage();
  ...
  it('should emulate viewport without screen size', async ({ contextFactory, copilotbrowser }) => {
    const device = copilotbrowser.devices['iPhone 6'];
    const context = await contextFactory(device);
    const page = await context.newPage();
    await page.setContent(`<meta name="viewport" content="width=device-width, user-scalable=no" />`);
