...
const test = base.extend<TestOptions>({
  startCCServer: async ({ asset }, use) => {
    ...
    await use(async options => {
      ...
      const host = options?.useFakeLocalhost ? 'local.copilotbrowser' : '127.0.0.1';
...
test.describe('fetch', () => {
  test('validate input', async ({ copilotbrowser }) => {
    for (const [contextOptions, expected] of kValidationSubTests)
      await expect(copilotbrowser.request.newContext(contextOptions)).rejects.toThrow(expected);
  });

  test('should fail with no client certificates provided', async ({ copilotbrowser, startCCServer }) => {
  ...
  test('should keep supporting http', async ({ copilotbrowser, server, asset }) => {
  ...
  test('should throw with untrusted client certs', async ({ copilotbrowser, startCCServer, asset }) => {
    const serverURL = await startCCServer();
    const request = await copilotbrowser.request.newContext({
      ignoreHTTPSErrors: true,
      clientCertificates: [{
  ...
  test('pass with trusted client certificates', async ({ copilotbrowser, startCCServer, asset }) => {
    const serverURL = await startCCServer();
    const request = await copilotbrowser.request.newContext({
      ignoreHTTPSErrors: true,
      clientCertificates: [{
  ...
  test('pass with trusted client certificates in pfx format', async ({ copilotbrowser, startCCServer, asset }) => {
    const serverURL = await startCCServer();
    const request = await copilotbrowser.request.newContext({
      ignoreHTTPSErrors: true,
      clientCertificates: [{
  ...
  test('pass with trusted client certificates and when a http proxy is used', async ({ copilotbrowser, startCCServer, proxyServer, asset }) => {
    const serverURL = await startCCServer();
  ...
  test('pass with trusted client certificates and when a socks proxy is used', async ({ copilotbrowser, startCCServer, asset }) => {
    const serverURL = await startCCServer();
  ...
  test('should throw a http error if the pfx passphrase is incorect', async ({ copilotbrowser, startCCServer, asset, browserName }) => {
    const serverURL = await startCCServer();
    const request = await copilotbrowser.request.newContext({
      ignoreHTTPSErrors: true,
      clientCertificates: [{
  ...
  test('should fail with matching certificates in legacy pfx format', async ({ copilotbrowser, startCCServer, asset, browserName }) => {
    const serverURL = await startCCServer();
    const request = await copilotbrowser.request.newContext({
      ignoreHTTPSErrors: true,
      clientCertificates: [{
  ...
  test('should work in the browser with request interception', async ({ browser, copilotbrowser, startCCServer, asset }) => {
    const serverURL = await startCCServer();
    const request = await copilotbrowser.request.newContext({
      ignoreHTTPSErrors: true,
      clientCertificates: [{
...
test.describe('browser', () => {
      ...
      await test.step(`TLS version: ${tlsVersion}`, async () => {
        ...
        const serverURL = await new Promise<string>(resolve => {
          server.listen(0, 'localhost', () => {
            const host = browserName === 'webkit' && platform === 'darwin' ? 'local.copilotbrowser' : 'localhost';
        ...
        await page.goto(serverURL);
        await expect(page.getByText('copilotbrowser client-certificate error: Client network socket disconnected before secure TLS connection was established')).toBeVisible();
        await page.close();
        await new Promise<void>(resolve => server.close(() => resolve()));
      });
    }
  ...
  test('should handle TLS renegotiation with client certificates', async ({ browser, asset, browserName, platform }) => {
    ...
    const origin = 'https://' + (browserName === 'webkit' && platform === 'darwin' ? 'local.copilotbrowser' : 'localhost');
  ...
  test('should have ignoreHTTPSErrors=false by default', async ({ browser, httpsServer, asset, browserName, platform }) => {
    ...
    await page.goto(browserName === 'webkit' && platform === 'darwin' ? httpsServer.EMPTY_PAGE.replace('localhost', 'local.copilotbrowser') : httpsServer.EMPTY_PAGE);
    await expect(page.getByText('copilotbrowser client-certificate error: self-signed certificate')).toBeVisible();
    await page.close();
  });
  ...
  test('support http2', async ({ browser, startCCServer, asset, browserName, isMac }) => {
    ...
    {
      await page.goto(serverURL.replace('127.0.0.1', 'local.copilotbrowser'));
      await expect(page.getByTestId('message')).toHaveText('Sorry, but you need to provide a client certificate to continue.');
      await expect(page.getByTestId('alpn-protocol')).toHaveText('h2');
      await expect(page.getByTestId('servername')).toHaveText('local.copilotbrowser');
    }
    {
      await page.goto(serverURL);
      await expect(page.getByTestId('message')).toHaveText('Hello Alice, your certificate was issued by localhost!');
  ...
  test('should handle rejected certificate in handshake with HTTP/2', async ({ browser, asset, browserName, platform }) => {
    ...
    await new Promise<void>(resolve => server.listen(0, 'localhost', resolve));
    const port = (server.address() as net.AddressInfo).port;
    const serverUrl = 'https://' + (browserName === 'webkit' && platform === 'darwin' ? 'local.copilotbrowser' : 'localhost') + ':' + port;
