...
import type { Page, Frame } from 'copilotbrowser-core';
...
async function generate(pageOrFrame: Page | Frame, target: string, expected?: string): Promise<string> {
  return pageOrFrame.$eval(target, (e, expected) => {
    const copilotbrowser = (window as any).copilotbrowser;
    const selector = copilotbrowser.selector(e);
    const expectedTarget = expected ? copilotbrowser.$(expected) : e;
    if (copilotbrowser.$(selector) === expectedTarget)
      return selector;
...
it.describe('selector generator', () => {
  ...
  it.beforeEach(async ({ context, page }) => {
    ...
    // initialization and copilotbrowser.selector(e) call in `generate()` function above.
  ...
  it('should use data-testid in strict errors', async ({ contextFactory, page, copilotbrowser }) => {
    const content = `
      <div>
        <div></div>
        <div>
    ...
    copilotbrowser.selectors.setTestIdAttribute('data-custom-id');
