...
it('should not contain internal pages', async ({ browserName, contextFactory, server }, testInfo) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/6743' });
...
it('should have security details', async ({ contextFactory, httpsServer, browserName, platform, mode }, testInfo) => {
  it.fail(browserName === 'webkit' && platform === 'linux', 'https://github.com/dayour/copilotbrowser/issues/6759');
  ...
  if (browserName === 'webkit' && platform === 'darwin')
    expect(securityDetails).toEqual({ protocol: 'TLS 1.3', subjectName: 'copilotbrowser-test', validFrom: 1691708270, validTo: 2007068270 });
  else
    expect(securityDetails).toEqual({ issuer: 'copilotbrowser-test', protocol: 'TLS 1.3', subjectName: 'copilotbrowser-test', validFrom: 1691708270, validTo: 2007068270 });

  expect(log.entries[1]._securityDetails).toEqual({ issuer: 'copilotbrowser-test', protocol: 'TLSv1.3', subjectName: 'copilotbrowser-test', validFrom: 1691708270, validTo: 2007068270 });
});
...
it('should return security details directly from response', async ({ contextFactory, httpsServer, browserName, platform, channel }) => {
  it.fail(browserName === 'webkit' && (platform === 'linux' || channel === 'webkit-wsl'), 'https://github.com/dayour/copilotbrowser/issues/6759');
  ...
  if (browserName === 'webkit' && platform === 'win32')
    expect({ ...securityDetails, protocol: undefined }).toEqual({ subjectName: 'copilotbrowser-test', validFrom: 1691708270, validTo: 2007068270 });
...
it('should not hang on resources served from cache', async ({ contextFactory, server, browserName, isBidi }, testInfo) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/11435' });
  server.setRoute('/one-style.css', (req, res) => {
    res.writeHead(200, {
...
it('should not hang on slow chunked response', async ({ browserName, browser, contextFactory, server }, testInfo) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/21182' });
  server.setRoute('/empty.html', (req, res) => {
    res.writeHead(200, {
