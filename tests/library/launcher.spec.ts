...
it('should have an errors object', async ({ copilotbrowser }) => {
  expect(String(copilotbrowser.errors.TimeoutError)).toContain('TimeoutError');
});

it('should have a devices object', async ({ copilotbrowser }) => {
  expect(copilotbrowser.devices['iPhone 6']).toBeTruthy();
  expect(copilotbrowser.devices['iPhone 6'].defaultBrowserType).toBe('webkit');
});
