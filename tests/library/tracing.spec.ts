...
import { artifactsFolderName } from '../../packages/copilotbrowser/src/isomorphic/folders';
...
test('should survive browser.close with auto-created traces dir', async ({ browserType }, testInfo) => {
  ...
  (browserType as any)._copilotbrowser._defaultTracesDir = undefined;
  const browser = await browserType.launch();
  const page = await browser.newPage();
...
test('should store global request traces separately', async ({ request, server, copilotbrowser, browserName, mode }, testInfo) => {
  const request2 = await copilotbrowser.request.newContext();
  await Promise.all([
...
test('should store postData for global request', async ({ request, server }, testInfo) => {
  testInfo.annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/15031' });
  await (request as any)._tracing.start({ snapshots: true });
  const url = server.PREFIX + '/simple.json';
