...
it.describe('screencast', () => {
  ...
  it('should continue recording main page after popup closes', async ({ browser, browserName }, testInfo) => {
    it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/30837' });
    // Firefox does not have a mobile variant and has a large minimum size (500 on windows and 450 elsewhere).
    const size = browserName === 'firefox' ? { width: 500, height: 400 } : { width: 320, height: 240 };
    const context = await browser.newContext({
      recordVideo: {
  ...
  it('should work with weird screen resolution', async ({ browser }, testInfo) => {
    it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/22069' });
    const videosPath = testInfo.outputPath('');
  ...
  it('should work with relative path for recordVideo.dir', async ({ browser }, testInfo) => {
    it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/27086' });
  ...
  it('should work for popups', async ({ browser, server, browserName, trace }, testInfo) => {
    it.fixme(browserName === 'firefox', 'https://github.com/dayour/copilotbrowser/issues/14557');
    const videosPath = testInfo.outputPath('');
    const size = { width: 600, height: 400 };
    const context = await browser.newContext({
      recordVideo: {
  ...
  it('should emulate an iphone', async ({ contextFactory, copilotbrowser, browserName }, testInfo) => {
    it.skip(browserName === 'firefox', 'isMobile is not supported in Firefox');

    const device = copilotbrowser.devices['iPhone 6'];
  ...
  it('should capture full viewport', async ({ browserType, browserName, isWindows, headless, isHeadlessShell }, testInfo) => {
    it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/22411' });
    it.fixme(browserName === 'chromium' && !isHeadlessShell, 'The square is not on the video');
    it.fixme(browserName === 'firefox' && isWindows, 'https://github.com/dayour/copilotbrowser/issues/14405');
    const size = { width: 600, height: 400 };
    const browser = await browserType.launch();
  ...
  it('should capture full viewport on hidpi', async ({ browserType, browserName, headless, isWindows, isLinux, isHeadlessShell, channel }, testInfo) => {
    ...
    it.fixme(browserName === 'chromium' && !isHeadlessShell, 'The square is not on the video');
    it.fixme(browserName === 'firefox' && isWindows, 'https://github.com/dayour/copilotbrowser/issues/14405');
    it.fixme(browserName === 'webkit' && !headless && (isLinux || (isWindows && channel === 'webkit-wsl')), 'https://github.com/dayour/copilotbrowser/issues/22617');
    const size = { width: 600, height: 400 };
    const browser = await browserType.launch();
