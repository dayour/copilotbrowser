...
test('should show empty trace viewer', async ({ showTraceViewer }, testInfo) => {
  ...
  await expect(traceViewer.page).toHaveTitle('copilotbrowser Trace Viewer');
});
...
test('should open two trace viewers', async ({ showTraceViewer }, testInfo) => {
  ...
  await expect(traceViewer1.page).toHaveTitle('copilotbrowser Trace Viewer');
  const traceViewer2 = await showTraceViewer(testInfo.outputPath(), { host: 'localhost', port });
  await expect(traceViewer2.page).toHaveTitle('copilotbrowser Trace Viewer');
});
...
test('should show tracing.group in the action list with location', async ({ runAndTrace, page, context }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/36483' });
...
test('should complain about newer version of trace in old viewer', async ({ showTraceViewer, asset }, testInfo) => {
  ...
  await expect(traceViewer.page.getByText('The trace was created by a newer version of copilotbrowser and is not supported by this version of the viewer.')).toBeVisible();
});
...
test('should have network requests', async ({ showTraceViewer }) => {
  ...
  await expect(traceViewer.networkRequests).toContainText([/script.jsGET200(?:application|text)\/javascript/]);
...
test('should have network request overrides 2', async ({ page, server, runAndTrace }) => {
  ...
  await expect.soft(traceViewer.networkRequests).toContainText([/script.jsGET200(?:application|text)\/javascript.*continued/]);
});
...
test('should work with adopted style sheets and all: unset', async ({ page, runAndTrace, browserName }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/31500' });
  test.fixme(browserName === 'chromium', 'Chromium bug #41416124 — adopted stylesheets with all: unset');
...
test('should work with nesting CSS selectors', async ({ page, runAndTrace }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/31607' });
...
test('should highlight expect failure', async ({ page, server, runAndTrace }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser-python/issues/2258' });
  const traceViewer = await runAndTrace(async () => {
    try {
      await page.goto(server.EMPTY_PAGE);
...
test('should include metainfo', async ({ showTraceViewer }) => {
  ...
  await expect(callLine.getByText('copilotbrowser version')).toHaveText(/copilotbrowser version:[\w.]+/);
...
test('should not crash with broken locator', async ({ page, runAndTrace, server }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/21832' });
  const traceViewer = await runAndTrace(async () => {
    try {
  ...
  await expect(traceViewer.page).toHaveTitle('copilotbrowser Trace Viewer');
  const header = traceViewer.page.getByText('copilotbrowser', { exact: true });
...
test('should display language-specific locators', async ({ runAndTrace, server, page, toImpl }) => {
  toImpl(page).attribution.copilotbrowser.options.sdkLanguage = 'python';
  const traceViewer = await runAndTrace(async () => {
  ...
  toImpl(page).attribution.copilotbrowser.options.sdkLanguage = 'javascript';
});
...
test('should open trace-1.31', async ({ showTraceViewer }) => {
  ...
  await expect(snapshot.locator('[__copilotbrowser_target__]')).toHaveText(['Submit']);
});
...
test('should show correct request start time', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/31133' },
...
test('should not record route actions', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/30970' },
}, async ({ page, runAndTrace, server }) => {
  const traceViewer = await runAndTrace(async () => {
...
test('should show baseURL in metadata pane', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/31847' },
...
test('should not leak recorders', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/33086' },
...
test('canvas clipping', async ({ runAndTrace, page, server }) => {
  ...
  const snapshot = await traceViewer.snapshotFrame('Navigate');
  await expect(snapshot.locator('canvas')).toHaveAttribute('title', `copilotbrowser couldn't capture full canvas contents because it's located partially outside the viewport.`);
});
...
test('should not trip over complex urls in style tags', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35681' },
