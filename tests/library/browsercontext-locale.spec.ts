...
it('should propagate locale to workers', async ({ browser, browserName, server }) => {
  it.fail(browserName === 'firefox', 'https://github.com/dayour/copilotbrowser/issues/38919');
  const context = await browser.newContext({ locale: 'ru-RU' });
  const page = await context.newPage();
  await page.goto(server.EMPTY_PAGE);
...
it('should affect Intl.DateTimeFormat().resolvedOptions().locale', async ({ browser, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/27802' });
