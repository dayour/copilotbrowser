  ...
  return new Promise(resolve => {
    void serverRequest.postBody.then(body => {
      ...
      form.parse(pt as any, (error, rawFields, rawFiles) => {
        // Normalize formidable v3 arrays to single values
        const fields: any = {};
...
const __testHookLookup = (hostname: string): LookupAddress[] => {
  if (hostname === 'localhost' || hostname.endsWith('copilotbrowser.dev'))
...
it('should add session cookies to request', async ({ context, server }) => {
  ...
  const [req] = await Promise.all([
    ...
    context.request.get(`http://www.my.copilotbrowser.dev:${server.PORT}/simple.json`, {
      __testHookLookup
    } as any),
  ]);
...
it('should filter cookies by domain', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/36069' }
}, async ({ context, server }) => {
  await context.addCookies([{
    name: 'first',
    value: '1',
    domain: 'copilotbrowser.dev',
    path: '/',
    expires: -1,
    httpOnly: false,
    secure: false,
  ...
  const [req] = await Promise.all([
    server.waitForRequest('/simple.json'),
    context.request.get(`http://my.copilotbrowser.dev:${server.PORT}/simple.json`, {
      __testHookLookup
    } as any),
  ]);
...
it('should not add context cookie if cookie header passed as a parameter', async ({ context, server }) => {
  ...
  const [req] = await Promise.all([
    ...
    context.request.get(`http://www.my.copilotbrowser.dev:${server.PORT}/empty.html`, {
      headers: {
        'Cookie': 'foo=bar'
      },
      __testHookLookup
...
it('should follow redirects', async ({ context, server }) => {
  ...
  const [req, response] = await Promise.all([
    server.waitForRequest('/simple.json'),
    context.request.get(`http://www.my.copilotbrowser.dev:${server.PORT}/redirect1`, { __testHookLookup } as any),
  ]);
  expect(req.headers.cookie).toEqual('username=John Doe');
  expect(response.url()).toBe(`http://www.my.copilotbrowser.dev:${server.PORT}/simple.json`);
  expect(await response.json()).toEqual({ foo: 'bar' });
});

it('should follow redirects correctly when Location header contains UTF-8 characters', async ({ context, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/30903' });
...
it('should preserve cookie order from Set-Cookie header', async ({ context, page, server, browserName, isLinux }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/23390' });
  server.setRoute('/setcookie.html', (req, res) => {
...
it('should not hang on a brotli encoded Range request', async ({ context, server, nodeVersion }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/18190' });
...
it('should support multipart/form-data', async function({ context, server }) {
  const formReceived = new Promise<{error: any, fields: Fields, files: Record<string, FormidableFile>, serverRequest: IncomingMessage}>(resolve => {
    server.setRoute('/empty.html', async (serverRequest, res) => {
      const form = new IncomingForm();
      const { error, fields, files } = await parseFormFromRequest(serverRequest as any, form);
...
it('should support multipart/form-data and keep the order', async function({ context, page, asset, server }) {
  ...
  const formReceived = new Promise<{error: any, fields: Fields}>(resolve => {
    server.setRoute('/empty.html', async (serverRequest, res) => {
      const form = new IncomingForm();
      const { error, fields } = await parseFormFromRequest(serverRequest as any, form);
      server.serveFile(serverRequest, res);
...
it('should support repeating names in multipart/form-data', async function({ context, server, nodeVersion }) {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/28070' });
...
it('fetch should not throw on long set-cookie value', async ({ context, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/27165' });
  server.setRoute('/empty.html', (req, res) => {
...
it('should update host header on redirect', async ({ context, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/26743' });
  let redirectCount = 0;
  server.setRoute('/redirect', (req, res) => {
    redirectCount++;
