...
it('should serialize storageState with lone surrogates', async ({ page, context, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser-dotnet/issues/2819' });
...
it('should support empty indexedDB', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35760' } }, async ({ page, server, contextFactory }) => {
  await page.goto(server.EMPTY_PAGE);
