...
it('should support clipboard read', async ({ page, context, server, browserName, isWindows, isLinux, headless, isHeadlessShell }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/27475' });
...
it('storage access', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/31227' }
...
it.describe(() => {
  ...
  it('should be able to use the local-fonts API', async ({ page, context, httpsServer, browserName, channel, headless }) => {
    ...
    it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/36113' });
...
it('local network request is allowed from public origin', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/37861' }
...
it('can request screen-wake-lock', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/39115' }
}, async ({ page, context }) => {
