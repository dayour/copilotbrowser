...
it('should not affect screenshots', async ({ page, server, browserName, headless, isWindows, isLinux, isHeadlessShell, channel }) => {
  it.skip(browserName === 'webkit' && isWindows && !headless, 'WebKit/Windows/headed has a larger minimal viewport. See https://github.com/dayour/copilotbrowser/issues/22616');
...
// @see https://github.com/dayour/copilotbrowser/issues/3476
...
browserTest('should not fire blur events when interacting with more than one page/context', async ({ contextFactory, browserName }) => {
  browserTest.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/30399' });
  const page1 = await (await contextFactory()).newPage();
  const page2 = await (await contextFactory()).newPage();
...
browserTest('should trigger hover state concurrently', async ({ browserType, browserName, headless, isBidi }) => {
  browserTest.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/27969' });
