...
it('should have passed URL when launching with ignoreDefaultArgs: true', async ({ browserType, server, createUserDataDir, toImpl, browserName }) => {
  const userDataDir = await createUserDataDir();
  const args = (await toImpl(browserType).defaultArgs((browserType as any)._copilotbrowser._defaultLaunchOptions, 'persistent', userDataDir, 0)).filter(a => a !== 'about:blank');
  const options = {
...
it('should respect selectors', async ({ copilotbrowser, launchPersistent }) => {
  ...
  await copilotbrowser.selectors.register('defaultContextCSS', defaultContextCSS);
...
it('dialog.accept should work', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35663' }
