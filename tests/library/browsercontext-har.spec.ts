...
it('should only handle requests matching url filter', async ({ context, asset }) => {
  ...
  const page = await context.newPage();
  await context.route('http://no.copilotbrowser/', async route => {
    expect(route.request().url()).toBe('http://no.copilotbrowser/');
    await route.fulfill({
      status: 200,
...
it('should apply overrides before routing from har', async ({ context, asset }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/29190' });
  const path = asset('har-fulfill.har');
  await context.routeFromHAR(path, { url: '**/*.js' });
  const page = await context.newPage();
  await context.route('http://no.copilotbrowser/my-script.js', async route => {
    await route.fallback({
      url: 'http://no.copilotbrowser/script2.js',
    });
  });
  await context.route('http://test.example/', async route => {
    await route.fulfill({
      status: 200,
      contentType: 'text/html',
      body: '<script src="http://no.copilotbrowser/my-script.js"></script><div>hello</div>',
    });
  });
...
it('should support regex filter', async ({ context, asset }) => {
  const path = asset('har-fulfill.har');
  await context.routeFromHAR(path, { url: /.*(\.js|.*\.css|no.copilotbrowser\/)$/ });
  const page = await context.newPage();
  await page.goto('http://no.copilotbrowser/');
  expect(await page.evaluate('window.value')).toBe('foo');
  await expect(page.locator('body')).toHaveCSS('background-color', 'rgb(255, 0, 0)');
...
it('should ignore boundary when matching multipart/form-data body', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/31495' }
...
it('should ignore aborted requests', async ({ contextFactory, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/29311' });
