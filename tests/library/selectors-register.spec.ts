...
it('should work', async ({ copilotbrowser, browser }) => {
  // Register one engine before creating context.
  await copilotbrowser.selectors.register('tag', `(${createTagSelector.toString()})()`);

  const context = await browser.newContext();
  // Register another engine after creating context.
  await copilotbrowser.selectors.register('tag2', `(${createTagSelector.toString()})()`);
...
it('should work when registered on global', async ({ browser, mode }) => {
  it.skip(mode === 'driver', 'We expect registering selectors on the right copilotbrowser instance.');

  await require('@copilotbrowser/copilotbrowser/test').selectors.register('oop-tag', `(${createTagSelector.toString()})()`);

  const context = await browser.newContext();
  // Register another engine after creating context.
  await require('@copilotbrowser/copilotbrowser/test').selectors.register('oop-tag2', `(${createTagSelector.toString()})()`);
...
it('should work with path', async ({ copilotbrowser, browser, asset }) => {
  await copilotbrowser.selectors.register('foo', { path: asset('sectionselectorengine.js') });
...
it('should work in main and isolated world', async ({ copilotbrowser, browser }) => {
  ...
  await copilotbrowser.selectors.register('main', createDummySelector);
  await copilotbrowser.selectors.register('isolated', createDummySelector, { contentScript: true });
  const page = await browser.newPage();
...
it('should handle errors', async ({ copilotbrowser, browser }) => {
  const page = await browser.newPage();
  ...
  error = await copilotbrowser.selectors.register('$', createDummySelector).catch(e => e);
  expect(error.message).toBe('selectors.register: Selector engine name may only contain [a-zA-Z0-9_] characters');

  // Selector names are case-sensitive.
  await copilotbrowser.selectors.register('dummy', createDummySelector);
  await copilotbrowser.selectors.register('duMMy', createDummySelector);

  error = await copilotbrowser.selectors.register('dummy', createDummySelector).catch(e => e);
  expect(error.message).toBe('selectors.register: "dummy" selector engine has been already registered');

  error = await copilotbrowser.selectors.register('css', createDummySelector).catch(e => e);
  expect(error.message).toBe('selectors.register: "css" is a predefined selector engine');
  await page.close();
});

it('should throw "already registered" error when registering', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/36467' } }, async ({ copilotbrowser }) => {
  await copilotbrowser.selectors.register('alreadyRegistered', createTagSelector);
  await expect(copilotbrowser.selectors.register('alreadyRegistered', createTagSelector)).rejects.toThrowError('selectors.register: "alreadyRegistered" selector engine has been already registered');
});

it('should not rely on engines working from the root', async ({ copilotbrowser, browser }) => {
  ...
  await copilotbrowser.selectors.register('__value', createValueEngine);
  const page = await browser.newPage();
...
it('should throw a nice error if the selector returns a bad value', async ({ copilotbrowser, browser }) => {
  ...
  await copilotbrowser.selectors.register('__fake', createFakeEngine);
  const page = await browser.newPage();
