...
it('should isolated contexts that share network process', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35870' }
...
it('serviceWorker should intercept document request', async ({ browser, httpsServer, browserName }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/27768' });
  it.fixme(browserName === 'chromium', 'SSL cert error when registering service worker with ignoreHTTPSErrors');
  const context = await browser.newContext({ ignoreHTTPSErrors: true });
  const page = await context.newPage();
