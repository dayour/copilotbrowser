...
const it = copilotbrowserTest.extend<GlobalFetchFixtures>({
...
it('should store cookie from Set-Cookie header even if it contains equal signs', async ({ request, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/11612' });
...
it('should preserve local storage on import/export of storage state', async ({ copilotbrowser, server }) => {
  ...
  const request = await copilotbrowser.request.newContext({ storageState });
...
it('should send cookies from storage state', async ({ copilotbrowser, server }) => {
...
it('storage state should round-trip through file', async ({ copilotbrowser, server }, testInfo) => {
  const storageState: StorageStateType = {
    'cookies': [
      {
        'name': 'a',
  ...
  const request1 = await copilotbrowser.request.newContext({ storageState });
  ...
  const request2 = await copilotbrowser.request.newContext({ storageState: path });
  const state2 = await request2.storageState();
  expect(state2).toEqual(storageState);
});

it('should work with empty storage state', async ({ copilotbrowser, server }, testInfo) => {
