...
// formidable v3 parse() awaits writeHeaders() before subscribing to 'data' events.
// The test server's postBody listener switches the IncomingMessage to flowing mode immediately,
// so data is consumed before formidable subscribes. Fix: await postBody accumulation,
// then feed data into a PassThrough AFTER formidable has subscribed (via setImmediate).
// Also normalizes formidable v3 array fields/files to single values (v1/v2 compat).
function parseFormFromRequest(serverRequest: IncomingMessage & { postBody: Promise<Buffer> }, form: InstanceType<typeof IncomingForm>): Promise<{error: any, fields: Fields, files: Record<string, FormidableFile>}> {
  return new Promise(resolve => {
    void serverRequest.postBody.then(body => {
      const pt = new PassThrough();
      (pt as any).headers = serverRequest.headers;
      form.parse(pt as any, (error, rawFields, rawFiles) => {
        const fields: any = {};
        for (const [k, v] of Object.entries(rawFields))
          fields[k] = Array.isArray(v) ? v[0] : v;
        const files: any = {};
        for (const [k, v] of Object.entries(rawFiles))
          files[k] = Array.isArray(v) ? v[0] : v;
        resolve({ error, fields: fields as Fields, files: files as Record<string, FormidableFile> });
      });
      setImmediate(() => {
        pt.write(body);
        pt.end();
...
const test = copilotbrowserTest.extend<ExtraFixtures>({
  connect: async ({ browserType }, use) => {
    ...
    await use(async (wsEndpoint, options = {}, redirectPortForTest): Promise<Browser> => {
      ...
      options.headers = {
        'x-copilotbrowser-launch-options': JSON.stringify((browserType as any)._copilotbrowser._defaultLaunchOptions || {}),
        ...options.headers,
      };
...
for (const kind of ['launchServer', 'run-server'] as const) {
  test.describe(kind, () => {
    ...
    test('should connect over wss', async ({ connect, startRemoteServer, httpsServer }) => {
      ...
      try {
        httpsServer.onceWebSocketConnection((ws, request) => {
          const headers = Object.fromEntries(Object.entries(request.headers).filter(entry => entry[0].startsWith('x-copilotbrowser')));
    ...
    test('should ignore page.pause when headed', async ({ connect, startRemoteServer, browserType, channel }) => {
      ...
      const headless = (browserType as any)._copilotbrowser._defaultLaunchOptions.headless;
      (browserType as any)._copilotbrowser._defaultLaunchOptions.headless = false;
      const remoteServer = await startRemoteServer(kind);
      const browser = await connect(remoteServer.wsEndpoint());
      const browserContext = await browser.newContext();
      const page = await browserContext.newPage();
      ...
      await browser.close();
      (browserType as any)._copilotbrowser._defaultLaunchOptions.headless = headless;
    });
    ...
    test('should send extra headers with connect request', async ({ connect, server }) => {
      const [request] = await Promise.all([
        ...
        connect(`ws://localhost:${server.PORT}/ws`, {
          headers: {
            'User-Agent': 'copilotbrowser',
            'foo': 'bar',
          },
          timeout: 100,
        }).catch(() => {})
      ]);
      expect(request.headers['user-agent']).toBe('copilotbrowser');
      expect(request.headers['foo']).toBe('bar');
    });

    test('should send default User-Agent and X-copilotbrowser-Browser headers with connect request', async ({ connect, browserName, server }) => {
      const [request] = await Promise.all([
        server.waitForWebSocketConnectionRequest(),
        connect(`ws://localhost:${server.PORT}/ws`, {
          headers: {
      ...
      expect(request.headers['x-copilotbrowser-browser']).toBe(browserName);
      expect(request.headers['foo']).toBe('bar');
    });
    ...
    test('should respect selectors', async ({ copilotbrowser, connect, startRemoteServer }) => {
      const remoteServer = await startRemoteServer(kind);
      ...
      await copilotbrowser.selectors.register(mycss1, mycss);

      const browser1 = await connect(remoteServer.wsEndpoint());
      const context1 = await browser1.newContext();

      // Register another engine after creating context.
      await copilotbrowser.selectors.register(mycss2, mycss);
      ...
      await copilotbrowser.selectors.register(mycss3, mycss);
    ...
    test('should saveAs videos from remote browser', async ({ connect, startRemoteServer }, testInfo) => {
      ...
      test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/36685' });
    ...
    test('should filter launch options', async ({ connect, startRemoteServer, server, browserType }, testInfo) => {
      ...
      const oldTracesDir = (browserType as any)._copilotbrowser._defaultTracesDir;
      (browserType as any)._copilotbrowser._defaultTracesDir = tracesDir;
      const remoteServer = await startRemoteServer(kind);
      const browser = await connect(remoteServer.wsEndpoint());
      const page = await browser.newPage();
      await page.goto(server.EMPTY_PAGE);
      await browser.close();
      (browserType as any)._copilotbrowser._defaultTracesDir = oldTracesDir;
    ...
    test('should fulfill with global fetch result', async ({ connect, startRemoteServer, copilotbrowser, server }) => {
      const remoteServer = await startRemoteServer(kind);
      const browser = await connect(remoteServer.wsEndpoint());
      const context = await browser.newContext();
      const page = await context.newPage();
    ...
    test('should upload large file', async ({ connect, startRemoteServer, server }, testInfo) => {
      ...
      const serverFilePromise = new Promise<FormidableFile>(fulfill => {
        server.setRoute('/upload', async (req, res) => {
          const form = new IncomingForm({ uploadDir: testInfo.outputPath() });
          const { files } = await parseFormFromRequest(req as any, form);
          res.end();
          fulfill(files.file1);
        });
      });
    ...
    test('setInputFiles should preserve lastModified timestamp', async ({ connect, startRemoteServer, asset }) => {
      test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/27452' });
      const remoteServer = await startRemoteServer(kind);
      const browser = await connect(remoteServer.wsEndpoint());
      const context = await browser.newContext();
      const page = await context.newPage();
    ...
    test.describe('socks proxy', () => {
      ...
      test('should proxy local.copilotbrowser requests', async ({ connect, server, dummyServerPort, startRemoteServer }, workerInfo) => {
        let reachedOriginalTarget = false;
        server.setRoute('/foo.html', async (req, res) => {
          reachedOriginalTarget = true;
          res.end('<html><body></body></html>');
        ...
        const browser = await connect(remoteServer.wsEndpoint(), { exposeNetwork: '*' }, dummyServerPort);
        const page = await browser.newPage();
        await page.goto(`http://local.copilotbrowser:${examplePort}/foo.html`);
        expect(await page.content()).toContain('from-dummy-server');
        expect(reachedOriginalTarget).toBe(false);
      });
      ...
      test('should check proxy pattern on the client', async ({ connect, startRemoteServer, server, dummyServerPort }) => {
        ...
        const browser = await connect(remoteServer.wsEndpoint(), {
          ...
          headers: {
            'x-copilotbrowser-proxy': '*',
          },
        }, dummyServerPort);
        const page = await browser.newPage();

        // local.copilotbrowser should fail on the client side.
        let failed = false;
        await page.goto(`http://local.copilotbrowser:${server.PORT}/foo.html`).catch(e => {
          failed = true;
        });
        expect(failed).toBe(true);
        expect(reachedOriginalTarget).toBe(false);
...
test('should refuse connecting when versions do not match', async ({ connect, childProcess }) => {
  ...
  expect(error.message).toContain('client version: v' + getcopilotbrowserVersion(true));
});
