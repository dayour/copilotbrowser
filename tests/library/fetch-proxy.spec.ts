...
it('global request should pick up proxy credentials', async ({ copilotbrowser, server, proxyServer }) => {
  proxyServer.forwardTo(server.PORT, { allowConnectRequests: true });
  let auth;
  proxyServer.setAuthHandler(req => {
...
it('should use socks proxy', async ({ copilotbrowser, server, socksPort }) => {
  it.skip(!!process.env.INSIDE_DOCKER, 'connect ECONNREFUSED 127.0.0.1:<port>');

  const request = await copilotbrowser.request.newContext({ proxy: {
...
it('should send correct ALPN protocol to HTTPS proxy', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/37676' } }, async ({ copilotbrowser, server, nodeVersion }) => {
