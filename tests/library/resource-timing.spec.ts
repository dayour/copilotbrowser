...
it('should work for subresource', async ({ contextFactory, server, browserName, platform }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/20654' });
...
it('should work when serving from memory cache', async ({ contextFactory, server, browserName, isBidi }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser-java/issues/1080' });
