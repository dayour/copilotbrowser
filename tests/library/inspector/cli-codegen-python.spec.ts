...
def run(copilotbrowser: copilotbrowser) -> None:
    browser = copilotbrowser.${browserName}.launch(${launchOptions(channel)})
...
def run(copilotbrowser: copilotbrowser) -> None:
    browser = copilotbrowser.chromium.launch(${launchOptions(channel)})
    context = browser.new_context(**copilotbrowser.devices["Pixel 2"])`;
  await cli.waitFor(expectedResult);
});
...
def run(copilotbrowser: copilotbrowser) -> None:
    browser = copilotbrowser.webkit.launch(${launchOptions(channel)})
    context = browser.new_context(**copilotbrowser.devices["iPhone 11"], color_scheme="light")`;
  await cli.waitFor(expectedResult);
});
