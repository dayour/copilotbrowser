...
async def run(copilotbrowser: copilotbrowser) -> None:
    browser = await copilotbrowser.${browserName}.launch(${launchOptions(channel)})
...
async def run(copilotbrowser: copilotbrowser) -> None:
    browser = await copilotbrowser.chromium.launch(${launchOptions(channel)})
    context = await browser.new_context(**copilotbrowser.devices["Pixel 2"])`;
  await cli.waitFor(expectedResult);
});
...
async def run(copilotbrowser: copilotbrowser) -> None:
    browser = await copilotbrowser.webkit.launch(${launchOptions(channel)})
    context = await browser.new_context(**copilotbrowser.devices["iPhone 11"], color_scheme="light")`;
  await cli.waitFor(expectedResult);
});
...
test('should work with --save-har', async ({ runCLI }, testInfo) => {
  ...
  expect(json.log.creator.name).toBe('copilotbrowser');
});
