...
it.describe('pause', () => {
  ...
  it('should resume from console', async ({ page }) => {
    ...
    await page.waitForFunction(() => (window as any).copilotbrowser && (window as any).copilotbrowser.resume() !== false);
    await scriptPromise;
  });
  ...
  it('should highlight locators with custom testId', async ({ page, copilotbrowser, recorderPageGetter }) => {
  ...
  it('should record from debugger', async ({ page, recorderPageGetter }) => {
    ...
    await expect(recorderPage.getByRole('combobox', { name: 'Source chooser' })).toHaveValue('copilotbrowser-test');
