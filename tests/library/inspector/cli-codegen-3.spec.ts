...
await page.GetByTestId("testid").HoverAsync();`);
  ...
  test('should record when top layer popover is open', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/39095' } }, async ({ openRecorder }) => {
    const { page, recorder } = await openRecorder();
  ...
  test('should record when top layer popover inside shadow DOM is open', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/39095' } }, async ({ openRecorder }) => {
    const { page, recorder } = await openRecorder();
  ...
  test('should record when manual popover with fullscreen backdrop is open', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/39095' } }, async ({ openRecorder }) => {
    const { page, recorder } = await openRecorder();
    ...
    // Difference: the dialog is not centered, because copilotbrowser recorder has trouble clicking on backdrops with centered occlusion.
    await recorder.setContentAndWait(`
      <style>
  ...
  test('should record when fullscreen element is open', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/39095' } }, async ({ openRecorder }) => {
    const { page, recorder } = await openRecorder();
