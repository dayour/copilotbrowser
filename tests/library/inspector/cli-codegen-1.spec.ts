...
await page.GetByRole(AriaRole.Button, new() { Name = "Submit" }).DblClickAsync();`);
  ...
  test('should ignore programmatic events', async ({ openRecorder }) => {
    ...
    const clicks = recorder.sources().get('copilotbrowser Test')!.actions!.filter(l => l.includes('Submit'));
...
await page.Locator("#input").FillAsync(\"てすと\");`);
  ...
  test('should fill textarea with new lines at the end', async ({ openRecorder }) => {
    test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/23774' });
    const { page, recorder } = await openRecorder();
...
await page.Locator(\"#age\").SelectOptionAsync(new[] { \"2\" });`);
  ...
  test('should await popup', async ({ openRecorder, server }) => {
    test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/36461' });
...
await page1.GetByRole(AriaRole.Button, new() { Name = \"Click me\" }).ClickAsync();`);
  ...
  await context.close();`);
    ...
    expect.soft(sources.get('copilotbrowser Test')!.text).toContain(`
  await page.goto('about:blank');
  await page.getByText('link').click();
});`);
...
await page.GetByRole(AriaRole.Textbox).FillAsync("h");`);
  ...
  test('should ignore AltGraph', async ({ openRecorder, browserName }) => {
    ...
    await recorder.page.type('input', 'copilotbrowser');
    ...
    expect(recorder.sources().get('JavaScript')!.text).toContain(`await page.getByRole('textbox').fill('copilotbrowser@example.com');`);
  });
...
await page.GetByRole(AriaRole.Slider).FillAsync("10");`);
  ...
  test('should click button with nested div', async ({ openRecorder }) => {
    test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/29067' });
...
await page.GetByRole(AriaRole.Button, new() { Name = "Submit" }).ClickAsync();`);
  ...
  test('should clear when recording is disabled', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/33802' } }, async ({ openRecorder }) => {
    const { recorder } = await openRecorder();
