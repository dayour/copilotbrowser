...
test('should print the correct imports and context options', async ({ browserName, channel, runCLI, server }) => {
  ...
  const expectedResult = `using Microsoft.copilotbrowser;
using System;
using System.Threading.Tasks;

using var copilotbrowser = await copilotbrowser.CreateAsync();
await using var browser = await copilotbrowser.${capitalize(browserName)}.LaunchAsync(new()
{
...
var context = await browser.NewContextAsync(copilotbrowser.Devices["Pixel 2"]);`;
  await cli.waitFor(expectedResult);
});
...
test(`should print a valid basic program in mstest`, async ({ runCLI, server }) => {
  ...
  await cli.waitFor(`Page.GotoAsync("${server.EMPTY_PAGE}")`);
  const expected = `using Microsoft.copilotbrowser.MSTest;
...
test(`should print a valid basic program in nunit`, async ({ runCLI, server }) => {
  ...
  await cli.waitFor(`Page.GotoAsync("${server.EMPTY_PAGE}")`);
  const expected = `using Microsoft.copilotbrowser.NUnit;
using Microsoft.copilotbrowser;
