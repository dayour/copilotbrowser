...
test('should print the correct imports and context options', async ({ runCLI, channel, browserName, server }) => {
  ...
  const expectedResult = `import com.microsoft.copilotbrowser.*;
import com.microsoft.copilotbrowser.options.*;
...
public class Example {
  public static void main(String[] args) {
    try (copilotbrowser copilotbrowser = copilotbrowser.create()) {
      Browser browser = copilotbrowser.${browserName}().launch(new BrowserType.LaunchOptions()
...
test('should print the correct imports in junit', async ({ runCLI, server }) => {
  ...
  const expectedImportResult = `import com.microsoft.copilotbrowser.junit.Usecopilotbrowser;
import com.microsoft.copilotbrowser.Page;
import com.microsoft.copilotbrowser.options.*;

import org.junit.jupiter.api.*;
import static com.microsoft.copilotbrowser.assertions.copilotbrowserAssertions.*;`;
  await cli.waitFor(expectedImportResult);
});

test('should print a valid basic program in junit', async ({ runCLI, server }) => {
  const cli = runCLI(['--target=java-junit', server.EMPTY_PAGE]);
  const expectedResult = `import com.microsoft.copilotbrowser.junit.Usecopilotbrowser;
import com.microsoft.copilotbrowser.Page;
import com.microsoft.copilotbrowser.options.*;
