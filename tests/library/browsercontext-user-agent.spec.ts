...
it('should emulate device user-agent', async ({ browser, server, copilotbrowser }) => {
  {
    const context = await browser.newContext();
    const page = await context.newPage();
  ...
  {
    const context = await browser.newContext({ userAgent: copilotbrowser.devices['iPhone 6'].userAgent });
    const page = await context.newPage();
    await page.goto(server.PREFIX + '/mobile.html');
...
it('custom user agent for download', async ({ server, contextFactory, browserVersion, browserName }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/22843' });
...
it('should work for navigator.userAgentData and sec-ch-ua headers', async ({ copilotbrowser, browserName, browser, server }) => {
  ...
  {
    const context = await browser.newContext(copilotbrowser.devices['Pixel 7']);
    const page = await context.newPage();
    const [request] = await Promise.all([
      server.waitForRequest('/empty.html'),
      page.goto(server.EMPTY_PAGE),
