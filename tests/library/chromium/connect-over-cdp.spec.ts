...
test('should use logger in default context', async ({ browserType }, testInfo) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/28813' });
  const port = 9339 + testInfo.workerIndex;
  const browserServer = await browserType.launch({
    args: ['--remote-debugging-port=' + port]
  });
...
test('should keep URL parameters when adding json/version', {
  annotation: {
    type: 'issue',
    description: 'https://github.com/dayour/copilotbrowser/issues/36097'
  }
...
test('should append /json/version with a slash if there isnt one', {
  annotation: {
    type: 'issue',
    description: 'https://github.com/dayour/copilotbrowser/issues/36378'
  }
}, async ({ browserType, server }) => {
  await Promise.all([
    server.waitForRequest('/browser/json/version/?foo=bar'),
...
test('emulate media should not be affected by second connectOverCDP', async ({ browserType }, testInfo) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/24109' });
  test.fixme(true, 'Media emulation state not isolated between connectOverCDP sessions');
  const port = 9339 + testInfo.workerIndex;
  const browserServer = await browserType.launch({
    args: ['--remote-debugging-port=' + port]
  });
