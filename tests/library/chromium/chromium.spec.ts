...
import { copilotbrowserTest } from '../../config/browserTest';
...
copilotbrowserTest('should close service worker together with the context', async ({ browserType, server }) => {
...
copilotbrowserTest('should pass args with spaces', async ({ browserType, createUserDataDir }, testInfo) => {
...
test('should intercept service worker update requests', async ({ context, page, server }) => {
  test.fixme(true, 'Service worker update interception times out — issue #14711');
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/14711' });
...
test('copilotbrowser_DISABLE_SERVICE_WORKER_NETWORK', async ({ mode, context, page, server }) => {
  test.skip(mode !== 'default', 'no env in non-default mode');
  process.env.copilotbrowser_DISABLE_SERVICE_WORKER_NETWORK = '1';
  ...
  delete process.env.copilotbrowser_DISABLE_SERVICE_WORKER_NETWORK;
});
