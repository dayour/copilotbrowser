...
it('should get the proper viewport', async ({ page, browser, server }) => {
  it.fixme(true, 'OOPIF viewport not propagated — screen.width returns 800 instead of 1280');
...
it('should support context options', async ({ browser, server, copilotbrowser }) => {
...
// @see https://github.com/dayour/copilotbrowser/issues/1240
...
it('should be able to click in iframe', async ({ page, server, browser }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/28023' });
  await page.goto(server.PREFIX + '/dynamic-oopif.html');
  await assertOOPIFCount(browser, 1);
  expect(page.frames().length).toBe(2);
  const frame = page.frames()[1];
...
it('should intercept response body from oopif', async function({ page, browser, server }) {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/20809' });
...
it('should allow to re-connect to OOPIFs with CDP when iframes were there already', async ({ browserType, server, browserMajorVersion }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/36095' });
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/17656' });
