...
it('should click when viewport size is larger than screen', async ({ page }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/22082' });
...
it('should not override viewport size when passed null', async function({ browserName, server, browser }) {
  ...
  const [popup] = await Promise.all([
    ...
    page.evaluate(() => {
      window.open(window.location.href, 'Title', 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=600,height=300,top=0,left=0');
    }),
  ]);
  await popup.waitForLoadState();
  await popup.evaluate(() => window.resizeTo(500, 450));
  // Use tolerant matching: Windows DWM and Edge's window chrome can round
  // outer dimensions by a few pixels from the requested value.
  await popup.waitForFunction(() => Math.abs(window.outerWidth - 500) <= 15 && Math.abs(window.outerHeight - 450) <= 15);
  await context.close();
});
...
it('headless and headful should use same default fonts', async ({ page, browserName, browserType, channel, isBidi }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/11177' });
  it.skip(browserName === 'firefox' && !isBidi, 'Text is misaligned in headed vs headless');
  it.skip(!!channel, 'Channel builds (e.g. msedge) render fonts differently in headed vs headless mode');
  ...
  const headlessBrowser = await browserType.launch({ headless: true, channel });
