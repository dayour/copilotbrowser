...
for (const scenario of ['launch', 'connect'] as const) {
  test.describe('reuse ' + scenario, () => {
    ...
    test('should not cache resources', async ({ reusedContext, server }) => {
      test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/19926' });
    ...
    test('should ignore binding from beforeunload', async ({ reusedContext }) => {
      test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/22803' });
    ...
    test('should reset mouse position', {
      annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/22432' },
    ...
    test('should continue issuing events after closing the reused page', async ({ reusedContext, server }) => {
      test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/24574' });
    ...
    test('should work with routeWebSocket', {
      annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/36480' },
