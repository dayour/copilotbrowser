...
import { test as baseTest, expect } from './copilotbrowser-test-fixtures';
import { RunServer } from '../config/remoteServer';
import type { copilotbrowserServer } from '../config/remoteServer';

const test = baseTest.extend<{ runServer: () => Promise<copilotbrowserServer> }>({
  runServer: async ({ childProcess }, use) => {
    let server: copilotbrowserServer | undefined;
    await use(async () => {
...
test('should reuse browser', async ({ runInlineTest, runServer }) => {
  ...
  const result = await runInlineTest({
    ...
    `,
  }, { workers: 2 }, { CB_TEST_REUSE_CONTEXT: '1', CB_TEST_CONNECT_WS_ENDPOINT: server.wsEndpoint() });
...
test('should produce correct test steps', async ({ runInlineTest, runServer }) => {
  ...
  const result = await runInlineTest({
    ...
    `,
  }, { reporter: './reporter.ts,list' }, { CB_TEST_REUSE_CONTEXT: '1', CB_TEST_CONNECT_WS_ENDPOINT: server.wsEndpoint() });
  ...
  expect(result.outputLines).toEqual([
    ...
    'onStepBegin [cb:api] connect',
    'onStepEnd [cb:api] connect',
    ...
    'onStepBegin [cb:api] Create page',
    'onStepEnd [cb:api] Create page',
    'onStepEnd [fixture] Fixture "page"',
    'onStepEnd [hook] Before Hooks',
    'onStepBegin [cb:api] Navigate to "about:blank"',
    'onStepEnd [cb:api] Navigate to "about:blank"',
    'onStepBegin [cb:api] Evaluate',
    'onStepEnd [cb:api] Evaluate',
