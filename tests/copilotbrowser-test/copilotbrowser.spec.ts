...
test('should respect context options in various contexts', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    ...
    'a.test.ts': `
      ...
      test('persistent context', async ({ copilotbrowser, browserName }) => {
        const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'user-data-dir-'));
        const context = await copilotbrowser[browserName].launchPersistentContext(dir);
      ...
      test('another browser', async ({ copilotbrowser, browserName }) => {
...
test('should use actionTimeout for APIRequestContext', async ({ runInlineTest, server }) => {
  ...
  const result = await runInlineTest({
    ...
    'a.test.ts': `
      ...
      test('newly created APIRequestContext without options', async ({ copilotbrowser }) => {
        const apiRequestContext = await copilotbrowser.request.newContext();
        await expect(apiRequestContext.get('/stall')).rejects.toThrow('apiRequestContext.get: Timeout 1111ms exceeded');
      });
      test('newly created APIRequestContext with options', async ({ copilotbrowser }) => {
        const apiRequestContextWithOptions = await copilotbrowser.request.newContext({ httpCredentials: { username: 'user', password: 'pass' } });
...
test('should save trace in two APIRequestContexts', async ({ runInlineTest, server }) => {
  const result = await runInlineTest({
    ...
    `,
    'a.test.ts': `
      import { test, request, BrowserContext, Page, APIRequestContext } from '@copilotbrowser/copilotbrowser/test';
...
test('window.copilotbrowser should be undefined by default', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'a.test.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
      ...
      test('test', async ({ page }) => {
        ...
        expect(await page.evaluate(() => window.copilotbrowser)).toBeUndefined();
      });
    `,
  }, {}, {});
  expect(result.exitCode).toBe(0);
...
test('window.copilotbrowser should not override existing property', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'a.test.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';

      test('test', async ({ page }) => {
        await page.setContent('<script>window.copilotbrowser = "foo"</script>');
        expect(await page.evaluate(() => window.copilotbrowser)).toBe('foo');
      });
    `,
  }, {}, { PWDEBUG: 'console' });
  expect(result.exitCode).toBe(0);
...
test('PWDEBUG=console should opt-in to exposing window.copilotbrowser', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'a.test.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
      ...
      test('test', async ({ page }) => {
        await page.setContent('<body></body>');
        expect(await page.evaluate(() => window.copilotbrowser)).toBeDefined();
      });
    `,
  }, {}, { PWDEBUG: 'console' });
  expect(result.exitCode).toBe(0);
...
test('init script should not observe copilotbrowser internals', async ({ server, runInlineTest }) => {
  test.skip(!!process.env.CB_CLOCK, 'clock installs globalThis.__pwClock');
  const result = await runInlineTest({
    'a.test.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
      ...
      test('test', async ({ page }) => {
        await page.addInitScript(() => {
          window['check'] = () => {
            ...
            return keys.find(name => name.includes('copilotbrowser') || name.includes('_pw')) || 'none';
          };
