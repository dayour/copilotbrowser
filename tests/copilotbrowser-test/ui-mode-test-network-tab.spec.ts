...
test('should not duplicate network entries from beforeAll', {
  annotation: [
    { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/34404' },
    { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/33106' },
  ]
  ...
  const { page } = await runUITest({
    ...
    `,
    'a.spec.ts': `
      import { test as base, expect, request, type APIRequestContext } from '@copilotbrowser/copilotbrowser/test';
...
test('should copy network request', async ({ runUITest, server }) => {
  ...
  await page.getByRole('button', { name: 'Copy request' }).hover();
  await page.getByRole('button', { name: 'Copy as copilotbrowser' }).click();
  await expect(async () => {
    const copilotbrowserRequest = await page.evaluate(() => (window as any).__clipboardCall);
    expect(copilotbrowserRequest).toContain(`await page.request.post('${server.PREFIX}/post-data-1', {`);
    expect(copilotbrowserRequest.replaceAll('\r\n', '\n')).toContain(`  data: \`{
  "data": {
    "key": "value",
    "array": [
      "value-1",
    ...
    expect(copilotbrowserRequest).toContain(`'content-type': 'application/json'`);
  }).toPass();
});
