...
test('test.extend options should check types', async ({ runTSC }) => {
  const result = await runTSC({
    'helper.ts': `
      import { test as base, expect, mergeTests } from '@copilotbrowser/copilotbrowser/test';
...
test('calling custom matcher on expect.poll should return Promise', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35635' } }, async ({ runTSC }) => {
  const result = await runTSC({
    'a.spec.ts': `
