...
const test = baseTest.extend<{ showReport: (reportFolder?: string) => Promise<void> }>({
  showReport: async ({ page }, use, testInfo) => {
    ...
    await use(async (reportFolder?: string) => {
      reportFolder ??=  testInfo.outputPath('copilotbrowser-report');
  ...
  test.describe(`${useIntermediateMergeReport ? 'merged' : 'created'}`, () => {
    ...
    test('should generate report', async ({ runInlineTest, showReport, page }) => {
      await runInlineTest({
        ...
        `,
      }, { reporter: 'dot,html', retries: 1 }, { copilotbrowser_HTML_OPEN: 'never' });
    ...
    test('should allow navigating to testId=test.id', async ({ runInlineTest, page, showReport }) => {
      const result = await runInlineTest({
        ...
        `,
        // Note: using CB_TEST_HTML_REPORT_OPEN to test backwards compatibility.
      }, { reporter: 'dot,html' }, { CB_TEST_HTML_REPORT_OPEN: 'never' });
    ...
    test('should not throw when copilotbrowser_HTML_OPEN value is invalid', async ({ runInlineTest, page, showReport }, testInfo) => {
      ...
      const result = await runInlineTest({
        ...
        `,
      }, { reporter: 'dot,html' }, { copilotbrowser_HTML_OPEN: invalidOption });
      expect(result.exitCode).toBe(0);
      expect(result.passed).toBe(1);
    });
    ...
    test('should not throw when attachment is missing', async ({ runInlineTest, page, showReport }, testInfo) => {
      const result = await runInlineTest({
        ...
        `,
      }, { reporter: 'dot,html' }, { copilotbrowser_HTML_OPEN: 'never' });
      expect(result.exitCode).toBe(0);
      expect(result.passed).toBe(1);
    ...
    test('should use different path if attachments base url option is provided', async ({ runInlineTest, page, showReport }, testInfo) => {
      const result = await runInlineTest({
        ...
        `
      }, {}, { copilotbrowser_HTML_OPEN: 'never' });
      expect(result.exitCode).toBe(0);
      expect(result.passed).toBe(1);
    ...
    test('should process URLs as links in report title', async ({ runInlineTest, page, showReport }, testInfo) => {
      const result = await runInlineTest({
        'copilotbrowser.config.ts': `
          module.exports = {
            reporter: [['html', { title: 'Custom report title https://copilotbrowser.dev separator http://microsoft.com end' }], ['line']]
          };
        `,
        'a.test.js': `
          import { test, expect } from '@copilotbrowser/copilotbrowser/test';
      ...
      await expect(page.locator('.header-title')).toHaveText('Custom report title https://copilotbrowser.dev separator http://microsoft.com end');
      await expect(anchorLocator).toHaveCount(2);
      await expect(anchorLocator.nth(0)).toHaveAttribute('href', 'https://copilotbrowser.dev');
      ...
      const result = await runInlineTest({
        ...
        'global-teardown.js': `
          ...
          export default async (config) => {
            ...
            process.env.copilotbrowser_HTML_TITLE = 'Omega Star Test Suite (Version: ' + apiVersion + ')';
          };
        `,
      }, { reporter: 'dot,html' }, { copilotbrowser_HTML_OPEN: 'never' });
      expect(result.exitCode).toBe(0);
    ...
    test('should include view trace button in header', async ({ runInlineTest, server, page, showReport }) => {
      const result = await runInlineTest({
        ...
        'a.test.js': `
          import { test, expect } from '@copilotbrowser/copilotbrowser/test';
          test('passes', async ({ copilotbrowser, page }) => {
            await page.evaluate('2 + 2');
    ...
    test('trace should not hang when showing parallel api requests', async ({ runInlineTest, page, server, showReport }) => {
      const result = await runInlineTest({
        ...
        `,
        'a.test.js': `
          import { test, expect, request } from '@copilotbrowser/copilotbrowser/test';
        ...
        `,
      }, { reporter: 'html,dot' }, { copilotbrowser_HTML_OPEN: 'never' });
      expect(result.exitCode).toBe(0);
      expect(result.passed).toBe(1);
    ...
    test('should warn user when viewing via file:// protocol', async ({ runInlineTest, page, showReport }, testInfo) => {
      ...
      await test.step('view via local file://', async () => {
        const reportFolder = testInfo.outputPath('copilotbrowser-report');
    ...
    test('should not crash on falsy non-string annotation', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35469' } }, async ({ runInlineTest, page, showReport }) => {
      const result = await runInlineTest({
        'copilotbrowser.config.js': `
          module.exports = { timeout: 1500 };
        `,
    ...
    test('should have link for opening HTML attachments in new tab', async ({ runInlineTest, page, showReport }) => {
      test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/32281' });
      test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35489' });
    ...
    test('should use file-browser friendly extensions for buffer attachments based on contentType', async ({ runInlineTest, showReport, page }, testInfo) => {
      ...
      for (const [visibleAttachmentName, downloadFileName, sha1] of expectedAttachments) {
        await test.step(`should download ${visibleAttachmentName}`, async () => {
          ...
          expect(await readAllFromStream(await download.createReadStream())).toEqual(await fs.promises.readFile(path.join(testInfo.outputPath('copilotbrowser-report'), 'data', sha1)));
        });
      }

      const files = await fs.promises.readdir(path.join(testInfo.outputPath('copilotbrowser-report'), 'data'));
    ...
    test('should differentiate repeat-each test cases', async ({ runInlineTest, showReport, page }) => {
      test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/10859' });
      const result = await runInlineTest({
        'a.spec.js': `
          import { test, expect } from '@copilotbrowser/copilotbrowser/test';
        ...
        `
      }, { 'reporter': 'dot,html', 'repeat-each': 3 }, { copilotbrowser_HTML_OPEN: 'never' });
      expect(result.exitCode).toBe(1);
      await showReport();
    ...
    test('should group similar / loop steps', async ({ runInlineTest, showReport, page }) => {
      test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/10098' });
      const result = await runInlineTest({
        'a.spec.js': `
          import { test, expect } from '@copilotbrowser/copilotbrowser/test';
          test('sample', async ({}, testInfo) => {
    ...
    test('open tests from required file', async ({ runInlineTest, showReport, page }) => {
      test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/11742' });
      const result = await runInlineTest({
        'inner.js': `
    ...
    test('should include commit metadata w/ CI', async ({ runInlineTest, writeFiles, showReport, page }) => {
      const files = {
        'uncommitted.txt': `uncommitted file`,
        'copilotbrowser.config.ts': `export default {}`,
        'example.spec.ts': `
          import { test, expect } from '@copilotbrowser/copilotbrowser/test';
          test('sample', async ({}) => { expect(2).toBe(2); });
        `,
    ...
    test('should not include git metadata w/o CI', async ({ runInlineTest, showReport, page }) => {
      const result = await runInlineTest({
        ...
        `,
      }, { reporter: 'dot,html' }, { copilotbrowser_HTML_OPEN: 'never' }, undefined);
    ...
    test.describe('report location', () => {
      test('with config should create report relative to config', async ({ runInlineTest, useIntermediateMergeReport }, testInfo) => {
        test.skip(useIntermediateMergeReport);
        const result = await runInlineTest({
          'nested/project/copilotbrowser.config.ts': `
          ...
          `,
        }, { reporter: '', config: './nested/project/copilotbrowser.config.ts' });
        expect(result.exitCode).toBe(0);
      ...
      test('without config should create relative to package.json', async ({ runInlineTest }, testInfo) => {
        const result = await runInlineTest({
          ...
          `
        }, { 'reporter': 'html,line' }, { copilotbrowser_HTML_OPEN: 'never' }, {
          cwd: 'foo/bar/baz/tests',
        });
        expect(result.exitCode).toBe(0);
        expect(result.passed).toBe(1);
        expect(fs.existsSync(testInfo.outputPath('copilotbrowser-report'))).toBe(false);
        expect(fs.existsSync(testInfo.outputPath('foo', 'copilotbrowser-report'))).toBe(true);
        expect(fs.existsSync(testInfo.outputPath('foo', 'bar', 'copilotbrowser-report'))).toBe(false);
        expect(fs.existsSync(testInfo.outputPath('foo', 'bar', 'baz', 'tests', 'copilotbrowser-report'))).toBe(false);
      });
      ...
      test('with env var should create relative to cwd', async ({ runInlineTest }, testInfo) => {
        const result = await runInlineTest({
          ...
          `
        }, { 'reporter': 'html,line' }, { 'copilotbrowser_HTML_OPEN': 'never', 'copilotbrowser_HTML_OUTPUT_DIR': '../my-report' }, {
          cwd: 'foo/bar/baz/tests',
        });
        expect(result.exitCode).toBe(0);
        expect(result.passed).toBe(1);
    ...
    test.describe('labels', () => {
      test('should show labels in the test row', async ({ runInlineTest, showReport, page }) => {
        const result = await runInlineTest({
          ...
          `,
          'a.test.js': `
            const { expect, test } = require('@copilotbrowser/copilotbrowser/test');
    ...
    test('should list tests in the right order', async ({ runInlineTest, showReport, page }) => {
      await runInlineTest({
        ...
        `,
      }, { reporter: 'html' }, { copilotbrowser_HTML_OPEN: 'never' });
    ...
    test('should print a user-friendly warning when opening a trace via file:// protocol', async ({ runInlineTest, showReport, page }) => {
      ...
      const reportPath = path.join(test.info().outputPath(), 'copilotbrowser-report');
      ...
      await expect(page.locator('#fallback-error')).toContainText(
          'The copilotbrowser Trace Viewer must be loaded over the http:// or https:// protocols.'
      );
      const expectedReportPath = reportPath.replace(/\\/g, '\\\\');
      await expect(page.locator('#fallback-error')).toContainText(
          `npx copilotbrowser show-report "${expectedReportPath}"`
      );
    });
    ...
    test('should not collate identical file names in different project directories', async ({ runInlineTest, page }) => {
      ...
      const reportPath = path.join(test.info().outputPath(), 'copilotbrowser-report', 'index.html');
    ...
    test('execSync doesnt produce a second stdout attachment', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/33886' } }, async ({ runInlineTest, showReport, page }) => {
      await runInlineTest({
        'a.test.js': `
          const { test, expect } = require('@copilotbrowser/copilotbrowser/test');
    ...
    test('should include diff in AI prompt', async ({ runInlineTest, writeFiles, showReport, page }) => {
      ...
      await page.waitForFunction(() => navigator.clipboard.readText());
      const prompt = await page.evaluate(() => navigator.clipboard.readText());
      expect(prompt, 'first line').toContain(`copilotbrowser test failed.`);
    ...
    test('worker test list', async ({ runInlineTest, showReport, page }) => {
      const result = await runInlineTest({
        ...
        'example.spec.ts': `
          ...
          test('success2', () => {});
        `,
      }, { reporter: 'html', workers: '1' }, { copilotbrowser_HTML_OPEN: 'never' });
      expect(result.exitCode).toBe(0);
      await showReport();
...
async function initGitRepo(baseDir: string) {
  ...
  await execGit(baseDir, ['add', 'copilotbrowser.config.ts']);
...
function ghaCommitEnv() {
  return {
    GITHUB_ACTIONS: '1',
    GITHUB_REPOSITORY: 'microsoft/copilotbrowser-example-for-test',
    GITHUB_SERVER_URL: 'https://copilotbrowser.dev',
