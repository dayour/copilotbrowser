...
const authFiles = {
  ...
  `,
  'auth.ts': `
    import { chromium, FullConfig } from '@copilotbrowser/copilotbrowser/test';
...
test('teardown order', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'copilotbrowser.config.ts': `
      ...
      export default { '@copilotbrowser/copilotbrowser/test': { plugins } };
    `,
    'a.test.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
      test('test', () => {});
...
test('globalSetup and globalTeardown should have copilotbrowser_TEST=1', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'copilotbrowser.config.ts': `
      import * as path from 'path';
      module.exports = {
    ...
    'globalSetup.ts': `
      export default () => {
        console.log('\\n%%setup env=' + process.env.copilotbrowser_TEST);
      };
    `,
    'globalTeardown.ts': `
      export default () => {
        console.log('\\n%%teardown env=' + process.env.copilotbrowser_TEST);
      };
    `,
    'a.test.js': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
      test('should work', async ({}, testInfo) => {
        console.log('\\n%%test env=' + process.env.copilotbrowser_TEST);
      });
    `,
  });
  expect(result.passed).toBe(1);
