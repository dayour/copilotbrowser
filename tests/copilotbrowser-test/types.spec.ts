...
test('should check types of fixtures', async ({ runTSC }) => {
  const result = await runTSC({
    'helper.ts': `
      import { test as base, expect, Page } from '@copilotbrowser/copilotbrowser/test';
    ...
    'copilotbrowser.config.ts': `
      ...
      import { Config } from '@copilotbrowser/copilotbrowser/test';
    ...
    'a.spec.ts': `
      ...
      test.afterAll(() => {});
    `,
    'copilotbrowser-props.config.ts': `
      import { copilotbrowserTestConfig } from '@copilotbrowser/copilotbrowser/test';
      const config0: copilotbrowserTestConfig = {
        use: {
      ...
      const config1: copilotbrowserTestConfig = {
        use: {
          ignoreHTTPSErrors: undefined,
          isMobile: true,
          javaScriptEnabled: false,
      ...
      const config2: copilotbrowserTestConfig = {
        use: {
          ignoreHTTPSErrors: undefined,
          isMobile: true,
          javaScriptEnabled: false,
      ...
      const config3: copilotbrowserTestConfig<{ foo: boolean }> = {
        use: {
          ignoreHTTPSErrors: undefined,
          isMobile: true,
          javaScriptEnabled: false,
      ...
      const config4: copilotbrowserTestConfig<{ foo: boolean }> = {
        use: {
          ignoreHTTPSErrors: undefined,
          isMobile: true,
          javaScriptEnabled: false,
    ...
    'copilotbrowser-define.config.ts': `
    ...
    `,
    'copilotbrowser-define-merge.config.ts': `
      import { defineConfig } from '@copilotbrowser/copilotbrowser/test';
      const config0 = defineConfig({
        timeout: 1,
        // @ts-expect-error
