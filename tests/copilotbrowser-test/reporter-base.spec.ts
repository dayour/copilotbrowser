...
for (const useIntermediateMergeReport of [false, true] as const) {
  test.describe(`${useIntermediateMergeReport ? 'merged' : 'created'}`, () => {
    ...
    test('should print an error in a codeframe', async ({ runInlineTest }) => {
      ...
      expect(result.output).toContain(`  2 |           import { test, expect } from '@copilotbrowser/copilotbrowser/test';`);
    ...
    test('should print if globalTimeout is reached', async ({ runInlineTest }) => {
      test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/29768' });
      const result = await runInlineTest({
        'copilotbrowser.config.ts': `
          module.exports = {
            globalTimeout: 3000,
      ...
      // TODO: uncomment once https://github.com/dayour/copilotbrowser/issues/30550 is fixed.
