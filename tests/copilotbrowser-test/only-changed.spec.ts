...
import { test as baseTest, expect, copilotbrowserCtConfigText } from './copilotbrowser-test-fixtures';
...
const test = baseTest.extend<{ git(command: string): void }>({
  git: async ({}, use, testInfo) => {
    ...
    const git = (command: string) => execSync(`git ${command}`, { cwd: baseDir, stdio: process.env.CBTEST_DEBUG ? 'inherit' : 'ignore' });
...
test('should run project dependencies of changed tests', {
  annotation: {
    type: 'issue',
    description: 'https://github.com/dayour/copilotbrowser/issues/32070',
  },
