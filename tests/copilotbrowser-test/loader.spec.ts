...
test('should validate configuration object', async ({ runInlineTest }) => {
  ...
  expect(result.exitCode).toBe(1);
  expect(result.failed).toBe(0);
  expect(result.output).toContain('copilotbrowser.config.ts: config.timeout must be a non-negative number');
});
...
test('should not resolve .mjs import to .ts file in non-ESM mode', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'a.test.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
      import { gimmeAOne } from './copilotbrowser-utils.mjs';
      test('pass', ({}) => {
        expect(gimmeAOne()).toBe(1);
      });
    `,
  ...
  expect(result.exitCode).toBe(1);
  expect(result.output).toContain(`Cannot find module './copilotbrowser-utils.mjs'`);
});

test('should resolve absolute .js import to .ts file', async ({ runInlineTest }) => {
  const filePath = test.info().outputPath('copilotbrowser-utils.js');
  const result = await runInlineTest({
    'a.test.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
...
test('should resolve no-extension import of module into .ts file', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'node_modules/copilotbrowser-utils/index.js': `
      exports.foo = 42;
    `,
    'a.test.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
    ...
    `,
    'copilotbrowser-utils.ts': `
      import { foo } from 'copilotbrowser-utils';
      export function gimmeAOne() {
        return foo - 41;
      }
    `,
...
test('should allow test.extend.ts and test.ts files', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'test.extend.ts': `
      export { test, expect } from '@copilotbrowser/copilotbrowser/test';
    `,
    'test.ts': `
...
test('should dynamically import re-exported cjs namespace', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35812' },
}, async ({ runInlineTest }) => {
  const result = await runInlineTest({
