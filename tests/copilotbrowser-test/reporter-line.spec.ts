...
for (const useIntermediateMergeReport of [false, true] as const) {
  test.describe(`${useIntermediateMergeReport ? 'merged' : 'created'}`, () => {
    ...
    test('should trim multiline step titles to first line', {
      annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/31266' }
    }, async ({ runInlineTest }) => {
      const result = await runInlineTest({
        'a.test.ts': `
          import { test, expect } from '@copilotbrowser/copilotbrowser/test';
        ...
        `,
      }, { reporter: 'line' }, { copilotbrowser_FORCE_TTY: '1' });
      const text = result.output;
...
test.describe('onTestPaused', () => {
  ...
  test('pause at end', async ({ interactWithTestRunner }) => {
    const runner = await interactWithTestRunner({
      ...
      `,
    }, { reporter: 'line' }, { CB_TEST_DEBUG_REPORTERS: '1', PWPAUSE: '1' });
