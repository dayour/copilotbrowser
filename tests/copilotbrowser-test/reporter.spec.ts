...
import type { Reporter, TestCase, TestResult, TestStep } from '../../packages/copilotbrowser-test/reporter';
...
`);
    ...
    test('should use sourceMap-based file suite names', async ({ runInlineTest }) => {
      test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/11028' });
      const result = await runInlineTest({
        'reporter.js': `
          class Reporter {
            onBegin(config, suite) {
...
var import_test = __toModule(require("@copilotbrowser/copilotbrowser/test"));
...
//# sourceMappingURL=data:application/json;base64,`,
    ...
    test('test and step error should have code snippet', async ({ runInlineTest }) => {
      const result = await runInlineTest({
        'reporter.ts': smallReporterJS,
        'copilotbrowser.config.ts': `module.exports = { reporter: [['./reporter', { printSteps: true, printErrorSnippet: true }]] };`,
        'a.spec.js': `
          const { test, expect } = require('@copilotbrowser/copilotbrowser/test');
          test('test', async () => {
...
onError: Error: test @ a.spec.js:3
  ======
    ...
    1 |
    2 |           const { test, expect } = require('@copilotbrowser/copilotbrowser/test');
...
test('tests skipped in serial mode receive onTestBegin/onTestEnd', async ({ runInlineTest }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/28321' });

  const result = await runInlineTest({
    'reporter.ts': smallReporterJS,
    'copilotbrowser.config.ts': `module.exports = { reporter: [['./reporter', { printTestStatus: true }]] };`,
    'a.spec.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
...
test('attachments are reported in onStepEnd', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/14364' } }, async ({ runInlineTest }) => {
  class TestReporter implements Reporter {
    onStepEnd(test: TestCase, result: TestResult, step: TestStep) {
