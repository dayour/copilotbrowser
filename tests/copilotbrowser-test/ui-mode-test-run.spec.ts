...
test('should show proper total when using deps', async ({ runUITest }) => {
  const { page } = await runUITest({
    ...
    `,
    'a.test.ts': `
      import { expect, test } from "@copilotbrowser/copilotbrowser/test";
...
test('should respect --tsconfig option', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/32797' }
}, async ({ runUITest }) => {
  const { page } = await runUITest({
    'copilotbrowser.config.ts': `
      import { foo } from '~/foo';
...
test('should respect --ignore-snapshots option', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/32868' }
}, async ({ runUITest }) => {
  const { page } = await runUITest({
    'a.test.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
...
test('should not leak websocket connections', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/33641' }
}, async ({ runUITest }) => {
  const { page } = await runUITest({
    'a.test.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
...
test('should run test defined outside of testdir', async ({ runUITest }) => {
  const { page } = await runUITest({
    'copilotbrowser.config.ts': `export default { testDir: 'tests' };`,
