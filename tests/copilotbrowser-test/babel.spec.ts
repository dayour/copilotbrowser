...
test('should be able to access |this| inside class properties', async ({ runInlineTest }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/21794' });
  const result = await runInlineTest({
    'example.spec.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
...
test('should work with |const| Type Parameters', async ({ runInlineTest }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/21900' });
  const result = await runInlineTest({
    'example.spec.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
...
test('should not read browserslist file', async ({ runInlineTest }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/23125' });
  const result = await runInlineTest({
...
test('should not transform external', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    ...
    `,
    'a.spec.ts': `
      const { test, expect, Page } = require('@copilotbrowser/copilotbrowser/test');
      let page: Page;
...
for (const type of ['module', undefined]) {
  test(`should support import assertions with type=${type} in the package.json`, {
    annotation: {
      type: 'issue',
      description: 'https://github.com/dayour/copilotbrowser/issues/32659'
    }
  }, async ({ runInlineTest }) => {
    const result = await runInlineTest({
      'copilotbrowser.config.ts': `
