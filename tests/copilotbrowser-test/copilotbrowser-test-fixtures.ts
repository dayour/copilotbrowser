...
import type { JSONReport, JSONReportSpec, JSONReportSuite, JSONReportTest, JSONReportTestResult } from '@copilotbrowser/copilotbrowser/reporter';
...
export const cliEntrypoint = path.join(__dirname, '../../packages/copilotbrowser-test/cli.js');

const configFile = (baseDir: string, files: Files): string | undefined => {
  for (const [name, content] of Object.entries(files)) {
    if (name.includes('copilotbrowser.config')) {
...
function startcopilotbrowserTest(childProcess: CommonFixtures['childProcess'], baseDir: string, params: any, env: NodeJS.ProcessEnv, options: RunOptions): TestChildProcess {
  ...
  return startcopilotbrowserChildProcess(childProcess, baseDir, args, env, options);
}

function startcopilotbrowserChildProcess(childProcess: CommonFixtures['childProcess'], baseDir: string, args: string[], env: NodeJS.ProcessEnv, options: RunOptions): TestChildProcess {
  return childProcess({
...
async function runcopilotbrowserTest(childProcess: CommonFixtures['childProcess'], baseDir: string, params: any, env: NodeJS.ProcessEnv, options: RunOptions, files: Files, mergeReports: (reportFolder: string, env?: NodeJS.ProcessEnv, options?: RunOptions) => Promise<CliRunResult>, useIntermediateMergeReport: boolean): Promise<RunResult> {
  let reporter;
  ...
  const envWithJsonReporter = {
    CB_TEST_REPORTER: path.join(__dirname, '../../packages/copilotbrowser/lib/reporters/json.js'),
    copilotbrowser_JSON_OUTPUT_NAME: reportFile,
    ...env,
  };
  const testProcess = startcopilotbrowserTest(childProcess, baseDir, params, useIntermediateMergeReport ? env : envWithJsonReporter, options);
...
async function runcopilotbrowserCLI(childProcess: CommonFixtures['childProcess'], args: string[], baseDir: string, env: NodeJS.ProcessEnv): Promise<{ output: string, stdout: string, stderr: string, exitCode: number }> {
...
export function cleanEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  return {
    ...
    // END: Reserved CI
    CB_TEST_HTML_REPORT_OPEN: undefined,
    copilotbrowser_HTML_OPEN: undefined,
    CB_TEST_DEBUG_REPORTERS: undefined,
    CB_TEST_REPORTER: undefined,
    CB_TEST_REPORTER_WS_ENDPOINT: undefined,
    CB_TEST_SOURCE_TRANSFORM: undefined,
    CB_TEST_SOURCE_TRANSFORM_SCOPE: undefined,
    CBTEST_BOT_NAME: undefined,
    CBTEST_SHARD_WEIGHTS: undefined,
...
export const test = base
    ...
    .extend<Fixtures>({
      ...
      runInlineTest: async ({ childProcess, mergeReports, useIntermediateMergeReport }, use, testInfo: TestInfo) => {
        const cacheDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'copilotbrowser-test-cache-'));
        await use(async (files: Files, params: Params = {}, env: NodeJS.ProcessEnv = {}, options: RunOptions = {}) => {
          const baseDir = await writeFiles(testInfo, files, true);
          return await runcopilotbrowserTest(childProcess, baseDir, params, { CBTEST_CACHE_DIR: cacheDir, ...env }, options, files, mergeReports, useIntermediateMergeReport);
        });
      ...
      runCLICommand: async ({ childProcess }, use, testInfo: TestInfo) => {
        ...
        await use(async (files: Files, command: string, args?: string[]) => {
          const baseDir = await writeFiles(testInfo, files, true);
          return await runcopilotbrowserCLI(childProcess, [command, ...(args || [])], baseDir, { CBTEST_CACHE_DIR: cacheDir });
        });
        await removeFolders([cacheDir]);
      },
      ...
      startCLICommand: async ({ childProcess }, use, testInfo: TestInfo) => {
        ...
        await use(async (files: Files, command: string, args?: string[], options: RunOptions = {}, env: NodeJS.ProcessEnv = {}) => {
          const baseDir = await writeFiles(testInfo, files, true);
          return startcopilotbrowserChildProcess(childProcess, baseDir, [command, ...(args || [])], { ...env, CBTEST_CACHE_DIR: cacheDir }, options);
        });
        await removeFolders([cacheDir]);
      },

      runWatchTest: async ({ interactWithTestRunner }, use, testInfo: TestInfo) => {
        await use((files, params, env, options) => interactWithTestRunner(files, params, { ...env, CBTEST_WATCH: '1' }, options));
      },
      ...
      interactWithTestRunner: async ({ childProcess }, use, testInfo: TestInfo) => {
        ...
        await use(async (files: Files, params: Params = {}, env: NodeJS.ProcessEnv = {}, options: RunOptions = {}) => {
          const baseDir = await writeFiles(testInfo, files, true);
          testProcess = startcopilotbrowserTest(childProcess, baseDir, params, { CBTEST_CACHE_DIR: cacheDir, ...env }, options);
          return testProcess;
        });
      ...
      useIntermediateMergeReport: async ({}, use) => {
        await use(process.env.CBTEST_INTERMEDIATE_BLOB_REPORT === '1');
      },
    });
