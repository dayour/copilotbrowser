...
test('should ignore globalTimeout when debugging', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/34911' },
}, async ({ runInlineTest }) => {
  const result = await runInlineTest({
...
test('should run fixture teardown with custom timeout after test timeout', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/31537' },
}, async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'a.spec.ts': `
      import { test as base, expect } from '@copilotbrowser/copilotbrowser/test';
