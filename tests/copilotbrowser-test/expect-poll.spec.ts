...
test('should propagate promise rejections', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/32256' } }, async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'a.spec.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
      test('should fail', async () => {
...
test('should propagate string exception from async arrow function', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/32256' } }, async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'a.spec.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
      test('should fail', async () => {
...
test('should show custom message', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/32582' }
}, async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'a.spec.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
