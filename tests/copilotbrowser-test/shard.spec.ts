...
test('should exit with shard=/3', async ({ runInlineTest }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/34463' });
...
test('should work with workers=1 and --fully-parallel', async ({ runInlineTest }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/21226' });
  const tests = {
    'a1.spec.ts': `
      import { test } from '@copilotbrowser/copilotbrowser/test';
...
test('should not shard mode:default suites', async ({ runInlineTest }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/22891' });
...
test('should shard tests with beforeAll based on shards total instead of workers', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/33077' },
}, async ({ runInlineTest }) => {
  const tests = {
    'a.spec.ts': `
      import { test } from '@copilotbrowser/copilotbrowser/test';
...
test('should respect custom shard weights', async ({ runInlineTest }) => {
  await test.step('shard 1', async () => {
    const result = await runInlineTest(tests, { 'shard': '1/2', 'workers': 1 }, { CBTEST_SHARD_WEIGHTS: '40:60' });
  ...
  await test.step('shard 2', async () => {
    const result = await runInlineTest(tests, { 'shard': '2/2', 'workers': 1 }, { CBTEST_SHARD_WEIGHTS: '40:60' });
    expect.soft(result.exitCode).toBe(0);
    expect.soft(result.outputLines).toEqual([
      'a2-test1-done',
      'a2-test2-done',
