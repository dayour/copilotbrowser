...
test('should report toHaveScreenshot step with expectation name in title', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    ...
    `,
    ...copilotbrowserConfig({ reporter: './reporter' }),
    'a.spec.js': `
      const { test, expect } = require('@copilotbrowser/copilotbrowser/test');
      test('is a test', async ({ page }) => {
        // Named expectation.
  ...
  expect(result.outputLines).toEqual([
    `end [cb:api] Launch browser`,
    `end [fixture] Fixture "browser"`,
    `end [cb:api] Create context`,
    `end [fixture] Fixture "context"`,
    `end [cb:api] Create page`,
...
test('should respect config.snapshotPathTemplate and sanitize the name', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35669' },
}, async ({ runInlineTest }) => {
  const result = await runInlineTest({
    ...copilotbrowserConfig({
      snapshotPathTemplate: '__screenshots__/{testFilePath}/{arg}{ext}',
    ...
    'a.spec.js': `
      ...
      test('is a test', async ({ page }) => {
        ...
        // An array is not sanitized - see https://github.com/dayour/copilotbrowser/pull/9156.
...
function copilotbrowserConfig(obj: any) {
  return {
