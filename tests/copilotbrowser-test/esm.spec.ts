...
import { test, expect, copilotbrowserCtConfigText } from './copilotbrowser-test-fixtures';
...
test('should import esm from ts when package.json has type module', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    ...
    'a.test.ts': `
      ...
      import { test, expect, Locator } from '@copilotbrowser/copilotbrowser/test';
      test('check project name', ({}, testInfo) => {
...
test('should use source maps', async ({ runInlineTest }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/15202' });
  const result = await runInlineTest({
...
test('should use source maps when importing a file throws an error', async ({ runInlineTest }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/29418' });
  ...
  2 |       import { test, expect } from '@copilotbrowser/copilotbrowser/test';
  3 |
...
test('should use source maps when importing a file throws an error in legacy mode', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35706' },
}, async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'package.json': `{ "type": "module" }`,
    'copilotbrowser.config.ts': `
    ...
    'a.test.ts': `
      ...
      throw new Error('Oh my!');
    `
  }, {}, { 'copilotbrowser_WAIT_FOR_SOURCE_MAPS': '1' });
  expect(result.exitCode).toBe(1);
  expect(result.output).toContain(`Error: Oh my!
...
test('should filter by line', async ({ runInlineTest }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/15200' });
  const result = await runInlineTest({
    'package.json': `{ "type": "module" }`,
    'copilotbrowser.config.ts': `
      export default { projects: [{name: 'foo'}] };
...
test('should resolve directory import to index.js file in ESM mode', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'package.json': `{ "type": "module" }`,
    'copilotbrowser.config.ts': `export default { projects: [{name: 'foo'}] };`,
    'a.test.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
      import { gimmeAOne } from './copilotbrowser-utils';
      test('pass', ({}) => {
        expect(gimmeAOne()).toBe(1);
      });
    `,
    'copilotbrowser-utils/index.js': `
...
test('should resolve directory import to index.tsx file in ESM mode', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    ...
    `,
    'copilotbrowser-utils/index.tsx': `
      export function gimmeAOne() {
        return 1;
      }
    `,
...
test('should resolve directory import to index.mjs file in ESM mode', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    ...
    `,
    'copilotbrowser-utils/index.mjs': `
      export function gimmeAOne() {
        return 1;
      }
    `,
...
test('should resolve directory import to index.jsx file in ESM mode', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    ...
    `,
    'copilotbrowser-utils/index.jsx': `
      export function gimmeAOne() {
        return 1;
      }
    `,
...
test('should resolve .js import to .ts file in ESM mode', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    ...
    'a.test.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
      import { gimmeAOne } from './copilotbrowser-utils.js';
      test('pass', ({}) => {
        expect(gimmeAOne()).toBe(1);
      });
    `,
    'copilotbrowser-utils.ts': `
      export function gimmeAOne() {
        return 1;
      }
    `,
...
test('should be able to use use execSync with a Node.js file inside a spec', async ({ runInlineTest }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/24516' });
  const result = await runInlineTest({
    'global-setup.ts': `
...
test('should be able to use mergeTests/mergeExpect', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'a.test.mjs': `
      import { test as base, expect as baseExpect, mergeTests, mergeExpects } from '@copilotbrowser/copilotbrowser/test';
...
test('should exit after merge-reports', async ({ runInlineTest, mergeReports }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/28699' });
  const result = await runInlineTest({
    'merge.config.ts': `
