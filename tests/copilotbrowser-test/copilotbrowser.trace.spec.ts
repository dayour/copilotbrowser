...
test('should record api trace', async ({ runInlineTest, server }, testInfo) => {
  const result = await runInlineTest({
    ...
    'a.spec.ts': `
      ...
      test('api pass', async ({copilotbrowser}, testInfo) => {
...
test('should not mixup network files between contexts', async ({ runInlineTest, server }, testInfo) => {
  ...
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/22089' });
...
test('should not throw when merging traces multiple times', async ({ runInlineTest, server }, testInfo) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/27286' });
  ...
  const result = await runInlineTest({
    ...
    `,
    'a.spec.ts': `
      import { BrowserContext, expect, Page, test as baseTest } from '@copilotbrowser/copilotbrowser/test';
...
test('should record trace for manually created context in a failed test', async ({ runInlineTest }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/31541' });
...
test('should not nest top level expect into unfinished api calls ', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/31959' }
...
test('should record trace after fixture teardown timeout', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/30718' },
}, async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'a.spec.ts': `
      import { test as base, expect } from '@copilotbrowser/copilotbrowser/test';
    ...
    `,
  }, { trace: 'on', timeout: '3000' }, { DEBUG: 'cb:test' });
  expect(result.exitCode).toBe(1);
  expect(result.failed).toBe(1);
