...
test('should add line in addition to file json without CI', async ({ runInlineTest }, testInfo) => {
  const result = await runInlineTest({
    ...
    `,
  }, { reporter: '' }, { CB_TEST_DEBUG_REPORTERS: '1' });
  expect(result.exitCode).toBe(0);
...
test.describe('report location', () => {
  ...
  test('with env var should create relative to cwd', async ({ runInlineTest }, testInfo) => {
    const result = await runInlineTest({
      ...
      `
    }, { 'reporter': 'json' }, { 'copilotbrowser_JSON_OUTPUT_NAME': '../my-report.json' }, {
  ...
  test('support copilotbrowser_JSON_OUTPUT_FILE', async ({ runInlineTest }, testInfo) => {
    const result = await runInlineTest({
      'foo/package.json': `{ "name": "foo" }`,
      // unused config along "search path"
      'foo/bar/copilotbrowser.config.js': `
      ...
      `
    }, { 'reporter': 'json' }, { 'copilotbrowser_JSON_OUTPUT_FILE': '../my-report.json' }, {
      cwd: 'foo/bar/baz/tests',
    });
    expect(result.exitCode).toBe(0);
    expect(result.passed).toBe(1);
  ...
  test('support copilotbrowser_JSON_OUTPUT_DIR and copilotbrowser_JSON_OUTPUT_NAME', async ({ runInlineTest }, testInfo) => {
    const result = await runInlineTest({
      ...
      `
    }, { 'reporter': 'json' }, { 'copilotbrowser_JSON_OUTPUT_DIR': 'foo/bar', 'copilotbrowser_JSON_OUTPUT_NAME': 'baz/my-report.json' });
    expect(result.exitCode).toBe(0);
    expect(result.passed).toBe(1);
    expect(fs.existsSync(testInfo.outputPath('foo', 'bar', 'baz', 'my-report.json'))).toBe(true);
  });
