...
import { createImage } from './copilotbrowser-test-fixtures';
...
test('should work with non-existing rootDir and testDir outside of it', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/38367' },
}, async ({ runUITest }) => {
  const { page } = await runUITest({
...
test('should show snapshots for steps', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35285' }
}, async ({ runUITest }) => {
  const { page } = await runUITest({
    'a.test.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
...
test('should work behind reverse proxy', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/33705' } }, async ({ runUITest, proxyServer: reverseProxy }) => {
  const { page } = await runUITest({
    'a.test.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
      test('trace test', async ({ page }) => {
...
test('should be able to create and dispose APIRequestContext inside Promise.all', async ({ runUITest }) => {
  const { page } = await runUITest({
    'a.test.ts': `
      import { test, request } from '@copilotbrowser/copilotbrowser/test';
