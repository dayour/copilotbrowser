...
test('should run web server with copilotbrowser_TEST=1 environment variable', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/37377' }
  ...
  const result = await runInlineTest({
    'test.spec.ts': `
      ...
      test('connect to the server', async ({baseURL, page}) => {
        expect(baseURL).toBe('http://localhost:${port}');
        await page.goto(baseURL + '/env-copilotbrowser_TEST');
...
test('should allow to unset copilotbrowser_TEST environment variable', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/37377' }
}, async ({ runInlineTest }, { workerIndex }) => {
  const port = workerIndex * 2 + 10500;
  const result = await runInlineTest({
...
test.describe('baseURL with plugins', () => {
  test('plugins do not set it', async ({ runInlineTest }, { workerIndex }) => {
    ...
    const result = await runInlineTest({
      ...
      `,
      'copilotbrowser.config.ts': `
        import { webServer } from 'copilotbrowser/lib/plugins';
        module.exports = {
          _plugins: [
            webServer({
              command: 'node ${JSON.stringify(SIMPLE_SERVER_PATH)} ${port}',
...
test('should check ipv4 and ipv6 with happy eyeballs when URL is passed', async ({ runInlineTest }, { workerIndex }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/20784' });
  const port = workerIndex * 2 + 10500;
  const result = await runInlineTest({
    'test.spec.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
