...
import type { copilotbrowserTestConfig } from '@copilotbrowser/copilotbrowser/test';
...
const test = baseTest.extend<{
  ...
  showReport: async ({ page }, use) => {
    ...
    await use(async (reportFolder?: string) => {
      reportFolder ??= test.info().outputPath('copilotbrowser-report');
...
test('should call methods in right order', async ({ runInlineTest, mergeReports }) => {
  ...
  await runInlineTest(files, { shard: `3/3` }, { CBTEST_BLOB_DO_NOT_REMOVE: '1' });
...
test('should merge into html with dependencies', async ({ runInlineTest, mergeReports, showReport, page }) => {
  ...
  for (let i = 0; i < totalShards; i++)
    await runInlineTest(files, { shard: `${i + 1}/${totalShards}` }, { CBTEST_BLOB_DO_NOT_REMOVE: '1' });
  const reportFiles = await fs.promises.readdir(reportDir);
  reportFiles.sort();
  ...
  const { exitCode, output } = await mergeReports(reportDir, { 'copilotbrowser_HTML_OPEN': 'never' }, { additionalArgs: ['--reporter', 'html'] });
  expect(exitCode).toBe(0);
...
test('should merge blob into blob', async ({ runInlineTest, mergeReports, showReport, page }) => {
  ...
  await runInlineTest(files, { shard: `1/2` }, { CBTEST_BLOB_DO_NOT_REMOVE: '1' });
  await runInlineTest(files, { shard: `2/2` }, { CBTEST_BLOB_DO_NOT_REMOVE: '1' });
  {
    const reportFiles = await fs.promises.readdir(reportDir);
    reportFiles.sort();
  ...
  }
  {
    ...
    const { exitCode } = await mergeReports(compinedBlobReportDir, { 'copilotbrowser_HTML_OPEN': 'never' }, { additionalArgs: ['--reporter', 'html,json'] });
    expect(exitCode).toBe(0);
...
test('should produce consistent step ids', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/31023' },
  ...
  reportFiles.sort();
  expect(reportFiles).toEqual(['report-1.zip', 'report-2.zip']);
  const { exitCode } = await mergeReports(reportDir, { 'copilotbrowser_HTML_OPEN': 'never' }, { additionalArgs: ['--reporter', 'html,json'] });
  expect(exitCode).toBe(0);
  await showReport();
...
test('be able to merge incomplete shards', async ({ runInlineTest, mergeReports, showReport, page }) => {
  ...
  reportFiles.sort();
  expect(reportFiles).toEqual([expect.stringMatching(/report-.*.zip/), expect.stringMatching(/report-.*.zip/)]);
  const { exitCode } = await mergeReports(reportDir, { 'copilotbrowser_HTML_OPEN': 'never' }, { additionalArgs: ['--reporter', 'html'] });
  expect(exitCode).toBe(0);
...
test('merge into list report by default', async ({ runInlineTest, mergeReports }) => {
  ...
  const { exitCode, output } = await mergeReports(reportDir, { CB_TEST_DEBUG_REPORTERS: '1', copilotbrowser_LIST_PRINT_STEPS: '1', copilotbrowser_FORCE_TTY: '80' }, { additionalArgs: ['--reporter', 'list'] });
  expect(exitCode).toBe(0);
...
test('generate html with attachment urls', async ({ runInlineTest, mergeReports, page, server }) => {
  ...
  const htmlReportDir = test.info().outputPath('copilotbrowser-report');
  ...
  await expect(page).toHaveTitle('copilotbrowser Trace Viewer');
...
test('multiple output reports', async ({ runInlineTest, mergeReports, showReport, page }) => {
  ...
  reportFiles.sort();
  expect(reportFiles).toEqual(['report-1.zip']);
  const { exitCode, output } = await mergeReports(reportDir, { 'copilotbrowser_HTML_OPEN': 'never' }, { additionalArgs: ['--reporter', 'html,line'] });
  expect(exitCode).toBe(0);
...
test('multiple output reports based on config', async ({ runInlineTest, mergeReports }) => {
  const reportDir = test.info().outputPath('blob-report');
  const files = {
    'merged/copilotbrowser.config.ts': `
      module.exports = {
  ...
  const { exitCode, output } = await mergeReports(reportDir, undefined, {
    ...
    additionalArgs: ['--config', 'copilotbrowser.config.ts'],
  });
  expect(exitCode).toBe(0);
...
test('preserve config fields', async ({ runInlineTest, mergeReports }) => {
  ...
  await runInlineTest(files, { shard: `3/3`, workers: 1 }, { CBTEST_BLOB_DO_NOT_REMOVE: '1' });
...
test('preserve steps in html report', async ({ runInlineTest, mergeReports, showReport, page }) => {
  ...
  const { exitCode, output } = await mergeReports(reportDir, { 'copilotbrowser_HTML_OPEN': 'never' }, { additionalArgs: ['--reporter', 'html'], cwd: mergeCwd });
  expect(exitCode).toBe(0);
...
test('support fileName option', async ({ runInlineTest, mergeReports }) => {
  ...
  await runInlineTest(files('two'), undefined, { CBTEST_BLOB_DO_NOT_REMOVE: '1' });
...
test('support copilotbrowser_BLOB_OUTPUT_DIR env variable', async ({ runInlineTest, mergeReports }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/30091' });
  const files = {
    'copilotbrowser.config.ts': `
      module.exports = {
        reporter: [['blob']],
  ...
  await runInlineTest(files, undefined, { copilotbrowser_BLOB_OUTPUT_DIR: 'my/dir' });
...
test('support copilotbrowser_BLOB_OUTPUT_NAME env variable', async ({ runInlineTest, mergeReports }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/30091' });
  const files = {
    'copilotbrowser.config.ts': `
      module.exports = {
  ...
  await runInlineTest(files, undefined, { copilotbrowser_BLOB_OUTPUT_NAME: 'report-one.zip' });
  await runInlineTest(files, undefined, { copilotbrowser_BLOB_OUTPUT_NAME: 'report-two.zip', CBTEST_BLOB_DO_NOT_REMOVE: '1' });
...
test('support copilotbrowser_BLOB_OUTPUT_FILE environment variable', async ({ runInlineTest, mergeReports }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/30091' });
  const files = {
    'copilotbrowser.config.ts': `
      module.exports = {
  ...
  await runInlineTest(files, { shard: `1/2` }, { copilotbrowser_BLOB_OUTPUT_FILE: 'subdir/report-one.zip' });
  await runInlineTest(files, { shard: `2/2` }, { copilotbrowser_BLOB_OUTPUT_FILE: test.info().outputPath('subdir/report-two.zip') });
...
test('keep projects with same name different global tag separate', async ({ runInlineTest, mergeReports, showReport, page }) => {
  ...
  await runInlineTest(files('second'), undefined, { GLOBAL_TAG: '@second', CBTEST_BLOB_DO_NOT_REMOVE: '1' });
...
test('blob-report should be next to package.json', async ({ runInlineTest }, testInfo) => {
  const result = await runInlineTest({
    ...
    'foo/bar/copilotbrowser.config.js': `
...
test('merge-reports should throw if report version is from the future', async ({ runInlineTest, mergeReports }) => {
  ...
  const { exitCode, output } = await mergeReports(reportDir, { 'copilotbrowser_HTML_OPEN': 'never' }, { additionalArgs: ['--reporter', 'html'] });
  expect(exitCode).toBe(1);
  expect(output).toContain(`Error: Blob report report-2.zip was created with a newer version of copilotbrowser.`);
...
test('merge-reports should merge old attachments (pre-1.53)', async ({ runInlineTest, mergeReports }) => {
  ...
  const { exitCode } = await mergeReports(reportDir, { 'copilotbrowser_HTML_OPEN': 'never' }, { additionalArgs: ['--reporter', 'html', '--config', 'merge.config.ts'] });
  expect(exitCode).toBe(0);

  const assets = await fs.promises.readdir(path.join(test.info().outputPath('copilotbrowser-report'), 'data'));
...
test('should merge blob reports with same name', async ({ runInlineTest, mergeReports, showReport, page }) => {
  ...
  const { exitCode } = await mergeReports(allReportsDir, { 'copilotbrowser_HTML_OPEN': 'never' }, { additionalArgs: ['--reporter', 'html'] });
  expect(exitCode).toBe(0);
  await showReport();
  await expect(page.locator('.subnav-item:has-text("All") .counter')).toHaveText('10');
...
test('reporter list in the custom config', async ({ runInlineTest, mergeReports }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/27281' });
  const reportDir = test.info().outputPath('blob-report');
  const files = {
    'copilotbrowser.config.ts': `
  ...
  const { exitCode, output } = await mergeReports(reportDir, undefined, { additionalArgs: ['--config', test.info().outputPath('merged/copilotbrowser.config.ts')] });
  expect(exitCode).toBe(0);
...
test('merge reports with different rootDirs', async ({ runInlineTest, mergeReports }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/27877' });
  const files1 = {
    'echo-reporter.js': `
    ...
    'dir1/copilotbrowser.config.ts': `module.exports = {
      testDir: 'tests1',
  ...
  await runInlineTest(files1, { workers: 1 }, undefined, { additionalArgs: ['--config', test.info().outputPath('dir1/copilotbrowser.config.ts')] });

  const files2 = {
    'dir2/copilotbrowser.config.ts': `module.exports = {
      reporter: [['blob', { outputDir: 'blob-report' }]]
    };`,
  ...
  await runInlineTest(files2, { workers: 1 }, undefined, { additionalArgs: ['--config', test.info().outputPath('dir2/copilotbrowser.config.ts')] });
...
test('merge reports same rootDirs', async ({ runInlineTest, mergeReports }) => {
  ...
  const files = {
    ...
    `,
    'copilotbrowser.config.ts': `module.exports = {
      testDir: 'tests',
      reporter: [['blob', { outputDir: 'blob-report' }]]
    };`,
...
test('merge reports should preserve attachments', async ({ runInlineTest, mergeReports, showReport, page }) => {
  ...
  reportFiles.sort();
  expect(reportFiles).toEqual(['report-1.zip', 'report-2.zip']);
  const { exitCode } = await mergeReports(reportDir, { 'copilotbrowser_HTML_OPEN': 'never' }, { additionalArgs: ['--reporter', 'blob,html'] });
  expect(exitCode).toBe(0);
...
test('merge reports must not change test ids when there is no need to', async ({ runInlineTest, mergeReports }) => {
  ...
  {
    const { exitCode, outputLines } = await runInlineTest(files, { workers: 1 }, { CBTEST_BLOB_DO_NOT_REMOVE: '1' }, { additionalArgs: ['--config', test.info().outputPath('shard-2.config.ts')] });
    expect(exitCode).toBe(0);
...
test('TestSuite.project() should return owning project', async ({ runInlineTest, mergeReports }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/29173' });
  const files1 = {
    'echo-reporter.js': `
      export default class EchoReporter {
        onTestBegin(test) {
...
test('open blob-1.42', async ({ runInlineTest, mergeReports }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/29984' });
  await runInlineTest({
    'echo-reporter.js': `
      export default class EchoReporter {
        lines = [];
...
test('preserve static annotations when tests did not run', async ({ runInlineTest, mergeReports, showReport, page }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/30260' });
  const reportDir = test.info().outputPath('blob-report');
  const files = {
    'copilotbrowser.config.ts': `
      module.exports = {
...
test('should report duration across all shards', async ({ runInlineTest, mergeReports }) => {
  ...
  await runInlineTest(files, { shard: `2/2`, workers: 1 }, { CBTEST_BLOB_DO_NOT_REMOVE: '1' });

  const { exitCode, outputLines } = await mergeReports(reportDir, {}, { additionalArgs: ['--reporter', test.info().outputPath('reporter.js'), '-c', test.info().outputPath('copilotbrowser.config.ts')] });
  expect(exitCode).toBe(0);
...
test('shard chart', async ({ runInlineTest, writeFiles, showReport, page, mergeReports }) => {
  ...
  await runInlineTest({}, { shard: '1/3' }, { CBTEST_BLOB_DO_NOT_REMOVE: '1', BOT_TAG: '@linux' });
  await runInlineTest({}, { shard: '2/3' }, { CBTEST_BLOB_DO_NOT_REMOVE: '1', BOT_TAG: '@linux' });
  await runInlineTest({}, { shard: '3/3' }, { CBTEST_BLOB_DO_NOT_REMOVE: '1', BOT_TAG: '@linux' });

  await runInlineTest({}, { shard: '1/2' }, { CBTEST_BLOB_DO_NOT_REMOVE: '1', BOT_TAG: '@mac' });
  await runInlineTest({}, { shard: '2/2' }, { CBTEST_BLOB_DO_NOT_REMOVE: '1', BOT_TAG: '@mac' });
...
test('workerIndex is rebased', async ({ runInlineTest, writeFiles, showReport, page, mergeReports }) => {
  ...
  await runInlineTest({}, { shard: '1/2', workers: '1' }, { CBTEST_BLOB_DO_NOT_REMOVE: '1' });
  await runInlineTest({}, { shard: '2/2', workers: '1' }, { CBTEST_BLOB_DO_NOT_REMOVE: '1' });
