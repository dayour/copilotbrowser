...
test('should work with custom copilotbrowserTest namespace', async ({ runTSC }) => {
  const result = await runTSC({
    'global.d.ts': `
      declare namespace copilotbrowserTest {
    ...
    `,
    'a.spec.ts': `
      import { test, expect, type Page, type APIResponse } from '@copilotbrowser/copilotbrowser/test';
      test.expect.extend({
        toBeWithinRange() {
          return {
            pass: true,
...
test('should propose only the relevant matchers when custom expect matcher classes were passed', async ({ runTSC }) => {
  const result = await runTSC({
    ...
    test('custom matchers', async ({ page }) => {
      ...
      // copilotbrowser-specific assertions do not apply to generic values.
      // @ts-expect-error
...
test('should respect expect.timeout in toHaveURL', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'copilotbrowser.config.js': `module.exports = { expect: { timeout: 1000 } }`,
    'a.test.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
...
test('should chain expect matchers and expose matcher utils (TSC)', async ({ runTSC }) => {
  const result = await runTSC({
    ...
    import type { Page, Locator, ExpectMatcherState, Expect } from '@copilotbrowser/copilotbrowser/test';
...
test('should support mergeExpects (TSC)', async ({ runTSC }) => {
  const result = await runTSC({
    'a.spec.ts': `
      import { test, mergeExpects, expect as baseExpect } from '@copilotbrowser/copilotbrowser/test';
...
test('should respect timeout from configured expect when used outside of the test runner', async ({ runInlineTest, writeFiles, runTSC }) => {
  ...
  const files = {
    'script.mjs': `
      import { test, expect as baseExpect, chromium } from '@copilotbrowser/copilotbrowser/test';
...
test('should expose timeout to custom matchers', async ({ runInlineTest, runTSC }) => {
  const files = {
    ...
    `,
    'a.test.ts': `
      import type { ExpectMatcherState, MatcherReturnType } from '@copilotbrowser/copilotbrowser/test';
      import { test, expect as base } from '@copilotbrowser/copilotbrowser/test';
...
test('should throw error when using .equals()', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'helper.ts': `
      import { test as base, expect as baseExpect } from '@copilotbrowser/copilotbrowser/test';
    ...
    'expect-test.spec.ts': `
      ...
      test('numeric ranges', () => {
        ...
        }).toThrowError('It looks like you are using custom expect matchers that are not compatible with copilotbrowser. See https://aka.ms/copilotbrowser/expect-compatibility');
      });
    `
  });
  expect(result.exitCode).toBe(0);
...
test('custom asymmetric matchers should work with expect.extend', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'expect-test.spec.ts': `
      import { test, expect as baseExpect, mergeExpects } from '@copilotbrowser/copilotbrowser/test';
      const expect1 = baseExpect.extend({
...
test('custom asymmetric matchers should function', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35138' } }, async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'expect-test.spec.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
      expect.extend({
...
test('single custom asymmetric matcher should present the correct error', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35138' } }, async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'expect-test.spec.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
      expect.extend({
...
test('multiple custom asymmetric matchers should present the correct error', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35138' } }, async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'expect-test.spec.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
      expect.extend({
...
test('multiple custom asymmetric matchers in async expect should present the correct error', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35138' } }, async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'expect-test.spec.ts': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
      expect.extend({
