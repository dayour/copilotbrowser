...
test('should throw when test() is called in config file', async ({ runInlineTest }) => {
  ...
  expect(result.output).toContain('copilotbrowser Test did not expect test() to be called here');
});
...
test('should merge configs', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'copilotbrowser.config.ts': `
      import { defineConfig, expect } from '@copilotbrowser/copilotbrowser/test';
  ...
  const result = await runInlineTest({
    'copilotbrowser.config.ts': `
      ...
      expect(derivedConfig).toEqual(expect.objectContaining({
        use: { foo: 1, bar: 2 },
        grep: 'hi',
        '@copilotbrowser/copilotbrowser/test': expect.objectContaining({
