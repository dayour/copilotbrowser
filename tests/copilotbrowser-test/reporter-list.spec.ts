  ...
  test.describe(`${useIntermediateMergeReport ? 'merged' : 'created'}`, () => {
    ...
    test('render steps', async ({ runInlineTest }) => {
      const result = await runInlineTest({
        ...
        `,
      }, { reporter: 'list' }, { CB_TEST_DEBUG_REPORTERS: '1', copilotbrowser_LIST_PRINT_STEPS: '1', copilotbrowser_FORCE_TTY: '80' });
      const text = result.output;
    ...
    test('render steps inline', async ({ runInlineTest }) => {
      ...
      }, { reporter: 'list' }, { CB_TEST_DEBUG_REPORTERS: '1', copilotbrowser_FORCE_TTY: '80' });
      const text = result.output;
      const lines = text.split('\n').filter(l => l.match(/^#.* :/)).map(l => l.replace(/[.\d]+m?s/, 'Xms'));
      lines.pop(); // Remove last item that contains [v] and time in ms.
      expect(lines).toEqual([
    ...
    test('render steps in non-TTY mode', async ({ runInlineTest }) => {
      const result = await runInlineTest({
        ...
        `,
      }, { reporter: 'list' }, { CB_TEST_DEBUG_REPORTERS: '1', copilotbrowser_LIST_PRINT_STEPS: '1' });
      const text = result.output;
      const lines = text.split('\n').filter(l => l.match(/^#.* :/)).map(l => l.replace(/[.\d]+m?s/, 'Xms'));
      expect(lines).toEqual([
    ...
    test('very long console line should not mess terminal', async ({ runInlineTest }) => {
      ...
      const result = await runInlineTest({
        ...
        `,
      }, { reporter: 'list' }, { copilotbrowser_FORCE_TTY: TTY_WIDTH + '' });
    ...
    test('render retries', async ({ runInlineTest }) => {
      const result = await runInlineTest({
        ...
        `,
      }, { reporter: 'list', retries: '1' }, { CB_TEST_DEBUG_REPORTERS: '1', copilotbrowser_FORCE_TTY: '80' });
      const text = result.output;
    ...
    test('should truncate long test names', async ({ runInlineTest }) => {
      const result = await runInlineTest({
        ...
        `,
      }, { reporter: 'list', retries: 0 }, { copilotbrowser_FORCE_TTY: '50' });
      expect(result.exitCode).toBe(1);
    ...
    test('should update test status row only within configured TTY height', async ({ runInlineTest }) => {
      const result = await runInlineTest({
        ...
        `,
      }, { reporter: 'list' }, { CB_TEST_DEBUG_REPORTERS: '1', copilotbrowser_FORCE_TTY: '80x80' });
      expect(result.exitCode).toBe(0);
...
test.describe('onTestPaused', () => {
  ...
  test('pause at end', async ({ interactWithTestRunner }) => {
    const runner = await interactWithTestRunner({
      ...
      `,
    }, { reporter: 'list' }, { CB_TEST_DEBUG_REPORTERS: '1', PWPAUSE: '1' });
