...
import { test, expect, parseTestRunnerOutput, countTimes } from './copilotbrowser-test-fixtures';
...
test('it should not allow a focused test when forbid-only is used', async ({ runInlineTest }) => {
  ...
  expect(result.exitCode).toBe(1);
  expect(result.output).toContain(`Error: item focused with '.only' is not allowed due to the 'forbidOnly' option in 'copilotbrowser.config.ts': \"tests${path.sep}focused-test.spec.js i-am-focused\"`);
...
test('sigint should stop workers', async ({ interactWithTestRunner }) => {
  ...
  const testProcess = await interactWithTestRunner({
    ...
    copilotbrowser_JSON_OUTPUT_NAME: 'report.json',
  });
...
test('should use the first occurring error when an unhandled exception was thrown', async ({ runInlineTest }) => {
  const result = await runInlineTest({
    'unhandled-exception.spec.js': `
      const { test: base, expect } = require('@copilotbrowser/copilotbrowser/test');
...
test('sigint should stop plugins', async ({ interactWithTestRunner }) => {
  ...
  const testProcess = await interactWithTestRunner({
    'copilotbrowser.config.ts': `
      ...
      module.exports = {
        '@copilotbrowser/copilotbrowser/test': { plugins }
      };
    `,
    'a.spec.js': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
...
test('sigint should stop plugins 2', async ({ interactWithTestRunner }) => {
  ...
  const testProcess = await interactWithTestRunner({
    'copilotbrowser.config.ts': `
      ...
      module.exports = { '@copilotbrowser/copilotbrowser/test': { plugins } };
    `,
    'a.spec.js': `
      import { test, expect } from '@copilotbrowser/copilotbrowser/test';
      test('test', async () => {
...
test('wait for workers to finish before reporter.onEnd', async ({ runInlineTest }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/30550' });
  test.fixme();
  const result = await runInlineTest({
    'copilotbrowser.config.ts': `
      export default {
