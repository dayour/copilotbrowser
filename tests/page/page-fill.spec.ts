...
for (const [type, value] of Object.entries({
  ...
  it(`input event.composed should be true and cross shadow dom boundary - ${type}`, async ({ page, server, browserName, isWindows }) => {
    it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/28726' });
...
it('should not double-fill in contenteditable with beforeinput handler in Firefox', {
  annotation: {
    type: 'issue',
    description: 'https://github.com/dayour/copilotbrowser/issues/36715'
  }
}, async ({ page, browserName }) => {
  it.fixme(browserName === 'firefox', 'https://github.com/dayour/copilotbrowser/issues/36715');
  ...
  const testValue = 'copilotbrowser';
...
it('fill back to back', async ({ page }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/15925' });
