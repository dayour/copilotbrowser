...
test('should respect timeout', async ({ page, copilotbrowser }) => {
  let error = null;
  await page.waitForEvent('filechooser', { timeout: 1 }).catch(e => error = e);
  expect(error).toBeInstanceOf(copilotbrowser.errors.TimeoutError);
});

test('should respect default timeout when there is no custom timeout', async ({ page, copilotbrowser }) => {
...
test('should prioritize exact timeout over default timeout', async ({ page, copilotbrowser }) => {
...
test('should emit event after navigation', async ({ page, server, browserName, browserMajorVersion }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/11375' });
