...
it.describe('Drag and drop', () => {
  ...
  it('should respect the drop effect', async ({ page, browserName, isLinux, isMac, headless, trace }) => {
    ...
    it.fixme(browserName === 'webkit' && isLinux && !headless, 'https://github.com/dayour/copilotbrowser/issues/21646');
    it.fixme(browserName === 'chromium' && !isMac && !headless, 'https://github.com/dayour/copilotbrowser/issues/21646');
    it.slow(trace === 'on');
...
it('should handle custom dataTransfer', async ({ page, browserName, isWindows }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/18013' });
...
it('what happens when dragging element is destroyed', async ({ page, browserName }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/21621' });
