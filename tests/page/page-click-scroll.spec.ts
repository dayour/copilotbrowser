...
it('should not hit scroll bar', async ({ page, browserName, platform }) => {
  ...
  it.fixme(browserName === 'webkit' && platform === 'win32', 'https://github.com/dayour/copilotbrowser/issues/18452');
...
it('should not crash when force-clicking hidden input', async ({ page }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/18183' });
...
it('should scroll into view span element', async ({ page, isAndroid }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/20165' });
  it.fixme(isAndroid);
  await page.setContent(`
...
it('should scroll into view element in iframe', async ({ page, isAndroid, server, browserName }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/27196' });
  it.fixme(isAndroid);
