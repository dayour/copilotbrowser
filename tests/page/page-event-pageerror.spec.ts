...
it('should emit error from unhandled rejects', async ({ page, browserName }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/14165' });
  const [error] = await Promise.all([
    page.waitForEvent('pageerror'),
    page.setContent(`
        <script>
...
it('should fire illegal character error', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/38388' },
