...
it('should scroll and click the button with smooth scroll behavior', async ({ page, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/12370' });
  await page.goto(server.PREFIX + '/input/scrollable.html');
...
it('should click the button behind position:absolute header', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/36339' },
}, async ({ page }) => {
  await page.setViewportSize({ width: 500, height: 240 });
  await page.setContent(`
    <style>
...
it('should click a button that is overlaid by a permission popup', async ({ page, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/23280' });
  await page.setViewportSize({ width: 500, height: 500 });
  await page.goto(server.EMPTY_PAGE);
  await page.setContent(`
...
it('ensure events are dispatched in the individual tasks', async ({ page, browserName }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/19685' });
  await page.setContent(`
...
it('should click if opened select covers the button', async ({ page }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/23618' });
  await page.setContent(`
    <div>
      <select>
...
it('should fire contextmenu event on right click in correct order', async ({ page, server, browserName }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/26515' });
...
it('should set PointerEvent.pressure on pointerdown', async ({ page, isLinux, headless }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35844' });
...
it('should click into shadow root with slotted div', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/37768' } }, async ({ page }) => {
  await page.setContent(`
    <my-button>
...
it('should click shadow root button', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/37768' } }, async ({ page }) => {
  await page.setContent(`
    <my-button>
      <template shadowrootmode="open">
        <button><slot></slot></button>
