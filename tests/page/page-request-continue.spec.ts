...
it('should delete header with undefined value', async ({ page, server, browserName }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/13106' });
...
it('should not throw if request was cancelled by the page', async ({ page, server, browserName }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/28490' });
...
it.describe('post data', () => {
  ...
  it('should compute content-length from post data', async ({ page, server }) => {
    it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/16027' });
    await page.goto(server.EMPTY_PAGE);
  ...
  it('should use content-type from original request', async ({ page, server, browserName }) => {
    it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/16736' });
...
it('should work with Cross-Origin-Opener-Policy', async ({ page, server, browserName }) => {
  it.fail(browserName === 'webkit', 'https://github.com/dayour/copilotbrowser/issues/8796');
  let serverHeaders;
...
it('should continue preload link requests', async ({ page, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/16745' });
...
it('should respect set-cookie in redirect response', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35154' }
}, async ({ page, server }) => {
  await page.goto(server.EMPTY_PAGE);
...
it('continue should not propagate cookie override to redirects', {
  annotation: [
    { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35168' },
  ]
...
it('continue should propagate headers to redirects', {
  annotation: [
    { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/28758' },
    { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/32045' },
  ]
}, async ({ page, server }) => {
...
it('continue should drop content-length on redirects', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/36029' }
}, async ({ page, server }) => {
  await page.goto(server.EMPTY_PAGE);
...
it('redirected requests should report overridden headers', {
  annotation: [
    { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/31351' },
    { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/32045' },
  ]
}, async ({ page, server }) => {
  await server.setRedirect('/redirect', '/empty.html');
...
it('continue should delete headers on redirects', {
  annotation: [
    { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/13106' },
    { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/32045' },
  ]
}, async ({ page, server }) => {
  await page.goto(server.PREFIX + '/empty.html');
...
it('propagate headers same origin redirect', {
  annotation: [
    { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/13106' },
    { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/32045' },
    { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35154' },
  ]
}, async ({ page, server }) => {
  await page.goto(server.PREFIX + '/empty.html');
  let resolve;
...
it('should intercept css variable with background url', async ({ page, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/19158' });
...
it('should not forward Host header on cross-origin redirect', {
  annotation: {
    type: 'issue',
    description: 'https://github.com/dayour/copilotbrowser/issues/36719'
  }
}, async ({ page, server, browserName }) => {
