...
it('should not override cookie header', async ({ page, server, browserName }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/16773' });
...
it('should work with redirect inside sync XHR', async ({ page, server, browserName }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/28461' });
...
it('should work with custom referer headers', async ({ page, server, browserName }) => {
  ...
  await page.route('**/*', route => {
    // See https://github.com/dayour/copilotbrowser/issues/8999
...
it('should not auto-intercept non-preflight OPTIONS without network interception', async ({ page, server, isAndroid, browserName }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/20469' });
  it.fixme(isAndroid);
...
it('should intercept when postData is more than 1MB', async ({ page, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/22753' });
  await page.goto(server.EMPTY_PAGE);
...
it('should be able to intercept every navigation to a page controlled by service worker', async ({ page, server, isElectron, browserName }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/33561' });
  it.skip(isElectron);
