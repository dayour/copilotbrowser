...
it('should show visible children of hidden elements', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/36296' }  }, async ({ page }) => {
  await page.setContent(`
...
it('return empty snapshot when iframe is not loaded', { annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/pull/36710' } }, async ({ page, server }) => {
  await page.setContent(`
...
it('should create incremental snapshot for prop change', async ({ page }) => {
  ...
  await page.evaluate(() => document.querySelector('a').setAttribute('href', 'https://copilotbrowser.dev'));
  expect(await snapshotForAI(page, { track: 'track', mode: 'incremental' })).toContainYaml(`
