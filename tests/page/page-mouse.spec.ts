...
it('should report correct pointerType property', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/38376' },
}, async ({ page }) => {
  await page.mouse.move(50, 60);
  await page.evaluate(() => {
    (window as any).__EVENTS = [];
...
it('should not crash on mouse drag with any button', async ({ page }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/16609' });
  await page.evaluate(() => {
...
it('should dispatch mouse move after context menu was opened', async ({ page, browserName, isWindows }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/20823' });
