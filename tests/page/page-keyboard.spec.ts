...
it('pressing Meta should not result in any text insertion on any platform', async ({ page, server, isMac }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/28495' });
...
it('should support MacOS shortcuts', async ({ page, server, platform, browserName }) => {
  ...
  // @see https://github.com/dayour/copilotbrowser/issues/5721
...
it('should support undo-redo', async ({ page, browserName, isLinux, channel }) => {
  it.fixme(browserName === 'webkit' && isLinux || channel === 'webkit-wsl', 'https://github.com/dayour/copilotbrowser/issues/12000');
  await page.setContent(`<div contenteditable></div>`);
...
it('should type repeatedly in contenteditable in shadow dom', async ({ page }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/12941' });
...
it('should have correct Keydown/Keyup order when pressing Escape key', async ({ page, server, browserName }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/27709' });
...
it('should close dialog on Escape key press in contenteditable', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/36727' }
