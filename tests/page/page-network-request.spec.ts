...
// https://github.com/dayour/copilotbrowser/issues/3993
...
it('should not get preflight CORS requests when intercepting', async ({ page, server, browserName, isAndroid, isBidi }) => {
  it.fail(isAndroid, 'copilotbrowser does not get CORS pre-flight on Android');
  await page.goto(server.PREFIX + '/empty.html');
...
it('should parse the data if content-type is application/x-www-form-urlencoded; charset=UTF-8', async ({ page, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/29872' });
  await page.goto(server.EMPTY_PAGE);
...
it('page.reload return 304 status code', async ({ page, server, browserName, isBidi }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/28779' });
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/29441' });
...
it('should handle mixed-content blocked requests', async ({ page, asset, browserName }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/29833' });
