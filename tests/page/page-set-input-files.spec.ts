...
// Also normalizes formidable v3 array files to single values (v1/v2 compat).
function parseFormFromRequest(serverRequest: IncomingMessage & { postBody: Promise<Buffer> }, form: InstanceType<typeof IncomingForm>): Promise<{error: any, files: Record<string, FormidableFile>}> {
  return new Promise(resolve => {
    void serverRequest.postBody.then(body => {
      const pt = new PassThrough();
      (pt as any).headers = serverRequest.headers;
      form.parse(pt as any, (error, _fields, rawFiles) => {
        const files: any = {};
        for (const [k, v] of Object.entries(rawFiles))
          files[k] = Array.isArray(v) ? v[0] : v;
        resolve({ error, files: files as Record<string, FormidableFile> });
      });
      setImmediate(() => {
        pt.write(body);
        pt.end();
...
test('should upload a file after popup', async ({ page, server, asset }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/29923' });
  await page.goto(server.PREFIX + '/input/fileupload.html');
  {
...
test('should upload the file with spaces in name', async ({ page, server, asset }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/17451' });
  await page.goto(server.PREFIX + '/input/fileupload.html');
  const filePath = path.relative(process.cwd(), asset('file to upload.txt'));
  const input = await page.$('input');
  await input.setInputFiles(filePath);
...
test('should detect mime type', async ({ page, server, asset }) => {
  ...
  server.setRoute('/upload', async (req, res) => {
    ...
    const { files: f } = await parseFormFromRequest(req as any, form);
    files = f;
    res.end();
  });
  await page.setContent(`<form action="${server.PREFIX}/upload" method="POST" enctype="multipart/form-data">
...
// @see https://github.com/dayour/copilotbrowser/issues/4704
...
test('input event.composed should be true and cross shadow dom boundary', async ({ page, server, asset }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/28726' });
  await page.goto(server.EMPTY_PAGE);
...
test('input should trigger events when files changed second time', async ({ page, asset }) => {
  test.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/20079' });
