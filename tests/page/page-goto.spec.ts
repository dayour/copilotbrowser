...
it('should fail when server returns 204', async ({ page, server, browserName, isLinux }) => {
  it.fixme(browserName === 'webkit' && isLinux, 'Regressed in https://github.com/dayour/copilotbrowser-browsers/pull/1297');
...
it('should fail when exceeding maximum navigation timeout', async ({ page, server, copilotbrowser }) => {
...
it('should fail when exceeding default maximum navigation timeout', async ({ page, server, copilotbrowser, isAndroid }) => {
...
it('should fail when exceeding browser context navigation timeout', async ({ page, server, copilotbrowser, isAndroid }) => {
  it.skip(isAndroid, 'No context per test');
...
it('should fail when exceeding default maximum timeout', async ({ page, server, copilotbrowser, isAndroid }) => {
  it.skip(isAndroid, 'No context per test');
...
it('should fail when exceeding browser context timeout', async ({ page, server, copilotbrowser, isAndroid }) => {
  it.skip(isAndroid, 'No context per test');
...
it('should prioritize default navigation timeout over default timeout', async ({ page, server, copilotbrowser }) => {
  // Hang for request to the empty.html
  server.setRoute('/empty.html', (req, res) => { });
  let error = null;
...
it('js redirect overrides url bar navigation ', async ({ page, server, browserName, trace }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/20749' });
...
it('should succeed on url bar navigation when there is pending navigation', async ({ page, server, browserName }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/21574' });
...
it('should send referer of cross-origin URL', async ({ page, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/27765' });
  const [request1, request2] = await Promise.all([
    server.waitForRequest('/grid.html'),
    server.waitForRequest('/digits/1.png'),
    page.goto(server.PREFIX + '/grid.html', {
...
it('should return url with basic auth info', async ({ page, server, loopback }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/23138' });
