...
it('textContent should be atomic', async ({ copilotbrowser, page }) => {
  ...
  await copilotbrowser.selectors.register('textContent', createDummySelector);
...
it('innerText should be atomic', async ({ copilotbrowser, page }) => {
  const createDummySelector = () => ({
  ...
  await copilotbrowser.selectors.register('innerText', createDummySelector);
  await page.goto(`data:text/html,<div>Hello</div>`);
...
it('innerHTML should be atomic', async ({ copilotbrowser, page }) => {
  const createDummySelector = () => ({
    query(root, selector) {
      const result = root.querySelector(selector);
      if (result)
  ...
  await copilotbrowser.selectors.register('innerHTML', createDummySelector);
...
it('getAttribute should be atomic', async ({ copilotbrowser, page }) => {
  const createDummySelector = () => ({
    query(root: HTMLElement, selector: string) {
      const result = root.querySelector(selector);
      if (result)
  ...
  await copilotbrowser.selectors.register('getAttribute', createDummySelector);
...
it('isVisible should be atomic', async ({ copilotbrowser, page }) => {
  const createDummySelector = () => ({
    query(root, selector) {
      const result = root.querySelector(selector);
      if (result)
  ...
  await copilotbrowser.selectors.register('isVisible', createDummySelector);
  await page.goto(`data:text/html,<div>Hello</div>`);
...
it('should take java-style string', async ({ copilotbrowser, page }) => {
  const createDummySelector = `{
    query(root, selector) {
  ...
  await copilotbrowser.selectors.register('objectLiteral', createDummySelector);
  await page.goto(`data:text/html,<div>Hello</div>`);
