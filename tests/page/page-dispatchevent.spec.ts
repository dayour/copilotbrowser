...
it('should be atomic', async ({ copilotbrowser, page }) => {
  ...
  await copilotbrowser.selectors.register('dispatchEvent', createDummySelector);
...
it('should dispatch wheel event', async ({ page, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/15562' });
...
it('should throw if argument is from different frame', async ({ page, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/28690' });
