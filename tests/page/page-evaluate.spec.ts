...
it('should work with large unicode strings', async ({ page, browserName, platform }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/16367' });
...
it('should throw for too deep reference chain', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/33997' }
}, async ({ page, browserName }) => {
  it.fixme(browserName === 'firefox', 'Firefox Juggler -> copilotbrowser serialiser does not throw for deep references.\nThis causes large objects to get serialised back to the copilotbrowser client.\nThere our validators throw \'Maximum call stack size exceeded\'.');
...
it('should work with overridden URL/Date/RegExp', async ({ page, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/21109' });
  const testCases = [
    // @ts-ignore
    () => globalThis.URL = 'foo',
    // @ts-ignore
...
it('should work with Array.from/map', async ({ page }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/28520' });
  expect(await page.evaluate(() => {
