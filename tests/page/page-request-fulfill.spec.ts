...
it('should fulfill with unuassigned status codes', async ({ page, server, browserName }) => {
  ...
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/30773' });
...
it('should fulfill with global fetch result', async ({ copilotbrowser, page, server, isElectron, electronMajorVersion, rewriteAndroidLoopbackURL }) => {
...
it('should fulfill with fetch response that has multiple set-cookie', async ({ copilotbrowser, page, server, isAndroid }) => {
  it.fixme(isAndroid);
...
it('headerValue should return set-cookie from intercepted response', async ({ page, server, browserName }) => {
  ...
  it.skip(browserName === 'webkit', 'Set-Cookie with \n in intercepted response does not pass validation in WebCore, see also https://github.com/dayour/copilotbrowser/pull/9273');
  await page.route('**/empty.html', async route => {
    void route.fulfill({
      status: 200,
      headers: {
...
it('should fulfill with har response', async ({ page, asset }) => {
  ...
  await page.goto('http://no.copilotbrowser/');
...
it('should fulfill with gzip and readback', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/29261' },
...
it('should not go to the network for fulfilled requests body', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/30760' },
}, async ({ page, server }) => {
...
it('should return body for fulfilled responses', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser-java/issues/1792' },
}, async ({ page, server }) => {
