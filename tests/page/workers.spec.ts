...
it('should attribute network activity for worker inside iframe to the iframe', async function({ page, server, browserName, browserMajorVersion }) {
  ...
  it.skip(browserName === 'firefox' && browserMajorVersion < 114, 'https://github.com/dayour/copilotbrowser/issues/21760');
...
it('should report worker script as network request', {
  annotation: [
    { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/33107' },
    { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35678' },
  ],
...
it('should report worker script as network request after redirect', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/35678' },
...
it('should dispatch console messages when page has workers', async function({ page, server }) {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/15550' });
  await page.goto(server.EMPTY_PAGE);
  await Promise.all([
    page.waitForEvent('worker'),
...
it('should report and intercept network from nested worker', async function({ page, server, browserName, browserMajorVersion }) {
  it.fixme(browserName === 'webkit', 'https://github.com/dayour/copilotbrowser/issues/27376');
  it.skip(browserName === 'chromium' && browserMajorVersion < 143, 'needs workerScriptLoaded event');
