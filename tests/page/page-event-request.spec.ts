...
it('should fire for fetches with keepalive: true', {
  annotation: {
    type: 'issue',
    description: 'https://github.com/dayour/copilotbrowser/issues/34497'
  }
...
it('main resource xhr should have type xhr', async ({ page, server }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/22812' });
  await page.goto(server.EMPTY_PAGE);
...
it('should finish 204 request', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/32752' }
}, async ({ page, server, browserName }) => {
...
it('<picture> resource should have type image', async ({ page }) => {
  it.info().annotations.push({ type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/33148' });
  const [request] = await Promise.all([
    page.waitForEvent('request'),
    page.setContent(`
      <picture>
...
it('should not expose preflight OPTIONS request', {
  annotation: { type: 'issue', description: 'https://github.com/dayour/copilotbrowser/issues/36311' }
}, async ({ page, server, browserName }) => {
