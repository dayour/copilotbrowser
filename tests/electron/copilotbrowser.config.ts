...
import type { Config, copilotbrowserTestOptions, copilotbrowserWorkerOptions } from '@copilotbrowser/copilotbrowser/test';
import * as path from 'path';

process.env.CBPAGE_IMPL = 'electron';

const outputDir = path.join(__dirname, '..', '..', 'test-results');
const testDir = path.join(__dirname, '..');
const config: Config<copilotbrowserWorkerOptions & copilotbrowserTestOptions> = {
  testDir,
  outputDir,
  expect: {
    timeout: 10000,
