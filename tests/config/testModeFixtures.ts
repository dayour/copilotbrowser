...
export const testModeTest = test.extend<TestModeTestFixtures, TestModeWorkerOptions & TestModeWorkerFixtures>({
  ...
  copilotbrowser: [async ({ mode }, run) => {
    ...
    const copilotbrowser = await testMode.setup();
    await run(copilotbrowser);
    await testMode.teardown();
  }, { scope: 'worker' }],

  toImplInWorkerScope: [async ({ copilotbrowser }, use) => {
    await use((copilotbrowser as any)._connection.toImpl);
  }, { scope: 'worker' }],
