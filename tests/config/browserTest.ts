...
interface StartRemoteServer {
  (kind: 'run-server' | 'launchServer'): Promise<copilotbrowserServer>;
...
const test = baseTest.extend<BrowserTestTestFixtures, BrowserTestWorkerFixtures>({
  ...
  browserType: [async ({ copilotbrowser, browserName, mode }, run) => {
    test.skip(mode === 'service2');
    await run(copilotbrowser[browserName]);
  }, { scope: 'worker' }],
  ...
  createUserDataDir: async ({ mode }, run) => {
    ...
    await run(async () => {
      const dir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'copilotbrowser-test-'));
      dirs.push(dir);
      return dir;
    });
...
export const copilotbrowserTest = test;
