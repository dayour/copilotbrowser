...
Create `packages/copilotbrowser/src/mcp/browser/tools/<your-tool>.ts`.
...
const myTool = defineTabTool({
  ...
  handle: async (tab, params, response) => {
    // Implementation using tab.page (copilotbrowser Page object)
    await tab.page.click(`[ref="${params.ref}"]`);

    // Add generated copilotbrowser code
...
- `response.addCode(code)` — add generated copilotbrowser code snippet
...
If your tool doesn't fit an existing capability, add a new one to `packages/copilotbrowser/src/mcp/config.d.ts`:
...
In `packages/copilotbrowser/src/mcp/browser/tools.ts`:
...
- `code` — generated copilotbrowser code (without ```js fences)
...
**Categories** (defined in `packages/copilotbrowser/src/cli/daemon/command.ts`):
...
1. Add it to `Category` type in `packages/copilotbrowser/src/cli/daemon/command.ts`
2. Add it to the `categories` array in `packages/copilotbrowser/src/cli/daemon/helpGenerator.ts`:
   ```typescript
...
Update `packages/copilotbrowser/src/skill/SKILL.md` with the new command documentation.
Add reference docs in `packages/copilotbrowser/src/skill/references/` if the feature is complex.

Run `npm run copilotbrowser-cli -- --help` to verify the help output includes your new command.
...
### 1. Type definition: `packages/copilotbrowser/src/mcp/config.d.ts`
...
### 2. CLI options type: `packages/copilotbrowser/src/mcp/browser/config.ts`
...
```typescript
options.myOption = envToString(process.env.copilotbrowser_MCP_MY_OPTION);
// For booleans: envToBoolean(process.env.copilotbrowser_MCP_MY_OPTION)
// For numbers: numberParser(process.env.copilotbrowser_MCP_MY_OPTION)
// For comma lists: commaSeparatedList(process.env.copilotbrowser_MCP_MY_OPTION)
// For semicolon lists: semicolonSeparatedList(process.env.copilotbrowser_MCP_MY_OPTION)
```

### 5. MCP server CLI: `packages/copilotbrowser/src/mcp/program.ts`
...
The skill file is located at `packages/copilotbrowser/src/skill/SKILL.md`. It contains documentation for all available CLI commands and MCP tools. Update it whenever you add new commands or tools.

Reference docs live in `packages/copilotbrowser/src/skill/references/`:
...
Run `npm run copilotbrowser-cli -- --help` to see the latest available commands and use them to update the skill file.
...
│   │   │   ├── runCode.ts   # run copilotbrowser code
...
CLI mode:
  User → `copilotbrowser-cli my-command arg1 --opt=val`
