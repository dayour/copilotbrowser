...
function runOSX() {
  ...
  if [[ -f "${SCRIPT_PATH}/EXPECTED_BUILDS" && -d "$WK_CHECKOUT_PATH/WebKitBuild/Release/copilotbrowser.app" ]]; then
    DYLIB_PATH="$WK_CHECKOUT_PATH/WebKitBuild/Release"
  elif [[ -d $SCRIPT_PATH/copilotbrowser.app ]]; then
    DYLIB_PATH="$SCRIPT_PATH"
  elif [[ -d $SCRIPT_PATH/WebKitBuild/Release/copilotbrowser.app ]]; then
    DYLIB_PATH="$SCRIPT_PATH/WebKitBuild/Release"
  else
    echo "Cannot find a copilotbrowser.app in neither location" 1>&2
    exit 1
  fi
  copilotbrowser="$DYLIB_PATH/copilotbrowser.app/Contents/MacOS/copilotbrowser"
  DYLD_FRAMEWORK_PATH="$DYLIB_PATH" DYLD_LIBRARY_PATH="$DYLIB_PATH" "$copilotbrowser" "$@"
}
