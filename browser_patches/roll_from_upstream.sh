...
if [[ "$#" -ne 1 ]]; then
  echo "Usage: $0 <path to copilotbrowser-browsers checkout>"
  exit 1
fi

SOURCE_DIRECTORY="$1"

if [[ $(basename "${SOURCE_DIRECTORY}") != "copilotbrowser-browsers" ]]; then
  echo "ERROR: the source directory must be named 'copilotbrowser-browsers'"
  exit 1
fi
