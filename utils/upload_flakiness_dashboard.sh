...
if [[ ("${GITHUB_REPOSITORY}" != "microsoft/copilotbrowser") && ("${GITHUB_REPOSITORY}" != "microsoft/copilotbrowser-browsers") ]]; then
  echo "NOTE: skipping dashboard uploading from fork"
  exit 0
fi

if [[ "${GITHUB_REF}" != "refs/heads/main" && "${GITHUB_REF}" != 'refs/heads/release-'* ]]; then
  echo "NOTE: skipping dashboard uploading from copilotbrowser branches"
  exit 0
fi
