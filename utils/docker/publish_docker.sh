...
publish_docker_images_with_arch_suffix() {
  ...
  ./build.sh "--${ARCH}" "${FLAVOR}" copilotbrowser:localbuild

  for ((i = 0; i < ${#TAGS[@]}; i++)) do
    local TAG="${TAGS[$i]}"
    tag_and_push copilotbrowser:localbuild "copilotbrowser.azurecr.io/public/${MCR_IMAGE_NAME}:${TAG}-${ARCH}"
  done
}
...
publish_docker_manifest () {
  ...
  for ((i = 0; i < ${#TAGS[@]}; i++)) do
    local TAG="${TAGS[$i]}"
    local BASE_IMAGE_TAG="copilotbrowser.azurecr.io/public/${MCR_IMAGE_NAME}:${TAG}"
    local IMAGE_NAMES=""
