...
const { docsLinkRendererForLanguage, rendercopilotbrowserDevLinks } = require('../doclint/linkUtils');
...
class TypesGenerator {
  ...
  async generateTypes(overridesFile) {
    ...
    let overrides = await parseOverrides(overridesFile, className => {
      ...
      if (!method) {
        if (new Set(['on', 'addListener', 'off', 'removeListener', 'once', 'prependListener', 'botName', '$', '$$', '$eval', '$$eval']).has(methodName))
          return '';
    ...
    {
      const copilotbrowser = this.documentation.classesArray.find(c => c.name === 'copilotbrowser');
      if (!copilotbrowser)
        throw new Error(`Unknown class "copilotbrowser"`);
      copilotbrowser.membersArray = copilotbrowser.membersArray.filter(member => !['errors', 'devices'].includes(member.name));
      copilotbrowser.index();
    }
    ...
    return [
      ...
      classes.map(classDesc => {
        return (classDesc.name === 'copilotbrowser') ? this.classBody(classDesc, true) : this.classToString(classDesc);
      }).join('\n'),
  ...
  writeComment(comment, indent) {
    ...
    comment = out.join('\n');
    comment = rendercopilotbrowserDevLinks(comment, '', '/api');
  ...
  async function generateTestTypes() {
    ...
    const generator = new TypesGenerator({
      ...
      overridesToDocsClassMapping: new Map([
        ...
        ['copilotbrowserTestArgs', 'Fixtures'],
        ['copilotbrowserTestOptions', 'TestOptions'],
        ['copilotbrowserWorkerArgs', 'Fixtures'],
        ['copilotbrowserWorkerOptions', 'TestOptions'],
        ['TestType', 'Test'],
      ]),
      ignoreMissing: new Set([
        ...
        'ExpectMatcherUtils',
        'Matchers',
        'copilotbrowserWorkerArgs.copilotbrowser',
        'copilotbrowserWorkerOptions.defaultBrowserType',
        'Project',
      ]),
