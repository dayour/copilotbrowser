...
import * as copilotbrowser from 'copilotbrowser';
...
copilotbrowser.chromium.launch().then(async browser => {
  const page = await browser.newPage();
...
// Test v0.12 features
(async () => {
  const launchOptions: copilotbrowser.LaunchOptions = {
    env: {
      TIMEOUT: '52',
      SOMETHING: '/some/path',
      JEST_TEST: 'true'
  ...
  const browser = await copilotbrowser.chromium.launch(launchOptions);
  const viewport: copilotbrowser.ViewportSize = {
    width: 100,
    height: 200,
  };
  const geolocation: copilotbrowser.Geolocation = {
    latitude: 0,
    longitude: 0,
    accuracy: undefined,
  };
  const httpCredentials: copilotbrowser.HTTPCredentials = {
    username: 'foo',
    password: 'bar',
  };
  const contextOptions: copilotbrowser.BrowserContextOptions = {
    viewport,
    geolocation,
    httpCredentials,
  };
  ...
  page.on('console', (event: copilotbrowser.ConsoleMessage, ...args: any[]) => {
  ...
  const cookies: copilotbrowser.Cookie[] = await page.context().cookies(['http://example.com']);
  ...
  // evaluateHandle with two different return types (ElementHandle)
  {
    ...
    const assertion: AssertType<copilotbrowser.ElementHandle<HTMLInputElement | HTMLTextAreaElement>, typeof handle> = true;
  }
...
// test $eval and $$eval
(async () => {
  const browser = await copilotbrowser.firefox.launch();
  const page = await browser.newPage();
  await page.goto('https://example.com');
...
// waitForEvent
(async () => {
  const browser = await copilotbrowser.webkit.launch();
  const page = await browser.newPage();
  {
    const frame = await page.waitForEvent('frameattached');
    const assertion: AssertType<copilotbrowser.Frame, typeof frame> = true;
  }
  {
    const worker = await page.waitForEvent('worker', {
      predicate: worker => {
        const condition: AssertType<copilotbrowser.Worker, typeof worker> = true;
        return true;
      }
    });
    const assertion: AssertType<copilotbrowser.Worker, typeof worker> = true;
  }
  {
    const newPage = await page.context().waitForEvent('page', {
      timeout: 500
    });
    const assertion: AssertType<copilotbrowser.Page, typeof newPage> = true;
  }
  {
    const response = await page.waitForEvent('response', response => response.url() === 'asdf');
    const assertion: AssertType<copilotbrowser.Response, typeof response> = true;
  }
})();
...
(async () => {
  ...
  }
  {
    ...
    const assertion: AssertType<copilotbrowser.JSHandle<string>, typeof value> = true;
  }
  {
    const handle = await page.locator('body').evaluateHandle(() => ([{a: '123'}]));
    const value = await handle.evaluateHandle((h, p) => ({ a: h[1].a, p}), 123);
    const assertion: AssertType<copilotbrowser.JSHandle<{a: string, p: number}>, typeof value> = true;
  }
  {
    const handle = await page.locator('body').evaluateHandle(() => ([{a: '123'}]));
    const value = await handle.evaluateHandle((h: ({a: string, b: number})[]) => h[1].b);
    const assertion: AssertType<copilotbrowser.JSHandle<number>, typeof value> = true;
  }
  {
  ...
  }
  {
    ...
    const assertion: AssertType<copilotbrowser.JSHandle<boolean | number>, typeof value> = true;
  }
  ...
  {
    type T = ({ s: string } | copilotbrowser.ElementHandle)[];
    const handle = await page.evaluateHandle(() => document.body);
...

  {
    ...
    const assertion: AssertType<copilotbrowser.ElementHandle<HTMLBodyElement>, typeof handle> = true;
...
(async () => {
  ...
  for (const elementLike of elementLikes) {
    {
      ...
      const bodyAssertion: AssertType<copilotbrowser.ElementHandle<HTMLBodyElement>, typeof handle> = true;
    }
    {
...

    {
      ...
      const bodyAssertion: AssertType<copilotbrowser.ElementHandle<HTMLBodyElement>[], typeof handles> = true;
    }
  ...
  for (const frameLike of frameLikes) {
    ...
    }
    {
      ...
      const elementAssertion: AssertType<copilotbrowser.ElementHandle<HTMLElement|SVGElement>, typeof handle> = true;
      const canBeNull: AssertCanBeNull<typeof handle> = false;
    }
    {
...
// top level
(async () => {
  copilotbrowser.chromium.connect;
  copilotbrowser.errors.TimeoutError;
  {
    copilotbrowser.devices['my device'] = {
      userAgent: 'foo',
    ...
    const iPhone = copilotbrowser.devices['iPhone 11'];
  ...
  {
    const agents = Object.entries(copilotbrowser.devices).map(([name, descriptor]) => descriptor.userAgent);
  ...
  await copilotbrowser.selectors.register('tag', createTagNameEngine);
})();

// Event listeners
(async function() {
  {
    const eventEmitter = {} as (copilotbrowser.Page | EventEmitter);
    const listener = () => { };
  ...
  {
    const eventEmitter = {} as (copilotbrowser.BrowserContext | EventEmitter);
    const listener = (c: copilotbrowser.BrowserContext) => { };
    eventEmitter.addListener('close', listener)
      .on('close', listener)
      .once('close', listener)
      .removeListener('close', listener)
  ...
  {
    const page: copilotbrowser.Page = {} as any;
...
(async () => {
  const browserType = {} as copilotbrowser.BrowserType<copilotbrowser.Browser & {foo: 'string'}>;
