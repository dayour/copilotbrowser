...
NODE_VERSION="24.13.1" # autogenerated via ./update-copilotbrowser-node.mjs
...
function build {
  ...
  echo "Building copilotbrowser-${PACKAGE_VERSION}-${SUFFIX}"
  ...
  if [[ "${ARCHIVE}" == "zip" ]]; then
    ...
    cd ..
    cp ./output/${NODE_DIR}/node.exe ./output/copilotbrowser-${SUFFIX}/
    NPM_PATH="node_modules/npm/bin/npm-cli.js"
  elif [[ "${ARCHIVE}" == "tar.gz" ]]; then
    tar -xzf ./output/${NODE_DIR}.tar.gz -C ./output/
    cp ./output/${NODE_DIR}/bin/node ./output/copilotbrowser-${SUFFIX}/
  ...
  cp ./output/${NODE_DIR}/LICENSE ./output/copilotbrowser-${SUFFIX}/
  cp ./output/api.json ./output/copilotbrowser-${SUFFIX}/package/
  cp ./output/protocol.yml ./output/copilotbrowser-${SUFFIX}/package/
  cd ./output/copilotbrowser-${SUFFIX}/package
  copilotbrowser_SKIP_BROWSER_DOWNLOAD=1 node "../../${NODE_DIR}/${NPM_PATH}" install --omit=dev --ignore-scripts
  ...
  # - https://github.com/dayour/copilotbrowser-dotnet/issues/2069
  find . -type f -exec touch {} +

  zip -q -r ../copilotbrowser-${PACKAGE_VERSION}-${SUFFIX}.zip .
}
