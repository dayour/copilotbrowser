...
class ApiParser {
  ...
  parseClass(node) {
    ...
    for (const member of node.children) {
      ...
      if (member.type === 'li' && member.liType === 'bullet' && member.text.startsWith('extends: ')) {
        const extendsText = member.text.substring('extends: '.length).replace(/^[`\[]/, '').replace(/[`\]]$/, '');
        extendsName = extendsText;
        continue;
      }
    }
  ...
  parseMember(spec) {
    ...
    let returnType = null;
    let optional = false;
    // Check if human-readable "**Returns:** `type`" or "> **Type:** `type`" format is used.
    let humanReadableReturn = false;
    let humanReadableTypeStr = null;
    for (const item of spec.children || []) {
      if (item.type === 'text') {
        // Match both "**Returns:** `type`" and "> **Type:** `type`" (blockquote) formats
        const retMatch = item.text.match(/^(?:> )?\*\*(?:Returns|Type):\*\*\s+`([^`]+)`/);
        if (retMatch) {
          humanReadableReturn = true;
          humanReadableTypeStr = retMatch[1].replace(/`/g, '');
          returnType = docs.Type.parse(humanReadableTypeStr);
          break;
        }
      }
    }
    if (!humanReadableReturn) {
      for (const item of spec.children || []) {
    ...
    } else if (humanReadableTypeStr) {
      // Collect inline object property children (e.g. `- \`name\` <\`string\`> ...`)
      // that define the shape of Object/Array<Object> types.
      const objectProperties = [];
      for (const child of spec.children || []) {
        if (child.type !== 'li' || child.liType !== 'default')
          continue;
        try {
          const { name: propName, optional: propOptional } = parseVariable(child.text);
          const childType = this.parseType(child, metainfo.since ?? 'v1.0');
          objectProperties.push(docs.Member.createProperty(
            { langs: {}, since: metainfo.since ?? 'v1.0', deprecated: undefined, discouraged: undefined },
            propName, childType.type, [], !propOptional
          ));
        } catch (e) {
          // skip non-parseable doc-only children
        }
      }
      if (objectProperties.length > 0) {
        try {
          returnType = docs.Type.parse(humanReadableTypeStr, objectProperties);
        } catch (e) {
          // keep existing returnType if properties can't be applied
        }
      }
    }
    if (!returnType)
   ...
   */
  parseArgument(spec) {
    // Check hidden first, before regex match, so language-specific doc sections
    // (like "### C# (.NET)") can be marked hidden and safely skipped.
    if (extractHidden(spec))
      return null;
...
function extractLangs(spec) {
  ...
  // Also handle "> **Languages:** JavaScript, Java" human-readable blockquote format
  const langNameToCode = { 'javascript': 'js', 'java': 'java', 'python': 'python', 'c#': 'csharp' };
  for (const child of spec.children || []) {
    if (child.type !== 'text' || !child.text.startsWith('> **Languages:**'))
      continue;
    const langsPart = child.text.replace(/^> \*\*Languages:\*\*\s*/, '').trim();
    // "*(all)*" means no language restriction — treat as undefined (all languages)
    if (!langsPart || langsPart === '*(all)*' || langsPart === '(all)') {
      return { only: undefined, aliases: {}, types: {}, overrides: {} };
    }
    const only = langsPart.split(',').map(l => (langNameToCode[l.trim().toLowerCase()] || l.trim().toLowerCase()));
    return { only, aliases: {}, types: {}, overrides: {} };
  }
  return {};
}
...
function extractSince(spec) {
  for (const child of spec.children) {
    ...
    // Also handle "> *Added in: v1.x*" blockquote format
    if (child.type === 'text') {
      const match = child.text.match(/^> \*Added in: (v[\d.]+)\*/);
      if (match)
        return match[1];
    }
  }
...
function childrenWithoutProperties(spec) {
  return (spec.children || []).filter(c => {
    ...
    // Skip human-readable language annotation blockquotes like "> **Languages:** JavaScript, Java"
    // and their continuation lines ("> *Added in: ...*", ">", etc.)
    const isLanguageAnnotation = c.type === 'text' && c.text.startsWith('>');
    return !isProperty && !isLanguageAnnotation;
  });
}
