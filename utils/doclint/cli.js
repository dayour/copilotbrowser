...
async function run() {
  ...
  let copilotbrowserVersion = require(path.join(PROJECT_DIR, 'package.json')).version;
  if (copilotbrowserVersion.endsWith('-next'))
    copilotbrowserVersion = copilotbrowserVersion.substring(0, copilotbrowserVersion.indexOf('-next'));
    ...
    for (const lang of langs) {
      try {
        ...
        for (const filePath of getAllMarkdownFiles(documentationRoot)) {
          ...
          // Also, Internally (copilotbrowser.dev generator) we merge test-api and test-reporter-api into api.
...
async function getBrowserVersions() {
  ...
  try {
    const browsers = await Promise.all(names.map(name => copilotbrowser[name].launch()));
    const result = {};
    ...
    for (const browser of browsersJSON.browsers) {
      if (names.includes(browser.name) && browser.browserVersion)
        result[browser.name] = browser.browserVersion;
    }
    return result;
  }
}
