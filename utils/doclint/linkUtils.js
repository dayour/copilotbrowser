...
function createMarkdownLink(languagePath, member, text, paramOrOption) {
  ...
  return `[${text}](https://copilotbrowser.dev${languagePath}/docs/api/class-${member.clazz.name.toLowerCase()}#${hash})`;
};
...
function createClassMarkdownLink(languagePath, clazz) {
  return `[${clazz.name}](https://copilotbrowser.dev${languagePath}/docs/api/class-${clazz.name.toLowerCase()})`;
};
...
function docsLinkRendererForLanguage(language, outputType) {
  ...
  return ({ clazz, member, param, option }) => {
    ...
    const className = member.clazz.varName === 'copilotbrowserAssertions' ? '' : member.clazz.varName + '.';
...
function assertionArgument(className) {
  switch (className.toLowerCase()) {
    ...
    case 'copilotbrowserassertions': return 'value';
  }
 ...
 * @returns {string}
 */
function rendercopilotbrowserDevLinks(content, languagePath, relativePath, releaseNotesMode = false) {
  return content.replace(/\[([^\]]+)\]\((\.[^\)]+)\)/g, (match, p1, p2) => {
    if (releaseNotesMode && p2.includes('/images/')) {
      const url = new URL(p2, `https://github.com/dayour/copilotbrowser/blob/main/docs/src/`);
      url.searchParams.set('raw', 'true');
      return `[${p1}](${url.toString()})`;
    }
    return `[${p1}](${new URL(p2.replace('.md', ''), `https://copilotbrowser.dev${languagePath}/docs${relativePath}/`).toString()})`;
  });
}

module.exports = {
  docsLinkRendererForLanguage,
  rendercopilotbrowserDevLinks,
